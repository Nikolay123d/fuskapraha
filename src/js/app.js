...
const PREMIUM_QR_IMG = './img/assets/img/csob-qr.png';
const PREMIUM_PLANS = {
...
// Default avatar
window.DEFAULT_AVATAR = "./img/assets/img/default-avatar.svg";
...
const SND = {
  chat: new Audio('./sounds/assets/audio/new_chat.wav'),
  dm: new Audio('./sounds/assets/audio/new_dm.wav'),
  ok: new Audio('./sounds/assets/audio/ok.wav'),
  err: new Audio('./sounds/assets/audio/err.wav'),
  party: new Audio('./sounds/assets/audio/celebration.wav')
};
...
// Apply cached wallpaper ASAP (prevents "old wallpaper flash")
try{
  const __city = (localStorage.getItem('city') || (document.getElementById('citySelect') && document.getElementById('citySelect').value) || 'praha');
  const __w = localStorage.getItem('wall_'+__city);
  if(__w && typeof __w==='string') document.documentElement.style.setProperty('--wall', `url('${__w}')`);
}catch(e){}
  ...
  if(t && t.id==='wallCamera'){
    ...
    const r=new FileReader();
	    r.onload=()=>{
	      ...
	      if(!data.startsWith('data:image')){ toast('Soubor není obrázek.'); return; }
	      try{ localStorage.setItem('wall_'+c, data); }catch(_){ }
	      document.documentElement.style.setProperty('--wall', `url('${data}')`);
	      // Persist for all devices (same wallpaper for all users in the city)
	      try{ db.ref('settings/wallpapers/'+c).set({url:data, ts:Date.now(), by:(auth.currentUser?auth.currentUser.uid:null)}); }catch(_){ }
...
// Prevent one rejected Promise from killing the whole app flow
...
function startWallpaperSync(){
  try{
    ...
    const handler=(snap)=>{
	      const val=snap.val();
	      const url = (val && typeof val==='object') ? String(val.url||'') : String(val||'');
	      if(url && url.startsWith('data:image')){
	        try{ localStorage.setItem('wall_'+city, url); }catch(_){ }
...
async function ensureMyPublic(u){
  ...
  if(u.email){
    ...
    try{ await db.ref('emails/'+key).set({uid:u.uid, ts: Date.now()}); }catch(e){}
  }
}
...
function setPresence(u){
  ...
  ref.on('value', snap=>{
    ...
    up.onDisconnect().set({state:'offline',ts:firebase.database.ServerValue.TIMESTAMP});
    up.set({state:'online',ts:firebase.database.ServerValue.TIMESTAMP});
  });
}
...
async function getUser(uid){
  ...
  try{
    ...
    u.online = (p.val() && p.val().state === 'online');
  }catch{}
...
async function ensureUserPublic(u, extra={}){
  ...
  if(u.email){
    ...
    try{ await db.ref('emails/'+key).update({uid: u.uid, ts: Date.now()}); }catch(e){}
  }
}
...
// === CHAT (stable realtime + paging + mobile UX) ===
let CHAT_LIVE_REF=null;
let CHAT_LIVE_CB=null;
let CHAT_CITY=null;
let CHAT_OLDEST_TS=null;
let CHAT_NEWEST_TS=null;
let CHAT_UNSEEN=0;
let CHAT_LOADING_OLDER=false;


function renderChatMessage(m, msgKey, {prepend=false}={}){
  const feed=$('#chatFeed'); if(!feed) return null;
  if(m.deleted) return null;
  const d=document.createElement('div'); d.className='msg';
  d.dataset.key = msgKey || '';
  // resolve user async
  (async ()=>{
  ...
  if(prepend) feed.insertBefore(d, feed.firstChild);
  else feed.appendChild(d);
  return d;
}

function chatIsAtBottom(feed){ return (feed.scrollTop + feed.clientHeight) >= (feed.scrollHeight - 24); }

function chatUpdateButtons(){
  const feed=$('#chatFeed'); if(!feed) return;
  const downBtn=$('#chatJumpBottom');
  const newBtn=$('#chatShowNew');
  if(downBtn){
    downBtn.style.display = chatIsAtBottom(feed) ? 'none' : 'flex';
  }
  if(newBtn){
    newBtn.textContent = CHAT_UNSEEN ? `Nové (${CHAT_UNSEEN})` : 'Nové';
    newBtn.style.display = CHAT_UNSEEN ? 'flex' : 'none';
  }
}
...
function chatJumpToBottom(){
  ...
  chatUpdateButtons();
  try{
    if(CHAT_NEWEST_TS) localStorage.setItem('lastSeen_'+CHAT_CITY, String(CHAT_NEWEST_TS));
  }catch{}
}

async function chatLoadInitial(city){
  const feed=$('#chatFeed'); if(!feed) return;
  feed.innerHTML='';
  CHAT_OLDEST_TS=null; CHAT_NEWEST_TS=null; CHAT_UNSEEN=0;

  const snap=await db.ref('messages/'+city).orderByChild('ts').limitToLast(20).get();
  const items=[];
  snap.forEach(s=>items.push({key:s.key, val:s.val()||{}}));
  items.sort((a,b)=>(a.val.ts||0)-(b.val.ts||0));

  for(const it of items){
    const m=it.val;
    CHAT_OLDEST_TS = CHAT_OLDEST_TS==null ? (m.ts||0) : Math.min(CHAT_OLDEST_TS, m.ts||0);
    CHAT_NEWEST_TS = CHAT_NEWEST_TS==null ? (m.ts||0) : Math.max(CHAT_NEWEST_TS, m.ts||0);
    renderChatMessage(m, it.key, {prepend:false});
  }

  // Start at top of loaded block (read from the beginning of the last N)
  feed.scrollTop = 0;
  chatUpdateButtons();
}

function chatSubscribeLive(city){
  if(CHAT_LIVE_REF && CHAT_LIVE_CB){
    try{ CHAT_LIVE_REF.off('child_added', CHAT_LIVE_CB); }catch(e){}
  }
  CHAT_LIVE_REF=null; CHAT_LIVE_CB=null;

  const startTs=(CHAT_NEWEST_TS||0)+1;
  const ref=db.ref('messages/'+city).orderByChild('ts').startAt(startTs);
  ...
  const cb=(snap)=>{
    ...
    const atBottom=chatIsAtBottom(feed);

    renderChatMessage(m, snap.key, {prepend:false});
    ...
    // sound & notification (skip own message)
    try{
      const me=auth.currentUser;
      if(!me || m.by!==me.uid){
        playSound('chat');
        // only notify if page hidden
        if(document.hidden){
          db.ref('usersPublic/'+m.by).get().then(s=>{
            const u=s.val()||{};
  ...
  CHAT_LIVE_REF=ref; CHAT_LIVE_CB=cb;
}

async function chatLoadOlder(){
  if(CHAT_LOADING_OLDER) return;
  const feed=$('#chatFeed'); if(!feed) return;
  if(CHAT_OLDEST_TS==null) return;

  CHAT_LOADING_OLDER=true;
  const prevScroll=feed.scrollTop;
  const prevHeight=feed.scrollHeight;

  const endTs=CHAT_OLDEST_TS-1;
  const city=getCity();
  const snap=await db.ref('messages/'+city).orderByChild('ts').endAt(endTs).limitToLast(20).get();
  const items=[];
  snap.forEach(s=>items.push({key:s.key, val:s.val()||{}}));
  items.sort((a,b)=>(a.val.ts||0)-(b.val.ts||0));
  ...
  for(const it of items){
    const m=it.val;
    CHAT_OLDEST_TS = CHAT_OLDEST_TS==null ? (m.ts||0) : Math.min(CHAT_OLDEST_TS, m.ts||0);
    renderChatMessage(m, it.key, {prepend:true});
  }

  // preserve scroll position after prepend
  const newHeight=feed.scrollHeight;
  feed.scrollTop = prevScroll + (newHeight - prevHeight);

  CHAT_LOADING_OLDER=false;
}

async function loadChat(){
  const city=getCity();
  CHAT_CITY=city;
  await chatLoadInitial(city);
  chatSubscribeLive(city);
  startWallpaperSync();
}
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  $('#chatJumpBottom')?.addEventListener('click', chatJumpToBottom);
  $('#chatShowNew')?.addEventListener('click', chatJumpToBottom);

  const feed=$('#chatFeed');
  if(feed){
    feed.addEventListener('scroll', ()=>{
      chatUpdateButtons();
      if(feed.scrollTop<=12){ chatLoadOlder().catch(()=>{}); }
    }, {passive:true});
  }
});
$('#citySelect')?.addEventListener('change', ()=> loadChat().catch(console.error));
...
async function loadRent(){
  ...
  for(const x of a){
    ...
    d.innerHTML = `<div class="ava" data-uid="${esc(x.by)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}"></div>`+
      `<div class="bubble"><div class="name" data-uid="${esc(x.by)}">${esc(u.nick||'Uživatel')}</div>`+
...
(function(){
  const btn = document.getElementById('wallCamBtn');
  const input = document.getElementById('wallCamInput');
  if(!btn || !input) return;

  function applyWall(dataUrl){
    if(typeof dataUrl!=='string') return;
    document.documentElement.style.setProperty('--wall', `url('${dataUrl}')`);
    try{ localStorage.setItem('wall_'+getCity(), dataUrl); }catch(e){}
  }

  btn.addEventListener('click', async ()=>{
    // Allow any user to set local wallpaper; admin additionally syncs to all users for the city
    input.value = '';
    input.click();
  });
  ...
  input.addEventListener('change', async ()=>{
    try{
      ...
      if(u && isAdmin(u)){
        const city = getCity();
        await db.ref('settings/wallpapers/'+city).set(dataUrl);
        toast('Pozadí nastaveno (globálně)');
      }else{
        toast('Pozadí nastaveno (jen na tomto zařízení)');
      }
    }catch(e){
      console.error(e);
      toast('Chyba při změně pozadí');
    }
  });
})();
