...
// Premium settings from admin (settings/premium)
let __premiumSettingsCache = null;
async function getPremiumSettings(){
  try{
    if(__premiumSettingsCache) return __premiumSettingsCache;
    const s = await db.ref('settings/premium').get();
    const v = s.exists() ? (s.val()||{}) : {};
    __premiumSettingsCache = {
      qr: (typeof v.qr==='string' && v.qr) ? v.qr : PREMIUM_QR_IMG,
      text: (typeof v.text==='string') ? v.text : '',
      supportUid: (typeof v.supportUid==='string' && v.supportUid) ? v.supportUid : (window.ADMIN_UID||'')
    };
    return __premiumSettingsCache;
  }catch(e){
    return { qr: PREMIUM_QR_IMG, text: '', supportUid: (window.ADMIN_UID||'') };
  }
}
...
// === Admin maintenance: delete old non-critical data (14 days) ===
// Runs client-side (no backend). Triggered automatically once per 24h when admin opens the site
// and also via a manual button in admin panel.
async function adminAutoCleanup14d(isAuto=false){
  const me = firebase.auth().currentUser;
  if(!me || !window.__isAdmin) return;
  const key = 'admin_cleanup_14d_last';
  const now = Date.now();
  const last = +localStorage.getItem(key) || 0;
  if(isAuto && (now - last) < 24*60*60*1000) return;
  ...
  const getCityKeys = ()=>{
    try{
      ...
      if(!sel) return ['praha'];
      const vals = Array.from(sel.querySelectorAll('option')).map(o=>String(o.value||'').trim()).filter(Boolean);
      return vals.length ? vals : ['praha'];
    }catch(e){ return ['praha']; }
  };

  const deleteOlder = async (refPath, tsKey='ts', batch=250)=>{
    const ref = db.ref(refPath);
    let total=0;
    while(true){
      const snap = await ref.orderByChild(tsKey).endAt(cutoff).limitToFirst(batch).get();
      const v = snap.val();
      if(!v) break;
      const upd={};
      for(const k of Object.keys(v)) upd[k]=null;
      await ref.update(upd);
      total += Object.keys(upd).length;
      if(Object.keys(upd).length < batch) break;
    }
    return total;
  };

  const deleteNestedOlder = async (rootPath, nestedKey='ts', batch=200)=>{
    // root has children lists; we clean each child list by ts
    const root = db.ref(rootPath);
    const rootSnap = await root.get();
    const rootVal = rootSnap.val() || {};
    let total=0;
    for(const child of Object.keys(rootVal)){
      try{ total += await deleteOlder(rootPath+'/'+child, nestedKey, batch); }catch(e){}
    }
    return total;
  };

  try{ toast((isAuto?'Auto ':'')+'Čistím stará data (14 dní)…'); }catch(e){}
  let removed=0;
  try{
    // Global-ish lightweight paths
    removed += await deleteNestedOlder('notifications', 'ts', 250);
  }catch(e){}
  try{
    // Bot inbox for this admin
    removed += await deleteNestedOlder('botsInbox/'+me.uid, 'ts', 250);
  }catch(e){}
  try{
    // Chat + rent messages (per city)
    for(const c of getCityKeys()){
      try{ removed += await deleteOlder('messages/'+c, 'ts', 300); }catch(e){}
      try{ removed += await deleteOlder('rentMessages/'+c, 'ts', 300); }catch(e){}
    }
  }catch(e){}

  try{ localStorage.setItem(key, String(Date.now())); }catch(e){}
  try{
    const el = document.getElementById('adminCleanupLast');
    if(el) el.textContent = new Date().toLocaleString();
  }catch(e){}
  try{ toast('Hotovo. Smazáno: '+removed); }catch(e){}
}
...
function cookieBanner(){
  ...
  const setChoice = async (val)=>{
    ...
    // Must be triggered by user gesture (the consent click) to unlock sounds.
    try{
      if(val !== 'reject') await unlockAudio();
    }catch(e){}

    // After cookie decision, ask notifications once.
    try{
      if(val !== 'reject') await ensureNotifications();
...
function showView(id){
  // IMPORTANT: avoid duplicate listeners when switching tabs.
  try{
    const prev = window.__currentViewId || null;
    if(prev && prev!==id){
      if(prev==='view-chat' && typeof offChat==='function'){ try{ offChat(); }catch(e){} offChat=null; }
      if(prev==='view-members' && typeof offMembers==='function'){ try{ offMembers(); }catch(e){} offMembers=null; }
      if(prev==='view-friends' && FR_REQ_REF){ try{ FR_REQ_REF.off(); }catch(e){} FR_REQ_REF=null; }
      // DM threads are loaded via .get() (no live listener). Room listener is managed inside openDMRoom.
    }
    window.__currentViewId = id;
  }catch(e){}
  ...
  try{
    if(id==='view-chat'){
      ...
      if(!already) setMiniLoad('chatMiniLoad','Načítáme chat…', true);
      else { /* chat already painted from cache */ setMiniLoad('chatMiniLoad','', false);
            try{ hidePreloader(); }catch(e){}
            try{ const p=document.getElementById('preloader'); if(p){ setTimeout(()=>{ try{ p.style.display='none'; }catch(_){} }, 180); } }catch(e){} }
    }
    ...
    if(id==='view-cabinet'){
      try{ openCabinet && openCabinet(); }catch(e){}
      return;
    }
    if(id==='view-map') setMiniLoad('mapPointsMiniLoad','Načítáme mapu…', true);
    if(id==='view-admin'){ setMiniLoad('adminUsersMiniLoad','Načítáme…', true); try{ __adminSelectTab('users');
      try{ initAdminUserPicker && initAdminUserPicker(); }catch(e){}
      try{ loadAdminTickets && loadAdminTickets(); }catch(e){} }catch(e){} }
  }catch(e){}
  ...
  try{
    ...
    ref.on('value', async (snap)=>{
      try{
        ...
        // update UI (public numbers are intentionally amplified x5 for 'busy' feel)
        const dTotal = Math.round(total*5);
        const dEmployers = Math.round(employers*5);
        const dWorkers = Math.round(workers*5);
...
async function fetchUserPublic(uid){
  // Public profile is the canonical source for everyone.
  try{
  ...
  // Private profile (/users) should be readable only by self or admin.
  try{
    const me = auth.currentUser;
    const isAdmin = !!(me && (me.email === 'urciknikolaj642@gmail.com' || me.email === 'urciknikolaj62@gmail.com'));
    if(me && (me.uid === uid || isAdmin)){
      const uSnap = await db.ref('users/'+uid).get();
      const u = uSnap.val();
      if(u) return { nick: u.name || u.nick || 'Uživatel', email: u.email, avatar: u.avatar, role: u.role, plan: u.plan, phone: u.phone, about: u.about, skills: u.skills, company: u.company };
    }
  }catch(e){
    // ignore
  }
...
async function googleSignIn(){
  ...
  // Always use redirect (most reliable; avoids popup blockers and COOP window.close issues).
  ...
  try{
    ...
    if(code.includes('unauthorized-domain')){
      const host = (location && location.host) ? location.host : 'tento web';
      toast('Google: nepovolená doména. Přidejte do Firebase Auth → Settings → Authorized domains: '+host);
    }else{
      toast(e?.message || 'Chyba Google přihlášení.');
    }
    try{ window.__GOOGLE_AUTH_IN_FLIGHT__ = false; }catch(_e){}
  }
}
    ...
    .catch((e)=>{
      try{
        ...
        if(code.includes('popup') || code.includes('redirect')){
          ...
          toast((e && e.code ? ('Google chyba: '+e.code+' — ') : 'Google chyba: ') + (e?.message||'Chyba přihlášení.'));
        }
      }catch(_e){}
...
function hidePreloader(){
  ...
  // Fully remove so it never blocks clicks (z-index overlays on some browsers)
  try{
    setTimeout(()=>{
      try{ p.style.display='none'; }catch(_e){}
      try{ p.remove(); }catch(_e){}
    }, 250);
  }catch(e){}
}
...
async function loadChat(){
  ...
  try{
    ...
    window.__chatSyncTimer = setTimeout(()=>{
      ...
      if(ml && ml.style.display!=='none'){
        setMiniLoad('chatMiniLoad','Zprávy jsou aktuální', true);
  ...
  const __markChatReady = ()=>{
    ...
    // Lazy loading: do NOT prefetch other heavy tabs on first load.
    // We'll initialize DM/friends/members/map only when the user opens those views.
  };
    ...
    refFast.once('value').then((s)=>{
      ...
      if(__chatBootTarget === 0 && !__chatBootDone){
        __chatBootDone = true;
          ...
          try{ hidePreloader(); }catch(e){}
          try{ const p=document.getElementById('preloader'); if(p){ setTimeout(()=>{ try{ p.style.display='none'; }catch(_){} }, 180); } }catch(e){}
          setTimeout(()=>{ try{ __chatAttachFull20(); }catch(e){} }, 120);
        if(stopSeq) stopSeq();
      }
    }).catch(()=>{
      ...
      if(snap && snap.key && !__chatBootDone){
        if(!__chatBootKeys.has(snap.key)){
          ...
          if(__chatBootKeys.size >= __chatBootTarget){
            __chatBootDone = true;
            setMiniLoad('chatMiniLoad','', false);
            // First meaningful paint of the app: remove the full-screen preloader.
            try{ if(!window.__PRELOADER_HIDDEN__){ window.__PRELOADER_HIDDEN__=true; hidePreloader(); } }catch(e){}
...
const m=snap.val()||{};
    ...
    // Safety: if for any reason the boot gating didn't fire, never keep the full-screen preloader once messages render.
    try{ if(!window.__PRELOADER_HIDDEN__){ window.__PRELOADER_HIDDEN__=true; hidePreloader(); } }catch(e){}
  ...
  try{
    db.ref('messages/'+city).limitToLast(5).once('value').then((s5)=>{
      ...
      setTimeout(()=>{ try{ __switchChatToFull(); }catch(e){} }, 220);
    }).catch(()=>{});
  }catch(e){}
  ...
  setTimeout(__switchChatToFull, 1200);
...
function showPremiumBotPanel(room){
  ...
  if(st.step==='choose'){
    ...
    const introBase = `Ahoj! Jsem bot pro nákup privilegií.
...
Po výběru ti pošlu QR kód. Zaplať a pošli sem fotku/printscreen platby, pak stiskni "Podat žádost".`;
    // Use admin-configured text (settings/premium/text) if available.
    getPremiumSettings().then((cfg)=>{
      const full = (cfg && cfg.text) ? (introBase + `\n\n---\n` + cfg.text) : introBase;
      ensureBotIntro(room, full);
    }).catch(()=>ensureBotIntro(room, introBase));
...
async function botSendQR(room){
  const cfg = await getPremiumSettings();
  const msg = cfg.text ? (cfg.text + '\n\nProsím zaplaťte pomocí QR a pošlete sem fotku/printscreen platby.')
                      : 'Prosím zaplaťte pomocí QR a pošlete sem fotku/printscreen platby.';
  await botSay(room, msg);
  await db.ref('privateMessages/'+room).push({by: auth.currentUser.uid, botUid: PREMIUM_BOT_UID, img: (cfg.qr||PREMIUM_QR_IMG), text:'QR platba', ts: Date.now(), bot:true});
}
...
async function openPremiumBotDM(planCode){
  ...
  try{
    ...
    showView('view-dm');
    // Mobile UX: open DM as a bottom-sheet overlay (so it opens immediately on phones).
    try{ setTimeout(()=>{ if(window.openDmMobile) window.openDmMobile(); }, 0); }catch(e){}
  ...
  try{
    ...
    await ref.set({
      ...
      bot: true,
      ts: now,
      text: `Dobrý den! Vybral(a) jste: ${planText}.\n\n1) Zaplaťte podle QR.\n2) Klikněte „Poslat žádost“ a nahrajte screenshot platby.\n3) Admin platbu potvrdí a tarif se aktivuje okamžitě.\n\nPokud chcete, napište mi: VIP / PREMIUM a váš e-mail.`
    });
    // QR as second bot message with image
    const ref2 = db.ref('privateMessages/'+room).push();
    await ref2.set({ by: PREMIUM_BOT_UID, bot:true, ts: now+1, text:'QR pro platbu:', photo: PREMIUM_QR_IMG });
  }catch(e){
    console.warn(e);
  }
}
...
document.getElementById('drawerCabinet')?.addEventListener('click', (e)=>{ e.preventDefault(); if(!auth.currentUser){ openAuth(); return; } showView('view-cabinet'); try{ if(typeof closeDrawer==='function') closeDrawer(); }catch(_e){} });
...
// Admin actions
function __pickedUid(){
  const sel = document.getElementById('adminUserPick');
  const v = (sel && sel.value) ? String(sel.value).trim() : '';
  return v || '';
}

// Fill dropdown by nick (limit ~300 last active)
async function initAdminUserPicker(){
  const sel = document.getElementById('adminUserPick');
  if(!sel) return;
  sel.innerHTML = '<option value="">— vyber uživatele —</option>';
  try{
    const snap = await db.ref('usersPublic').orderByChild('lastActive').limitToLast(300).get();
    const v = snap.val()||{};
    const rows = Object.entries(v).map(([uid,u])=>({uid, u:u||{}}));
    rows.sort((a,b)=> (String(a.u.nick||'').localeCompare(String(b.u.nick||''), 'cs')));
    for(const r of rows){
      const opt = document.createElement('option');
      opt.value = r.uid;
      opt.textContent = `${r.u.nick||'Uživatel'} · ${r.uid.slice(0,6)}`;
      sel.appendChild(opt);
    }
  }catch(e){
    // fallback: empty
...
async function loadAdminTickets(){
  const me = auth.currentUser; if(!me || !isAdmin(me)) return;
  const box = document.getElementById('adminTicketsList');
  if(!box) return;
  box.innerHTML = '<div class="muted">Načítám…</div>';
  try{
    const snap = await db.ref('support/tickets').orderByChild('ts').limitToLast(80).get();
    const v = snap.val()||{};
    const items = Object.entries(v).map(([id,t])=>({id, t:t||{}})).sort((a,b)=>(b.t.ts||0)-(a.t.ts||0));
    box.innerHTML = '';
    if(!items.length){ box.innerHTML = '<div class="muted">Zatím žádné záznamy.</div>'; return; }
    for(const it of items){
      const u = it.t.uid ? await getUser(it.t.uid) : null;
      const row = document.createElement('div');
      row.className = 'card';
      row.style.padding = '10px';
      row.innerHTML = `<div class="row" style="justify-content:space-between;gap:10px">
        <div class="row" style="gap:10px;margin:0">
          <div class="ava" style="width:28px;height:28px"><img src="${esc(u?.avatar||window.DEFAULT_AVATAR)}"></div>
          <div><b>${esc(u?.nick||it.t.email||'Uživatel')}</b><div class="muted small">${fmtTime(it.t.ts||0)} · ${esc(it.t.type||'ticket')}</div></div>
        </div>
        <div class="muted small">${esc(it.id)}</div>
      </div>
      <div class="text" style="margin-top:8px;white-space:pre-wrap">${esc(it.t.text||'')}</div>
      ${it.t.img?`<div class="text" style="margin-top:8px"><img src="${esc(it.t.img)}"></div>`:''}`;
      box.appendChild(row);
    }
  }catch(e){
    box.innerHTML = '<div class="muted">Chyba načítání.</div>';
  }
}
document.getElementById('adminTicketsReload')?.addEventListener('click', ()=>{ try{ loadAdminTickets(); }catch(e){} });

document.getElementById('adminOpenBroadcast')?.addEventListener('click', ()=>{
  try{ openModal('broadcastModal'); }catch(e){}
});
document.getElementById('adminOpenMapPoints')?.addEventListener('click', ()=>{
  try{ openModal('adminMapPointsModal'); }catch(e){}
});
document.getElementById('botInboxOpen')?.addEventListener('click', ()=>{
  try{ openModal('botInboxModal'); loadBotInboxModal && loadBotInboxModal(); }catch(e){}
});

document.getElementById('adminOpenPickedProfile')?.addEventListener('click', async ()=>{
  const uid = __pickedUid(); if(!uid) return toast('Vyber uživatele');
  try{
    if(window.showUserCard) await window.showUserCard(uid);
    else if(window.adminOpenUserCard) await window.adminOpenUserCard(uid);
  }catch(e){}
});
...
$('#ban60')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  const reason=$('#banReason').value.trim()||'porušení pravidel';
  await db.ref('bans/'+uid).set({until: Date.now()+60*60*1000, reason}); toast('Ban 60 min');
});
$('#ban24')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  const reason=$('#banReason').value.trim()||'porušení pravidel';
  await db.ref('bans/'+uid).set({until: Date.now()+24*60*60*1000, reason}); toast('Ban 24 h');
});
$('#ban250')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  const reason=$('#banReason').value.trim()||'porušení pravidel';
  await db.ref('bans/'+uid).set({until: Date.now()+250*60*60*1000, reason}); toast('Ban 250 h');
});
$('#ban1000')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  const reason=$('#banReason').value.trim()||'porušení pravidel';
  await db.ref('bans/'+uid).set({until: Date.now()+1000*60*60*1000, reason}); toast('Ban 1000 h');
});
$('#mute60')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  const reason=$('#banReason').value.trim()||'spam';
  await db.ref('mutes/'+uid).set({until: Date.now()+60*60*1000, reason}); toast('Mute 60 min');
});
$('#mute24')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  const reason=$('#banReason').value.trim()||'spam';
  await db.ref('mutes/'+uid).set({until: Date.now()+24*60*60*1000, reason}); toast('Mute 24 h');
});
$('#mute250')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  const reason=$('#banReason').value.trim()||'spam';
  await db.ref('mutes/'+uid).set({until: Date.now()+250*60*60*1000, reason}); toast('Mute 250 h');
});
$('#mute1000')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  const reason=$('#banReason').value.trim()||'spam';
  await db.ref('mutes/'+uid).set({until: Date.now()+1000*60*60*1000, reason}); toast('Mute 1000 h');
});
$('#unmute')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  await db.ref('mutes/'+uid).remove(); toast('Unmute');
});
$('#vip7')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  await db.ref('usersPublic/'+uid).update({plan:'vip', planUntil: Date.now()+7*24*60*60*1000}); toast('VIP 7 dní');
});
$('#vip30')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  await db.ref('usersPublic/'+uid).update({plan:'premium', planUntil: Date.now()+30*24*60*60*1000}); toast('VIP 30 dní');
});
$('#unvip')?.addEventListener('click', async ()=>{
  const me=auth.currentUser; if(!me || !isAdmin(me)) return alert('Jen admin');
  const uid=__pickedUid(); if(!uid) return toast('Vyber uživatele');
  await db.ref('usersPublic/'+uid).update({plan:'free', planUntil:null}); toast('VIP OFF');
});
...
function _setAdminUI(is, user){
  ...
  // Auto DB cleanup once per 24h when admin opens the site.
  try{
    if(!prev && window.__isAdmin){
      // defer so UI can render first
      setTimeout(()=>{ try{ adminAutoCleanup14d && adminAutoCleanup14d(true); }catch(e){} }, 1200);
    }
  }catch(e){}
}
...
// profile display
  if(u){
    ...
    setPresence(u);
    try{ startPremiumPresence(); }catch(e){}
    await refreshMe();
    ...
    // Full-screen preloader is hidden by the first chat paint (loadChat). This avoids a blank screen.
    try{ maybeStartTour(); }catch(e){}

  }else{
    try{ stopPremiumPresence(); }catch(e){}
...
// Admin cleanup button + last run label
window.addEventListener('DOMContentLoaded', ()=>{
  try{
    const b=document.getElementById('adminCleanupNow');
    if(b) b.addEventListener('click', ()=>adminAutoCleanup14d(false));
    const last=+localStorage.getItem('admin_cleanup_14d_last')||0;
    const el=document.getElementById('adminCleanupLast');
...
document.getElementById('authGoogleBtn')?.addEventListener('click', async ()=>{
  ...
  async function renderFriendCard(uid, st){
    let u = {}; try{ u = await getUser(uid); }catch(e){ u = {nick:'Uživatel', avatar: window.DEFAULT_AVATAR}; }
    const wrap=document.createElement('div'); wrap.className='msg';
    wrap.innerHTML = `
  ...
  async function loadFriendsUI(){
    ...
    const frEntries = Object.entries(fr).filter(([_,st])=>st===true || st==='accepted');
...
let NOTIF_REF=null;
  ...
  function __updatePremiumUI(planVal){
    try{
      const pv = String(planVal||'').toLowerCase().replace(/\s+/g,'');
      const isCab = ['premium','premium+','premiumplus','premium_plus'].includes(pv);
      const cab = document.getElementById('drawerCabinet');
      if(cab) cab.style.display = isCab ? '' : 'none';
      const fab = document.getElementById('fabPremium');
      if(fab) fab.style.display = isCab ? '' : 'none';
    }catch(e){}
  }
...
// --- DM threads / inbox ---
  ...
  async function loadDmThreads(){
    ...
    try{
      ...
      // IMPORTANT: reset container before rendering to avoid duplicates on repeated loads
...
// Premium presence (so bots can run without admin, but only while at least one Premium/Premium+ user is online)
let __PREM_PRES_TIMER=null;
function startPremiumPresence(){
  const me = auth.currentUser; if(!me) return;
  db.ref('usersPublic/'+me.uid+'/plan').get().then((s)=>{
    const up = {plan: s.val()||''};
    if(!isPremiumCabinet(up) && !isAdminUser(me) && !isAdmin(me)) return;
    const ref = db.ref('presencePremium/'+me.uid);
    const beat = async()=>{ try{ await ref.set({ts: Date.now()}); }catch(e){} };
    try{ ref.onDisconnect().remove(); }catch(e){}
    if(__PREM_PRES_TIMER) clearInterval(__PREM_PRES_TIMER);
    __PREM_PRES_TIMER = setInterval(beat, 30000);
    beat();
  });
}
function stopPremiumPresence(){
  const me = auth.currentUser; if(!me) return;
  try{ db.ref('presencePremium/'+me.uid).remove(); }catch(e){}
  if(__PREM_PRES_TIMER){ clearInterval(__PREM_PRES_TIMER); __PREM_PRES_TIMER=null; }
}
async function _hasAnyPremiumOnline(){
  try{
    const cut = Date.now()-90000;
    const snap = await db.ref('presencePremium').orderByChild('ts').startAt(cut).limitToLast(5).get();
    return snap.exists();
  }catch(e){
    return false;
  }
...
async function _tryAcquireBotHost(){
  const me = auth.currentUser;
  if(!me) return false;
  // Only Premium/Premium+ (or admin) clients can host bot engine
  try{
    const up = await getUserPublic(me.uid);
    if(!isPremiumCabinet(up) && !isAdminUser(me) && !isAdmin(me)) return false;
  }catch(e){ return false; }
  // If nobody Premium is online -> stop hosting
  if(!(await _hasAnyPremiumOnline())) return false;
...
// User bots host tick (posts on behalf of Premium users; by=ownerUid)
async function _userBotsHostTick(){
  try{
    const now = Date.now();
    const snap = await db.ref('userBots').get();
    const tree = snap.val()||{};
    for(const [ownerUid, bots] of Object.entries(tree)){
      if(!bots) continue;
      // Only run for owners that still have Premium/Premium+
      let ownerUp=null;
      try{ ownerUp = await getUserPublic(ownerUid); }catch(e){}
      if(!isPremiumCabinet(ownerUp)) continue;

      for(const [botId, b] of Object.entries(bots)){
        if(!b || !b.active) continue;
        const intervalMs = Math.max(60_000, Number(b.intervalMin||30)*60_000);
        const nextAt = Number(b.nextAt||0);
        if(nextAt && now < nextAt) continue;

        await userBotPostForOwner(ownerUid, botId, b);
        // schedule next
        await db.ref('userBots/'+ownerUid+'/'+botId).update({lastTs: now, nextAt: now + intervalMs});
        // one post per tick to avoid spam load
        return;
      }
    }
  }catch(e){
    console.warn('userBotsHost tick', e);
  }
}
  ...
  async function loadBotInboxModal(){
    ...
    const renderFor = async (botUid)=>{
      ...
      __BOT_INBOX_REF.on('value', async (snap)=>{
        const vv=snap.val()||{};
        // Group messages by sender -> show dialog list (like messenger) inside each bot.
        const byUser = {};
        for(const [k,it0] of Object.entries(vv)){
          const it = it0||{};
          const from = it.from||'';
          if(!from) continue;
          (byUser[from] ||= []).push({k, ...it});
        }
        // Sort dialogs by last message ts
        const dialogs = Object.entries(byUser)
          .map(([uid,arr])=>{
            arr.sort((a,b)=>(a.ts||0)-(b.ts||0));
            const last = arr[arr.length-1];
            return {uid, arr, lastTs: (last?.ts||0), last};
          })
        ...
        listWrap.className = 'feed';
        listWrap.style.gap = '10px';

        const openDialog = async (peerUid)=>{
          const d = dialogs.find(x=>x.uid===peerUid);
          if(!d) return;
          // render conversation thread for this user
          box.innerHTML='';
          const head=document.createElement('div');
          head.className='row';
          head.style.justifyContent='space-between';
          const peer = await getUser(peerUid);
          head.innerHTML = `<div class="row" style="gap:10px;margin:0">
              <div class="ava" style="width:34px;height:34px"><img src="${esc(peer?.avatar||window.DEFAULT_AVATAR)}"></div>
              <div><b>${esc(peer?.nick||'Uživatel')}</b><div class="muted small">Dialog s botem</div></div>
            </div>
            <div class="row" style="margin:0;gap:8px">
              <button class="ghost" data-act="back" type="button">← Zpět</button>
              <button class="ghost" data-act="open" type="button">Otevřít DM</button>
            </div>`;
          ...
          thread.className='feed';
          thread.style.marginTop='10px';
          const arr = (d.arr||[]).slice().sort((a,b)=>(a.ts||0)-(b.ts||0));
          for(const it of arr){
            const el=document.createElement('div');
            el.className='msg';
            el.innerHTML = `
              <div class="bubble" style="width:100%">
                <div class="meta"><span class="muted">${new Date(it.ts||0).toLocaleString()}</span>
                  <button class="ghost" data-act="del" data-key="${esc(it.k)}" type="button">Smazat</button>
                </div>
            ...
            el.addEventListener('click', async (e)=>{
              ...
              if(act==='del'){
                ...
                if(key) await db.ref('botsInbox/'+adminUid+'/'+botUid+'/'+key).remove();
              }
            });
        ...
        for(const d of dialogs){
          const fromU = await getUser(d.uid);
          if(term){
            const nn = (fromU?.nick||'').toString().toLowerCase();
            if(!nn.includes(term)) continue;
          }
          const lastText = (d.last?.text||'').toString();
          const preview = lastText ? lastText.slice(0,64) : (d.last?.img ? '[image]' : '');
          const el=document.createElement('div');
          el.className='msg';
          el.innerHTML = `
            <div class="ava" data-uid="${esc(d.uid)}"><img src="${esc(fromU?.avatar||window.DEFAULT_AVATAR)}"></div>
            <div class="bubble" style="width:100%">
              <div class="meta"><b>${esc(fromU?.nick||'Uživatel')}</b><span class="time">${fmtTime(d.lastTs)}</span></div>
              <div class="muted small">${esc(preview)}</div>
              <div class="muted small">${d.arr.length} zpráv</div>
            </div>`;
          el.addEventListener('click', ()=>openDialog(d.uid));
          listWrap.appendChild(el);
        }
        if(!listWrap.children.length){
          box.innerHTML='<div class="muted">Nenalezeno.</div>';
          return;
        }
        box.appendChild(listWrap);
      });
...
function initAdminSettings(){
  ...
  try{
    db.ref('settings/premium').on('value', (s)=>{
      const v=s.val()||{};
      try{ __premiumSettingsCache = null; }catch(e){}
...
function initInfoPages(){
  ...
  document.getElementById('drawerHelpSupport')?.addEventListener('click', (e)=>{ e.preventDefault(); openModal('supportModal'); try{ window.__closeDrawer?.(); }catch{} });
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  // NOTE: keep all UI bootstrap above. Everything below must stay in the global scope.
});
...
function isPremiumPlan(u){
  const planRaw = (u && (u.plan||u.premiumPlan||u.vipPlan)) || '';
  const plan = String(planRaw).toLowerCase().replace(/\s+/g,'');
  return ['premium','premium+','premiumplus','premium_plus','vip150','vip','vip_worker','vip100','premium200'].includes(plan);
}
function isPremiumCabinet(u){
  const planRaw = (u && (u.plan||u.premiumPlan||u.vipPlan)) || '';
  const plan = String(planRaw).toLowerCase().replace(/\s+/g,'');
  return ['premium','premium+','premiumplus','premium_plus'].includes(plan);
}
...
async function openCabinet(){
  ...
  if(!isPremiumCabinet(up)){
    toast('Kabinet je pouze pro Premium/Premium+');
    showView('view-chat');
    return;
  }
  // Render DM bots list
  const box = document.getElementById('cabinetBots');
  if(box){
    const bots = [
      {uid: PREMIUM_BOT_UID, title:'Bot — Privilegia', desc:'Nákup / aktivace tarifu'},
    ];
    box.innerHTML = '';
    for(const b of bots){
      const u = await getUser(b.uid);
      const row = document.createElement('div');
      row.className = 'row card';
      row.style.padding = '10px';
      row.style.justifyContent = 'space-between';
      row.innerHTML = `<div class="row" style="gap:10px;margin:0">
          <div class="ava" style="width:34px;height:34px"><img src="${esc(u?.avatar||window.DEFAULT_AVATAR)}"></div>
          <div><b>${esc(u?.nick||b.title)}</b><div class="muted small">${esc(b.desc||'')}</div></div>
        </div>
        <button class="btn primary" type="button">Otevřít DM</button>`;
      row.querySelector('button')?.addEventListener('click', async ()=>{
        await openDMRoom(uid, b.uid);
        showView('view-dm');
        try{ setTimeout(()=>{ if(window.openDmMobile) window.openDmMobile(); }, 0); }catch(e){}
      });
      box.appendChild(row);
    }
    if(!bots.length) box.innerHTML='<div class="muted">Zatím žádní boti.</div>';
  }
  // Wire cabinet user bots
  document.getElementById('cabinetBotAdd')?.addEventListener('click', async ()=>{
    if(!auth.currentUser) return;
    const id = db.ref('userBots/'+uid).push().key;
    await db.ref('userBots/'+uid+'/'+id).set({nick:'Reklama', city:'Praha', intervalMin:30, templates:['[REKLAMA] Dobrý den!'], active:false, ts:Date.now()});
    await renderUserBots();
  }, {once:true});
  document.getElementById('cabinetBotRefresh')?.addEventListener('click', ()=>{ try{ renderUserBots(); }catch(e){} }, {once:true});
  await renderUserBots();
}
...
function startActiveUserBots(bots){
  // Client-side intervals disabled: user bots are executed by the shared host lock,
  // so they can continue while any Premium/Premium+ user is online.
  stopAllUserBots();
}
async function userBotPostForOwner(ownerUid, botId, b){
  ...
  const msg = {
    ...
    by: ownerUid,              // IMPORTANT: writes as the real user UID
    bot:true,
    botOwner: ownerUid,
    botId,
    ad:true
...
// (end of premium cabinet bootstrap)
// =====================
...
async function adminLoadUsers(fromSearch){
  ...
  for(const u of filtered.slice(0,120)){
    ...
    row.innerHTML = `
      <div class="userMain">
        <div class="userNick"><b>${esc(u.nick||'Uživatel')}</b> <span class="muted">${esc(u.role||'')}</span></div>
...
// === Drawer Help submenu (Pomoc / Kontakt / Stížnost) ===
(function(){
  const btn = document.getElementById('drawerHelpSupport');
  const box = document.getElementById('helpSubmenu');
  if(!btn || !box) return;
  ...
  document.getElementById('helpOpenHelp')?.addEventListener('click', (e)=>{ e.preventDefault(); box.hidden=true; showView('view-help'); try{ closeDrawer?.(); }catch(_){} });
  document.getElementById('helpOpenContacts')?.addEventListener('click', (e)=>{ e.preventDefault(); box.hidden=true; showView('view-contacts'); try{ closeDrawer?.(); }catch(_){} });
  document.getElementById('helpOpenComplaint')?.addEventListener('click', (e)=>{ e.preventDefault(); box.hidden=true; showView('view-complaint'); try{ closeDrawer?.(); }catch(_){} });
})();
