...
async function googleSignIn(){
  ...
  // Always use redirect (mobile + desktop) to avoid popup blockers / COOP issues.
...
async function handleForgot(){
  ...
  try{
    ...
    toast('Odesláno. Zkontroluj e‑mail (i spam).');
  }catch(e){
    console.warn('forgot error', e);
    toast(e?.message || 'Chyba. Nelze odeslat reset.');
  }
}
...
async function resolveUidByEmail(email){
  // emails/* is disabled for privacy + rules stability.
  return null;
}
let DM_REF=null;
...
document.getElementById('authGoogleBtn')?.addEventListener('click', async ()=>{
  ...
  document.getElementById('friendAddBtn')?.addEventListener('click', async ()=>{
    ...
    if(me.emailVerified===false){
      ...
      const uid=null; // email lookup disabled
