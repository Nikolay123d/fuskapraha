...
export async function openView(view, opts = {}) {
  ...
  if (requireAuthForView(view) && !isAuthed()) {
    // Guest mode: bounce to chat.
    // IMPORTANT: on restore (F5) we do NOT force-open auth overlay.
    if (!opts.restore) {
      try{ openAuth(); }catch{}
    }
    view = 'chat';
...
export function restoreAfterReload() {
  ...
  if (s.view === 'dm' && s.dm && s.dm.room) {
    return openView('dm', { room: s.dm.room, restore: true });
  }
  if (s.view) return openView(s.view, { restore: true });
  return openView('chat', { restore: true });
}
