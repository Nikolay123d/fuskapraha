...
// ---- Auth wiring guard ----
// Attach onAuthStateChanged exactly once (prevents double init + broken F5 restore).
export function wireAuthOnce(auth, cb){
  try{
    if (!auth || typeof auth.onAuthStateChanged !== 'function') return ()=>{};
    if (window.__authStateUnsub) return window.__authStateUnsub;
    const unsub = auth.onAuthStateChanged((u)=>{
      try{ cb && cb(u); }catch(e){}
    });
    window.__authStateUnsub = typeof unsub === 'function' ? unsub : ()=>{};
    return window.__authStateUnsub;
  }catch(e){
    return ()=>{};
  }
}
