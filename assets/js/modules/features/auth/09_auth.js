...
export async function googleSignIn(){
  const { auth } = getAccess();
  if(!auth) throw new Error("[auth] Firebase auth not available");
...
async function emailLogin(){
  ...
  // Client-side rate limit: 3 failed tries => 5 min cooldown (best-effort; not a server-side protection)
  const keyBase = `mk_email_login_${email.toLowerCase()}`;
  const lockUntil = Number(localStorage.getItem(keyBase + '_lockUntil') || '0');
  const now = Date.now();
  if(lockUntil && now < lockUntil){
    const s = Math.ceil((lockUntil - now)/1000);
    return alert(`Zkus to později. Pauza: ${s}s`);
  }

  try{
    await auth.signInWithEmailAndPassword(email, pass);
    localStorage.removeItem(keyBase + '_fails');
    localStorage.removeItem(keyBase + '_lockUntil');
    closeAuth();
  }catch(e){
    const fails = Number(localStorage.getItem(keyBase + '_fails') || '0') + 1;
    localStorage.setItem(keyBase + '_fails', String(fails));
    if(fails >= 3){
      localStorage.setItem(keyBase + '_lockUntil', String(now + 5*60*1000));
      localStorage.setItem(keyBase + '_fails', '0');
    }
    throw e;
  }
}
