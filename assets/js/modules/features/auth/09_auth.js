...
export function openAuthIfNeeded(){
  try{
    ...
    const auth = access && access.auth ? access.auth : null;
    const user = auth ? auth.currentUser : null;
    if(!user){
      const s = getState();
      const view = s.view || "chat";
      const needs = (view==="dm" || view==="admin" || view==="friends" || view==="profile" || view==="premium");
      if(needs) openAuth();
    }
  }catch(e){
    // never crash app because of auth overlay logic
    console.warn('[auth] openAuthIfNeeded failed', e);
  }
}
