...
export function initDM(){
  ...
  // initial mode: follow mk_state (SSOT), do NOT clobber room restore
  try{
    const s = JSON.parse(localStorage.getItem("mk_state")||"{}");
    if(s && s.view==="dm" && s.dmMode==="room" && s.room && s.peer){
      // Restore room directly
      openRoom(s.room, s.peer, {restore:true});
    }else{
      // Default to inbox; skip state write to avoid overwriting a room state during boot races
      showInbox({skipStateWrite:true});
    }
  }catch(e){
    showInbox({skipStateWrite:true});
  }
...
async function showInbox(opts={}){
  ...
  // By default we persist state; during init/restore we may skip to avoid clobbering room restore.
  if(!opts.skipStateWrite){
