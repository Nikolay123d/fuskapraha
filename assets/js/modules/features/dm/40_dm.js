...
export function stopDMListeners(){
...
export function initDM(){
  ...
  box.innerHTML = `
    <div class="dm-shell">
      <div class="card dm-inbox" id="dmInboxCard">
        <div class="dm-inbox-head">
          <div class="dm-title">Konverzace</div>
          <button class="btn sm" id="dmNew">+ Nová</button>
        </div>
        <div class="muted sm">Tip: klikněte na konverzaci pro otevření historie.</div>
        <div id="dmThreads" class="dm-threads"></div>
      </div>

      <div class="card dm-room" id="dmRoomWrap">
        <div class="dm-header">
          <button class="btn sm ghost dm-back" id="dmBack" type="button">⬅</button>
          <div class="dm-peer">
            <div class="dm-peer-name" id="dmPeerName">Osobní zprávy</div>
            <div class="dm-peer-sub muted sm" id="dmPeerSub"></div>
          </div>
          <button class="btn sm ghost" id="dmClear" type="button">Vyčistit</button>
        </div>

        <div class="dm-feedwrap" id="dmFeedWrap">
          <div id="dmFeed" class="dm-feed"></div>
          <div class="mini-load" id="dmMiniLoad" style="display:none"><div class="mini-status">Načítáme zprávy…</div><div class="mini-tip"></div></div>
          <button class="scroll-down" id="dmScrollDown" style="display:none" title="Dolů" type="button">⬇</button>
        </div>

        <div class="dm-composer">
          <input id="dmText" placeholder="Zpráva…" class="dm-input">
          <label class="dm-attach" title="Foto">📎<input id="dmProof" type="file" accept="image/*"></label>
          <button class="btn dm-send" id="dmSend">Odeslat</button>
        </div>
      </div>
    </div>`;
  ...
  box.querySelector("#dmClear").addEventListener("click", clearRoom);
  box.querySelector("#dmNew").addEventListener("click", openNewDM);
  box.querySelector("#dmProof").addEventListener("change", sendProofImage);
...
async function showInbox(){
  ...
  const inbox = document.getElementById("dmInboxCard");
  const roomWrap = document.getElementById("dmRoomWrap");
  
  roomWrap.classList.add("hidden");
  inbox.classList.remove("hidden");
  __dmRoom=null; __peer=null;
  localStorage.setItem("mk_state", JSON.stringify({view:"dm", dmMode:"inbox"}));
...
async function openRoom(room, peerUid, opts={}){
  ...
  localStorage.setItem("mk_state", JSON.stringify({view:"dm", dmMode:"room", room, peer: peerUid}));
  ...
  try{ const pu = await getUser(peerUid); document.getElementById("dmPeerName").textContent = pu?.nick ? ("DM: "+pu.nick) : "Osobní zprávy"; document.getElementById("dmPeerSub").textContent = pu?.email?pu.email:""; }catch(e){}
...
function openNewDM(){
  try{
    const peer = prompt("Zadej UID uživatele:");
    if(!peer) return;
    const u = firebase.auth().currentUser;
    if(!u) return alert("Login required");
    const room = [u.uid, peer].sort().join("_");
    openRoom(room, peer);
...
async function clearRoom(){
  try{
    if(!__dmRoom) return;
    // MVP: local clear only (DB remove может быть запрещён правилами)
    const feed = document.getElementById("dmFeed");
    if(feed) feed.innerHTML = "";
    toast("Vyčištěno (lokálně)");
  }catch(e){}
}
