...
function showView(id){
  ...
  try{
    ...
    if(id==='view-dm'){
      ...
      try{ if(!window. }catch(e){}
      try{ if(!window. currentDmPeerUid=null; } }catch(e){}
      try{ if(!window. }catch(e){}
...
document.addEventListener('click',(e)=>{
  ...
  // IMPORTANT: DM must always enter through the unified inbox entry-point.
  // Otherwise "envelope" / menu / tab can initialize different flows.
  if(v === 'view-dm'){
    try{ openDMInbox && openDMInbox(true); }
    catch(e){
      try{ showView('view-dm'); }catch(_){}
      try{ ensureDMInboxLoaded && ensureDMInboxLoaded('nav'); }catch(_){}
    }
    return;
  }
  showView(v);
  ...
  function restoreLastView(){
    ...
    try{
      if(view==='view-dm'){
        ...
        showView('view-dm');
        try{ window. }catch(e){}
        // start inbox load even before auth is ready (keeps loader and sets  }catch(e){}
        try{ loadDmThreads && loadDmThreads(true); }catch(e){}
    ...
    // If we restored DM before auth became ready, loadDmThreads() sets 
