...
function initDmMobileModal(){
  ...
  function close(){
    try{
      overlay.hidden=true;
      try{ setDmState('list',''); }catch(e){}
      try{ localStorage.setItem('mk_last_dm_seen', String(Date.now())); }catch(e){}
      // move back
      if(origParent){
...
// === DM state (single source of truth) ===
// mk_dm_mode: "list" | "thread"
// mk_dm_peer: uid of current peer (only when mode==="thread")
function getDmMode(){ try{ return (localStorage.getItem('mk_dm_mode')||'').trim() || 'list'; }catch(e){ return 'list'; } }
function getDmPeer(){ try{ return (localStorage.getItem('mk_dm_peer')||'').trim(); }catch(e){ return ''; } }
function setDmState(mode, peer){
  try{
    const m = (mode==='thread') ? 'thread' : 'list';
    localStorage.setItem('mk_dm_mode', m);
    localStorage.setItem('mk_dm_peer', m==='thread' ? String(peer||'') : '');
  }catch(e){}
}
try{ if(!localStorage.getItem('mk_dm_mode')) localStorage.setItem('mk_dm_mode','list'); }catch(e){}
window.getDmMode=getDmMode;
window.getDmPeer=getDmPeer;
window.setDmState=setDmState;
...
async function restoreAfterReload(){
  try{
    ...
    // Only restore one tab (lazy strategy)
        if(t === 'dm'){
      try{
        const mode=getDmMode();
        const peer=getDmPeer();
        if(mode==='thread' && peer){
          try{ await openDM(peer); }catch(e){ try{ openDMRoom(auth.currentUser.uid, peer); }catch(_){} }
        }else{
          try{ await openDM(); }catch(e){ try{ openDMInbox(true); }catch(_){} }
        }
      }catch(e){
        try{ await openDM(); }catch(_){}
      }
      return;
    }
    if(t === 'friends'){
...
// Global unified DM opener
// openDM() -> open list
// openDM(peerUid) -> open thread
async function openDM(peerUid){
  ...
  if(!peerUid){
    return openDMInbox(true);
  }
  try{ showView('view-dm'); }catch(e){}
  try{ setMiniLoad('dmMiniLoad','Načítáme…', true); }catch(e){}
  try{ setDmState('thread', peerUid); }catch(e){}
  try{ localStorage.setItem('mk_last_dm_peer', String(peerUid)); }catch(e){}
  try{ localStorage.setItem('mk_last_dm_seen', String(Date.now())); }catch(e){}
  // open exact room
  setTimeout(()=>{ try{ openDMRoom(me.uid, peerUid); }catch(e){} }, 0);
}
...
try{ setDmState('thread', (a===me.uid)?b:(b===me.uid?a:b)); }catch(e){}
  ...
  if(u){
    ...
    try{
      ...
      if(activeId==='view-dm' || document.getElementById('view-dm')?.classList.contains('active')){
        ...
        // Restore DM deterministically (list vs thread)
        try{
          const mode = (localStorage.getItem('mk_dm_mode')||'list').trim() || 'list';
          const peer = (localStorage.getItem('mk_dm_peer') || localStorage.getItem('mk_last_dm_peer') || '').trim();
          if(mode==='thread' && peer && !window.__DM_ROOM_RESTORED__){
            window.__DM_ROOM_RESTORED__ = true;
            setTimeout(()=>{
              try{ setMiniLoad('dmMiniLoad','Načítám konverzaci…', true); }catch(e){}
              try{ openDMRoom(u.uid, peer); }catch(e){}
            }, 450);
          }
          // If mode is list, DO NOT auto-open last peer.
        }catch(e){}
}
...
let NOTIF_REF=null;
  function listenNotifications(){
    ...
    NOTIF_REF.on('value', async snap=>{
      ...
      const v=snap.val()||{};
      const ids=Object.keys(v).sort((a,b)=>(v[b].ts||0)-(v[a].ts||0));
      // DM badge: count 'dm' notifications newer than last seen (local only)
      let dmNew=0;
      try{
        const seen = Number(localStorage.getItem('mk_last_dm_seen')||'0')||0;
        for(const k of ids){
          const n=v[k]||{};
          if(n.type==='dm' && (n.ts||0) > seen) dmNew++;
        }
      }catch(e){}
      const dmBadge=document.getElementById('dmBadge');
      if(dmBadge){ dmBadge.textContent = dmNew ? String(dmNew) : ''; dmBadge.style.display = dmNew ? 'inline-flex' : 'none'; }
...
async function startDM(toUid, opts={}){
  ...
  try{
    ...
    await Promise.all([
      db.ref('inboxMeta/'+me.uid+'/'+room).update({ ts, lastTs: ts, with: toUid, title: toPub.nick||'Uživatel' }),
      db.ref('inboxMeta/'+toUid+'/'+room).update({ ts, lastTs: ts, with: me.uid, title: myPub.nick||'Uživatel' })
    ]);
  }catch(e){}
...
(function initMobileNav(){
  ...
  document.getElementById('btnDMTop')?.addEventListener('click', ()=>{
    try{ openDM(); }catch(e){ try{ openDMInbox(true); }catch(_){} }
  });
