...
function ensureDMInboxLoaded(reason){
  try{
    // Only when DM view is active
    const dmView = document.getElementById('view-dm');
    if(!dmView || !dmView.classList.contains('active')) return;

    // Avoid re-entrant loops
    if(window.__DM_ENSURE_RUNNING__) return;
    window.__DM_ENSURE_RUNNING__ = true;

    try{ setMiniLoad('dmMiniLoad','Načítáme soukromé zprávy…', true); }catch(e){}

    const me = auth?.currentUser;
    if(!me){
      // Auth not ready yet: keep loader, mark for later
      window.__DM_NEEDS_LOAD__ = true;
      return;
    }

    // First attempt (now)
    setTimeout(()=>{ try{ loadDmThreads && loadDmThreads(true); }catch(e){} }, 0);

    // Safety retry: some mobiles paint later
    setTimeout(()=>{
      try{
    ...
    try{ window.__DM_ENSURE_RUNNING__ = false; }catch(e){}
  }
}
...
try{
  ...
  if(peer) localStorage.setItem('mk_last_dm_peer', peer);
  const room = dmRoomId(me.uid, peer);
  if(room) localStorage.setItem('mk_last_dm_room', room);
}catch(e){}
  ...
  try{
    const now=Date.now();
    const up={};
    up['inboxMeta/'+a+'/'+room+'/peer']=b;
    up['inboxMeta/'+b+'/'+room+'/peer']=a;
    // Create/refresh thread timestamps so inbox renders immediately.
    up['inboxMeta/'+a+'/'+room+'/lastTs']=now;
    up['inboxMeta/'+b+'/'+room+'/lastTs']=now;
    up['inboxMeta/'+a+'/'+room+'/ts']=now;
    up['inboxMeta/'+b+'/'+room+'/ts']=now;
    // do NOT bump unread here (only on new messages)
    if(meUid){ up['inboxMeta/'+meUid+'/'+room+'/unread']=0; }
...
$('#dmSend')?.addEventListener('click', async ()=>{
  ...
  try{
    ...
    try{
      await db.ref('notifications/'+currentDmPeerUid).push({ ts, type:'dm', from: me.uid, room, text: preview });
    }catch(e){}
...
// profile display
  if(u){
    ...
    // --- Post-auth restore (critical for DM after F5) ---
    // We only *painted* the view during DOMContentLoaded. Now that auth is ready,
    // start the proper loaders/subscriptions for the restored tab.
    try{
      if(!window.__RESTORE_DONE__){
        const v = (window.__RESTORE_TARGET_VIEW__ || (localStorage.getItem('mk_last_view')||localStorage.getItem('lastView')||'view-chat')).trim() || 'view-chat';
        window.__RESTORE_DONE__ = true;
        // Do not restore profile as a landing view
        const vv = (v==='view-profile') ? 'view-chat' : v;
        try{ showView(vv); }catch(e){}

        // Start tab data loads
        if(vv==='view-dm'){
          try{ setMiniLoad('dmMiniLoad','Načítáme soukromé zprávy…', true); }catch(e){}
          // Ensure inbox loads even if the first attempt happened before DOM/auth
          setTimeout(()=>{ try{ ensureDMInboxLoaded && ensureDMInboxLoaded('post-auth-restore'); }catch(e){} }, 0);

          // Restore exact conversation if it was open before reload
          try{
            const peer = (window.__DM_RESTORE_PEER__ || localStorage.getItem('mk_last_dm_peer') || '').trim();
            if(peer){
              setTimeout(()=>{ try{ openDMRoom(u.uid, peer); }catch(e){} }, 500);
            }
          }catch(e){}
        }
        if(vv==='view-friends'){
          try{ setMiniLoad('friendsMiniLoad','Načítáme přátele…', true); }catch(e){}
          setTimeout(()=>{ try{ loadFriends && loadFriends(true); }catch(e){} }, 0);
        }
        if(vv==='view-members'){
          try{ setMiniLoad('membersMiniLoad','Načítáme…', true); }catch(e){}
          setTimeout(()=>{ try{ loadMembers && loadMembers(true); }catch(e){} }, 0);
        }
        if(vv==='view-map'){
          try{ setMiniLoad('mapMiniLoad','Načítáme mapu…', true); }catch(e){}
          setTimeout(()=>{ try{ ensureMapLoadedOnce && ensureMapLoadedOnce(); }catch(e){} }, 0);
        }
        if(vv==='view-rent'){
          try{ setMiniLoad('rentMiniLoad','Načítáme…', true); }catch(e){}
          setTimeout(()=>{ try{ loadRent && loadRent(true); }catch(e){} }, 0);
        }
      }
    }catch(e){}
    ...
    if(view){
      if(view==='view-dm'){
        ...
        try{ openDMInbox(true); }catch(e){
          ...
          try{ loadDmThreads && loadDmThreads(true); }catch(_){}
        }
      }else{
        showView(view);
      }
...
(function(){
  ...
  function restoreLastView(){
    // Rule:
    // - Brand new visitor => always Praha chat
    // - Returning visitor => restore the last non-profile view
    // IMPORTANT: we only *paint* the view here. Data loads are started after auth is ready.
    let view = 'view-chat';
    let hasSaved = false;
    ...
    // Prefer explicit URL hash view request (deep links)
    try{
    ...
    // Never land on profile as the initial screen (it breaks "Praha chat first").
    try{
    ...
    // Paint only + remember target for post-auth restore.
    try{
      window.__RESTORE_TARGET_VIEW__ = view;
      window.__RESTORE_DONE__ = false;
    }catch(e){}

    try{
      showView(view);
    }catch(e){
      try{ showView('view-chat'); window.__RESTORE_TARGET_VIEW__='view-chat'; }catch(_){}
    }

    // Minimal loaders for non-chat views so UI doesn't look "dead" on reload.
    try{
      if(view==='view-dm'){ setMiniLoad('dmMiniLoad','Načítáme soukromé zprávy…', true); }
      if(view==='view-friends'){ setMiniLoad('friendsMiniLoad','Načítáme přátele…', true); }
      if(view==='view-members'){ setMiniLoad('membersMiniLoad','Načítáme…', true); }
      if(view==='view-map'){ setMiniLoad('mapMiniLoad','Načítáme mapu…', true); }
      if(view==='view-rent'){ setMiniLoad('rentMiniLoad','Načítáme…', true); }
    }catch(e){}

    // Chat must be ready immediately even for new visitors.
    try{
      if(view==='view-chat' && !window.__chatLoaded){
        window.__chatLoaded = true;
        const st = document.getElementById('preloaderStatus');
        if(st) st.textContent = 'Načítáme chat Praha…';
        try{ loadChat(); }catch(e){}
      }
    }catch(e){}
  }
  ...
  function mkBootstrap(){
    ...
    // Persist last active view right before reload/close (some mobiles lose it otherwise)
    try{
      if(!window.__MK_BEFOREUNLOAD_INSTALLED__){
        window.__MK_BEFOREUNLOAD_INSTALLED__ = true;
        window.addEventListener('beforeunload', ()=>{
          try{
            const activeView = document.querySelector('.view.active')?.id;
            if(activeView){
              localStorage.setItem('mk_last_view', activeView);
              localStorage.setItem('lastView', activeView);
              localStorage.setItem('mk_last_tab', activeView.replace('view-',''));
            }
          }catch(e){}
        });
      }
    ...
    // Some browsers restore from BFCache; ensure DM inbox restarts if needed
    try{
      if(!window.__MK_PAGESHOW_INSTALLED__){
        window.__MK_PAGESHOW_INSTALLED__ = true;
