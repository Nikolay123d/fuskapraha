...
window.FIREBASE_CONFIG = {
  ...
  // Use the canonical Firebase Auth domain. It reduces Google OAuth redirect_uri_mismatch issues.
  // (/__/auth/handler exists on both firebaseapp.com and web.app.)
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  // Always start in the main chat on hard reload.
  ...
  async function loadDmThreads(){
    ...
    try{
      ...
      for(const room of rooms){
        ...
        const row=document.createElement('div');
        row.className='msg';
        // IMPORTANT: Do NOT put data-uid on the whole row/name, иначе глобальный делегат кликов
        // может открыть профиль вместо переписки.
        row.innerHTML = `
        <div class="ava"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}" alt=""></div>
