...
function showView(id){
  ...
  if(el){
    ...
    try{
      ...
      localStorage.setItem('mk_last_view', id);
      // Timestamp = single source of truth for reliable restore-after-F5.
      // (Fixes cases when an old "view-dm" value keeps forcing DM on every reload.)
      localStorage.setItem('mk_last_view_ts', String(Date.now()));
    }catch(e){}
    try{
      window.__ACTIVE_TAB__ = id.replace('view-','');
      localStorage.setItem('mk_last_tab', window.__ACTIVE_TAB__);
      localStorage.setItem('mk_last_tab_ts', String(Date.now()));
    }catch(e){}
  }
...
(function(){
  ...
  function restoreLastView(){
    ...
    try{
      ...
      const ts = Number(localStorage.getItem('mk_last_view_ts')||0);

      // If we don't have a timestamp yet (older builds), do NOT trust the saved view.
      // This prevents a stale "view-dm" from forcing DM on every reload.
      if(ts > 0){
        hasSaved = !!(mv || lv);
    ...
    try{
      if(view==='view-dm'){
        // Always enter DM through the unified entry-point (envelope/menu/router).
        // This guarantees identical code-paths and prevents "DM empty until click".
        try{ window.__RESTORING_VIEW__ = true; }catch(e){}
        try{ window.openDMInbox?.(true); }catch(e){ showView('view-dm'); }
