...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  // Auth modal: close on backdrop click + ESC (reliable on mobile)
  try{
  ...
  // Activate last view (default chat) ASAP (so reload returns user where they were)
  let __startView = 'view-chat';
  try{ __startView = localStorage.getItem('lastView') || 'view-chat'; }catch(e){}
  try{ showView(__startView); }catch(e){}

  // Preload Praha chat only if we are actually in chat (keeps DM reload fast + shows correct mini loaders)
  try{
    if(__startView==='view-chat'){
...
function startAuthHeroRotator(){
  ...
  const lines=[
    "Práce začíná tady.",
    "Nejčerstvější nabídky práce v Praze — v reálném čase.",
    "Chat 24/7: rychle se domluvíš, rychle začneš.",
    "Přidej se: brigády, směny, práce na HPP i DPČ.",
    "Zaměstnavatelé i pracující — vše na jednom místě."
  ];
  let i=0;
  const typeLine=(s,cb)=>{
  ...
  __dmThreadsLiveCb = ()=>{
    ...
    __dmThreadsLiveTimer = setTimeout(async ()=>{
      ...
      try{
        ...
        const cur = (localStorage.getItem('lastView')||'view-chat');
...
async function loadChat(){
  ...
  // Failsafe: never let chat mini-loader spin forever
  setTimeout(()=>{ try{ if(feed && feed.children && feed.children.length){ setMiniLoad('chatMiniLoad','', false); } }catch(e){} }, 2500);
