...
// --- Auth-ready gates (fix DM after F5) ---
let __AUTH_READY__ = false;
let __NEED_DM_LOAD__ = false;
let __PENDING_DM_PEER__ = '';
let __RESTORE_REQUESTED__ = false;
...
async function openDMInbox(forceReload=true){
  // Auth gate: after F5 auth.currentUser is null for a short time.
  // Do NOT start DM queries before auth is ready; keep loader and request a post-auth load.
  if(!__AUTH_READY__){
    __NEED_DM_LOAD__ = true;
    try{ showView('view-dm'); }catch(e){}
    try{ setMiniLoad('dmMiniLoad','Přihlašujeme…', true); }catch(e){}
    return;
  }
...
// Unified DM entry-point (one source of truth):
// window.openDM() -> open inbox list
// window.openDM(uid) -> open a specific thread with uid
window.openDM = async function(peerUid){
  // Always switch to DM view and show a mini loader immediately (no empty screen).
  try{ showView('view-dm'); }catch(e){}
  try{ setMiniLoad('dmMiniLoad','Načítáme soukromé zprávy…', true); }catch(e){}

  if(!__AUTH_READY__){
    __NEED_DM_LOAD__ = true;
    __PENDING_DM_PEER__ = (peerUid||'');
    try{ setMiniLoad('dmMiniLoad','Přihlašujeme…', true); }catch(e){}
    return;
  }

  const me = auth.currentUser;
  if(!me){ __NEED_DM_LOAD__ = true; __PENDING_DM_PEER__=(peerUid||''); return; }

  // Load inbox first (ensures list + meta are ready).
  try{ await loadDmThreads?.(); }catch(e){}

  if(peerUid){
    // Open thread deterministically (me + peer).
    try{ await window.openDMRoom?.(me.uid, peerUid); }catch(e){}
  }else{
    // Ensure list mode.
    try{ setDmState('list', null); }catch(e){}
  }
...
function ensureDMInboxLoaded(reason){
  try{
    if(!__AUTH_READY__){ window.__DM_NEEDS_LOAD__ = true; return; }
...
$('#dmOpen')?.addEventListener('click', async ()=>{
  try{ await window.openDM?.(); }catch(e){
    const me=auth.currentUser;
...
// profile display
  if(u){
    ...
    __AUTH_READY__ = true;
    // Restore view only after auth is ready (prevents DM reads before auth on F5)
    if(!window.__RESTORED_ON_AUTH__){
      window.__RESTORED_ON_AUTH__ = true;
      try{ restoreLastView(); }catch(e){}
    }
    ...
    // If DM was requested before auth became ready (e.g., after F5), load it now.
    try{
      if(__NEED_DM_LOAD__ || window.__DM_NEEDS_LOAD__){
        __NEED_DM_LOAD__ = false;
        window.__DM_NEEDS_LOAD__ = false;
        await loadDmThreads?.();
        if(__PENDING_DM_PEER__){
          const peer = __PENDING_DM_PEER__;
          __PENDING_DM_PEER__ = '';
          try{ await window.openDMRoom?.(u.uid, peer); }catch(e){}
        }
      }
    }catch(e){}
hidePreloader();
...
(function initMobileNav(){
  ...
  drawer?.addEventListener('click', (e)=>{
    ...
    if(view){
      if(view==='view-dm'){
        // paint DM view first; data loads only after auth becomes ready
        ...
        // remember desired DM peer if user was in a thread
        try{
          const st = getDmState();
          __PENDING_DM_PEER__ = (st.mode==='thread' && st.peer) ? st.peer : '';
        }catch(e){ __PENDING_DM_PEER__=''; }
      }else{
        showView(view);
      }
    }
...
(function(){
  ...
  function mkBootstrap(){
    ...
    // 6) Initial paint: do NOT restore before auth is ready (fix DM after F5)
    __RESTORE_REQUESTED__ = true;
    try{
      const h = (location.hash||'').toLowerCase();
      if(h.includes('dm')){
        showView('view-dm');
        ...
        showView('view-chat');
        if(!window.__chatLoaded){ window.__chatLoaded = true; try{ loadChat(); }catch(e){} }
      }
    }catch(e){ try{ showView('view-chat'); }catch(_){} }
