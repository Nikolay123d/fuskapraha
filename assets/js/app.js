...
// Persist last active view/tab even if switching was done programmatically
window.addEventListener('beforeunload', ()=>{
  try{
    const activeId = document.querySelector('.view.active')?.id;
    if(activeId){
      localStorage.setItem('mk_last_view', activeId);
      const tab = activeId.replace('view-','') || 'chat';
...
async function restoreAfterReload(){
  // Prefer view (more specific), fallback to tab
  const lastView = localStorage.getItem('mk_last_view') || '';
  const lastTab  = localStorage.getItem('mk_last_tab') || (lastView ? lastView.replace('view-','') : 'chat') || 'chat';

  // Only restore ONE view (lazy tabs stay lazy)
  if(lastTab === 'dm' || lastView === 'view-dm'){
    await openDMInbox(true); // shows mini loader + loads inbox once

    // If we know the last peer, try to reopen the dialog
    const peerUid = localStorage.getItem('mk_last_dm_peer');
    if(peerUid){
      try{
        const snap = await db.ref('usersPublic/'+peerUid).once('value');
        const p = snap.val() || {};
        const nick = p.nick || p.name || 'User';
        const ava  = p.avatar || p.photoURL || '';
        await openDMRoom(peerUid, nick, ava);
      }catch(e){}
    }
    return;
  }

  if(lastTab === 'friends' || lastView === 'view-friends'){
    showView('view-friends');
    await loadFriends();
    return;
  }

  if(lastTab === 'members' || lastView === 'view-members'){
    showView('view-members');
    await loadMembers();
    return;
  }

  if(lastTab === 'rent' || lastView === 'view-rent'){
    showView('view-rent');
    await loadRent();
    return;
  }

  // Default: Praha chat should be the first thing a new visitor sees
  showView('view-chat');
  await loadChat();
}
...
// profile display
  if(u){
    ...
    try{
      ...
      try{ await restoreAfterReload(); }catch(e){ console.warn("restoreAfterReload failed", e); }
