...
async function openDMInbox(forceReload=true){
  const me = auth?.currentUser;
  if(!me){
    // Do not navigate to a "broken" DM route when auth isn't ready.
    // We just mark the intent and the main auth handler will restore DM reliably.
    window.__PENDING_VIEW__ = 'view-dm';
    try{ window.__DM_NEEDS_LOAD__ = true; }catch(e){}
    // IMPORTANT: no modal spam on reload; open auth only on explicit user action
...
function ensureDMInboxLoaded(reason){
  try{
    ...
    // First attempt (now)
    setTimeout(()=>{ try{ ensureDMInboxLoaded && ensureDMInboxLoaded('restore'); }catch(e){} }, 0);
    ...
    setTimeout(()=>{
      try{
        ...
        if(box && box.children && box.children.length===0){
          try{ ensureDMInboxLoaded && ensureDMInboxLoaded('restore'); }catch(e){}
        }
      }catch(e){}
    }, 450);
...
async function googleSignIn(){
  ...
  // Redirect-only (POPUP causes COOP issues + blocked on many mobiles)
...
$('#dmOpen')?.addEventListener('click', async ()=>{
  try{ await window.openDMInbox?.(true); }catch(e){
    ...
    showView('view-dm');
    try{ try{ ensureDMInboxLoaded && ensureDMInboxLoaded('auth-active'); }catch(e){} }catch(_){ }
  }
});
...
auth.onAuthStateChanged(async (u)=>{
  try{ window.__AUTH_READY__ = !!u; }catch(e){}
  ...
  if(u){
    ...
    try{
      ...
      if(activeId==='view-dm' || document.getElementById('view-dm')?.classList.contains('active')){
        try{ setMiniLoad('dmMiniLoad','Načítáme soukromé zprávy…', true); }catch(e){}
        try{ ensureDMInboxLoaded && ensureDMInboxLoaded('auth-active'); }catch(e){}
...
(function initMobileNav(){
  ...
  document.getElementById('btnDMTop')?.addEventListener('click', ()=>{
    try{ showView('view-dm'); }catch(e){}
    try{ ensureDMInboxLoaded && ensureDMInboxLoaded('btnDMTop'); }catch(e){ try{ openDMInbox?.(true); }catch(_){} }
  });
