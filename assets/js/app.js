...
// === Unified Guard Pipeline (SSoT: Auth → Verified → Access) ===
(function initGuardPipeline(){
  if(window.MK_GUARD) return;

  async function requireAuth(){
    const u = (window.auth && auth.currentUser) ? auth.currentUser : null;
    if(u) return u;
    try{ window.__PENDING_VIEW__ = window.__PENDING_VIEW__ || (localStorage.getItem('mk_last_view')||'view-chat'); }catch(e){}
  ...
  function requireVerified(u){
    if(!u) return false;
    // If app already marked verification required, enforce immediately.
    if(window.__EMAIL_VERIFY_REQUIRED__){
      try{ toast && toast('Nejdřív potvrďte e-mail.'); }catch(e){}
      try{ openModalAuth && openModalAuth('login'); }catch(e){}
      return false;
    }
    // Only block when the verification window expired
    try{
      if(u.emailVerified===false){
        const until = (typeof getVerifyDeadline==='function') ? getVerifyDeadline(u) : 0;
        if(until && Date.now()>until){
          window.__EMAIL_VERIFY_REQUIRED__ = true;
          try{ toast && toast('Potvrzení e-mailu vypršelo.'); }catch(e){}
          try{ openModalAuth && openModalAuth('login'); }catch(e){}
          return false;
        }
      }
  ...
  function requireCanChat(){
    const g = window.MK_ACCESS ? window.MK_ACCESS.canChat() : {ok:true};
    if(g && g.ok) return true;
    try{ alert(window.MK_ACCESS.explainBlock(g)); }catch(e){}
    return false;
  }

  function requireCanDM(){
    const g = window.MK_ACCESS ? window.MK_ACCESS.canDM() : {ok:true};
    if(g && g.ok) return true;
    try{ alert(window.MK_ACCESS.explainBlock(g)); }catch(e){}
    return false;
  }

  window.MK_GUARD = { requireAuth, requireVerified, requireCanChat, requireCanDM };
})();
...
ref.set({ nick: (u.displayName||u.email||'Uživatel'), email: u.email||null, avatar: window.DEFAULT_AVATAR, role:'seeker', plan:'free', createdAt: Date.now() });
  }else{
    const v=s.val()||{};
...
async function openDMRoom(a,b){
  ...
  currentDmRoom = room;
  try{ localStorage.setItem('mk_last_dm_mode','room'); localStorage.setItem('mk_last_dm_peer', String((auth.currentUser && auth.currentUser.uid===a)?b:a)); }catch(e){}
...
$('#dmSend')?.addEventListener('click', async ()=>{
  const me = await (window.MK_GUARD?.requireAuth?.()||Promise.resolve(auth.currentUser));
  if(!me) return;
  if(!(window.MK_GUARD?.requireVerified?.(me))) return;
  if(!(window.MK_GUARD?.requireCanDM?.())) return;
  ...
  try{
    ...
    // No forced re-render: DM updates come from the realtime listener.
...
async function ensureMapLoadedOnce(){
  try{
    ...
    if(!MAP){
      MAP = L.map('map');
      // Restore map position (F5 identical restore)
      try{
        const st = JSON.parse(localStorage.getItem('mk_last_map_state')||'null');
        if(st && Array.isArray(st.center) && typeof st.zoom==='number'){
          MAP.setView(st.center, st.zoom, { animate:false });
        }else{
          MAP.setView([50.0755, 14.4378], 11, { animate:false });
        }
      }catch(e){ MAP.setView([50.0755, 14.4378], 11, { animate:false }); }
      ...
      try{
        MAP.on('moveend zoomend', ()=>{
          try{
            ...
            const z = MAP.getZoom();
            localStorage.setItem('mk_last_map_state', JSON.stringify({ center:[c.lat, c.lng], zoom:z, city:getCity(), ts:Date.now() }));
          }catch(e){}
        });
      }catch(e){}
...
async function openView(viewId, opts={}){
  ...
  if(v==='view-dm'){
    // DM entry point: restore inbox vs room identically
    const me = auth?.currentUser;
    const mode = opts.dmMode || (()=>{
      try{ return (localStorage.getItem('mk_last_dm_mode')||'').trim(); }catch(e){ return ''; }
    })();
    const restorePeer = opts.restorePeer || (()=>{
      try{ return (localStorage.getItem('mk_last_dm_peer')||'').trim(); }catch(e){ return ''; }
    })();
    if(me && mode==='room' && restorePeer){
  ...
  if(u){
    ...
    try{
      ...
      if(activeId==='view-dm' || document.getElementById('view-dm')?.classList.contains('active')){
        ...
        // Restore DM state identically (inbox vs room)
        try{
          const mode = (localStorage.getItem('mk_last_dm_mode')||'').trim();
          const peer = (window.__DM_RESTORE_PEER__ || localStorage.getItem('mk_last_dm_peer') || '').trim();
          if(mode==='room' && peer && !window.__DM_ROOM_RESTORED__){
            window.__DM_ROOM_RESTORED__ = true;
            setTimeout(()=>{ try{ openDMRoom(u.uid, peer); }catch(e){} }, 450);
          }
        }catch(e){}
}
