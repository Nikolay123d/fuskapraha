...
async function restoreAfterReload(){
  try{
    ...
    if(t === 'dm'){
      // Restore DM deterministically:
      // - if mk_dm_mode === 'thread' -> restore that peer
      // - else -> stay in list (do NOT open old thread)
      try{ window.__RESTORING_VIEW__ = true; }catch(e){}
      try{ showView('view-dm'); }catch(e){}
      try{ window.__RESTORING_VIEW__ = false; }catch(e){}

      // Always show a mini-loader so the user understands DM is loading
      try{ setMiniLoad('dmMiniLoad','Načítáme soukromé zprávy…', true); }catch(e){}

      const st = (typeof getDmState==='function') ? getDmState() : {mode:'list', peer:''};
      const me = auth?.currentUser;

      if(st.mode==='thread' && st.peer){
        // If auth is ready already, open the thread now; otherwise auth handler will restore it.
        try{ window.__DM_RESTORE_PEER__ = st.peer; }catch(e){}
        if(me){
          try{ setMiniLoad('dmMiniLoad','Načítáme konverzaci…', true); }catch(e){}
          setTimeout(()=>{ try{ openDMRoom(me.uid, st.peer); }catch(e){} }, 280);
        }else{
...
async function startDM(toUid, opts={}){
  ...
  try{
    ...
    await Promise.all([
      db.ref('inboxMeta/'+me.uid+'/'+room).update({ ts, lastTs: ts, with: toUid, peer: toUid, title: toPub.nick||'Uživatel' }),
      db.ref('inboxMeta/'+toUid+'/'+room).update({ ts, lastTs: ts, with: me.uid, peer: me.uid, title: myPub.nick||'Uživatel' })
    ]);
  }catch(e){}
...
// --- DM threads / inbox ---
  ...
  async function loadDmThreads(){
    ...
    // If cache is empty, still show a clear "loading" placeholder (no blank state)
    try{
      if(!box.innerHTML || !box.innerHTML.trim()){
        box.innerHTML = '<div class="muted" style="padding:10px">Načítáme konverzace…</div>';
      }
    }catch(e){}
