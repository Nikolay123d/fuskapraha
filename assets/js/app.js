...
// === Unified Router (ONE entry-point for tabs/menu/envelope + reload restore) ===
window.APP = window.APP || { authReady:false, user:null, currentView:null, initializedViews:{} };

function viewFromHash(){
  try{
    const h = (location.hash||'').replace('#','').trim().toLowerCase();
    if(!h) return '';
    if(h.startsWith('dm')) return 'view-dm';
    if(h.startsWith('friends')) return 'view-friends';
    if(h.startsWith('members')) return 'view-members';
    if(h.startsWith('rent')) return 'view-rent';
    if(h.startsWith('map')) return 'view-map';
...
function viewFromStorage(){
  try{
    ...
    const v=(mv||lv||'').trim();
    if(!v) return '';
    // safety: never land on profile as initial
    if(v==='view-profile') return 'view-chat';
    return v;
  }catch(e){ return ''; }
}

// One public function: openView('view-dm') / openView('dm')
async function openView(view, opts){
  opts = opts || {};
  try{
    const v = String(view||'').startsWith('view-') ? String(view) : ('view-'+String(view||''));
    // remember desired tab for reload
    try{ localStorage.setItem('mk_last_view', v); localStorage.setItem('lastView', v); }catch(e){}
    APP.currentView = v;

    // Auth gating: allow paint, but DM needs special handling
    if(v==='view-dm'){
      // unified DM entry-point (handles auth-not-ready by keeping loader + pending intent)
      await (openDMInbox ? openDMInbox(true) : Promise.resolve());
      return;
    }

    // Other views can be shown even before auth; data loads inside are guarded where needed.
    try{ openView && openView(v); }catch(e){ try{ showView(v); }catch(_){} }
  }catch(e){
    try{ showView('view-chat'); }catch(_){}
  }
}
window.openView = openView;

// Start router exactly once, but ONLY after auth becomes ready (prevents "empty DM until click").
function startRouterOnce(){
  try{
    if(window.__ROUTER_STARTED__) return;
    if(!window.__AUTH_READY__) return; // wait for auth callback
    window.__ROUTER_STARTED__ = true;

    // Priority: URL hash > last view > first visit defaults to chat
    const v = viewFromHash() || viewFromStorage() || 'view-chat';
    openView(v, { restore:true });
  }catch(e){
    try{ showView('view-chat'); }catch(_){}
  }
}
window.startRouterOnce = startRouterOnce;
...
document.addEventListener('click',(e)=>{
  ...
  if(!t) return;
  e.preventDefault();
  try{ openView && openView(t.dataset.view); }
  catch(err){ try{ showView(t.dataset.view); }catch(_){} }
});
...
try{
  ...
  // single source of truth: we are inside a THREAD now
...
auth.onAuthStateChanged(async (u)=>{
  ...
  try{ window.APP = window.APP || {}; APP.authReady = true; APP.user = u||null; }catch(e){}
  try{ startRouterOnce && startRouterOnce(); }catch(e){}
  ...
  if(u){
    ...
    try{
      if(window.__PENDING_VIEW__){
        ...
        try{ openView && openView(v, {restore:true}); }catch(e){ try{ try{ openView && openView(v); }catch(e){ try{ showView(v); }catch(_){} } }catch(_){} }
      }
    }catch(e){}
...
(function initMobileNav(){
  ...
  drawer?.addEventListener('click', (e)=>{
    ...
    if(view){
      try{ openView && openView(view); }catch(e){ try{ showView(view); }catch(_){} }
    }
  ...
  document.getElementById('btnDMTop')?.addEventListener('click', ()=>{
    try{ openView && openView('view-dm'); }
    catch(e){ try{ openDMInbox && openDMInbox(true); }catch(_){ try{ showView('view-dm'); }catch(__){} } }
  });
document.getElementById('btnFriendsTop')?.addEventListener('click', ()=>{ try{ openView && openView('view-friends'); }catch(e){ try{ showView('view-friends'); loadFriends && loadFriends(true); }catch(_){} } });
  document.getElementById('btnMe')?.addEventListener('click', ()=>{ try{ openView && openView('view-profile'); }catch(e){ try{ showView('view-profile'); }catch(_){} } });
...
function openAdmin(){
  try{ openView && openView('view-admin'); }catch(e){ try{ showView('view-admin'); }catch(_){} }
}
...
function openSupportChat(){
  ...
  try{ openDMRoom(window._user.uid, adminUid); }catch(e){}
  try{ openView && openView('view-dm'); }catch(e){ try{ showView('view-dm'); }catch(_){} }
}
