...
async function ensureUserPublic(u, extra={}){
  ...
  const cur = snap.val() || {};
  const nick = (cur.nick || extra.nick || u.displayName || u.email || 'Uživatel').trim();
  const role = (cur.role || extra.role || 'worker');
  const merged = {
    email: u.email || cur.email || '',
    nick,
    nickLower: (cur.nickLower || nick.toLowerCase()),
    role,
...
$('#saveProfile')?.addEventListener('click', async ()=>{
  ...
  // Prefill profile form and lock identity fields
  try{
    const nickEl = document.getElementById('setNick');
    if(nickEl){ nickEl.value = pub.nick || ''; nickEl.setAttribute('readonly','readonly'); }
    const roleEl = document.getElementById('setRole');
    if(roleEl){ roleEl.value = pub.role || ''; roleEl.setAttribute('disabled','disabled'); }
  }catch(e){}
  const up = {};
  const a=$('#setAvatarUrl')?.value.trim(); if(a) up.avatar=a;

  // free fields (non-identity)
  ...
  // identity fields: nick + role only via request
  const n=$('#setNick')?.value.trim();
  const r=$('#setRole')?.value;
  const wantNick = n && (n !== (pub.nick||''));
  const wantRole = r && (r !== (pub.role||''));
  if(wantNick || wantRole){
    const id = String(Date.now());
    await db.ref('profileChangeRequests/'+u.uid+'/'+id).set({
      ts: Date.now(),
      uid: u.uid,
      email: u.email || '',
      currentNick: pub.nick || '',
      currentRole: pub.role || '',
      requestedNick: wantNick ? n : null,
      requestedRole: wantRole ? r : null,
      status: 'pending'
    });
    toast('Žádost o změnu nick/role byla odeslána (čeká na schválení).');
  }
...
document.addEventListener('click', (e)=>{
  // Global profile open by data-uid (but do NOT hijack DM clicks)
  if(e.target.closest('#dmThreads') || e.target.closest('#dmConversationCard') || e.target.closest('#dmModal')) return;
