...
const PREMIUM_QR_IMG = './assets/img/csob-qr.png';
const PREMIUM_PLANS = {
...
function showView(id){
  ...
  try{
    ...
    if(id==='view-dm'){
      ...
      try{ loadDMThreads && loadDMThreads(); }catch{}
      return;
    }
    if(id==='view-map'){
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  // Admin quick navigation: scroll to sections (keeps single page, avoids "chaos")
  try{
...
// Auth hooks (hero copy)
let __authHookTimer=null;
let __authHookTypingTimer=null;
let __authHookIdx=0;
const __authHooks=[
  "Spojujeme zaměstnavatele a uchazeče v reálném čase.",
  "Dlouho hledáš práci a máš dost šablonových inzerátů?",
  "Přidej se k nám a najdi nejlepší nabídku 24/7.",
  "Makáme.cz — práce začíná tady."
];
function stopAuthHooks(){
  try{ if(__authHookTimer) clearInterval(__authHookTimer); }catch(e){}
  try{ if(__authHookTypingTimer) clearInterval(__authHookTypingTimer); }catch(e){}
  __authHookTimer=null; __authHookTypingTimer=null;
}
function startAuthHooks(){
  const el=document.getElementById('authHook');
  if(!el) return;
  stopAuthHooks();
  function typeText(txt){
    ...
    el.innerHTML='';
    let i=0;
    __authHookTypingTimer=setInterval(()=>{
      i++;
      const part=txt.slice(0,i);
      el.innerHTML=part+'<span class="cursor">▍</span>';
      if(i>=txt.length){
        clearInterval(__authHookTypingTimer);
        __authHookTypingTimer=null;
        el.innerHTML=txt;
      }
    }, 22);
  }
  function showNext(){
    const txt=__authHooks[__authHookIdx%__authHooks.length];
    __authHookIdx++;
    typeText(txt);
  }
  showNext();
  __authHookTimer=setInterval(showNext, 5200);
}
...
function openModalAuth(mode='login'){
  ...
  try{ startAuthHooks(); }catch(e){}
}
function closeModalAuth(){
  const m=$('#modalAuth');
  if(m) m.hidden=true;
  try{ stopAuthHooks(); }catch(e){}
  document.body.classList.remove('auth-open');
}
...
function setBadge(el, n){
  ...
  if(v > 0){
    ...
    try{ el.style.display = 'inline-flex'; }catch(e){}
  }else{
    el.textContent = '';
...
function updateTopBadges(){
  ...
  // bell badge element id in UI = bellBadge
  setBadge(document.getElementById('bellBadge'), __notifUnread);
}
// Notifications "unread" is computed client-side using last seen timestamp.
// This avoids changing DB schema and works immediately.
function __notifSeenKey(uid){ return 'mk_notif_seen_'+uid; }
function getNotifSeenTs(uid){
  try{ return Number(localStorage.getItem(__notifSeenKey(uid))||0) || 0; }catch(e){ return 0; }
}
function setNotifSeenNow(uid){
  const ts = Date.now();
  try{ localStorage.setItem(__notifSeenKey(uid), String(ts)); }catch(e){}
  return ts;
}
function markNotificationsRead(){
  ...
  if(!me) return;
  __notifUnread = 0;
  try{ setNotifSeenNow(me.uid); }catch(e){}
  updateTopBadges();
}
...
async function openDMRoom(a,b){
  ...
  if(isMobileViewport()) document.body.classList.add('dm-room-open');
  ...
  // Reset my unread counter for this room (so ✉️ badge drops immediately)
  try{
    if(meUid){
      const peerUid = currentDmPeerUid;
      const peerPub = peerUid ? await getUser(peerUid) : null;
      await db.ref('inboxMeta/'+meUid+'/'+room).transaction((cur)=>{
        cur = cur || {};
        cur.with = peerUid;
        cur.title = (peerPub && peerPub.nick) ? peerPub.nick : (cur.title||'Uživatel');
        cur.ts = cur.ts || Date.now();
        cur.lastTs = cur.lastTs || cur.ts;
        cur.unread = 0;
        return cur;
      });
    }
...
$('#dmSend')?.addEventListener('click', async ()=>{
  ...
  try{
    ...
    // 4) update inbox meta (room-keyed) + unread counters
    const myPub = await getUser(me.uid);
    const peerPub = await getUser(currentDmPeerUid);
    const snippet = (text||'').slice(0, 140) || (img ? '📷 Foto' : '');
    // peer: increment unread
    try{
      await db.ref('inboxMeta/'+currentDmPeerUid+'/'+room).transaction((cur)=>{
        cur = cur || {};
        cur.with = me.uid;
        cur.title = myPub.nick || 'Uživatel';
        cur.ts = ts;
        cur.lastTs = ts;
        cur.lastText = snippet;
        cur.unread = (Number(cur.unread||0) || 0) + 1;
        return cur;
      });
    }catch(e){}
    // me: reset unread
    try{
      await db.ref('inboxMeta/'+me.uid+'/'+room).transaction((cur)=>{
        cur = cur || {};
        cur.with = currentDmPeerUid;
        cur.title = peerPub.nick || 'Uživatel';
        cur.ts = ts;
        cur.lastTs = ts;
        cur.lastText = snippet;
        cur.unread = 0;
    ...
    // Bell notification for peer (client-side, for 🔔 feed)
    try{
      await db.ref('notifications/'+currentDmPeerUid).push({
        ts, type:'dm', from: me.uid, room, text: snippet, title: (myPub.nick||'Uživatel')
      });
    }catch(e){}
  ...
  wrap.addEventListener('click', async (e)=>{
    ...
    try{
      if(a==='accept'){
        ...
        try{ await db.ref('notifications/'+uid).push({ts:Date.now(), type:'friendAccepted', from:me.uid, title:'Přátelství potvrzeno', text:'Uživatel potvrdil vaše přátelství.'}); }catch{}
        toast('Přidáno');
        loadFriends();
        return;
      }
...
async function loadFriends(){
  ...
  try{
    // Clear containers before fresh render (prevents duplicates when cached HTML was painted)
    if(reqBox){ reqBox.innerHTML=''; reqBox.style.display='none'; }
    if(listBox){ listBox.innerHTML=''; }
    // incoming requests
...
$('#friendAddBtn')?.addEventListener('click', async ()=>{
  try{
    ...
    try{ await db.ref('notifications/'+uid).push({ts:Date.now(), type:'friend', from:me.uid, title:'Žádost o přátelství', text:'Nová žádost o přátelství'}); }catch(e){}
...
let NOTIF_REF=null;
  ...
  const stopSeq = startMiniSequence('chatMiniLoad', [
    ...
    'Synchronizuji chat…'
      ...
      // badge (unread only, computed via last-seen ts)
      const seenTs = getNotifSeenTs(me.uid);
      let unreadCount = 0;
      for(const id of ids){
        const t = Number(v[id]?.ts || 0);
        if(t > seenTs) unreadCount++;
      }
      __notifUnread = unreadCount;
      updateTopBadges();
    });
  }
  ...
  const __bellBtn = document.getElementById('btnBell');
  if(__bellBtn && !__bellBtn.dataset.wiredBell){
    __bellBtn.dataset.wiredBell = '1';
    __bellBtn.addEventListener('click', ()=>{
      if(!auth.currentUser) return toast('Přihlaste se');
      try{ openModal('modalNotif'); }catch(e){ try{ openModal(document.getElementById('modalNotif')); }catch(_){} }
...
async function startDM(toUid, opts={}){
  ...
  showView('view-dm');
  // refresh left thread list so the opened peer appears there immediately
...
// Backward compatible helper (some UI buttons still call openDM(uid))
window.openDM = function(uid){ return startDM(uid); };
...
// --- DM threads / inbox ---
  ...
  async function loadDmThreads(){
    ...
    const me = auth.currentUser;
    if(!me){
      // Auth not ready yet. Do not "lock" DM in an empty state.
    ...
    try{
      ...
      for(const room of rooms){
        ...
        <div class="bubble" style="width:100%;cursor:pointer">
          <div class="name"><b>${esc(u.nick||'Uživatel')}</b> <span class="muted">${u.online?'online':'offline'}</span>${Number(v[room]?.unread||0) > 0 ? ` <span class="badge-count" style="margin-left:8px">${Number(v[room].unread) > 99 ? '99+' : Number(v[room].unread)}</span>` : ''}</div>
...
(function initMobileNav(){
  ...
  const __bellBtn2 = document.getElementById('btnBell');
  if(__bellBtn2 && !__bellBtn2.dataset.wiredBell){
    __bellBtn2.dataset.wiredBell = '1';
    __bellBtn2.addEventListener('click', ()=>{
      const me = auth?.currentUser;
      if(!me){ try{ openModalAuth('login'); }catch{}; return; }
      try{ openModal(document.getElementById('modalNotif')); }catch(e){ try{ openModal('modalNotif'); }catch(_){} }
      try{ markNotificationsRead(); }catch(e){}
    });
  }
...
window.addEventListener('DOMContentLoaded', ()=> {
  ...
  // This fixes "empty circles" in DM list/history on some devices/browsers.
