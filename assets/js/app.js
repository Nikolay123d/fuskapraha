...
async function googleSignIn(){
  ...
  try{
    sessionStorage.setItem('__mk_auth_redirecting','1');
...
async function handleGoogleRedirectResult(){
  ...
  try{
    ...
    try{ sessionStorage.removeItem('__mk_auth_redirecting'); }catch(_e){}
  }
}
...
auth.onAuthStateChanged(async (u)=>{
  ...
  // Redirect glue guard (Google sign-in redirect)
  const __redirecting = (sessionStorage.getItem('__mk_auth_redirecting') === '1');
  if(!u && __redirecting){
    try{ await handleGoogleRedirectResult(); }catch(e){ console.error('[AUTH] redirect retry failed', e); }
    u = auth.currentUser;
  }
