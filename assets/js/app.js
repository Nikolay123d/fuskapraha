...
function startAuthHeroRotator(){
  ...
  const lines=[
    "Práce začíná tady.",
    "Spojujeme zaměstnavatele a pracující v reálném čase.",
    "Už tě nebaví šablonové inzeráty?",
    "Přidej se k nám a najdi práci 24/7."
  ];
  let i=0;
  const typeLine=(s,cb)=>{
...
async function openDMRoom(a,b){
  ...
  try{
    ...
    up['inboxMeta/'+a+'/'+room+'/lastTs']=up['inboxMeta/'+a+'/'+room+'/lastTs']||now;
    up['inboxMeta/'+b+'/'+room+'/lastTs']=up['inboxMeta/'+b+'/'+room+'/lastTs']||now;
...
(function initMobileNav(){
  ...
  showView('view-dm');
  setTimeout(()=>{ try{ loadDmThreads(true); }catch(e){} }, 0);
});
