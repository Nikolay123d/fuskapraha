...
window.FIREBASE_CONFIG = {
  ...
  // IMPORTANT (Google redirect login on GitHub Pages):
  // Use the firebaseapp.com authDomain so the OAuth redirect handler is ALWAYS available
  // even if Firebase Hosting (web.app) has not been deployed yet.
  // GitHub Pages will still be the final redirectUrl (your app URL).
  // In Google Cloud Console (OAuth Client), allow BOTH redirect URIs:
...
// === Redirect "glue" guard (must run BEFORE any heavy UI) ===
window.__MK_REDIRECT_GLUE_DONE__ = false;
async function __mkProcessRedirectEarly(){
  if(window.__MK_REDIRECT_GLUE_DONE__) return null;
  window.__MK_REDIRECT_GLUE_DONE__ = true;

  // Force local persistence (helps across reloads / some browsers)
  try{ await auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL); }catch(e){}

  const redirecting = sessionStorage.getItem('__mk_auth_redirecting') === '1';
  if(!redirecting) return null;

  // Try a few times: some browsers deliver redirect result slightly later
  for(let i=0;i<4;i++){
    try{
      const res = await auth.getRedirectResult();
      if(res && res.user){
        console.log('[AUTH] redirect glued early:', res.user.uid, res.user.email);
        sessionStorage.removeItem('__mk_auth_redirecting');
        return res;
      }
    }catch(e){
      console.error('[AUTH] early getRedirectResult attempt failed', e);
      // if error is fatal, still keep trying once or twice
    }
...
// Run immediately (do not wait for DOM load)
try{ __mkProcessRedirectEarly(); }catch(e){}
...
// --- Google Auth (redirect-only; works on mobile + avoids popup blockers) ---
async function googleSignIn(){
  ...
  try{
    // persist auth across redirect + mark redirecting to avoid UI reset race
try{ await auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL); }catch(e){}
try{ sessionStorage.setItem('__mk_auth_redirecting','1'); }catch(e){}
await auth.signInWithRedirect(provider);
    ...
    if(code==='auth/unauthorized-domain'){
      ...
      toast('Google přihlášení: '+(e?.message||code||'Chyba'));
    }
    console.error('[AUTH] googleSignIn failed', e);
    throw e;
  }
}
...
// Process Google redirect ASAP (do not wait for window.load)
try{ handleGoogleRedirectResult(); }catch(e){}
document.addEventListener('DOMContentLoaded', ()=>{ try{ handleGoogleRedirectResult(); }catch(e){} });
window.addEventListener('load', ()=>{ try{ handleGoogleRedirectResult(); }catch(e){} });
...
// If we're returning from Google redirect, Firebase may briefly report null.
// Avoid resetting UI/state until redirect result is processed.
const __redirecting = (()=>{ try{return sessionStorage.getItem('__mk_auth_redirecting')==='1';}catch(e){return false;} })();
if(!u && __redirecting){
  // Coming back from Google redirect: do NOT reset UI/listeners yet.
  try{ await __mkProcessRedirectEarly(); }catch(e){}
