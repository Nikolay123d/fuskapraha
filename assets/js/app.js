...
window.FIREBASE_CONFIG = {
  ...
  // IMPORTANT (Google redirect login): use the *.web.app authDomain.
  // Some setups/extensions/VPNs break the firebaseapp.com hosted handler page,
  // which causes the redirect flow to return to the site but NOT complete login.
  // Ensure these are authorized:
  //  - Firebase Console → Authentication → Settings → Authorized domains:
  //      nikolay123d.github.io, web-app-b4633.web.app, web-app-b4633.firebaseapp.com
...
// Make auth stick across reloads (important for redirect login).
// If the browser blocks IndexedDB/3rd-party cookies, Firebase may fall back,
// but this still improves reliability on both desktop and mobile.
try{
...
async function googleSignIn(){
  ...
  try{
    // Ensure auth survives the redirect round-trip.
...
async function handleGoogleRedirectResult(){
  ...
  try{
    // Re-apply persistence on return (some browsers reset it between redirects).
    try{ await auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL); }catch(e){}
...
auth.onAuthStateChanged(async (u)=>{
  ...
  // Redirect glue guard: during Google redirect round-trip, Firebase may emit null before getRedirectResult() completes.
  const __redirecting = (function(){ try{ return sessionStorage.getItem('__mk_auth_redirecting')==='1'; }catch(e){ return false; } })();
  if(!u && __redirecting){
