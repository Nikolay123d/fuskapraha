// === Firebase init ===
// Guard against double-initialization if scripts are injected twice.
...
// Admin helpers
// Single source of truth for admin UI: /roles/{uid}/admin (kept live in window.__isAdmin by watchAdminRole()).
// Fallback: hard UID allowlist (ADMIN_UIDS) for founders (UI only).
function isAdminUser(u){
  if(!u) return false;

  // If we already have a live role result for the current user, trust it.
  try{
    const cur = firebase.auth().currentUser;
    if(cur && u.uid === cur.uid && typeof window.__isAdmin === 'boolean') return window.__isAdmin === true;
  }catch(e){}

  // Fallback (UI only): hard UID list.
  try{
...
async function ensureMyPublic(u){
  ...
  if(!s.exists()){
    ...
    if(u.email && !v.email) up.email=u.email;
    if(u.email){
      const el = String(u.email).toLowerCase();
      if(!v.emailLower || String(v.emailLower).toLowerCase() !== el) up.emailLower = el;
    }
    if(!v.plan) up.plan='free';
    if(typeof v.planUntil !== 'number') up.planUntil = 0;
    if(typeof v.notifLastSeenTs !== 'number') up.notifLastSeenTs = Date.now();
