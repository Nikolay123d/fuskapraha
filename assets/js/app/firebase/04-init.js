// === Firebase init ===
// Must be idempotent. Some hosts / double script injection / SPA-like restores
// can execute init twice and crash with "Firebase App named '[DEFAULT]' already exists".
try{
  if(!firebase.apps || !firebase.apps.length){
    firebase.initializeApp(window.FIREBASE_CONFIG);
  }else{
    // Ensure default app is accessible even if initializeApp raced.
    firebase.app();
  }
}catch(e){
  // Failsafe: keep going, Firebase might already be initialized.
  try{ firebase.app(); }catch(_e){}
}
...
function isAdminUser(u){
  if(!u) return false;
  // Single source of truth: roles/{uid}/admin|moderator is watched in 16-auth-state.js and stored in window.__isAdmin.
  try{ if(window.__isAdmin === true) return true; }catch(e){}
  return false;
}
...
async function ensureMyPublic(u){
  ...
  if(!s.exists()){
    ...
    const v=s.val()||{};
    const nick = v.nick || u.displayName || 'Uživatel';
    const upd = {
      nick,
    ...
    // Privacy: never store any e-mail derived fields in usersPublic.
    // If a legacy record still has them, remove in the SAME update (so rules with strict validation pass).
    if(v.email) upd.email = null;
    if(v.emailLower) upd.emailLower = null;
    if(v.emailHash) upd.emailHash = null;
    if(v.email_hash) upd.email_hash = null;
