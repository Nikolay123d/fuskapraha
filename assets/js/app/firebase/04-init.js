...
let fn = null;
try{ fn = firebase.functions(); }catch(e){ fn = null; }
// Expose for modules (safe even if functions SDK is not loaded)
try{ window.fn = fn; }catch(e){}

// callFn(name, data) -> data (unwraps callable result)
window.callFn = async function(name, data){
  data = data || {};
  if(!fn || typeof fn.httpsCallable !== "function"){
    throw new Error("Functions SDK not available");
  }
  const callable = fn.httpsCallable(String(name||""));
