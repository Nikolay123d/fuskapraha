...
async function ensureMyPublic(u){
  try{
    ...
    // Build a safe patch that cannot violate rules (length limits / whitelisted fields)
    const patch = { updatedAt: nowTs };

    // Nick: prefer existing nick, then displayName, then fallback. Always clamp.
    const rawNick = String((v && v.nick) || u.displayName || '').trim();
    patch.nick = (rawNick || fallbackNick).slice(0, 60);

    // Backfill defaults (older rows or legacy create flows)
    if(!(v && typeof v.role === 'string' && v.role.length)) patch.role = 'seeker';
    if(!(v && typeof v.plan === 'string' && v.plan.length)) patch.plan = 'free';

    // Avatar: write only if missing OR invalid/too large.
    // This prevents permission_denied for legacy huge data: URLs.
    const curAvatar = (v && typeof v.avatar === 'string') ? v.avatar : '';
    const avatarOk = curAvatar && curAvatar.length <= 400000;
    if(!avatarOk){
      let av = String(u.photoURL || window.DEFAULT_AVATAR || './img/default-avatar.svg');
      if(/^data:image\//i.test(av) && av.length > 400000){
        av = String(window.DEFAULT_AVATAR || './img/default-avatar.svg');
      }
      if(av.length > 400000){
        av = String(window.DEFAULT_AVATAR || './img/default-avatar.svg');
      }
    ...
    if(!(v && typeof v.createdAt === 'number' && isFinite(v.createdAt))) patch.createdAt = nowTs;

    // Privacy cleanup: remove legacy public e-mail field if it exists.
    if(v && v.email) patch.email = null;

    if(!snap.exists()){
      // On create, write only whitelisted keys.
      await ref.set({
        nick: patch.nick,
        avatar: patch.avatar || (window.DEFAULT_AVATAR || './img/default-avatar.svg'),
        role: patch.role || 'seeker',
        plan: patch.plan || 'free',
        createdAt: patch.createdAt || nowTs,
        updatedAt: patch.updatedAt
      });
    }else{
      // On update, do not touch fields we didn't explicitly patch.
      await ref.update(patch);

      // Legacy cleanup: if email existed, hard-remove it.
      if(v && v.email){
        try{ await ref.child('email').remove(); }catch(e){}
      }
    }
  }catch(e){
    console.warn('ensureMyPublic failed', e);
  }
}
