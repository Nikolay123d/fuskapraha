...
async function ensureMyPublic(u){
  ...
  try{
    ...
    const v = s.val() || {};
    const needInit = (!s.exists()) || !!v.email || !v.nick || !v.role || !v.avatar || !v.createdAt || !v.plan;

    if(needInit){
      if(typeof window.callFn === 'function'){
        try{
          await window.callFn('profileInit', {
            nick: (v.nick || u.displayName || fallbackNick),
            role: (v.role || 'seeker'),
        ...
        // No functions bridge -> avoid noisy permission_denied writes
        console.warn('profileInit skipped: functions bridge not ready');
      }
    }
  }catch(e){
    console.warn('ensureMyPublic read failed', e);
  }
}
