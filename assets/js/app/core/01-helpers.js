...
function __startPrefetchStages(){
  ...
  // On mobile / slow networks it can make the first load feel slower,
  // because it competes with chat/DM rendering.
  // Enable manually via: localStorage.setItem('mk_prefetch_on','1')
  try{
    const on = String(localStorage.getItem('mk_prefetch_on')||'').trim();
    if(on !== '1') return;
  }catch(e){ /* ignore */ }
