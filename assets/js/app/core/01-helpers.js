...
function setMiniLoad(id, text, show){
  ...
  if(!el) return;
  let st = el.querySelector('.mini-status');
      if(!st){
        // fallback for legacy loaders that use a dedicated text node (e.g., autopostMiniText)
        try{
          const g1 = document.getElementById(String(loaderId).replace('MiniLoad','MiniText'));
          const g2 = document.getElementById(String(loaderId).replace('Load','Text'));
          const g3 = document.getElementById(String(loaderId)+'Text');
          st = g1 || g2 || g3 || null;
        }catch(e){}
      }
...
// === Slow network hint (optional): if a mini-loader is visible for too long, show "tap to retry".
function startSlowHint(loaderId, text, delayMs, onRetry){
  const el = document.getElementById(loaderId);
  if(!el) return null;
  const delay = Math.max(800, Number(delayMs)||2000);
  const prevClick = el.onclick;
  let active = true;
  const t = setTimeout(()=>{
    if(!active) return;
    try{
      const shown = (!el.hidden) && (el.style.display !== 'none');
      if(!shown) return;
      const st = el.querySelector('.mini-status');
      if(st && typeof text === 'string' && text.length) st.textContent = text;
      // Make the loader actionable: tap to retry the load.
      el.style.cursor = 'pointer';
      el.classList.add('slow-hint');
      el.onclick = (ev)=>{
        try{ ev.preventDefault(); ev.stopPropagation(); }catch(e){}
        try{ if(typeof onRetry === 'function') onRetry(); }catch(e){}
      };
    }catch(e){}
  }, delay);
  ...
  return ()=>{
    ...
    try{
      el.classList.remove('slow-hint');
      el.style.cursor = '';
      el.onclick = prevClick || null;
    }catch(e){}
  };
}
...
function __cacheKey(kind, extra){
  const uid = (typeof auth !== 'undefined' && auth && auth.currentUser && auth.currentUser.uid) ? auth.currentUser.uid : 'anon';
