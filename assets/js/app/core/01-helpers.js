...
function __startPrefetchStages(){
  // Strict lazy-loading: prefetch is disabled by default.
  // It can be enabled manually for experiments by setting window.MK_PREFETCH_ENABLED = true.
  if(window.MK_PREFETCH_ENABLED !== true) return;
  try{
    if('requestIdleCallback' in window){
...
// Blur active element (prevents mobile keyboard popping after navigation/restore)
...
try{ window.blurActiveElement = window.blurActiveElement || blurActiveElement; }catch(e){}
