...
async function getPremiumQrImg(){
  try{
    // Backward compatibility: older admin UI saved QR under settings/premium/qr
    // while the payment UI uses settings/payments/qrImg.
    const v1=(await db.ref('settings/premium/qr').get()).val();
    if(v1) return v1;
    const v2=(await db.ref('settings/payments/qrImg').get()).val();
    return v2 || PREMIUM_QR_IMG_DEFAULT;
