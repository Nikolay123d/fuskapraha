...
// If true, users must verify email after the grace period to keep sending chat/DM.
// If false, the app will NOT block sending due to e-mail verification.
window.REQUIRE_EMAIL_VERIFY = false;
...
async function resolveMsgUser(m){
  try{
    ...
    // Prefer "lite" profile (no presence) for message rendering to keep loads fast.
    try{ if(typeof window.getUserLite === 'function') return await window.getUserLite(m && m.by); }catch(e){}
...
const PREMIUM_PLANS = {
  vip: { code:'vip', title:'VIP', price:250, period:'měsíc', desc:'VIP: více limitů, autopost kampaně, priorita. (Cena: 250 Kč / měsíc)' },
...
// Sanitize image src/href to prevent svg/script payloads and javascript: links.
function safeImgSrc(url, fallback){
  try{
    ...
    // allow data image but block svg
    if(s.startsWith('data:image/')){
      // accept only common raster formats (strict)
      if(/^data:image\/(png|jpeg|jpg|webp|gif);/i.test(s)) return s;
      return fb;
    }
...
function safeHref(url, fallback){
  try{
    ...
    // allow only http(s), blob, or safe data:image (raster)
    if(s.startsWith('https://') || s.startsWith('http://') || s.startsWith('blob:')) return s;
    if(s.startsWith('data:image/')){
      if(/^data:image\/(png|jpeg|jpg|webp|gif);/i.test(s)) return s;
      return fb;
    }
