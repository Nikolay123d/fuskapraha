...
function __exitView(viewId){
  ...
  if(!v) return;
  try{
    // Unified listener registry: always clear tab-scoped subscriptions first.
    // (Global subscriptions MUST NOT be touched by tab navigation.)
    try{ window.MK?.subs?.clear('tab:'+v); }catch(e){}
    ...
    else if(v==='view-admin'){ window.__adminUnsub?.(); }
...
function showView(id, opts){
  ...
  try{
    ...
    if(id==='view-profile'){
      // Profile has no long-lived listeners, but needs a refresh after auth restore.
      try{ window.loadProfile && window.loadProfile(); }catch(e){}
      return;
    }
    if(id==='view-dm'){
      if(entering || opts.forceEnter || !window.__dmThreadsLoaded){
        window.__dmThreadsLoaded = true;
        // DM view lifecycle is handled by the DM module (lazy subs + restore-safe state)
        try{
          if(window.MK?.dm?.subs?.onEnterView) window.MK.dm.subs.onEnterView(opts);
          else {
            // Fallback (minimal)
            try{ setMiniLoad('dmMiniLoad','Načítáme soukromé zprávy…', true); }catch(e){}
            try{ ensureDMInboxLoaded && ensureDMInboxLoaded('tab-enter'); }catch(e){}
    ...
    if(id==='view-admin'){
      if(entering || opts.forceEnter || !window.__adminLoaded){
        window.__adminLoaded = true;
        try{ if(typeof window.enterAdminView === 'function') window.enterAdminView(); }catch(e){}
        // Legacy Stage5 admin loaders (kept for compatibility, but now called from router)
        try{ if(typeof window.__enterAdminStage5 === 'function') window.__enterAdminStage5(); }catch(e){}
      }
      return;
    }
  }catch(e){}
...
// Unified DM entry-point (top envelope, drawer item, tabs)
// The heavy logic lives in features/10-dm.js (DM.actions.openInbox).
async function openDMInbox(forceReload=true){
  try{
    if(window.MK?.dm?.actions?.openInbox){
      return await window.MK.dm.actions.openInbox(forceReload);
    }
  }catch(e){ console.warn(e); }
  // Fallback (should not happen): just show the tab and try to load threads.
  try{ showView('view-dm'); }catch(e){}
  try{ setMiniLoad('dmMiniLoad','Načítáme soukromé zprávy…', true); }catch(e){}
  try{ if(window.loadDmThreads) return await window.loadDmThreads(true); }catch(e){}
}
window.openDMInbox = openDMInbox;

// Unified DM entry-point by UID (from profile/usercard/friends/notifications)
// Unified DM entry-point by UID (from profile/usercard/friends/notifications)
// Implementation lives in features/10-dm.js (DM.actions.openByUid).
async function openDM(toUid){
  try{
    if(window.MK?.dm?.actions?.openByUid){
      return await window.MK.dm.actions.openByUid(toUid);
    }
  }catch(e){ console.warn(e); }
  // Fallback (minimal)
  ...
  try{ showView('view-dm'); }catch(e){}
  try{ if(typeof window.startDM === 'function') await window.startDM(peer); }catch(e){}
  try{ if(typeof window.openDMRoom === 'function') window.openDMRoom(me.uid, peer); }catch(e){}
}
...
// Ensure DM inbox loads after reload without requiring tab switching.
// Implementation lives in features/10-dm.js (DM.actions.ensureInboxLoaded).
function ensureDMInboxLoaded(reason){
  try{
    if(window.MK?.dm?.actions?.ensureInboxLoaded){
      return window.MK.dm.actions.ensureInboxLoaded(reason);
    }
  }catch(e){}
  // Fallback (minimal)
