...
function unsubscribeAll(){
  try{
    // Primary: registry-based (prevents "global ручки" and leaks)
...
function showView(id, opts){
  ...
  try{
    ...
    if(id==='view-admin'){
      ...
      // Always refresh pending requests + bot profiles when entering admin panel
      try{
        if(auth?.currentUser && isAdminUser(auth.currentUser)){
          try{ window.loadAdminRequests && window.loadAdminRequests(); }catch(e){}
          try{ window.loadBotProfiles && window.loadBotProfiles(); }catch(e){}
        }
      }catch(e){}

      if(needInit){
        try{ window.MK?.subs?.set && window.MK.subs.set('tab:admin', ()=>{ /* no live listeners by default */ }, 'tab'); }catch(e){}
      }
      return;
    }
  }catch(e){}
