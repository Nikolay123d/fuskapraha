...
// ---------------------------------------------------------------------------
// "По-взрослому": все realtime подписки должны регистрироваться в MK.subs
// и сниматься централизованно при уходе со вкладки.
// Scope policy:
//   - global: auth/usersPublic/roles/badges
//   - tab: heavy listeners per-view (chat/dm/friends/members/map)
...
function __exitView(viewId){
  ...
  try{
    ...
    else if(v==='view-admin'){ try{ window.__adminUnsub?.(); }catch(e){} }
    // rent/profile have no long-lived listeners in current MVP
  }catch(e){}
}

function unsubscribeAll(){
  // Always stop tab-scope listeners.
  // 1) Modern modules: via MK.subs registry.
  // 2) Legacy modules: via __exitView() (e.g., __chatUnsub/__dmUnsub/...)
  // NOTE: previously we returned early if MK.subs existed, which left legacy
  // listeners running in the background and caused occasional duplicate renders.
  const active = window.__MK_ACTIVE_VIEW__ || document.querySelector('.view.active')?.id || '';
  try{
    if(window.MK && window.MK.subs && typeof window.MK.subs.clearScope==='function'){
      window.MK.subs.clearScope('tab');
    }
  }catch(e){}
  try{ __exitView(active); }catch(e){}
}
...
function showView(id, opts){
  ...
  // Any navigation should close overlays (drawer, petal menu, sheets, etc.)
  // even when re-opening the same view.
  ...
  window.__profileLoaded = window.__profileLoaded || false;
  window.__adminLoaded = window.__adminLoaded || false;

  try{
    if(id==='view-chat'){
      const needInit = (entering || !window.__chatLoaded);
      if(needInit){
        window.__chatLoaded=true;
        // Always show a mini-loader when (re)entering chat (first paint is important).
        try{ setMiniLoad('chatMiniLoad', 'Načítám chat…', true); }catch(e){}
        try{ loadChat(); }catch(e){
          // If something unexpected happens, at least show a visible error.
          try{ mkToast('Chat: chyba načtení'); }catch(_e){}
          try{ setMiniLoad('chatMiniLoad', false); }catch(_e){}
        }
      }
      if(needInit){
        // Register tab-scope unsubscriber (prevents duplicate listeners)
        try{ window.MK?.subs?.set && window.MK.subs.set('tab:chat', ()=>{ try{ window.__chatUnsub?.(); }catch(e){} }, 'tab'); }catch(e){}
      }
      return;
    }

    if(id==='view-premium'){
      const needInit = (entering || !window.__premiumLoaded);
      if(needInit){
        window.__premiumLoaded = true;
      }
      // Always refresh UI on entry (cheap)
      try{ window.enterPremiumView && window.enterPremiumView(); }catch(e){}

      // Ensure tab-scope cleanup exists
      try{ window.MK?.subs?.set && window.MK.subs.set('tab:premium', ()=>{ try{ window.__premiumUnsub?.(); }catch(e){} }, 'tab'); }catch(e){}
      return;
    }

    if(id==='view-autopost'){
      const needInit = (entering || !window.__autopostLoaded);
      if(needInit) window.__autopostLoaded = true;
      // Autopost is server-scheduled. Client only watches UI data while the tab is open.
      try{ const u = auth?.currentUser; if(u && window.startAutopostWatch) window.startAutopostWatch(u.uid); }catch(e){}
      try{ window.enterAutopostView && window.enterAutopostView(); }catch(e){}
      try{ window.MK?.subs?.set && window.MK.subs.set('tab:autopost', ()=>{ try{ window.stopAutopostWatch?.(); }catch(e){} }, 'tab'); }catch(e){}
      return;
    }
    if(id==='view-rent'){
    ...
    if(id==='view-friends'){
      const needInit = (entering || !window.__friendsLoaded);
      if(needInit){
        window.__friendsLoaded=true;
        try{ loadFriends(); }catch{}
      }
      if(needInit){
        try{ window.MK?.subs?.set && window.MK.subs.set('tab:friends', ()=>{ try{ window.__friendsUnsub?.(); }catch(e){} }, 'tab'); }catch(e){}
      }
      return;
    }
    if(id==='view-members'){
      const needInit = (entering || !window.__membersLoaded);
      if(needInit){
        window.__membersLoaded=true;
        try{ loadMembers(); }catch{}
      }
      if(needInit){
        try{ window.MK?.subs?.set && window.MK.subs.set('tab:members', ()=>{ try{ window.__membersUnsub?.(); }catch(e){} }, 'tab'); }catch(e){}
      }
      return;
    }
    if(id==='view-dm'){
      const needInit = (entering || !window.__dmThreadsLoaded);
      if(needInit){
        ...
        try{ if(auth?.currentUser && window.loadDmRequests){ loadDmRequests(true); } }catch(e){}
        // Live update DM inbox only while DM tab is open (lazy subscriptions)
        try{
          const uid = auth?.currentUser?.uid;
          if(uid){
            if(window.startDmInboxLive) startDmInboxLive(uid);
            else if(window.watchDmThreadsLive) watchDmThreadsLive(uid);
          }
        }catch(e){}
        try{ if(auth?.currentUser && window.startDmRequestsLive){ startDmRequestsLive(auth.currentUser.uid); } }catch(e){}
      }
      if(needInit){
        try{ window.MK?.subs?.set && window.MK.subs.set('tab:dm', ()=>{ try{ window.__dmUnsub?.(); }catch(e){} }, 'tab'); }catch(e){}
      }
      return;
    }
    if(id==='view-map'){
      const needInit = (entering || !window.__mapLoaded);
      if(needInit){
      ...
      if(needInit){
        try{ window.MK?.subs?.set && window.MK.subs.set('tab:map', ()=>{ try{ window.__mapUnsub?.(); }catch(e){} }, 'tab'); }catch(e){}
      }
      return;
    }

    if(id==='view-profile'){
      const needInit = (entering || !window.__profileLoaded);
      if(needInit){
        window.__profileLoaded = true;
        // Ensure profile is always refreshed on entry (cheap, fixes "empty profile" after reload)
        try{ if(auth?.currentUser && window.loadMyProfileUI) loadMyProfileUI(auth.currentUser); }catch(e){}
      }else{
        try{ if(auth?.currentUser && window.loadMyProfileUI) loadMyProfileUI(auth.currentUser); }catch(e){}
      }
      if(needInit){
        try{ window.MK?.subs?.set && window.MK.subs.set('tab:profile', ()=>{ /* no live listeners */ }, 'tab'); }catch(e){}
      }
      return;
    }
    if(id==='view-admin'){
      const needInit = (entering || !window.__adminLoaded);
      if(needInit) window.__adminLoaded = true;
      try{ window.enterAdminView ? enterAdminView() : (window.initAdminPanelUX && initAdminPanelUX()); }catch(e){}
      return;
    }
  }catch(e){}
}
