  ...
  try{
    ...
    if(id==='view-premium'){
      ...
      return;
	    ...
	    if(id==='view-profile'){
	      // Profile is private; enterProfileView() will open auth modal if needed.
	      try{ window.enterProfileView && window.enterProfileView(); }catch(e){}

	      // No long-lived listeners here; still keep tab-scope cleanup slot for future.
	      try{ window.MK?.subs?.set && window.MK.subs.set('tab:profile', ()=>{ try{ window.__profileUnsub?.(); }catch(e){} }, 'tab'); }catch(e){}
	      return;
	    }
    if(id==='view-rent'){
...
try{ await openDMInbox(true); }catch(e){ try{ ensureDMInboxLoaded && ensureDMInboxLoaded('restore'); }catch(_){} }
    ...
    if(t === 'premium'){
      try{ showView('view-premium'); }catch(e){}
      try{ enterPremiumView && enterPremiumView(); }catch(e){}
      return;
    }

    if(t === 'profile'){
      try{ showView('view-profile'); }catch(e){}
      try{ enterProfileView && enterProfileView(); }catch(e){}
      return;
    }
    // default chat
