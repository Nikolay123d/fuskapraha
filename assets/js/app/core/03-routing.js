        ...
        if(overlay){
          ...
          if(buyBtn){
            ...
            buyBtn.onclick = ()=>{
              ...
              // default: open Premium/VIP purchase UI (no DM dependency)
              const prem = document.getElementById('premiumOverlay');
              if(window.openPremium) openPremium();
              else if(prem) openModal('premiumOverlay');
              else toast(title);
            };
          }
...
function showView(id){
  // On tab switch: tear down all tab-scoped subscriptions.
  // (Global badge/watchers live under scope "global" and are not touched.)
  try{
    const prev = document.querySelector('.view.active')?.id || '';
    if(prev && prev !== id){
      window.unsubscribeAll && window.unsubscribeAll();
    }
  }catch(e){}
