...
(function initMobileNav(){
  ...
  // Profile UI should load only after auth-ready (prevents "as if logged out" state)
  if(!window.__AUTH_READY__){
    try{ setMiniLoad('profileMiniLoad','Načítám profil…', true); }catch(e){}
    await new Promise(res=>window.addEventListener('auth:ready', ()=>res(), {once:true}));
    try{ setMiniLoad('profileMiniLoad','', false); }catch(e){}
  }
  const u = auth.currentUser;
  if(!u){
    try{ toast('Přihlaste se'); }catch(e){}
    try{ openModalAuth('login'); }catch(e){}
    return;
  }
  try{ if(typeof loadMyProfileUI === 'function') await loadMyProfileUI(u); }catch(e){ console.warn(e); }
});
  ...
  document.getElementById('notifClear')?.addEventListener('click', async ()=>{
    ...
    // Users should not delete the notifications feed (delivered by server/admin).
    // Allow cleanup only for admins.
    try{
      if(typeof isAdminUser==='function' && isAdminUser(me)){
