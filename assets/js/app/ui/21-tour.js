...
(function(){
  // Bump the key when onboarding steps/layout change.
  const TOUR_KEY = 'mk_tour_done_v2';
  ...
  function position(target){
    ...
    if(!r || (r.width < 8 && r.height < 8)){
      // Element exists but is hidden (e.g., auth-only button) → skip this step
      next();
      return;
    }
  ...
  function end(){
    ...
    // Use the real helper (scheduleNotifPromptAfterConsent) from core/02-config.js.
    try{
      const need = (('Notification' in window) && Notification.permission==='default' && localStorage.getItem('notifAllowed')!=='1');
      if(need){
        // Prefer direct prompt (user already clicked Skip/Next)
        if(typeof showNotifPrompt==='function') showNotifPrompt();
        else if(typeof scheduleNotifPromptAfterConsent==='function') scheduleNotifPromptAfterConsent(250);
      }
    }catch(e){}
  }
    ...
    steps = [
      {
        selector: '#drawerBtn',
        title: 'Меню (слева)',
        text: 'Левая кнопка ☰ — меню сайта (профиль, правила, помощь, админка).'
      },
      {
        selector: '#citySelect',
        title: 'Выбор чата',
        text: 'Рядом — выбор города. Это переключает общий чат.'
      },
      {
        selector: '#btnBell',
        title: 'Колокольчик',
        text: 'Уведомления и входящие запросы (друзья/DM/платежи).'
      },
      {
        selector: '#btnMe',
        title: 'Профиль (справа)',
        text: 'Справа — ваш профиль: ник, аватар, настройки.'
      },
      {
        // DM entry point is the floating envelope button (header DM is intentionally removed)
        selector: '#fabDm',
        title: 'Личные сообщения',
        text: 'Конверт снизу — личные сообщения (DM).'
      },
      {
        selector: '#drawerPremium',
        title: 'Premium / VIP',
        text: 'Здесь можно отправить заявку на Premium / Premium+ / VIP и смотреть статус.',
        before: () => openDrawer()
      }
    ];
