...
(function(){
  ...
  function mkBootstrap(){
    ...
    // UX fix: when the user FIRST lands on the site, the default view is the chat.
    // Previously chat data load was started only after auth-state fired (auth-ready restore),
    // so visitors could see only the background with no loader.
    // Start chat load immediately for the default active view (works for guests too).
    try{
      const active = document.querySelector('.view.active')?.id || '';
      if(active === 'view-chat' && !window.__chatLoaded){
        window.__chatLoaded = true;
        try{ setMiniLoad('chatMiniLoad','Načítáme chat…', true); }catch(e){}
        try{ loadChat(); }catch(e){}
      }
    }catch(e){}
