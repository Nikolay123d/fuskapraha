...
(function(){
  ...
  function mkBootstrap(){
    ...
    // 4.5) First paint must be fast: start PUBLIC chat immediately (even for guests).
    // Guests are allowed to read chat, so we do NOT wait for auth-ready.
    // NOTE: we do NOT mark __chatLoaded here.
    // Auth-state restore still needs to run and will re-subscribe cleanly.
    try{
      const chatView = document.getElementById('view-chat');
      if(chatView && chatView.classList.contains('active')){
        try{ setMiniLoad('chatMiniLoad','Načítáme posledních 15 zpráv…', true); }catch(e){}
        try{ loadChat(); }catch(e){}
      }
    }catch(e){}

    // 4.6) Never keep the whole UI blocked behind the fullscreen preloader.
    // After this point we rely on per-view mini-loaders.
    try{ hidePreloader(); }catch(e){}

    // 5) Syncs (wallpapers/sounds) — defer to avoid blocking first paint (mobile)
    try{
      if(!window.__MK_GLOBAL_SYNCS_STARTED__){
        const kick = ()=>{
          try{
            if(window.__MK_GLOBAL_SYNCS_STARTED__) return;
            window.__MK_GLOBAL_SYNCS_STARTED__ = true;
            startGlobalSyncs();
          }catch(e){}
        };
