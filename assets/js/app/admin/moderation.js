...
async function adminLoadMapPoints(){
  // Admin OR moderator can review map points.
  if(!window.__isStaff) return;
    ...
    for(const it of items){
      ...
      row.innerHTML = `
        ...
        <div class="bubble" style="width:100%">
          ...
          ${photos.length?('<div class="row" style="flex-wrap:wrap;gap:6px">'+photos.map(p=>`<img src="${esc(safeImgSrc(p,''))}" style="width:78px;height:78px;object-fit:cover;border-radius:12px">`).join('')+'</div>'):''}
...
try{
  ...
  window.loadPoi = function(){
    ...
    POI_REF.on('child_added', async snap=>{
      ...
      const photo = (Array.isArray(v.photos) && v.photos[0]) ? `<br><img src="${esc(safeImgSrc(v.photos[0],''))}" style="max-width:220px;border-radius:12px">` : '';
