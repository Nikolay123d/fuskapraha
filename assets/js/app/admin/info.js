...
document.getElementById('supportSend')?.addEventListener('click', async ()=>{
  try{
    ...
    // Server-first (бетон): create ticket through Cloud Functions, do not let client write into /support directly.
    if(typeof window.callFn !== 'function') throw new Error('functions_unavailable');
    const r = await window.callFn('supportTicketCreate', {
      text: txt||'',
      img: _supportImgData||null,
      ua: (navigator.userAgent||'')
    });
    if(!r || r.ok !== true){
      if(String(r?.reason||'')==='limit') toast('Лимит заявок исчерпан');
...
document.getElementById('broadcastSave')?.addEventListener('click', async ()=>{
  try{
    ...
    try{ window.adminShowPanel && adminShowPanel(null); }catch(e){}
