...
(function fabMenuModule(){
  function initFabMenu(){
    ...
    // Any FAB menu action should behave like navigation: close all overlays first.
    // Fixes cases where the "petal menu" stays open after clicking an action.
    const closeAll = ()=>{ try{ if(typeof closeAllOverlays==='function') closeAllOverlays({keep:null}); }catch(e){} };
    ...
    const render = ()=>{
      ...
      // UX: FAB (+) is admin-only. Regular users use drawer/topbar.
      try{ btn.style.display = isAdm ? 'flex' : 'none'; }catch(e){}
      try{ menu.style.display = isAdm ? 'block' : 'none'; }catch(e){}
      if(!isAdm){
        try{ menu.hidden = true; }catch(e){}
        return;
      }

      // Logged out (FAB hidden anyway)
      if(!me){ return; }
      ...
      menu.appendChild(mkBtn('Koupit privilegia', {
        ...
        onClick: ()=>{
          closeAll();
          close();
          try{ showView('view-premium'); }catch(e){ console.warn(e); toast('Privilegia nejsou dostupná'); }
        }
      }));
