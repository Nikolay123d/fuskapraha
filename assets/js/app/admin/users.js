...
async function adminLoadUsers(){
  ...
  try{
    if(_adminUsersMode==='complaints'){
      ...
      for(const it of items){
        ...
        row.innerHTML = `
          ...
          <div class="bubble" style="width:100%">
            ...
            ${it.img?`<div class="text"><img src="${esc(safeImgSrc(it.img,''))}" style="max-width:220px;border-radius:12px"></div>`:''}
      ...
      const filtered=q?items.filter(x=> (String(x.nick||'').toLowerCase().includes(q) || String(x.uid||'').toLowerCase().includes(q)) ): items;
...
async function adminOpenUserCard(uid){
  ...
  try{
    ...
    document.getElementById('adminUserCardAva').src = safeImgSrc(pub.avatar || window.DEFAULT_AVATAR, window.DEFAULT_AVATAR);
    document.getElementById('adminUserCardUid').textContent = 'UID: '+uid;
    document.getElementById('adminUserCardEmail').textContent = 'Email: —';
    ...
    const parts=[];
    parts.push('Stats: chat=' + (stats.chatCount||0));
    parts.push('dm=' + (stats.dmCount||0));
    parts.push('lastSeen=' + (stats.lastSeen? new Date(stats.lastSeen).toLocaleString():'—'));
    if(stats.lastActiveDay) parts.push('day=' + String(stats.lastActiveDay));
    if(typeof stats.streakDays === 'number') parts.push('streak=' + (stats.streakDays||0));
    if(typeof stats.maxStreakDays === 'number') parts.push('maxStreak=' + (stats.maxStreakDays||0));
    if(typeof stats.totalActiveDays === 'number') parts.push('activeDays=' + (stats.totalActiveDays||0));
    if(typeof stats.lastVisitGapHours === 'number') parts.push('gapH=' + stats.lastVisitGapHours);
    document.getElementById('adminUserStats').textContent = parts.join(' · ');
  }catch(e){}
}
...
document.getElementById('adminUserCardClose')?.addEventListener('click', ()=>{ try{ window.adminShowPanel && adminShowPanel('adminUsersPanel'); }catch(e){} });
document.getElementById('adminUsersClose')?.addEventListener('click', ()=>{ try{ window.adminShowPanel && adminShowPanel(null); }catch(e){} });
document.getElementById('adminBroadcastClose')?.addEventListener('click', ()=>{ try{ window.adminShowPanel && adminShowPanel(null); }catch(e){} });
document.getElementById('adminMapPointsClose')?.addEventListener('click', ()=>{ try{ window.adminShowPanel && adminShowPanel(null); }catch(e){} });
