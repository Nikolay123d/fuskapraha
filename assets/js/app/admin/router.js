...
(function adminRouter(){
  ...
  function wireBotsButtons(){
    const once = (el, fn)=>{
      if(!el || el.dataset.wired === '1') return;
      el.dataset.wired = '1';
      el.addEventListener('click', fn);
    };
    // Keep bots UI as a single modal layer (no stacking). openModal() already closes other modals.
    once($('adminOpenBotsManage'), (e)=>{
      e.preventDefault();
      try{ openModal('modalBots'); }catch(err){}
      try{ window.loadBotsModal && loadBotsModal(); }catch(err){}
    });
    once($('adminOpenBotInbox'), (e)=>{
      e.preventDefault();
      try{ openModal('modalBotInbox'); }catch(err){}
      try{ window.loadBotInboxModal && loadBotInboxModal(); }catch(err){}
    });
  }
