...
(function(){
  ...
  async function loadBot(uid){
    uid = normUid(uid);
    if(!uid) return;
    // IMPORTANT: this module is bundled globally, and the admin DOM exists even for non-admin users (hidden).
    // Do not attempt any reads from protected botProfiles unless the current user is admin.
    if(!isAdmin() && !(window.isAdminUser && window.auth && auth.currentUser && isAdminUser(auth.currentUser))){
      return; // silent: avoids permission_denied noise for non-admins
    }
    try{
  ...
  // Auto-load default only if admin is actually signed in.
  // Wait briefly for auth/roles hydration.
  (function waitAdminHydration(){
    let tries = 0;
    const t = setInterval(()=>{
      tries++;
      const me = (window.auth && auth.currentUser) ? auth.currentUser : null;
      const ok = !!me && (rootRole('admin') || (window.isAdminUser && isAdminUser(me)));
      if(ok){
        clearInterval(t);
        loadBot(elUid.value);
      }
