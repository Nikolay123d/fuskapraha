  ...
  try{
    ...
    rows.forEach(n=>{
      ...
      if(n.kind==='friend' || n.kind==='dm_request'){
        ...
        // Extra CTA: Add to friends from bell (requested)
        // - For dm_request we allow "Přidat do přátel" without leaving the bell.
        // - This calls server-first friendSend via friends.js.
        let btnAddFriend = null;
        if(n.kind === 'dm_request'){
          btnAddFriend = document.createElement('button');
          btnAddFriend.type = 'button';
          btnAddFriend.className = 'btn-purple';
          btnAddFriend.textContent = 'Přidat do přátel';
          btnAddFriend.addEventListener('click', async (ev)=>{
            ev.preventDefault(); ev.stopPropagation();
            const uid = String(n.fromUid||'');
            if(!uid) return;
            try{
              if(typeof window.sendFriendRequest !== 'function'){
                toast('Friends není dostupné');
                return;
              }
        ...
        if(btnAddFriend) actions.appendChild(btnAddFriend);
