...
async function loadNotificationsFeed(limit=100){
  ...
  try{
    ...
    const [frSnap, dmSnap, paySnap, notifSnap, friendsSnap, vacSeenSnap, vacIdxSnap] = await Promise.all([
    ...
    rows.forEach(n=>{
      ...
      // Person strip (avatar + nick + quick actions) for friend/DM/vacancy
      try{
        const personUid = (n.kind==='vacancy') ? (n.uid||'') : (n.fromUid||'');
        if(personUid && (n.kind==='friend' || n.kind==='dm' || n.kind==='vacancy')){
          const u = users[personUid] || {};
          const strip = document.createElement('div');
          strip.className = 'notif-person';

          const img = document.createElement('img');
          img.alt = '';
          img.className = 'ava-sm';
          img.src = safeImgSrc(u.avatar || n.fromAvatar || window.DEFAULT_AVATAR, window.DEFAULT_AVATAR);

          const name = document.createElement('div');
          name.className = 'notif-name';
          name.textContent = (n.fromNick || u.nick || personUid);
          ...
          btnProfile.addEventListener('click', (e)=>{ e.stopPropagation(); try{ window.showUserCard?.(String(personUid)); }catch(err){} });
          ...
          if(n.kind==='friend'){
            ...
            btnAcc.addEventListener('click', async (e)=>{ e.stopPropagation(); try{ await window.acceptFriend?.(String(personUid)); await loadNotificationsFeed(limit); }catch(err){} });
            actions.appendChild(btnAcc);

            const btnDec = document.createElement('button');
            btnDec.type = 'button';
            btnDec.className = 'ghost danger';
            btnDec.textContent = 'Odmítnout';
            btnDec.addEventListener('click', async (e)=>{ e.stopPropagation(); try{ await window.declineFriend?.(String(personUid)); await loadNotificationsFeed(limit); }catch(err){} });
          ...
          if(n.kind==='dm'){
            ...
            btnDm.type = 'button';
            btnDm.className = 'btn primary';
            btnDm.textContent = 'DM';
            btnDm.addEventListener('click', async (e)=>{ e.stopPropagation(); try{ await openDM(String(personUid)); }catch(err){} });
