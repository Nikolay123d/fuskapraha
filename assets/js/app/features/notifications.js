...
async function loadNotificationsFeed(limit=100){
  ...
  try{
    ...
    const [frSnap, dmReqSnap, dmSnap, paySnap, notifSnap, friendsSnap, vacSeenSnap, vacIdxSnap] = await Promise.all([
      ...
      db.ref('dmRequests/'+me.uid).orderByChild('ts').limitToLast(50).get(),
    ...
    for(const [fromUid, req] of Object.entries(dmReqVal)){
      ...
      items.push({
        ...
        ts,
        fromUid,
        previewText: String(req?.previewText || ''),
        fromNick: String(req?.fromNick || ''),
        fromAvatar: String(req?.fromAvatar || ''),
      });
    }
    ...
    rows.forEach(n=>{
      ...
      if(n.kind==='friend'){
        ...
        const u = users[n.fromUid] || {};
        const nick = (n.fromNick || u.nick || n.fromUid || 'Uživatel');
        title = 'DM žádost';
        const preview = String(n.previewText||'').trim();
        bodyText = nick + (preview ? (': ' + preview) : '') + '\n(žádost o soukromé zprávy)';
