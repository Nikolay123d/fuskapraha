...
// --- Admin tab lifecycle (subscriptions must be tab-scoped) ---
let __ADMIN_SETTINGS_WIRED = false;
let __adminLiveRefs = null; // {wp:{ref,cb}, snd:{ref,cb}, prem:{ref,cb}}

function initAdminSettings(){
  // Wire UI handlers only once. (No realtime .on() listeners here.)
  if(__ADMIN_SETTINGS_WIRED) return;
  __ADMIN_SETTINGS_WIRED = true;
  ...
  // Privacy cleanup: remove legacy email fields from usersPublic (OSINT hardening)
  const btnClean = document.getElementById('adminCleanUsersPublicEmails');
  if(btnClean && !btnClean.dataset.wired){
    btnClean.dataset.wired = '1';
    btnClean.addEventListener('click', async ()=>{
      try{
        if(!auth.currentUser || !window.__isAdmin){ toast('Pouze administrátor.'); return; }
        if(!confirm('Smazat email* z usersPublic pro všechny uživatele?')) return;
        btnClean.disabled = true;
        const out = document.getElementById('adminCleanUsersPublicEmailsRes');
        if(out) out.textContent = 'Načítám usersPublic…';
        ...
        async function flush(){
          ...
          for(const uid of batch){
            updates['usersPublic/'+uid+'/email'] = null;
            updates['usersPublic/'+uid+'/emailLower'] = null;
            updates['usersPublic/'+uid+'/emailHash'] = null;
            updates['usersPublic/'+uid+'/email_hash'] = null;
          }
        ...
        for(const uid of uids){
          const p = all[uid] || {};
          if(p.email || p.emailLower || p.emailHash || p.email_hash){
            batch.push(uid);
            cleaned++;
            if(batch.length >= 200){
              if(out) out.textContent = 'Čistím… ('+cleaned+')';
              await flush();
            }
          }
        }
        if(out) out.textContent = 'Čistím… ('+cleaned+')';
        await flush();
        if(out) out.textContent = cleaned ? ('Hotovo. Vyčištěno: '+cleaned) : 'Nic k čištění.';
        toast(cleaned ? ('Vyčištěno: '+cleaned) : 'Nic k čištění');
        try{ if(typeof auditLog==='function') auditLog('usersPublic_email_cleanup', '-', {cleaned}); }catch(e){}
      }catch(e){
        console.warn(e);
        toast('Chyba čištění');
      }finally{
        try{ btnClean.disabled = false; }catch(e){}
      }
    });
  }
...
function startAdminSettingsLive(){
  // Real-time admin previews should exist ONLY while admin tab is open.
  // They are registered in MK.subs under 'tab:view-admin'.
  try{ if(__adminLiveRefs) return; }catch(e){}
  __adminLiveRefs = {};
  ...
  try{
    ...
    ref.on('value', cb);
    __adminLiveRefs.prem = {ref, cb};
  }catch(e){}

  const off = ()=>{
    try{ if(__adminLiveRefs?.wp) __adminLiveRefs.wp.ref.off('value', __adminLiveRefs.wp.cb); }catch(e){}
    try{ if(__adminLiveRefs?.snd) __adminLiveRefs.snd.ref.off('value', __adminLiveRefs.snd.cb); }catch(e){}
    try{ if(__adminLiveRefs?.prem) __adminLiveRefs.prem.ref.off('value', __adminLiveRefs.prem.cb); }catch(e){}
    __adminLiveRefs = null;
  };
  try{ window.MK?.subs?.set('tab:view-admin', 'adminSettingsLive', off); }catch(e){}
}

function stopAdminSettingsLive(){
  try{ window.MK?.subs?.off('tab:view-admin', 'adminSettingsLive'); }catch(e){}
  try{
    if(__adminLiveRefs?.wp) __adminLiveRefs.wp.ref.off('value', __adminLiveRefs.wp.cb);
    if(__adminLiveRefs?.snd) __adminLiveRefs.snd.ref.off('value', __adminLiveRefs.snd.cb);
    if(__adminLiveRefs?.prem) __adminLiveRefs.prem.ref.off('value', __adminLiveRefs.prem.cb);
  }catch(e){}
  __adminLiveRefs = null;
}

// Router entry/exit hooks
window.enterAdminView = function(){
  try{ initAdminSettings(); }catch(e){}
  try{ if(window.__isAdmin) startAdminSettingsLive(); }catch(e){}
};
window.__adminUnsub = function(){
  try{ stopAdminSettingsLive(); }catch(e){}
  try{ window.MK?.subs?.clear('tab:view-admin'); }catch(e){}
};
...
function initFabMenu(){
  ...
  // Toggle
  btn.addEventListener('click', (e)=>{ e.preventDefault(); toggle(); render(); });
  ...
  // Keep content in sync when role/auth changes
  try{ window.addEventListener('admin-changed', render); }catch(e){}
  try{ auth.onAuthStateChanged(()=>{ try{ render(); }catch(e){} }); }catch(e){}
...
function initBotsModalUI(){
  ...
  document.getElementById('botsModalAdd')?.addEventListener('click', async ()=>{
    ...
    const btn = document.getElementById('botsModalAdd');
    if(btn && btn.dataset.busy==='1') return;
    if(btn){ btn.dataset.busy='1'; btn.disabled = true; }
    try{
      const id = db.ref('bots').push().key;
      // Default nick should be unique to avoid accidental duplicates on double click.
      const nick = 'Bot-'+String(id).slice(-4);
      const city = getCity();

      // Client-side dedupe (best effort)
      try{
      ...
      await db.ref('bots/'+id).set({
        ownerUid: me.uid,
        nick,
        nickLower: nick.trim().toLowerCase(),
        city,
        intervalMin:15,
        text:'Ahoj!',
        enabled:true,
      ...
      try{ await auditLog('bot_create', 'bot_'+id, {id, nick, city}); }catch(e){}
      await loadBotsModal();
    }finally{
...
let __broadcastRef = null;
let __broadcastCb = null;
function watchBroadcast(){
  // Global watcher (light) + registered in MK.subs to avoid duplicates.
  try{ window.MK?.subs?.off('global', 'broadcast'); }catch(e){}
  try{ if(__broadcastRef && __broadcastCb) __broadcastRef.off('value', __broadcastCb); }catch(e){}
  __broadcastRef = null;
  __broadcastCb = null;

  try{
    const ref = db.ref('settings/broadcast');
    const cb = (s)=>{ const v=s.val(); if(v) showBroadcastBanner(v); };
    ref.on('value', cb);
    __broadcastRef = ref;
    __broadcastCb = cb;
    try{
      window.MK?.subs?.set('global', 'broadcast', ()=>{
        try{ if(__broadcastRef && __broadcastCb) __broadcastRef.off('value', __broadcastCb); }catch(e){}
        __broadcastRef = null;
        __broadcastCb = null;
      });
...
// (legacy) stats bump hooks were removed.
// If you want per-user counters, increment directly inside send handlers where the push is confirmed.
