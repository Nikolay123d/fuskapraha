...
function initBotsModalUI(){
  ...
  document.getElementById('botsModalAdd')?.addEventListener('click', async ()=>{
    ...
    // Anti-double click / re-entrancy
    try{ if(window.MK && !window.MK.lockTake('admin:bot:add', 1500)) return; }catch(e){}
    ...
    const city = getCity();
    try{
      // Lightweight duplicate shield: if the same default bot was created by this admin within ~2s, skip.
      try{
        const snap = await db.ref('bots').orderByChild('createdAt').limitToLast(50).get();
        const bots = snap.val()||{};
        const recentDup = Object.values(bots).some(b=>b && b.ownerUid===me.uid && (b.nick||'')==='Bot' && (b.city||'')===city && (now - (b.createdAt||0)) < 2000);
        if(recentDup){ toast('Bot už byl přidán'); return; }
      }catch(e){}
