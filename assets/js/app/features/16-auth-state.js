...
(function initAuthState(){
  ...
  let offRoles = null;
  let offPublic = null;
  let __rolesBootstrapped = false;

  // --- Topbar/profile + roles watchers (single source of truth) ---
  function renderTopBar(profile, roles){
    profile = profile || {};
    roles = roles || {};
    const ava = profile.avatar || window.DEFAULT_AVATAR || 'assets/img/default-avatar.svg';
    try{
  ...
  function watchMyRoles(uid){
    ...
    try{
      ...
      const cb = (snap)=>{
        ...
        // Bootstrap roles for known admin UIDs (one-time, safe).
        // This removes dependence on hardcoded email checks.
        try{
          if(!__rolesBootstrapped && (!snap.exists() || Object.keys(v||{}).length===0)){
            if(Array.isArray(window.ADMIN_UIDS) && window.ADMIN_UIDS.includes(uid)){
              __rolesBootstrapped = true;
              // Rules allow writing only once when roles/{uid} doesn't exist and uid is whitelisted.
              ref.set({ admin:true }).catch(()=>{});
            }
          }
        }catch(e){}
