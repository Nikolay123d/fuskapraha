...
function _setAdminUI(is, user){
  ...
  // Let UI modules react (FAB menu, admin-only widgets, etc.)
  try{
    window.dispatchEvent(new CustomEvent('admin-changed', { detail: { is: !!is, uid: user?user.uid:null } }));
  }catch(e){
    try{ window.dispatchEvent(new Event('admin-changed')); }catch(_){}
  }
}
