...
(function initAuthState(){
  ...
  // --- Admin role watcher (kept from legacy) ---
  async function watchAdminRole(uid){
    ...
    try{
      const ref = db.ref('usersPublic/'+uid+'/role');
      const cb = (snap)=>{
        const role = (snap.val()||'');
        const isAdmin = role === 'admin' || role === 'mod';
        try{ window.__isAdmin = isAdmin; }catch(e){}
        try{
          const adminEl = document.getElementById('drawerAdmin');
          if(adminEl){
            if(isAdmin) adminEl.removeAttribute('hidden');
            else adminEl.setAttribute('hidden','');
          }
        }catch(e){}
      };
      ref.on('value', cb);
