...
(function initAuthState(){
  ...
  function renderTopBar(profile, roles){
    ...
    try{
      ...
      // Role flags
      // __isAdmin  -> only full admin
      // __isMod    -> moderator (limited)
      // __isStaff  -> admin OR moderator
      try{
        window.__isAdmin = !!isAdmin;
        window.__isMod = !!isMod;
        window.__isStaff = !!(isAdmin || isMod);
      }catch(_e){}
    }catch(e){}
  }
    ...
    auth.onAuthStateChanged(async (u)=>{
      ...
      if(!u){
        ...
        try{
          ...
          window.__isMod = false;
          window.__isStaff = false;
        }catch(e){}
        ...
        try{ stopDmRequestsLive && stopDmRequestsLive(); }catch(e){}
        try{ if(typeof watchMyPlan === 'function') watchMyPlan(null); }catch(e){}
        try{ if(window.stopAutopostEngine) stopAutopostEngine(); }catch(e){}
        try{ stopDmThreadsLive && stopDmThreadsLive(); }catch(e){}
        try{ stopDmInboxLive && stopDmInboxLive(); }catch(e){}
      ...
      // Clear local unverified counters when user verifies e-mail
      try{
        if(u.emailVerified){
          localStorage.removeItem('unv_msgs_'+u.uid);
          localStorage.removeItem('verify_deadline_'+u.uid);
        }
      }catch(e){}
      ...
      // Autoposting campaigns watcher (server posts on schedule)
      ...
      // Fix: if user opened Profile before login (view already active), profile UI stayed blank.
      // After sign-in, refresh profile tab if it is currently visible.
      try{
        const active = (window.__MK_ACTIVE_VIEW__ || document.querySelector('.view.active')?.id || '').trim();
        if(active === 'view-profile'){
          try{ if(typeof loadMyProfileUI === 'function') loadMyProfileUI(u); }catch(e){}
        }
      }catch(e){}
    });
  }
