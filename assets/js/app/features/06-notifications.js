...
function updateTopBadges(){
  ...
  // Bell badge = events (admin notifications + friend requests + payments decisions)
  setBadge(document.getElementById('bellBadge') || document.getElementById('notifBadge'), (__notifUnread + __friendReqUnread + __payUnread));
}
...
// DM Requests badge (incoming) = count(dmRequests/{uid})
let __dmReqRef = null;
let __dmReqCb = null;

function watchDmRequestsBadge(uid){
  try{ window.MK?.subs?.off('global', 'dmRequests'); }catch(e){}
  ...
  __dmReqRef = db.ref('dmRequests/'+uid);
  __dmReqCb = (snap)=>{
    const v = snap.val() || {};
    __dmReqUnread = Object.keys(v).length;
    updateTopBadges();
  };
  __dmReqRef.on('value', __dmReqCb);

  try{
    window.MK?.subs?.set('global','dmRequests', ()=>{
      try{ if(__dmReqRef && __dmReqCb) __dmReqRef.off('value', __dmReqCb); }catch(e){}
      __dmReqRef = null; __dmReqCb = null;
    });
  }catch(e){}
}
window.watchDmRequestsBadge = watchDmRequestsBadge;

// Payments "decision" events (granted/rejected) after lastSeen
let __payRef = null;
let __payCb = null;
let __payCache = null;

function __calcPayUnread(val){
  let c = 0;
  if(val){
    for(const k of Object.keys(val)){
      ...
      if((st==='granted' || st==='rejected') && decidedAt && decidedAt > (__notifLastSeenTs||0)){
        c += 1;
      }
    }
  }
...
function watchPaymentsDecisionBadge(uid){
  try{ window.MK?.subs?.off('global', 'payDecisions'); }catch(e){}
  try{ if(__payRef && __payCb) __payRef.off('value', __payCb); }catch(e){}
  __payRef = null;
  __payCb = null;
  __payCache = null;
  __payUnread = 0;
  ...
  __payRef = db.ref('payments/requests/'+uid).limitToLast(25);
  __payCb = (snap)=>{
    __payCache = snap.val() || {};
    __calcPayUnread(__payCache);
  };
  __payRef.on('value', __payCb);

  try{
    window.MK?.subs?.set('global','payDecisions', ()=>{
      try{ if(__payRef && __payCb) __payRef.off('value', __payCb); }catch(e){}
      __payRef = null; __payCb = null;
      __payCache = null;
    });
  }catch(e){}
}
window.watchPaymentsDecisionBadge = watchPaymentsDecisionBadge;
...
function watchNotificationsUnread(uid){
  // Unified registry (global): prevent duplicated listeners across auth rebinds
  try{ window.MK?.subs?.off('global', 'notifUnread'); }catch(e){}
  ...
  __notifLastSeenCb = (snap)=>{
    ...
    try{ if(__payCache) __calcPayUnread(__payCache); }catch(e){}
  ...
  try{
    window.MK?.subs?.set('global', 'notifUnread', ()=>{
      try{ if(__notifLastSeenRef && __notifLastSeenCb) __notifLastSeenRef.off('value', __notifLastSeenCb); }catch(e){}
      try{ if(__notifUnreadRef && __notifUnreadCb) __notifUnreadRef.off('value', __notifUnreadCb); }catch(e){}
      __notifLastSeenRef = null;
      __notifLastSeenCb = null;
...
async function clearNotifications(){
  // Notifications are admin-only write: regular users cannot delete them.
  // "Clear" = just mark as read (lastSeenTs).
...
function watchDmUnread(uid){
  // Unified registry (global): prevent duplicated listeners across auth rebinds
  try{ window.MK?.subs?.off('global', 'dmUnread'); }catch(e){}
  ...
  // Register unified global off
  try{
    window.MK?.subs?.set('global', 'dmUnread', ()=>{
      try{ if(__inboxMetaRef && __inboxMetaCb) __inboxMetaRef.off('value', __inboxMetaCb); }catch(e){}
      __inboxMetaRef = null;
      __inboxMetaCb = null;
    });
