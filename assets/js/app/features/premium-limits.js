...
(function premiumLimitsModule(){
  ...
  async function incLimit(action, delta=1){
    ...
    try{
      ...
      console.warn('[premium-limits] consumeLimit failed', e);
    }

    // Local fallback removed (server-only). If Cloud Functions is unreachable,
    // we do NOT write to RTDB directly (rules would/should block it).
    return false;
  }
