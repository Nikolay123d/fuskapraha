...
async function rentAdd(){
  ...
  let img=null; if(f){ img=await fileToDataURL(f, { max: 1280, quality: 0.78 }); }
...
async function loadRent(){
  ...
  for(const x of a){
    ...
    d.innerHTML = `<div class="ava" data-uid="${esc(x.by)}"><img src="${esc(safeImgSrc(u.avatar||window.DEFAULT_AVATAR, window.DEFAULT_AVATAR))}"></div>`+
      ...
      (safeImgSrc(x.img,'')?`<div class="text"><img src="${esc(safeImgSrc(x.img,''))}"></div>`:'')+
