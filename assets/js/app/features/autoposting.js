...
(function autopostingModule(){
  ...
  function _wireUIOnce(){
    ...
    document.addEventListener('mk:plan-changed', ()=>{
      ...
      // If plan was upgraded (free -> paid), start posting without waiting for the next interval.
      try{ if(_uid) setTimeout(_tickOnce, 600); }catch(e){}
    });
  }
  ...
  async function _tryPostCampaign(cid, c){
    try{
      ...
      if(window.checkLimit){
        const lim = await checkLimit('autopost_posts');
        if(lim && lim.ok === false) return;
      }
  ...
  function startAutopostEngine(uid){
    try{
      ...
      const cb = (snap)=>{
        try{ if(typeof _stopSlow === 'function') _stopSlow(); }catch(e){}
      _setMini(false);
        _camps = snap.val() || {};
        try{ _renderList(); }catch(e){}
        // When campaigns change (new/edited/activated), run a near-immediate tick.
        try{
          if(_tickDebounce){ clearTimeout(_tickDebounce); }
          _tickDebounce = setTimeout(_tickOnce, 600);
        }catch(e){}
      };
      ref.on('value', cb);
      ...
      // Scheduler tick
      _tick = setInterval(_tickOnce, 15000);
