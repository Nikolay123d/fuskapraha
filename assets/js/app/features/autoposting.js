// Autoposting campaigns (server-scheduled; client only manages campaigns)
(function autopostingModule(){
  ...
  function _renderCounters(){
    ...
    if(!el) return;
    const slots=_slots();
    const total=Object.keys(_camps||{}).length;
    const active=_activeCount();
    el.textContent = slots ? `Kampaně: ${total}/${slots} · aktivní: ${active}` : `Kampaně: ${total} · aktivní: ${active}`;
  }
...
if(!can){
  if(h) h.textContent = 'Autoposting je dostupný v Premium / Premium+ / VIP. Upgrade najdeš v „Privilegia“.';
}else{
  if(h) h.textContent = '✅ Autopost běží na serveru (funguje i když web zavřeš).';
}
  }
  ...
  function _campCard(id, c){
    ...
    const np = Number(c.nextPostTs||0);
    const lpStr = lp ? new Date(lp).toLocaleString() : '–';
    const npStr = np ? new Date(np).toLocaleString() : '–';
    meta.textContent = `Last: ${lpStr} · Next: ${npStr}`;
  ...
  try{
    if(!_uid) return;
    const want = !c.isActive;

    _setMini(true, want ? 'Zapínám…' : 'Vypínám…');
    ...
    const r = await callFn('autopostCampaignToggle', { campaignId: id, isActive: want });
    _setMini(false);

    if(r && r.ok){
      toast(want ? 'Zapnuto' : 'Vypnuto');
      _reloadCampaigns('toggle').catch(()=>{});
    }else{
      toast((r && r.reason) ? String(r.reason) : 'Chyba');
    }
  }catch(e){
    console.warn(e);
    _setMini(false);
  ...
  try{
    if(!_uid) return;
    if(typeof callFn !== 'function'){ toast('Functions nejsou dostupné'); return; }
    _setMini(true, 'Mažu…');
    const r = await callFn('autopostCampaignRemove', { campaignId: id });
    _setMini(false);
    if(r && r.ok){
      toast('Smazáno');
      _reloadCampaigns('remove').catch(()=>{});
    }else{
      toast((r && r.reason) ? String(r.reason) : 'Chyba');
    }
  }catch(e){
  ...
  // Load campaigns list via callable (бетон). This avoids any client-side writes to server-only RTDB paths.
  // Optional: you can still keep RTDB read rules for owner, but UI will work even if reads are restricted.
  async function _reloadCampaigns(reason){
    try{
      if(!_uid) return;
      ...
      try{ _setMini(true, (String(reason||'')==='poll') ? '' : 'Načítám…'); }catch(e){}
      const r = await callFn('autopostCampaignList', { reason: String(reason||'') });
      try{ _setMini(false); }catch(e){}
      ...
      try{ _setMini(false); }catch(_e){}
      if(String(reason||'') !== 'poll'){
        try{ toast(e && e.code ? e.code : 'Chyba načtení'); }catch(_e){}
      }
    }finally{
      _loadingList = false;
    }
  ...
  _setMini(true, 'Ukládám…');
  try{
    const r = await callFn('autopostCampaignUpdate', {
      campaignId: id,
      text,
      intervalMin,
  ...
  try{
    ...
    _setMini(true, 'Vytvářím…');
    const r = await callFn('autopostCampaignCreate', {
      city,
      text,
      intervalMin,
    ...
    if(r && r.ok){
      ...
      _reloadCampaigns('create').catch(()=>{});
    }else{
      let msg = 'Chyba';
      if(r && r.reason){
        if(r.reason==='slots') msg = `Limit kampaní: ${r.used||0}/${r.slots||0}`;
        else if(r.reason==='bad_interval') msg = `Interval min ${r.minIntervalMin||_minInterval()}m`;
        else if(r.reason==='no_feature') msg = 'Není v plánu';
        else msg = String(r.reason);
      }
      toast(msg);
    }
  }catch(e){
  ...
  function enterAutopostView(){
    ...
    _fillIntervalSelect();
    _renderList();
    _reloadCampaigns('enter').catch(()=>{});
  }
  window.enterAutopostView = enterAutopostView;

    // -------- server-scheduled autopost --------
  // Client only reads + manages campaigns via Cloud Functions.
  // Posting itself happens in a scheduled Cloud Function (autopostTick).
  ...
  function startAutopostEngine(uid){
    try{
      ...
      // Initial load via callable (бетон)
      _reloadCampaigns('start').catch(()=>{});

      // Poll to reflect lastPostTs/nextPostTs updates by server tick
      _pollTimer = setInterval(()=>{ _reloadCampaigns('poll').catch(()=>{}); }, 30000);
      _watchUnsub = ()=>{ try{ if(_pollTimer) clearInterval(_pollTimer); }catch(e){} _pollTimer=null; };

      // One-time migration (best-effort): ensure active campaigns are indexed for server scheduler
      try{
        if(typeof callFn === 'function'){
          callFn('autopostReindexMyCampaigns', {}).catch(()=>{});
        }
      }catch(e){}
  ...
  function stopAutopostEngine(){
    try{ if(_tickDebounce){ clearTimeout(_tickDebounce); _tickDebounce=null; } }catch(e){}
