// Autoposting campaigns (server-scheduled)
(function autopostingModule(){
  ...
  function _renderPlanHint(){
    ...
    if(!can){
      ...
      if(h) h.textContent = 'Autoposting je dostupný v Premium / Premium+ / VIP. Upgrade najdeš v „Privilegia“. (Běží na serveru i když máš web zavřený.)';
    }
  }
  ...
  function _campCard(id, c){
    ...
    btnToggle.onclick = async ()=>{
      try{
        ...
        const r = await window.callFn('toggleAutopostCampaign', { campaignId: id, isActive: !c.isActive });
        if(!r || r.ok !== true) throw new Error('toggle_failed');
      }catch(e){
        console.warn(e);
    ...
    btnDel.onclick = async ()=>{
      ...
      try{
        if(typeof window.callFn !== 'function') throw new Error('functions_unavailable');
        const r = await window.callFn('deleteAutopostCampaign', { campaignId: id });
        if(!r || r.ok !== true) throw new Error('delete_failed');
      }catch(e){
        console.warn(e);
        toast(e && e.code ? e.code : 'Chyba');
      }
  ...
  function _openEdit(id, c){
    ...
    (async ()=>{
      try{
        if(typeof window.callFn !== 'function') throw new Error('functions_unavailable');
        const r = await window.callFn('updateAutopostCampaign', { campaignId: id, patch });
        if(!r || r.ok !== true) throw new Error('update_failed');
        toast('Uloženo');
      }catch(e){ console.warn(e); toast(e && e.code ? e.code : 'Chyba'); }
    })();
  }
  ...
  async function _createCampaign(){
    ...
    try{
      if(typeof window.callFn !== 'function') throw new Error('functions_unavailable');
      const r = await window.callFn('createAutopostCampaign', {
        city,
        text,
        intervalMin,
        imageUrl: _pendingImg || null
      });
      if(!r || r.ok !== true){
        const reason = String(r?.reason||'');
        if(reason==='slots') toast('Limit kampaní vyčerpán');
        else if(reason==='no_feature') toast('Autopost není dostupný v tomto plánu');
        else toast('Nelze vytvořit kampaň');
        return;
      }
  ...
  // -------- campaigns live watch (only while view is open) --------

  function startAutopostWatch(uid){
    try{ stopAutopostWatch(); }catch(e){}
    _uid = uid || null;
    _camps = {};
    if(!_uid) return;
  ...
  window.startAutopostWatch = startAutopostWatch;
  window.stopAutopostWatch = stopAutopostWatch;
