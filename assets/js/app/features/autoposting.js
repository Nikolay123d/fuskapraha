...
if(!can){
  ...
  try{
    ...
    const r = await callFn('toggleAutopostCampaign', { campaignId: id, isActive: want });
    _setMini(false);
  ...
  try{
    ...
    _setMini(true, 'Mažu…');
    const r = await callFn('deleteAutopostCampaign', { campaignId: id });
    _setMini(false);
    if(r && r.ok){
      toast('Smazáno');
    }else{
  ...
  try{
    const r = await callFn('updateAutopostCampaign', {
      campaignId: id,
      text,
      intervalMin,
    ...
    _setMini(false);
    if(r && r.ok) toast('Uloženo');
    else toast((r && r.reason) ? String(r.reason) : 'Chyba');
  }catch(e){
    console.warn(e);
    _setMini(false);
    toast(e && e.code ? e.code : 'Chyba');
  ...
  try{
    ...
    const r = await callFn('createAutopostCampaign', {
      city,
      text,
      intervalMin,
