...
(function friendsModule(){
  ...
  async function sendFriendRequest(toUid){
    ...
    return withLock('send:'+toUid, async ()=>{
      // Server-first (бетон): friend requests must go through Cloud Functions.
      // This enforces daily limits and prevents client-side rule bypass.
      try{
        ...
        const r = await window.callFn('friendSend', { toUid });
        if(!r || r.ok !== true){
          const reason = String(r?.reason||'');
          if(reason==='limit') toast('Limit přátel vyčerpán');
          else if(reason==='already_friends') toast('Už jste přátelé');
          else if(reason==='already_requested') toast('Žádost už byla odeslána');
          else if(reason==='has_incoming') toast('Máte od něj žádost — přejděte do „Přátelé“');
          else if(reason==='blocked') toast('Nelze odeslat (blokováno)');
          else toast('Nelze odeslat žádost');
          return false;
        }
  ...
  async function acceptFriend(fromUid){
    ...
    return withLock('accept:'+fromUid, async ()=>{
      try{
        ...
        const r = await window.callFn('friendAccept', { fromUid });
        if(!r || r.ok !== true){
          const reason = String(r?.reason||'');
          if(reason==='blocked') toast('Nelze potvrdit (blokováno)');
          else toast('Žádost už není aktuální');
          return false;
        }
  ...
  async function declineFriend(fromUid){
    ...
    return withLock('decline:'+fromUid, async ()=>{
      try{
        ...
        const r = await window.callFn('friendDecline', { fromUid });
        if(!r || r.ok !== true){ toast('Žádost už není aktuální'); return false; }
        toast('Odmítnuto');
        try{ await loadFriends(); }catch(e){}
        return true;
      }catch(e){
  ...
  async function removeFriend(uid){
    ...
    return withLock('remove:'+uid, async ()=>{
      try{
        ...
        const r = await window.callFn('friendRemove', { uid });
        if(!r || r.ok !== true){ toast('Nelze odebrat'); return false; }
        toast('Odebráno');
        try{ playSound?.('ok'); }catch(e){}
        try{ await loadFriends(); }catch(e){}
        return true;
  ...
  function _setMiniLoad(on){
    try{
      const el = document.getElementById('friendsMiniLoad');
  ...
  function watchFriendRequestsBadge(uid){
    ...
    try{ window.MK?.subs?.clear && window.MK.subs.clear('global:friendReqBadge'); }catch(e){}
    // detach
    ...
    try{
      const off = ()=>{ try{ if(__badgeRef && __badgeCb) __badgeRef.off('value', __badgeCb); }catch(e){}; __badgeRef=null; __badgeCb=null; };
      window.MK?.subs?.set && window.MK.subs.set('global:friendReqBadge', off, 'global');
    }catch(e){}
  }
