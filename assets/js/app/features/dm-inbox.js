...
function _ensureReqHeader(){
  ...
  head.textContent = 'Žádosti o zprávu';
...
function _updateReqRow(fromUid, r, myUid){
  ...
  try{
    ...
    const btnAccept = btns && btns[1];
    const btnDecline = btns && btns[2];
    if(btnOpen){
      btnOpen.onclick = (ev)=>{
      ...
      btnAccept.onclick = async (ev)=>{
        ...
        try{
          if(typeof callFn === 'function'){
            await callFn('dmConfirmAtomic', {peerUid:fromUid});
            toast('Dialog potvrzen');
          }else{
            toast('Server není dostupný');
            return;
          }
        }catch(e){ console.warn(e); toast('Chyba'); return; }
        try{ startDM && startDM(fromUid); }catch(e){ try{ openDMRoom(myUid, fromUid); showView('view-dm'); }catch(_){ } }
      };
    }
    if(btnDecline){
      btnDecline.onclick = async (ev)=>{
        ...
        try{ await db.ref('dmRequests/'+myUid+'/'+fromUid).remove(); toast('Odmítnuto'); }catch(e){ console.warn(e); toast('Chyba'); }
      };
    }
    row.onclick = ()=>{
      try{ startDM && startDM(fromUid); }catch(e){ try{ openDMRoom(myUid, fromUid); showView('view-dm'); }catch(_){ } }
