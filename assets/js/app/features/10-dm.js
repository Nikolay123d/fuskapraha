    ...
    try{
      ...
      if(peerUid && !peerIsBot && !isPremiumRoom){
        ...
        if(!confirmed){
          ...
          if(hasIncoming){
            // Confirm DM before first message
            try{ await db.ref('dmConfirmed/'+room).set({ a: me.uid, b: peerUid, ts, by: me.uid }); }
            catch(e){ console.warn('dmConfirmed write failed', e); toast('Nelze potvrdit DM (rules)'); playSound('err'); return; }
