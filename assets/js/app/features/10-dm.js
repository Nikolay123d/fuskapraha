...
const isBotPeer = String(currentDmPeerUid||'').startsWith('bot_') || isPremiumBotRoom(room);

if(!isBotPeer){
  // Server-first (user↔user): write via Cloud Function, client does NOT touch privateMessages/inboxMeta.
  try{
    if(typeof window.callFn !== 'function'){
      toast('Server není dostupný (Functions)');
      playSound('err');
    ...
    const r = await window.callFn('dmSend', { room, text: text||'', img: img||'' });
    if(!r || r.ok !== true){
      const reason = String(r?.reason||'');
      if(reason==='not_confirmed') toast('Nejdřív musí uživatel potvrdit žádost');
      else if(reason==='blocked') toast('Nelze poslat (blokováno)');
      else if(reason==='dm_banned' || reason==='banned') toast('Nelze poslat (ban)');
    ...
    toast('Chyba odeslání (server)');
    playSound('err');
    return;
  }
}else{
  // Bot rooms keep MVP direct writes (rules allow). Update ONLY my meta (peer meta is server/admin-only).
