...
async function renderDmRequestBanner(room){
  try{
    ...
    banner.style.border = '1px solid rgba(168,85,247,.55)';
    banner.style.borderRadius = '12px';
    banner.style.padding = '10px';
    banner.style.margin = '6px 0 10px';
    banner.style.background = 'rgba(168,85,247,.15)';
    ...
    t.textContent = 'ЛС запрос';
    t.style.background = 'rgba(168,85,247,.95)';
    t.style.color = '#fff';
...
async function renderDmOutgoingRequestBanner(room){
  try{
    ...
    const t = document.createElement('b');
    t.textContent = 'ЛС запрос отправлен';
    t.style.background = 'rgba(168,85,247,.95)';
    t.style.color = '#fff';
    t.style.padding = '4px 10px';
    t.style.borderRadius = '999px';
...
async function ensureBotIntroSimple(room){
  try{
    ...
    if(anyBot) return;
    // Server-authoritative message send (avoids client rules issues)
    if(typeof window.callFn === 'function'){
      await window.callFn('dmSend', {
        room,
        peerUid: PREMIUM_BOT_UID,
        bot: true,
...
$('#dmPhoto')?.addEventListener('change', async (e)=>{
  ...
  pendingDmFile = f;
  // Compress DM images to stay within RTDB rules (~400k) and avoid permission_denied rollbacks.
  try{
    const raw = await fileToDataURL(f);
    if(typeof shrinkImageDataURL === 'function'){
      pendingDmImg = await shrinkImageDataURL(raw, 350000);
    }else{
      pendingDmImg = raw;
    }
  }catch(err){ pendingDmImg=null; }
...
$('#dmSend')?.addEventListener('click', async ()=>{
  const me=auth.currentUser;
  if(!me){ toast('Войдите, чтобы отправлять сообщения.'); return; }
  const isPassword = (me.providerData||[]).some(p=>p && p.providerId==='password');
  if(isPassword && me.emailVerified===false){
    const until = (window.getVerifyDeadline ? getVerifyDeadline(me) : 0);
    if(until && Date.now()>until){
      toast('Подтвердите e‑mail, чтобы продолжить писать.');
      try{ showView('view-verify'); }catch(e){}
      return;
    }
    const k='unv_msgs_'+me.uid;
  ...
  // Optional gate: e-mail verification. Make it configurable because it can hurt UX on mobile.
  ...
  try{
        ...
        if(!confirmed){
          ...
          if(hasIncoming){
            ...
            try{
              ...
              // Send full text so it can be persisted as the first message after accept.
              const r = await window.callFn('dmRequestSend', { toUid: peerUid, text: String(text).slice(0,1200) });
              if(!r || r.ok !== true){
    ...
    // 3) send message through server (server-authoritative)
    try{
      if(typeof window.callFn !== 'function') throw new Error('functions_unavailable');
      const r = await window.callFn('dmSend', { room, peerUid: currentDmPeerUid, text: text||null, img: img||null });
      if(!r || r.ok !== true){
        const reason = String(r?.reason||'');
        if(reason==='not_confirmed') toast('Nejdřív počkejte na potvrzení.');
        else if(reason==='blocked') toast('Нельзя отправить (блокировка)');
        else if(reason==='dm_banned') toast('DM zablokováno');
        else toast('Nelze odeslat');
        playSound('err');
        return;
      }
    }catch(e){
