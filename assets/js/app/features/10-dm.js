...
async function openDMRoom(a,b,opts){
...
try{
 ...
 // IMPORTANT: do NOT overwrite peer's inboxMeta here (it would reset their lastReadTs/unread logic).
 // Peer meta is created/updated on acceptance and on actual messages.
 const now = Date.now();
 const up = {};
...
async function renderDM(room){
  ...
  try{
    window.MK?.subs?.set('tab:view-dm', 'dmRoom', ()=>{ try{ ref.off(); }catch(e){} });
  }catch(e){}
...
$('#dmSend')?.addEventListener('click', async ()=>{
  ...
  try{
    ...
    // Messages are allowed only after acceptance (both members true), except bots.
    if(!peerIsBot){
      let peerMember = false;
      try{
        const pm = await db.ref('privateMembers/'+room+'/'+currentDmPeerUid).get();
      ...
      if(!peerMember){
        ...
        try{
          const inc = await db.ref('dmRequests/'+me.uid+'/'+currentDmPeerUid).get();
          if(inc.exists()){
            const up = {};
            up['dmRequests/'+me.uid+'/'+currentDmPeerUid] = null;
            up['privateMembers/'+room+'/'+me.uid] = true;

            up[`inboxMeta/${me.uid}/${room}/with`] = currentDmPeerUid;
            up[`inboxMeta/${me.uid}/${room}/lastReadTs`] = ts;

            up[`inboxMeta/${currentDmPeerUid}/${room}/with`] = me.uid;
            await db.ref().update(up);
            toast('Žádost přijata');
          }else{
            const previewReq = (text && text.length) ? text.slice(0,180) : (img ? '📷 Foto' : '');
            const up = {};
            up[`dmRequests/${currentDmPeerUid}/${me.uid}`] = { from: me.uid, ts, textPreview: previewReq };
...
function startDmThreadsLive(uid){
  ...
  // Unified registry (tab-scoped): DM threads live list
  try{ window.MK?.subs?.set('tab:view-dm', 'dmThreadsLive', stopDmThreadsLive); }catch(e){}
}
