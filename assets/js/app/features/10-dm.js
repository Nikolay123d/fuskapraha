...
async function renderDmRequestBanner(room){
  try{
    ...
    hint.textContent = 'Odpovědí potvrdíte dialog.';
...
$('#dmSend')?.addEventListener('click', async ()=>{
  ...
  try{
      ...
      if(peerUid && !peerIsBot && !isPremiumRoom){
        ...
        if(!confirmed){
          ...
          try{ const rs = await db.ref('dmRequests/'+me.uid+'/'+peerUid).get(); hasIncoming = rs.exists(); }catch(e){}
	          ...
	          if(hasIncoming){
	            // Confirm request (dmConfirmed + members in one atomic update).
	            // IMPORTANT: dmConfirmed rules validate against dmRequests, so we must
	            // NOT delete dmRequests in the same multi-location update.
	            try{
	              const upd={};
	              upd['privateMembers/'+room+'/'+me.uid] = true;
	              // Make both participants members immediately.
	            ...
	            // Best-effort cleanup in a separate call.
	            try{ await db.ref('dmRequests/'+me.uid+'/'+peerUid).remove(); }catch(e){}
	            try{ window.MK_usage && MK_usage('accept_dm_req', 1); }catch(e){}
            ...
            try{ if(typeof incLimit==='function') incLimit('dm_init'); }catch(e){}
	            try{ window.MK_usage && MK_usage('send_dm_req', 1); }catch(e){}
    ...
    try{ window.MK_stats_inc && MK_stats_inc('dmCount', 1); }catch(e){}
	    try{ window.MK_usage && MK_usage('send_dm_msg', 1); }catch(e){}
