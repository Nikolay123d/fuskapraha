...
// Read receipts (dmReads/{room}/{uid})
let __DM_PEER_READ_TS = 0;
let __DM_READ_REF = null;
let __DM_READ_CB = null;

// Turn-taking: prevent sending 2 messages in a row until peer replies
let __DM_WAIT_REPLY = false;

function __isBotRoom(room){
  const r = String(room||'');
  return !!r && (r.startsWith('bot_') || r.includes('_bot_'));
}

function __detachDmReads(){
  try{ if(__DM_READ_REF && __DM_READ_CB) __DM_READ_REF.off('value', __DM_READ_CB); }catch(e){}
  __DM_READ_REF = null;
  __DM_READ_CB = null;
  __DM_PEER_READ_TS = 0;
}

function __updateDmSeen(){
  try{
    const box = document.getElementById('dmFeed');
    const me = auth?.currentUser;
    if(!box || !me) return;
    const meUid = me.uid;
    // only show on the latest outgoing message
    let lastEl = null;
    let lastTs = 0;
    const msgs = Array.from(box.querySelectorAll('.msg'));
    for(const el of msgs){
      const by = el.dataset.by || '';
      if(by !== meUid) continue;
      const ts = Number(el.dataset.ts || 0);
      if(ts && ts >= lastTs){ lastTs = ts; lastEl = el; }
    }
    for(const el of msgs){
      const by = el.dataset.by || '';
      if(by !== meUid){
        // remove any stray seen markers on incoming
        try{ const s=el.querySelector('.dm-seen'); if(s) s.remove(); }catch(e){}
        continue;
      }
      if(el !== lastEl){
        try{ const s=el.querySelector('.dm-seen'); if(s) s.remove(); }catch(e){}
      ...
      if(!bubble) continue;
      let s = bubble.querySelector('.dm-seen');
      if(!s){
        s = document.createElement('div');
        s.className = 'dm-seen muted';
        s.style.fontSize = '11px';
        s.style.marginTop = '4px';
        s.style.textAlign = 'right';
        bubble.appendChild(s);
      }
      const peerRead = Number(__DM_PEER_READ_TS || 0);
      const seen = peerRead && lastTs && peerRead >= lastTs;
      s.textContent = seen ? '✓✓ прочитано' : '✓ отправлено';
    }
  }catch(e){}
}

async function __markDmRead(room){
  const me = auth?.currentUser;
  if(!me || !room) return;
  const ts = Date.now();
  // Prefer server function (if deployed), fallback to direct RTDB write
  try{
    if(typeof callFn === 'function'){
      await callFn('dmMarkRead', {room});
      return;
    }
  }catch(e){ /* fallback */ }
  try{ await db.ref('dmReads/'+room+'/'+me.uid).set(ts); }catch(e){}
  try{ await db.ref('inboxMeta/'+me.uid+'/'+room+'/unread').set(0); }catch(e){}
  try{ await db.ref('inboxMeta/'+me.uid+'/'+room+'/lastReadTs').set(ts); }catch(e){}
}
...
async function renderDmRequestBanner(room){
  try{
    ...
    btnDecline.onclick = async ()=>{
      if(!confirm('Odmítnout žádost?')) return;
      try{ await db.ref('dmRequests/'+me.uid+'/'+peerUid).remove(); toast('Odmítnuto'); }catch(e){ console.warn(e); toast('Chyba'); }
      try{ banner.remove(); }catch(e){}
    };

    const btnAccept = document.createElement('button');
    btnAccept.className = 'ghost';
    btnAccept.type = 'button';
    btnAccept.textContent = 'Přijmout';
    btnAccept.onclick = async ()=>{
      try{
        if(typeof callFn === 'function'){
          await callFn('dmConfirmAtomic', {peerUid});
          toast('Dialog potvrzen');
          try{ banner.remove(); }catch(e){}
        }else{
    ...
    body.textContent = prev ? `"${prev}"` : 'Nová zpráva od neznámého uživatele.';

    const hint = document.createElement('div');
    hint.className = 'muted';
    hint.style.marginTop = '6px';
    hint.textContent = 'Přijmi nebo odpověz (odpovědí se dialog také potvrdí).';
...
// Stable entrypoint used across the app (profiles, notifications, friends)
async function startDM(toUid, opts={}){
  const me = auth?.currentUser;
  if(!me){
    window.__PENDING_VIEW__ = 'view-dm';
    window.__PENDING_DM_PEER__ = String(toUid||'');
    try{ openModalAuth('login'); }catch(e){}
    return null;
  }
  const peer = String(toUid||'').trim();
  if(!peer || peer===me.uid) return null;
  try{ if(opts && opts.closeModalId) closeModal(opts.closeModalId); }catch(e){}
  try{ await openDMRoom(me.uid, peer); }catch(e){ try{ openDMRoom(me.uid, peer); }catch(_){} }
  return dmKey(me.uid, peer);
}
...
async function renderDM(room){
  ...
  // Reset/attach read receipts listener for this room
  try{ __detachDmReads(); }catch(e){}
  try{ __DM_WAIT_REPLY = false; }catch(e){}
  try{
    const me = auth?.currentUser;
    const peer = currentDmPeerUid;
    if(me && peer){
      __DM_READ_REF = db.ref('dmReads/'+room+'/'+peer);
      __DM_READ_CB = (snap)=>{
        __DM_PEER_READ_TS = Number(snap.val()||0) || 0;
        __updateDmSeen();
      };
      __DM_READ_REF.on('value', __DM_READ_CB);
    }
  }catch(e){}

  // Mark room as read (server-first)
  ...
  box.onscroll = async ()=>{
    ...
    try{
      ...
      for(let i=items.length-1;i>=0;i--){
        ...
        const el=document.createElement('div'); el.className='msg'; el.dataset.mid=it.k; try{ el.dataset.by = String(m.by||''); }catch(e){}
  ...
  ref.on('child_added', async snap=>{
    ...
    const u = await resolveMsgUser(m);
    const el=document.createElement('div'); el.className='msg'; el.dataset.mid=snap.key; try{ el.dataset.by = String(m.by||''); }catch(e){} try{ el.dataset.ts = String(m.ts||0); }catch(e){}
    el.innerHTML = `<div class="ava" data-uid="${esc(m.by)}"><img src="${esc(safeImgSrc(normalizeAvatarUrl(u.avatar), window.DEFAULT_AVATAR))}"></div>`+
      `<div class="bubble"><div class="meta"><div class="name" data-uid="${esc(m.by)}">${esc(u.nick||'Uživatel')}</div><div class="time">${fmtTime(m.ts||0)}</div></div>`+
      (m.text?`<div class="text">${esc(m.text)}</div>`:'')+
      (safeImgSrc(m.img,'')?`<div class="text"><img src="${esc(safeImgSrc(m.img,''))}"></div>`:'')+
    ...
    // Turn-taking state (do not allow sending twice in a row)
    try{
      const meNow = auth?.currentUser;
      const meUid = meNow ? meNow.uid : '';
      const by = String(m.by||'');
      const peer = String(currentDmPeerUid||'');
      if(meUid && !__isBotRoom(room)){
        if(by === meUid) __DM_WAIT_REPLY = true;
        else if(peer && by === peer) __DM_WAIT_REPLY = false;
      }else{
        __DM_WAIT_REPLY = false;
      }
    }catch(e){}

    // Read receipts (update marker when peer read ts changes)
    try{ __updateDmSeen(); }catch(e){}

    // Notify only on incoming
    try{
      const meNow = auth?.currentUser;
      if(meNow && String(m.by||'') !== meNow.uid){
        if(!firstLoad){
          playSound('dm');
          notify('Nová DM', (u.nick||'Uživatel')+': '+(m.text||''));
        }
        if(wasNearBottom) __markDmRead(room);
      }
    }catch(e){}
  }, __onDmCancel);
}
...
$('#dmSend')?.addEventListener('click', async ()=>{
  ...
  try{
    ...
    // --- Server DM (atomic) ---
    // Preferred path: all critical DM logic is executed on Cloud Functions.
    // Fallback (legacy client writes) remains only for cases when functions are not deployed.
    try{
      const peerUid = currentDmPeerUid;
      const roomIdNow = room;
      // Client-side guard for turn-taking (server also enforces).
      if(!isPremiumReq && !__isBotRoom(roomIdNow) && __DM_WAIT_REPLY){
        toast('Nelze psát 2× po sobě. Počkejte na odpověď.');
        playSound('err');
        return;
      }
      if(!isPremiumReq && typeof callFn === 'function'){
        const payload = { peerUid, text, img: (img||'') };
        const res = await callFn('dmSendAtomic', payload);
        ...
        try{ document.getElementById('dmReqBanner')?.remove(); }catch(e){}

        if(res && res.status === 'request_sent'){
          toast('Žádost o zprávu odeslána. Počkejte na odpověď.');
        }else{
          toast('Odesláno');
        }
        playSound('ok');
        try{ __markDmRead(roomIdNow); }catch(e){}
        return;
      }
    }catch(err){
      // Handle expected server errors (do NOT fallback to legacy writes)
      try{
        const code = String(err && (err.code||''));
        const msg = String(err && (err.message||''));
        if(code.includes('failed-precondition') && msg.includes('WAIT_REPLY')){
          toast('Nelze psát 2× po sobě. Počkejte na odpověď.');
          playSound('err');
          return;
        }
        if(code.includes('resource-exhausted')){
          toast('Limit DM vyčerpán');
          playSound('err');
          return;
        }
        if(msg.toLowerCase().includes('images are not allowed')){
          toast('Nejdřív počkejte na potvrzení. Obrázek lze poslat až po přijetí.');
          playSound('err');
          return;
        }
        if(msg.toLowerCase().includes('dm request already sent')){
          toast('Žádost už byla odeslána');
          playSound('ok');
          return;
        }
      }catch(_e){}
      // Unexpected error -> allow legacy flow below (useful on dev / no-functions deployments)
      console.warn('dmSendAtomic failed (falling back to legacy)', err);
    }
    ...
    try{
      ...
      if(peerUid && !peerIsBot && !isPremiumRoom){
        ...
        if(!confirmed){
          ...
          if(hasIncoming){
            // Atomic confirm: dmRequest -> dmConfirmed -> privateMembers
            // (all in one multi-location update to avoid race conditions / half-state)
            try{
              const upd={};
              upd['privateMembers/'+room+'/'+me.uid] = true;
              upd['dmConfirmed/'+room] = { a: me.uid, b: peerUid, ts, by: me.uid };
              upd['dmRequests/'+me.uid+'/'+peerUid] = null;
              await db.ref().update(upd);
            }catch(e){
              console.warn('dmConfirmed atomic update failed', e);
              toast('Nelze potvrdit DM (rules)');
              playSound('err');
              return;
            }
            ...
            // DM initiation limit: only when creating a new request.
            if(typeof window.checkLimit === 'function'){
              try{
                const lim = await window.checkLimit('dm_init');
                if(lim && lim.ok === false){
                  toast(lim.msg || 'Limit DM vyčerpán');
                  playSound('err');
                  return;
                }
              }catch(e){ console.warn('checkLimit(dm_init) failed', e); }
            }

            const reqPath = 'dmRequests/'+peerUid+'/'+me.uid;
            try{ const ex = await db.ref(reqPath).get(); if(ex.exists()){ toast('Žádost už byla odeslána'); playSound('ok'); return; } }catch(e){}

            const fromNick = window.__myPublic?.nick || null;
            const fromAvatar = window.__myPublic?.avatar || null;
            try{ await db.ref(reqPath).set({ ts, fromUid: me.uid, previewText: String(text).slice(0,240), fromNick, fromAvatar }); }
            catch(e){ console.warn('dmRequest write failed', e); toast('Nelze odeslat žádost'); playSound('err'); return; }

	            // Update my own thread preview so I can see the pending request in the inbox.
	            try{
	              await db.ref('inboxMeta/'+me.uid+'/'+room).update({
	                lastTs: ts,
	                lastText: '[Žádost] '+String(text).slice(0,120),
	                lastBy: me.uid,
	                with: peerUid,
	                unread: 0
	              });
            ...
            try{ if(typeof incLimit==='function') incLimit('dm_init'); }catch(e){}
            toast('Žádost o zprávu odeslána. Počkejte na odpověď.');
            playSound('ok');
    ...
    try{ window.MK_stats_inc && MK_stats_inc('dmCount', 1); }catch(e){}
