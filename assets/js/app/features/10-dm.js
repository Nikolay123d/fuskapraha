...
// Room confirmation state (server-authority)
// NOTE: this is per-current thread (currentDmRoom).
let __DM_IS_CONFIRMED = false;

// Watcher used for pending rooms (so the sender sees acceptance without reload)
let __dmConfirmRef = null;
let __dmConfirmCb = null;

function __stopDmConfirmWatch(){
  try{
    if(__dmConfirmRef && __dmConfirmCb) __dmConfirmRef.off('value', __dmConfirmCb);
  }catch(e){}
  __dmConfirmRef = null;
  __dmConfirmCb = null;
}

function __startDmConfirmWatch(room){
  __stopDmConfirmWatch();
  const me = auth?.currentUser;
  if(!me || !room) return;
  // This path is readable by the user and flips to true once the room is confirmed.
  // It avoids permission_denied noise from dmConfirmed/privateMembers.
  __dmConfirmRef = db.ref('privateRoomsByUser/'+me.uid+'/'+room);
  __dmConfirmCb = (snap)=>{
    try{
      if(currentDmRoom !== room) return;
      if(snap && snap.val && snap.val() === true){
        __stopDmConfirmWatch();
        // Re-render now that permissions are granted.
        try{ renderDM(room); }catch(e){}
      }
    }catch(e){}
  };
  try{ __dmConfirmRef.on('value', __dmConfirmCb); }catch(e){}
}

async function __isDmConfirmedForMe(room){
  try{
    const me = auth?.currentUser;
    if(!me || !room) return false;
    const peerUid = currentDmPeerUid;
    if(String(peerUid||'').startsWith('bot_')) return true;
    // Premium bot room is treated like bot.
    try{ if(typeof isPremiumBotRoom === 'function' && isPremiumBotRoom(room)) return true; }catch(e){}
    const s = await db.ref('privateRoomsByUser/'+me.uid+'/'+room).get();
...
async function renderDmRequestBanner(room){
  try{
    ...
    // Don't show if room already has messages or already confirmed.
    try{ if(__DM_ROOM_HAS_ANY) return; }catch(e){}
    try{ if(__DM_IS_CONFIRMED) return; }catch(e){}
    try{ if(await __isDmConfirmedForMe(room)) return; }catch(e){}
...
try{
  ...
  __DM_IS_CONFIRMED = false;

  // Stop any previous pending-room watchers (switching between rooms)
  try{ __stopDmConfirmWatch(); }catch(e){}

  // NOTE (бетон): client does NOT create privateMembers / inboxMeta.
  // Those are server-owned and created by callable functions:
  // - dmRequestSend (sender inboxMeta)
  // - dmConfirmAtomic + dmSend (both sides)
  // Exception: for bot rooms we allow the user to self-join (read/write gated to bot rooms only).
  try{ if(String(peerUid||'').startsWith('bot_')) await db.ref('privateMembers/'+room+'/'+meUid).set(true); }catch(e){}
...
async function renderDM(room){
  ...
  box.innerHTML='';
  // Detach previous room listeners immediately (important when current room is not confirmed).
  try{ if(DM_REF){ DM_REF.off(); DM_REF=null; } }catch(e){}
  try{ box.onscroll = null; }catch(e){}
  // Reset per-room stream state
  try{ __DM_OLDEST_TS = null; }catch(e){}
  try{ __DM_ROOM_HAS_ANY = false; }catch(e){}
  try{ __DM_SEEN_IDS = new Set(); }catch(e){}
  try{ __setDmComposerLocked(false); }catch(e){}
  try{ __DM_HAS_PSEUDO = false; }catch(e){}
  try{ __DM_IS_CONFIRMED = false; }catch(e){}

  // Always update the premium bot UI state (it will hide itself for normal rooms)
  try{ setupPremiumComposer(room); }catch(e){}

  // Determine if we can subscribe to privateMessages without hitting permission_denied.
  // (For unconfirmed rooms we show request UI only.)
  let confirmed = false;
  try{ confirmed = await __isDmConfirmedForMe(room); }catch(e){ confirmed = false; }

  // Legacy heuristic: if privateRoomsByUser is missing but inboxMeta looks like a normal thread,
  // treat as confirmed to avoid breaking old rooms.
  let meta = null;
  ...
  if(!confirmed){
    ...
    try{
      if(me){
        const ms = await db.ref('inboxMeta/'+me.uid+'/'+room).get();
        meta = ms.exists() ? (ms.val()||{}) : null;
      }
    }catch(e){ meta = null; }
    try{ pendingOutgoing = !!(meta && meta.pendingRequest === true); }catch(e){ pendingOutgoing = false; }
    // Incoming request exists (receiver side)
    if(me && peer && !String(peer||'').startsWith('bot_')){
      try{ const rs = await db.ref('dmRequests/'+me.uid+'/'+peer).get(); hasIncoming = rs.exists(); }catch(e){ hasIncoming = false; }
    }
    try{ if(meta && meta.confirmed === true) confirmed = true; }catch(e){}
    if(!confirmed && meta && !pendingOutgoing && !hasIncoming){
      const lt = String(meta.lastText||'').trim();
      if(lt && !lt.startsWith('[Запрос]') && !lt.startsWith('[Отклонено]')) confirmed = true;
    }
  }

  __DM_IS_CONFIRMED = confirmed;
  ...
  // If not confirmed and not a bot room → do NOT subscribe to privateMessages.
  // This avoids permission_denied + fixes the bug where sender never sees the reply until reload.
  const peerIsBot = String(currentDmPeerUid||'').startsWith('bot_');
  const isPremiumRoom = (typeof isPremiumBotRoom==='function') ? !!isPremiumBotRoom(room) : false;
  if(!confirmed && !peerIsBot && !isPremiumRoom){
    try{ __stopDmReadWatch(); }catch(e){}
    try{ __startDmConfirmWatch(room); }catch(e){}
    // Hide loader immediately (no realtime messages stream yet)
    setMiniLoad('dmMiniLoad','', false);
    // If there is no request banner/preview, show a simple hint.
    try{
      if(!__DM_HAS_PSEUDO){
        const empty=document.createElement('div');
        empty.className='muted';
        empty.style.padding='10px 12px';
        empty.textContent='Диалог ещё не подтверждён. Отправьте запрос или дождитесь ответа.';
        box.appendChild(empty);
      }
    }catch(e){}
    // No message listeners in this mode.
    return;
  }

  // Confirmed (or bot) → mark read via server + enable read receipts.
  try{ if(typeof window.callFn === 'function'){ await window.callFn('dmMarkRead', { room }); } }catch(e){}
  try{ if(!peerIsBot) __markDmRead(room); }catch(e){}
  try{ if(!peerIsBot && currentDmPeerUid) __startDmReadWatch(room, currentDmPeerUid); }catch(e){}
  ...
  const __onDmCancel = (err)=>{
    ...
    // If the room isn't confirmed yet, Firebase will cancel listeners with PERMISSION_DENIED.
    // In that case we don't show a scary error – we fall back to the request UI.
    try{
      const code = String((err && (err.code || err.message)) || '').toLowerCase();
      if(code.includes('permission')){
        try{ __DM_IS_CONFIRMED = false; }catch(e){}
        try{ __startDmConfirmWatch(room); }catch(e){}
        // If we already have pseudo UI (request banner/preview) – keep it.
        try{ if(__DM_HAS_PSEUDO) return; }catch(e){}
        const empty=document.createElement('div');
        empty.className='muted';
        empty.style.padding='10px 12px';
        empty.textContent='Диалог ещё не подтверждён. Ждите ответа или ответьте на запрос.';
        box.appendChild(empty);
        return;
      }
    }catch(e){}
    try{
      ...
      empty.style.padding='10px 12px';
      empty.textContent='DM nelze načíst ('+(err && err.code ? err.code : 'error')+').';
      box.appendChild(empty);
    }catch(e){}
  };
...
async function submitPlanRequest(plan, text, proofImg, proofFile){
  ...
  const id = (db.ref('payments/requestsIndex').push().key) || ('req_'+Date.now());
  ...
  // Server-first: create request via Cloud Functions (prevents client-side forging).
  if(typeof window.callFn !== 'function') throw new Error('functions_unavailable');
  const r = await window.callFn('createPaymentRequest', {
    plan,
    text: text || null,
    city: city || null,
  ...
  if(!r || r.ok !== true) throw new Error(String(r?.reason||'payment_request_failed'));
  return r.id;
}
  ...
  try{
    ...
    try{
      ...
      if(peerUid && !peerIsBot && !isPremiumRoom){
        ...
        // Confirmed state is determined without reading dmConfirmed/privateMembers (avoids permission_denied).
        let confirmed = false;
        try{ confirmed = (__DM_IS_CONFIRMED === true); }catch(e){ confirmed = false; }
        ...
        if(!confirmed){
          ...
          if(hasIncoming){
            ...
            // Room permissions change (you become a member). Re-render to attach listeners.
            try{ renderDM(room); }catch(e){}
    ...
    // NOTE (бетон): client no longer writes to privateMembers/privateMessages/inboxMeta.
    // All message delivery goes through Cloud Functions (dmSend).
    ...
    if(isPremiumBotRoom(room) && __premiumSelectedPlan){
      ...
      try{
        // Store the user's message in the bot thread (server-owned).
        try{ if((text || img) && typeof window.callFn === 'function'){ await window.callFn('dmSend', { room, text: text||null, img: img||null }); } }catch(_e){}
    ...
    // 3) Normal DM message (server-owned)
    try{
      if(typeof window.callFn !== 'function') throw new Error('functions_unavailable');
      const r = await window.callFn('dmSend', { room, text: text||null, img: img||null });
      if(!r || r.ok !== true) throw new Error('send_failed');
    }catch(e){
      console.warn('dmSend failed', e);
      toast('Nelze odeslat (server)');
      playSound('err');
      return;
    }
    ...
    // No re-render: child_added listener will append.
