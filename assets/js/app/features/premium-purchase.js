...
(function premiumPurchaseModule(){
  ...
  async function _submitRequest(){
    ...
    let id = null;
    try{
      // Server-first: anti-duplicate and validation is enforced in Cloud Functions.

      const ts = Date.now();
      const city = _getCitySafe();
      const note = String($('premiumNote')?.value||'').trim();

      // Optional promo (created server-side): promoOffers/chat25/{uid}
      let promo = null;
      try{
        const planNow = (window.getMyPlanState && getMyPlanState().plan) || 'free';
        if(String(planNow).toLowerCase()==='free' && window.__mkPromoChat25Active && window.__mkPromoChat25Offer){
          const o = window.__mkPromoChat25Offer;
          if(String(o.status||'')==='active' && (+o.expiresAt||0) > Date.now()){
            promo = {
              kind: 'chat25',
              discountPct: Number(o.discountPct||50),
              offerCreatedAt: Number(o.createdAt||0)||null,
              offerExpiresAt: Number(o.expiresAt||0)||null
            };
          }
        }
      }catch(e){}

      // Local id used only for Storage path. The server generates the real request id.
      id = db.ref('payments/requestsIndex').push().key;

      // 1) Upload proof
      const proofUrl = await _uploadProof(me.uid, id, __proofFile);

      // 2) Create request via Cloud Functions (server-owned)
      ...
      const rr = await window.callFn('createPaymentRequest', {
        plan: __selectedPlan,
        text: note || null,
        city: city || null,
      ...
      if(!rr || rr.ok !== true){
        ...
        if(reason==='pending_exists') throw new Error('pending_exists');
