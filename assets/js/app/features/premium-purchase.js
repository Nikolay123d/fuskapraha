...
(function premiumPurchaseModule(){
  ...
  const DEFAULT_PLANS = {
    premium:     { title:'Premium',     price:'', period:'30 dní', desc:'Odznak v chatu + vyšší limity.' },
    premiumPlus: { title:'Premium+',    price:'', period:'30 dní', desc:'Ještě vyšší limity + status.' },
    vip:         { title:'VIP',         price:'', period:'30 dní', desc:'Maximální limity + VIP odznak.' }
  };
  ...
  async function _submitRequest(){
    ...
    try{
      ...
      try{ toast(String(err?.message||'Chyba při odeslání')); }catch(e){}
    }finally{
      __submitLock = false;
      _enableSubmitUI();
    }
  ...
  function _renderCompareTable(){
    const tbl=document.getElementById('premiumCompareTable');
    if(!tbl) return;

    const limits = (window.getPlanLimits && window.getPlanLimits()) || null;
    if(!limits){
      tbl.textContent='';
      const tr=document.createElement('tr');
      const td=document.createElement('td');
      td.className='muted';
      td.textContent='Limity nejsou dostupné.';
      tr.appendChild(td);
      tbl.appendChild(tr);
      return;
    }

    const plans=['free','premium','premiumplus','vip'];
    const labels={free:'Free', premium:'Premium', premiumplus:'Premium+', vip:'VIP'};
    const badges={free:'—', premium:'PREMIUM', premiumplus:'PREMIUM+', vip:'VIP'};

    const rows=[
      {label:'Cena', get:(p)=> (PREMIUM_PLANS[p]?.price!=null ? (PREMIUM_PLANS[p].price+' Kč') : '—')},
      {label:'Odznak', get:(p)=>badges[p]||'—'},
      {label:'DM / den', key:'dm_day'},
      {label:'Přátelé / den', key:'friend_day'},
      {label:'Chat / den', key:'chat_day'},
      {label:'Inzeráty / den', key:'rent_day'},
      {label:'Reporty / den', key:'report_day'},
    ];

    tbl.innerHTML='';
    tbl.classList.add('plan-table');
    ...
    rows.forEach(r=>{
      ...
      plans.forEach(p=>{
        const td=document.createElement('td');
        let val='—';
        if(r.get) val=r.get(p);
        else if(r.key) val = (limits[p] && limits[p][r.key]!=null) ? String(limits[p][r.key]) : '—';
        td.textContent=val;
        tr.appendChild(td);
      });
  ...
  function _renderMyRequests(val){
    ...
    for(const r of rows.slice(0, 20)){
      ...
      meta.textContent = r.ts ? new Date(Number(r.ts)).toLocaleString() : '';
