...
(function mkProfileModule(){
  ...
  // --- Profile locking & requests ---
    ...
    async function _renderLimitsCard(plan, until){
      ...
      try{ if(window.checkLimit){ const r=await window.checkLimit('dm'); dm={limit:r.limit, count:r.count}; } }catch(e){}
    ...
    async function loadMyProfileUI(u){
      ...
      try{ await _renderLimitsCard(up.plan||'free', up.planUntil||0); }catch(e){}
