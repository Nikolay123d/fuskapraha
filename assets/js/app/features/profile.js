...
(function mkProfileModule(){
  ...
  // --- Profile locking & requests ---
    ...
    async function _renderLimitsCard(plan, until){
      ...
      // Battery UI
      const dmFill=document.getElementById('limitDmFill');
      const dmPct=document.getElementById('limitDmPct');
      const frFill=document.getElementById('limitFriendFill');
      const frPct=document.getElementById('limitFriendPct');
      const apFill=document.getElementById('limitAutoPostsFill');
      const apPct=document.getElementById('limitAutoPostsPct');
      const slFill=document.getElementById('limitAutoSlotsFill');
      const slPct=document.getElementById('limitAutoSlotsPct');
      ...
      let dm = {limit:'—', count:0, remaining:0};
      let fr = {limit:'—', count:0, remaining:0};
      let ap = {limit:'—', count:0, remaining:0};
      try{ if(window.checkLimit){ const r=await window.checkLimit('dm_init'); dm={limit:r.limit, count:r.count, remaining:r.remaining}; } }catch(e){}
      try{ if(window.checkLimit){ const r=await window.checkLimit('friend'); fr={limit:r.limit, count:r.count, remaining:r.remaining}; } }catch(e){}
      try{ if(window.checkLimit){ const r=await window.checkLimit('autopost_posts'); ap={limit:r.limit, count:r.count, remaining:r.remaining}; } }catch(e){}
      ...
      try{
        ...
        if(uid){
          // Prefer server list (works even if autopostCampaigns is read-restricted)
          if(typeof window.callFn === 'function'){
            const r = await window.callFn('autopostCampaignList', { slim:true });
            if(r && r.ok){
              if(Number.isFinite(Number(r.count))) slotsUsed = Number(r.count);
              else slotsUsed = Object.keys((r.campaigns||{})||{}).length;
            }
          }else{
            const ss = await db.ref('autopostCampaigns/'+uid).get();
            const obj = ss.val() || {};
            slotsUsed = Object.keys(obj||{}).length;
          }
        }
      }catch(e){ slotsUsed = 0; }
      ...
      // Battery-style percent: shows remaining quota (like phone charge).
      const pctRemain = (remaining, limit)=>{
        const l = Number(limit);
        if(!Number.isFinite(l) || l>=1e11) return 100;
        if(l<=0) return 0;
        const r = Math.max(0, Math.min(l, Number(remaining)||0));
        return Math.max(0, Math.min(100, Math.round((r/l)*100)));
      };
      const setBattery = (fillEl, pctEl, remaining, limit)=>{
        const p = pctRemain(remaining, limit);
        try{ if(fillEl) fillEl.style.width = p + '%'; }catch(e){}
        try{ if(pctEl) pctEl.textContent = p + '%'; }catch(e){}
      };

      setBattery(dmFill, dmPct, dm.remaining, dm.limit);
      setBattery(frFill, frPct, fr.remaining, fr.limit);
      setBattery(apFill, apPct, ap.remaining, ap.limit);
      setBattery(slFill, slPct, Math.max(0, (slotsTotal - slotsUsed)), Math.max(0, slotsTotal));
    ...
    async function loadMyProfileUI(u){
      ...
      // Avatar upload (📷) -> dataURL (compressed) -> stored in usersPublic/avatar
      // Fixes: "avatar file selected but nothing happens" + keeps under rules size limit.
      try{
        const inp = document.getElementById('avatarFile');
        const urlInp = document.getElementById('setAvatarUrl');
        const avaImg = document.getElementById('myAvatar');
        if(inp && inp.dataset.wired !== '1'){
          ...
          inp.addEventListener('change', async ()=>{
            try{
              ...
              if(!f) return;
              if(!String(f.type||'').startsWith('image/')){ toast('Vyberte obrázek'); inp.value=''; return; }

              if(typeof window.fileToDataURL !== 'function'){
                toast('Chybí fileToDataURL');
                return;
              }
              const dataUrl = await window.fileToDataURL(f, { maxSide: 512, maxLen: 390000, mime: 'image/jpeg', quality: 0.82 });
              if(urlInp) urlInp.value = String(dataUrl||'');
              if(avaImg) avaImg.src = safeImgSrc(String(dataUrl||''), window.DEFAULT_AVATAR);
              toast('Avatar připraven — klikni Uložit');
        ...
        if(urlInp && urlInp.dataset.wired !== '1'){
          urlInp.dataset.wired = '1';
          urlInp.addEventListener('input', ()=>{
            try{ if(avaImg) avaImg.src = safeImgSrc(String(urlInp.value||''), window.DEFAULT_AVATAR); }catch(e){}
          });
        }
      }catch(e){}
  ...
  setMiniLoad('profileMiniLoad','', false); }catch(e){}
      ...
      if(u.emailVerified===false){
        ...
        if(Object.keys(patch).length){
          ...
          const r = await window.callFn('profileUpdate', patch);
          if(!(r && r.ok)){
            toast((r && r.reason) ? String(r.reason) : 'Chyba profilu');
            return;
          }
        }
