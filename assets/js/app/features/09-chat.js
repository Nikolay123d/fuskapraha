...
function __nearEl(){ return document.getElementById('chatNearBar'); }
let __nearHideTimer=null;

function __nearKey(city){ return 'mk_chat_sent_once_'+String(city||''); }
function __nearHiddenKey(city){ return 'mk_chat_near_hidden_'+String(city||''); }
function __hasSentInCity(city){ try{ return localStorage.getItem(__nearKey(city))==='1'; }catch(e){ return false; } }
function __markSentInCity(city){ try{ localStorage.setItem(__nearKey(city),'1'); }catch(e){} }
function __isNearHidden(city){ try{ return localStorage.getItem(__nearHiddenKey(city))==='1'; }catch(e){ return false; } }
function __setNearHidden(city,v){ try{ localStorage.setItem(__nearHiddenKey(city), v?'1':'0'); }catch(e){} }
...
async function __renderTyping(){
  ...
  if(!active.length){
    ...
    el.textContent = '';
    try{ const nb=__nearEl(); if(nb){ nb.style.display='none'; nb.innerHTML=''; } }catch(e){}
    return;
  }
  ...
  // Near bar (avatars + nicks) — shown only after user sent at least 1 message in this city
  try{
    const city = __typingCity || getCity();
    const nb = __nearEl();
    if(!nb) return;
    if(__isNearHidden(city)) { nb.style.display='none'; nb.innerHTML=''; return; }
    if(!__hasSentInCity(city)) { nb.style.display='none'; nb.innerHTML=''; return; }
    ...
    for(const it of top){
      try{
        const u = await getUserLite(it.uid);
        const nick = (u && u.nick ? String(u.nick) : '').trim() || '—';
        const ava  = safeImgSrc((u && u.avatar) || window.DEFAULT_AVATAR, window.DEFAULT_AVATAR);
        parts.push(`<span class="near-chip"><img src="${esc(ava)}" alt=""><span class="nick">${esc(nick)}</span><span class="dot">·</span><span class="muted" style="font-size:11px">online</span></span>`);
      }catch(e){}
    }
    const closeBtn = `<button class="ghost x near-close" id="chatNearClose" type="button">×</button>`;
    nb.innerHTML = `<span class="muted" style="font-size:12px">Сейчас рядом (online x${active.length}):</span>` + parts.join('') + closeBtn;
    nb.style.display = 'flex';

    const c = document.getElementById('chatNearClose');
    if(c){
      c.onclick = ()=>{ __setNearHidden(city, true); nb.style.display='none'; nb.innerHTML=''; };
    }

    // Auto-hide after ~60s to avoid clutter
    if(__nearHideTimer) try{ clearTimeout(__nearHideTimer); }catch(e){}
    __nearHideTimer = setTimeout(()=>{ try{ nb.style.display='none'; nb.innerHTML=''; }catch(e){} }, 60000);
  }catch(e){}
}
...
async function loadChat(){
  ...
  // NOTE: upsert can be used for initial batch rendering. Use opts.silent=true
  // to suppress sounds/notifications/autoscroll during initial paint.
  const upsert=async (snap, opts={})=>{
    // opts can be null when caller passes a payload coming from RTDB listeners.
    ...
    const planVal = String(u.plan||'').toLowerCase();
    const isPrem = !!planVal && !['free','none','0','basic'].includes(planVal);
    const canAdmin = !!(auth.currentUser && window.isAdminUser && isAdminUser(auth.currentUser));
    const canMod = !!(auth.currentUser && window.isModeratorUser && isModeratorUser(auth.currentUser));
    const isAdm = (canAdmin && auth.currentUser && m.by === auth.currentUser.uid);
    const badges = (isAdm?'<span class="badge admin">ADMIN</span>':'') + (isPrem?'<span class="badge premium">PREMIUM</span>':'');
    ...
    d.innerHTML = `<div class="ava" data-uid="${esc(m.by)}"><img src="${esc(safeImgSrc(normalizeAvatarUrl(u.avatar), window.DEFAULT_AVATAR))}"></div>`+
                  ...
                  (m.text? `<div class="text">${esc(m.text)}</div>`:'' )+
                  (safeImgSrc(m.img||m.photo,'')? `<div class="text"><img src="${esc(safeImgSrc(m.img||m.photo,''))}"></div>`:'')+
                  ((canAdmin || canMod) && m.by ? `<div class="adminActions">
                    <button data-act="del" title="Smazat">🗑</button>
                    ${canAdmin ? `<button data-act="ban" title="Ban 12h (chat+DM)">⛔</button>
  ...
  // Smooth scroll helper (medium speed, no "jerk")
  const __smoothScrollTo = (el, to, durationMs=650)=>{
    if(!el) return;
    const maxTop = Math.max(0, (el.scrollHeight||0) - (el.clientHeight||0));
    const target = Math.max(0, Math.min(maxTop, Number(to)||0));
    const start = Number(el.scrollTop)||0;
    const delta = target - start;
    if(Math.abs(delta) < 2){ try{ el.scrollTop = target; }catch(e){} return; }
    const t0 = (window.performance && performance.now) ? performance.now() : Date.now();
    const easeInOut = (p)=> (p < 0.5 ? (2*p*p) : (1 - Math.pow(-2*p + 2, 2)/2));
    const step = (now)=>{
      const t = ((now - t0) / durationMs);
      const p = Math.max(0, Math.min(1, t));
      try{ el.scrollTop = start + delta * easeInOut(p); }catch(e){}
      if(p < 1) requestAnimationFrame(step);
    };
    requestAnimationFrame(step);
  };
  const __smoothScrollToBottom = (el, durationMs=650)=>{
    if(!el) return;
    const maxTop = Math.max(0, (el.scrollHeight||0) - (el.clientHeight||0));
    __smoothScrollTo(el, maxTop, durationMs);
  };

  // === Initial load (batch render of last 15) ===
  // Without this, child_added paints messages one-by-one and the list "twitches".
  // We keep a mini loader until the whole batch is ready.
  let __initialRendered = false;
  try{
    if(!__hadCache){
      try{ feed.style.visibility = 'hidden'; }catch(e){}
      try{ feed.innerHTML=''; }catch(e){}
      try{ __CHAT_SEEN_IDS = new Set(); }catch(e){}
    }
    ...
    const items = Object.entries(v)
      ...
      .filter(it=>it && it.k)
      .sort((a,b)=>{
        const ta = Number(a.m.ts||a.m.createdAt||0);
        const tb = Number(b.m.ts||b.m.createdAt||0);
        if(ta !== tb) return ta - tb;
        return String(a.k).localeCompare(String(b.k));
      });

    // Establish oldestTs so the "Starší +15" button can appear.
    try{
      const tsList = items.map(it=>Number(it && it.m && it.m.ts)).filter(Number.isFinite);
      if(tsList.length){
        const minTs = Math.min(...tsList);
        __CHAT_OLDEST_TS = (__CHAT_OLDEST_TS===null) ? minTs : Math.min(__CHAT_OLDEST_TS, minTs);
      }
    }catch(e){}
    ...
    if(!__hadCache){
      for(const it of items){
        const m = it.m||{};
        if(!m || m.deleted) continue;
        // Render silently (no sounds/notifications/autoscroll)
        await upsert({ key: it.k, val: ()=>m }, { silent:true });
      }
      __initialRendered = true;
    ...
    if(!__hadCache){
      try{ feed.style.visibility = ''; }catch(e){}
      // Medium-speed smooth scroll down AFTER the batch is fully painted.
      try{ setTimeout(()=>{ __smoothScrollToBottom(feed, 650); }, 140); }catch(e){}
    }
  }catch(e){
    // On any failure (rules, permissions, offline) – stop loaders and show a hint.
    try{ setMiniLoad('chatMiniLoad','', false); }catch(_e){}
    try{ stopSeq && stopSeq(); }catch(_e){}
    try{ stopSlow && stopSlow(); }catch(_e){}
    try{ __markChatReady(); }catch(_e){}
    try{
      if(!__hadCache){
        feed.style.visibility = '';
        feed.innerHTML = '<div class="mini-hint">Chat nelze načíst.</div>';
      }
    }catch(_e){}
  }
...
$('#sendBtn')?.addEventListener('click', async ()=>{
  try{
    const u=auth.currentUser; if(!u){ toast('Войдите, чтобы отправлять сообщения.'); return; }
    const isPassword = (u.providerData||[]).some(p=>p && p.providerId==='password');
    if(isPassword && u.emailVerified===false){
      const until = (window.getVerifyDeadline ? getVerifyDeadline(u) : 0);
      if(until && Date.now()>until){
        toast('Подтвердите e‑mail, чтобы продолжить писать.');
        try{ showView('view-verify'); }catch(e){}
        return;
      }
      const k='unv_msgs_'+u.uid;
      const cnt=parseInt(localStorage.getItem(k)||'0',10)||0;
      if(cnt>=5){
        toast('Подтвердите e‑mail, чтобы продолжить писать.');
        try{ showView('view-verify'); }catch(e){}
        return;
    ...
    // Optional gate: e-mail verification (configurable; can hurt UX on mobile).
    // Set window.REQUIRE_EMAIL_VERIFY = true in config to enable.
    if(window.REQUIRE_EMAIL_VERIFY === true){
      if(window.__EMAIL_VERIFY_REQUIRED__){ toast('Nejdřív potvrďte e-mail.'); try{ showView('view-verify'); }catch(e){} return; }
      if(u.emailVerified===false){
        const until=getVerifyDeadline(u);
        if(until && Date.now()>until){ window.__EMAIL_VERIFY_REQUIRED__=true; toast('Potvrzení e-mailu vypršelo.'); try{ showView('view-verify'); }catch(e){} return; }
      }
    }
    ...
    if(isPassword && u.emailVerified===false){
      const k='unv_msgs_'+u.uid;
      const cnt=parseInt(localStorage.getItem(k)||'0',10)||0;
      localStorage.setItem(k, String(cnt+1));
    }
...
// Emoji picker (simple, lightweight)
// Requirement: add real emojis (not textual symbols) to the main chat.
(function ensureChatEmojiPicker(){
  try{
    const composer = $('#composerChat');
    const input = $('#msgText');
    ...
    // Insert before file button (if present)
    const fileBtn = composer.querySelector('.filebtn');
    if(fileBtn) composer.insertBefore(btn, fileBtn);
    else composer.insertBefore(btn, composer.firstChild);

    const panel = document.createElement('div');
    panel.id = 'chatEmojiPanel';
    panel.className = 'emojiPanel';
    ...
    const toggle = ()=>{
      ...
      if(!isOpen){
        // Position just above the composer
        try{
    ...
    document.addEventListener('click', ()=>{ try{ panel.style.display='none'; }catch(e){} });
  }catch(e){}
})();
