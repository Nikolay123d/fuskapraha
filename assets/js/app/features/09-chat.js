...
async function loadChat(){
  ...
  feed.onscroll = async ()=>{
    ...
    try{
      ...
      for(const it of items){
        ...
        d.innerHTML = `<div class="ava" data-uid="${esc(m.by)}"><img src="${esc(normalizeAvatarUrl(u.avatar))}"></div>`+
  ...
  const upsert=async (snap)=>{
    ...
    const m=snap.val()||{};
    const wasNearBottom = (()=>{
      try{ return (feed.scrollHeight - feed.scrollTop - feed.clientHeight) < 220; }catch(e){ return true; }
    })();
    if(m.deleted) return;
    ...
    try{ if(wasNearBottom) feed.scrollTop = feed.scrollHeight; }catch(e){}
