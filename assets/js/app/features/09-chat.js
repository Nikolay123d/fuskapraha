...
async function loadChat(){
  ...
  const upsert=async (snap)=>{
    ...
    // Auto-follow when user is already at the bottom
    if(wasNearBottom){ try{ feed.scrollTop = feed.scrollHeight; }catch(e){} }
  ...
  // Register tab-scoped subscription so tab switches call unsubscribeAll().
  try{ if(window.MK && window.MK.subs) window.MK.subs.add(offChat, {scope:'tab', key:'chatFeed'}); }catch(e){}
...
$('#sendBtn')?.addEventListener('click', async ()=>{
  try{
    ...
    try{ if(window.MK && !window.MK.lockTake('chat:send', 900)) return; }catch(e){}

    // Plan limits
    try{ if(typeof checkLimit==='function'){ const ok = await checkLimit('chat'); if(!ok) return; } }catch(e){}
    ...
    try{ if(typeof incLimit==='function') await incLimit('chat'); }catch(e){}
    // cleanup
    $('#msgText').value='';
    $('#msgPhoto').value='';
    pendingChatImg=null;
