...
async function loadChat(){
  ...
  // Gate for sounds/notifications: initial batch must NOT trigger them.
  let __INITIAL_BATCH_DONE = false;
  // Failsafe: if network is slow, keep loader but update the hint (avoid "dead" UI)
  setTimeout(()=>{ try{ setMiniLoad('chatMiniLoad','Stále načítám… zkontroluj internet', true); }catch(e){} }, 6500);
  ...
  const upsert=async (snap)=>{
    ...
    if(m.deleted){
      ...
      const ex = feed.querySelector(`[data-mid="${CSS.escape(key)}"]`);
      if(ex) ex.remove();
      try{ __CHAT_SEEN_IDS && __CHAT_SEEN_IDS.delete(key); }catch(e){}
      return;
    }
    ...
    let d = feed.querySelector(`[data-mid="${CSS.escape(key)}"]`);
    const isNew = !d;
    if(!d){ d=document.createElement('div'); d.className='msg'; d.dataset.mid = key;
    try{ d.dataset.ts = String(m.ts||m.createdAt||Date.now()); }catch(e){} }
    const isMine = !!(auth.currentUser && m.by===auth.currentUser.uid);
    const isAdm = isMine && isAdminUser(auth.currentUser);

    // Render instantly with placeholders; resolve profile async to avoid blocking the UI.
    ...
    const plan0 = String(u0.plan||'').toLowerCase();
    const isPrem0 = !!plan0 && !['free','none','0','basic'].includes(plan0);
    const msgImg = safeImgSrc(m.img||m.photo,'');
    // Only gate *very* large inline data images (huge base64 can lag on mobile).
    // Medium images (new uploads are resized) should render immediately.
    const isBigDataImg = !!msgImg && msgImg.startsWith('data:image') && msgImg.length > 900000;
    const showAdminActions = isAdminUser(auth.currentUser) && m.by && (!auth.currentUser || m.by !== auth.currentUser.uid);

    const name = `<span class="nick" data-uid="${esc(m.by)}">${esc(nick0)}</span>`+
                 `<span class="badge admin" style="display:${isAdm?'':'none'}">ADMIN</span>`+
                 `<span class="badge premium" style="display:${isPrem0?'':'none'}">PREMIUM</span>`+
                 `<span class="online" style="display:none"></span>`;

    d.innerHTML = `<div class="ava" data-uid="${esc(m.by)}"><img class="avaImg" src="${esc(avatar0)}" loading="lazy" decoding="async"></div>`+
                  `<div class="bubble"><div class="meta"><div class="name">${name}</div><div class="time">${fmtTime(m.ts||m.createdAt||Date.now())}</div></div>`+
                  (m.text? `<div class="text">${esc(m.text)}</div>`:'' )+
                  ...
                  (showAdminActions ? `<div class="adminActions">
    ...
    // Wire on-demand image render for large data: URLs
    if(isBigDataImg){
      try{
        const btn = d.querySelector('.showImgBtn');
        if(btn && !btn.__wired){
          btn.__wired = true;
    ...
    // Async user resolve (shared for UI + notifications)
    let userPromise = null;
    if(!myPub || !isMine){
      userPromise = resolveMsgUser(m).then(u=>{
        if(!u) return null;
        if(!d.isConnected) return u;

        // avatar
        try{
          const imgEl = d.querySelector('.avaImg') || d.querySelector('.ava img');
          if(imgEl) imgEl.src = safeImgSrc(normalizeAvatarUrl(u.avatar||''), window.DEFAULT_AVATAR);
        }catch(e){}

        // nick
        try{
          const nickEl = d.querySelector('.nick');
          if(nickEl) nickEl.textContent = u.nick || u.name || nickEl.textContent;
        }catch(e){}
        ...
        try{
          const planVal = String(u.plan||'').toLowerCase();
          const isPrem = !!planVal && !['free','none','0','basic'].includes(planVal);
          const premEl = d.querySelector('.badge.premium');
          if(premEl) premEl.style.display = isPrem ? '' : 'none';
        }catch(e){}

        // online
        try{
          const onEl = d.querySelector('.online');
          if(onEl) onEl.style.display = u.online ? '' : 'none';
        }catch(e){}
    ...
    try{ __CHAT_SEEN_IDS && __CHAT_SEEN_IDS.add(key); }catch(e){}
    ...
    if(isNew){
      ...
      // Alert only for LIVE incoming messages (not the initial batch, and not own messages)
      if(__INITIAL_BATCH_DONE && !isMine){
        try{ if(document.visibilityState==='visible') playSound('chat'); }catch(e){}
        ...
        if(userPromise){
          userPromise.then(u=>{
            ...
            notify('Nová zpráva', (nn+': '+(m.text||'')).slice(0, 140), 'chat');
          }).catch(()=>{
            notify('Nová zpráva', body0, 'chat');
          });
        }else{
          notify('Nová zpráva', body0, 'chat');
        }
...
$('#msgPhoto')?.addEventListener('change', async (e)=>{
  ...
  // Medium image for mobile (fast render, still readable)
  pendingChatImg = await fileToDataURL(f, { max: 1280, quality: 0.78 });
