...
(function mkBotsEngine(){
  ...
  async function _getBotConfigByUid(botUid){
    ...
    // Dynamic bots: bot_<pushId> => bots/<pushId>
    if(String(botUid).startsWith('bot_')){
      const id = String(botUid).slice(4);
      if(!id) return null;
    ...
    // Fixed bots: botProfiles/<uid>
    const psnap = await db.ref('botProfiles/'+botUid).get();
    if(psnap.exists()){
      const p = psnap.val() || {};
      return {
        uid: botUid,
        id: botUid,
        nick: p.nick || 'Bot',
        avatar: p.avatar || './img/default-avatar.svg',
        mode: p.mode || 'dm',
        text: p.text || p.welcome || '',
        img: p.img || '',
  ...
  async function _ensureBotPublic(bot){
    // Server-authoritative: bots must be provisioned by admin / Cloud Functions.
    // Client must NOT write to usersPublic (rules are strict).
    return;
  }
