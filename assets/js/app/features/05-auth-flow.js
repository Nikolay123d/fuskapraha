...
(function authFlowModule(){
  ...
  async function ensureUserPublic(u, extra={}){
    if(!u) return;
    // ✅ "По-взрослому": profile is initialized server-side.
    // This prevents plan/role abuse via DevTools when rules are strict.
    try{
      if(typeof window.callFn === 'function'){
        await window.callFn('profileInit', {
          nick: (extra.nick || u.displayName || 'Uživatel'),
          role: (extra.role || '')
        });
        return;
      }
    }catch(e){
      console.warn('profileInit failed', e);
    }

    // Fallback (legacy) — may fail with permission_denied if rules are strict.
