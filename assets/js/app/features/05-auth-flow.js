...
function openVerifyView(user){
  ...
  async function ensureUserPublic(u, extra={}){
    ...
    // Preferred: server-authoritative init.
    // This works even when RTDB client writes are locked down (server-only paths).
    try{
      if(window.callFn){
        const nick = (extra.nick || u.displayName || '').toString();
        const role = (extra.role || '').toString();
        const avatar = (extra.avatar || u.photoURL || window.DEFAULT_AVATAR || '').toString();
        const r = await window.callFn('profileInit', { nick, role, avatar });
        if(r && r.ok) return;
      }
    }catch(e){
      console.warn('[profileInit] failed, fallback to client init', e);
    }
