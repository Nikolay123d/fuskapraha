...
(function authFlowModule(){
  ...
  async function ensureUserPublic(u, extra={}){
    if(!u) return;
    const nick = String((extra && extra.nick) || u.displayName || 'Uživatel').trim();
    const role = String((extra && extra.role) || 'seeker').trim();
    const avatar = String((extra && extra.avatar) || window.DEFAULT_AVATAR || '').trim();

    // Server-authoritative usersPublic bootstrap
    if(typeof window.callFn === 'function'){
      try{
        await window.callFn('profileInit', { nick, role, avatar });
      }catch(e){
        console.warn('profileInit', e);
      }
    }else{
      console.warn('profileInit skipped: functions bridge not ready');
    }
