...
(function(){
  ...
  async function showUserCard(uid){
    ...
    const isAdm = (Array.isArray(window.ADMIN_UIDS) && window.ADMIN_UIDS.includes(uid));
...
// --- DM threads / inbox ---
  ...
  async function loadMyProfileUI(u){
    ...
    // Email is not needed in public UX; show only for the owner (here).
    document.getElementById('profileEmail').textContent = u.email ? ('E-mail: '+u.email) : 'E-mail: —';
  ...
  async function loadAdminRequests(){
    ...
    // MVP 2026: plan requests come to support/planRequests/{requestId}
    const ps = await db.ref('support/planRequests').orderByChild('ts').limitToLast(150).get();
    const pv = ps.val() || {};

    const all = [];
    for(const [id,r] of Object.entries(pv)){
      if(r && r.status==='pending') all.push({id, r});
    }
    all.sort((a,b)=>( (b.r.ts||0) - (a.r.ts||0) ));

    for(const item of all.slice(0,120)){
      const r=item.r||{};
      const uid = r.fromUid || r.by;
      if(!uid) continue;
      const u=await getUser(uid);

      const plan = String(r.plan || 'premium');
      ...
      const textLine = r.text ? `<div class="muted" style="margin-top:4px">${esc(String(r.text))}</div>` : '';
      const cityLine = r.city ? `<div class="muted">City: ${esc(String(r.city))}</div>` : '';
      ...
      el.addEventListener('click', async (e)=>{
        ...
        if(act==='grant'){
          const now=Date.now();
          let planUntil=0;
          // VIP is "navždy" by default → planUntil = 0
          if(plan !== 'vip'){
            // month-ish plans default to 30 days
            planUntil = now + 30*24*60*60*1000;
          }
          await db.ref('usersPublic/'+uid).update({plan, planUntil, premiumSince: now});
          await db.ref('support/planRequests/'+item.id).update({status:'granted', decidedAt:now, decidedBy: me.uid});
        ...
        if(act==='reject'){
          const now=Date.now();
          await db.ref('support/planRequests/'+item.id).update({status:'rejected', decidedAt: now, decidedBy: me.uid});
          toast('Zamítnuto');
          loadAdminRequests();
        }
      });
