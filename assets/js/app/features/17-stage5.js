...
// --- DM threads / inbox ---
  ...
  async function loadBotsModal(){
    ...
    const s = await db.ref('bots').limitToLast(50).get();
    const v = s.val()||{};
...
const _botChatTick = async ()=>{
    try{
      const snap = await db.ref('bots').limitToLast(50).get();
      const bots = snap.val()||{};
      const now = Date.now();
...
document.getElementById('botAdd')?.addEventListener('click', async ()=>{
    ...
    if(btn){ btn.dataset.lock='1'; btn.disabled=true; }
    try{
      const nick=prompt('Nick bota:', 'Bot'); if(!nick) return;
      const city=prompt('Město (praha/brno/olomouc):', getCity())||getCity();
      const interval=prompt('Interval (min, min 5):','15')||'15';
      const text=prompt('Text zprávy:', 'Ahoj!')||'';

      const nickT = nick.trim();
      const cityT = city.trim();
      const intervalMin = Math.max(5, parseInt(interval,10)||15);

      // Simple duplicate guard: owner+nick+city
      try{
        const snap = await db.ref('bots').limitToLast(200).get();
        const bots = snap.val()||{};
        const dup = Object.values(bots).some(b=>{
          if(!b) return false;
          const on=(b.ownerUid||'');
          const nn=(b.nick||'').toString().trim().toLowerCase();
          const cc=(b.city||'').toString().trim().toLowerCase();
          return (on===me.uid && nn===nickT.toLowerCase() && cc===cityT.toLowerCase());
        });
        if(dup){
          if(!confirm('Bot s tímto nickem a městem už existuje. Přidat duplikát?')) return;
        }
      }catch(e){ /* ignore */ }
      ...
      try{ auditLog('bot_add', me.uid, {botId:id, nick, city}); }catch(e){}
      toast('Bot přidán');
      loadBotsUI();
    }finally{
      if(btn){ btn.dataset.lock='0'; btn.disabled=false; }
    }
  });
