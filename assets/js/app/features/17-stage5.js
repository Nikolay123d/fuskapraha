...
const boxPay=document.getElementById('adminPaymentRequests'); // (legacy placeholder)
if(boxP) boxP.innerHTML = '';

// New flow: payments/requests/{uid}/{requestId}
// Admin reads all requests and can grant planUntil + status.
const ps = await db.ref('payments/requests').get();
const pv = ps.val() || {};
const all = [];
for(const [uid, byUid] of Object.entries(pv)){
  if(!byUid) continue;
  for(const [rid, r] of Object.entries(byUid)){
    if(!r) continue;
...
all.sort((a,b)=>(Number(b.r.ts||0) - Number(a.r.ts||0)));

for(const item of all.slice(0,120)){
  const r=item.r || {};
  const uid = item.uid || r.uid || r.by || '';
  if(!uid) continue;
  const u=await getUser(uid);
  ...
  el.addEventListener('click', async (e)=>{
    ...
    // Compute planUntil (MVP): VIP = 0 (no expiry), Premium* = 30 days.
    const planUntil = (plan==='vip') ? 0 : (now + 30*24*60*60*1000);
    ...
    if(act==='grant'){
      ...
      await db.ref('payments/requests/'+uid+'/'+item.id).update({
        status:'granted',
        decidedAt: now,
        decidedBy: me.uid,
        planUntil
    ...
    if(act==='reject'){
      ...
      try{
        await db.ref('notifications/'+uid).push({
          ...
          text: `Zamítnuto: ${planTitle}`
        });
      }catch(_e){}
      try{ if(typeof auditLog==='function') auditLog('premium_reject', uid, {plan, requestId: item.id}); }catch(_e){}
      toast('Zamítnuto');
      loadAdminRequests();
    }
  });
