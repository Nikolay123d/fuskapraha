...
(function(){
  ...
  // v32: Modern modules own Friends + Notifications.
  // Stage5 keeps legacy code only when flags are off.
  const USE_MODERN_FRIENDS = !!window.MK_FRIENDS_MODERN;
  const USE_MODERN_NOTIFS  = !!window.MK_NOTIFS_MODERN;
  ...
  async function loadFriendsUI(){
    if(USE_MODERN_FRIENDS){
      try{ return window.loadFriends?.(); }catch(e){ return; }
    }
  ...
  // Hook: when tab opened (legacy friends). Modern friends module handles this via showView().
  if(!USE_MODERN_FRIENDS){
    document.addEventListener('click', (e)=>{
  ...
  // Friends: add by email (legacy). Modern friends module wires this itself.
  if(!USE_MODERN_FRIENDS) document.getElementById('friendAddBtn')?.addEventListener('click', async ()=>{
    try{
  ...
  async function showUserCard(uid){
    ...
    addBtn && (addBtn.onclick = async ()=>{
      ...
      // Prefer the modern friends module (single source of truth) if present.
      try{
        if(state==='incoming' && typeof window.acceptFriend==='function'){
          await window.acceptFriend(uid);
          closeModal('modalUserCard');
          loadFriendsUI?.();
          return;
        }
        if(state==='none' && typeof window.sendFriendRequest==='function'){
      ...
      // Legacy fallback (no notifications; those are admin-only in rules).
      if(state==='incoming'){
        const updates={};
        updates['friends/'+me.uid+'/'+uid]='accepted';
        updates['friends/'+uid+'/'+me.uid]='accepted';
  ...
  async function notifyFriendsAboutVacancy(meUid, vac){
    // SECURITY: notifications are only writable by the recipient themselves or admins.
    // For MVP we avoid user->user notification fanout from the client.
    try{ if(!isAdminUser(auth.currentUser)) return; }catch(e){ return; }
...
let NOTIF_REF=null;
  ...
  function watchMyPlan(){
    if(window.MK_LIMITS_MODERN) return;
    const me=auth.currentUser; if(!me) return;
...
async function startDM(toUid, opts={}){
  ...
  // membership (strict rules: each user adds only themselves)
  try{
    // set myself
    await db.ref('privateMembers/'+room+'/'+me.uid).set(true);

    // If DM is with a bot, register room meta (virtual bot, no forced memberships).
    try{
      if(String(toUid).startsWith('bot_')){
        // Bot rooms are virtual (no server). We only register room meta (no forced memberships).
  ...
  // inbox meta for quick thread list (self only; peer meta is created when a real message is sent)
  try{
    const toPub = await getUser(toUid);
    await db.ref('inboxMeta/'+me.uid+'/'+room).update({ ts, lastTs: ts, with: toUid, title: toPub.nick||'Uživatel', lastText:'', unread:0 });
}catch(e){}
  ...
  if(opts && opts.closeModalId){ try{ window.closeModal && window.closeModal(opts.closeModalId); }catch(e){} }
  return room;
}
...
// --- DM threads / inbox ---
  ...
  async function loadMyProfileUI(u){
    ...
    document.getElementById('reqPremium').onclick = async ()=>{
      if(window.openPremium) openPremium();
      else openPremiumBot();
    };
  }
  ...
  async function loadBotsModal(){
    ...
    // Keep the admin UI snappy even if DB has many bots.
    const s = await db.ref('bots').orderByChild('createdAt').limitToLast(50).get();
    const v = s.val()||{};
    const entries = Object.entries(v).sort((a,b)=> ((b[1]?.createdAt||0) - (a[1]?.createdAt||0)));
    ...
    try{
      ...
      for(const [id,b] of Object.entries(bots)){
        ...
        for(const city of targets){
          // IMPORTANT: RTDB rules enforce by === auth.uid. We post as the host admin
          // but include botNick/botAvatar so UI renders it as the bot.
          const msg = { by: hostUid, ts: now, text: text||null, img: img||null, bot:true, botUid, botNick: b.nick||'Bot', botAvatar: b.avatar||window.DEFAULT_AVATAR };
...
async function botTick(){
    ...
    for(const [id,b] of Object.entries(v)){
      ...
      // IMPORTANT: RTDB rules enforce by === auth.uid. We post as the host admin
      // but include botNick/botAvatar so UI renders it as the bot.
      const botText = (b.text||'').toString();
      const botImg = (b.img||'').toString();
      if(!botText && !botImg) continue;
      await db.ref('messages/'+city).push({
        ...
        botUid,
        botNick: b.nick||'Bot',
        botAvatar: b.avatar||window.DEFAULT_AVATAR
      });
