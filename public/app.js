...
async function ensureUserPublic(u, extra={}){
  ...
  if(u.email){
    ...
    await db.ref('emails/'+key).update({uid: u.uid, ts: Date.now()});
  }
}
...
// profile display
  if(u){
    ...
    try{ await ensureMyPublic(u); }catch(e){ console.warn('ensureMyPublic failed', e); }
    setPresence(u);
    await refreshMe();
    ...
    try{ Promise.resolve(listenNotifications()).catch(e=>console.warn('listenNotifications',e)); }catch(e){}

    // v15: start bot DM engine for admin (auto-replies)
    try{ Promise.resolve(startBotDmEngine()).catch(e=>console.warn('startBotDmEngine',e)); }catch(e){}
    try{ Promise.resolve(startBotHostEngine()).catch(e=>console.warn('startBotHostEngine',e)); }catch(e){}
