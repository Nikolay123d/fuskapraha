...
async function googleSignIn(){
  ...
  // IMPORTANT: Use redirect on ALL devices (mobile + desktop).
  // Also request permissions/unlock audio on the same user gesture BEFORE redirect.
...
// Handle Google redirect return (redirect is used for ALL devices)
try{
  auth.getRedirectResult()
    ...
    .catch((e)=>{
      ...
      try{ toast('Chyba přihlášení přes Google'); }catch(_){}
...
(function(){
  if(window.__AUTH_STATE_WIRED__) return;
  window.__AUTH_STATE_WIRED__ = true;
...
document.getElementById('botAdd')?.addEventListener('click', async ()=>{
  ...
  if(window.__AUTH_STATE_WIRED__) {
    // main auth handler already registered above
  } else {
  auth.onAuthStateChanged(async (u)=>{
    try{
