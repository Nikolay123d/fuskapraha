...
exports.consumeLimit = functions.region('europe-west1').https.onCall(async (data, context) => {
  ...
  const planSnap = await rtdb.ref(`usersPublic/${uid}/plan`).get();
  const plan = normPlan(planSnap.val() || 'free');
...
exports.friendSend = functions.region('europe-west1').https.onCall(async (data, context) => {
  ...
  const planSnap = await rtdb.ref(`usersPublic/${me}/plan`).get();
  const plan = normPlan(planSnap.val() || 'free');
  ...
  const payload = {
    ...
    fromAvatar: String(mePub.avatar || '')
  };

  // If already friends, do nothing
  const frSnap = await rtdb.ref(`friends/${me}/${toUid}`).get();
  if(String(frSnap.val()||'') === 'accepted'){
    return { ok: true, already: true };
  }

  await rtdb.ref(`friendRequests/${toUid}/${me}`).set(payload);
...
exports.onChatMessageCreate = functions.region('europe-west1').database.ref('/messages/{city}/{mid}').onCreate(async (snap, ctx) => {
  ...
  const planSnap = await rtdb.ref(`usersPublic/${by}/plan`).get();
  const plan = normPlan(planSnap.val() || 'free');
