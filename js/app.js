...
// --- Ждём DOM и только потом вешаем обработчики ---
document.addEventListener('DOMContentLoaded', setupSend);
...
function setupSend(){
  ...
  if (!sendBtn || !input) {
    console.warn('sendButton/messageInput не найдены в DOM');
    return;
  }
  ...
  async function onSend(){
    ...
    const imageUrl = window.__pendingImageUrl || null; // если прикрепляешь ссылкой
    if (!text && !imageUrl) return;

    const msg = {
      uid: u.uid,  // ОБЯЗАТЕЛЬНО под правила!
      nick: u.displayName || (u.email ? u.email.split('@')[0] : 'Користувач'),
    ...
    try {
      ...
      input.value = '';
      window.__pendingImageUrl = null;
    } catch (e) {
      console.error('push failed:', e);
      alert('Не вдалося відправити: ' + (e?.message || e));
    }
  }
}
