...
function renderDMInbox(){
  ...
  db.ref('inboxMeta/'+u.uid).orderByChild('ts').limitToLast(200).on('child_added', async snap=>{
    ...
    const card=document.createElement('div'); card.className='help-card'; card.style.display='flex'; card.style.alignItems='center'; card.style.gап='8px';
...
// ===== Админка участников (модалка строится на лету)
