...
async function googleSignIn(){
  const provider = new firebase.auth.GoogleAuthProvider();
  try{ provider.addScope('email'); provider.addScope('profile'); }catch(e){}
  // Desktop: prefer popup (no page reload). Mobile: redirect is more reliable.
  // Also avoid double-click races.
  if(window.__GOOGLE_AUTH_IN_FLIGHT__) return;
  window.__GOOGLE_AUTH_IN_FLIGHT__ = true;
  const isMobile = (window.matchMedia && window.matchMedia('(max-width: 720px)').matches) || /Android|iPhone|iPad|iPod/i.test(navigator.userAgent||'');
  try{
    try{ await auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL); }catch(e){}
    ...
    if(!isMobile){
      try{
        ...
        const code = String(e?.code||'');
        if(code.includes('popup') || code.includes('auth/web-storage-unsupported')){
...
try{
  auth.getRedirectResult()
    .then(async (r)=>{
      if(r && r.user){
        try{ await ensureUserPublic(r.user, {}); }catch(e){}
        try{ closeModalAuth(); }catch(e){}
...
document.getElementById('authGoogleBtn')?.addEventListener('click', async ()=>{
    try{ await googleSignIn(); }catch(e){ toast(e.message||'Chyba'); }
  });
