...
// ===== AUTH click delegation (SAFE) =====
// NOTE: Previously this block matched buttons by *text* (e.g. any element containing 'přihl' or 'registrace').
// That accidentally hijacked clicks on:
// - Google sign-in button (text contains 'přihl' -> was treated as normal login)
// - 'Nemáte účet? Registrace' switch link (text contains 'registrace' -> was treated as submit)
// Result: Google/login/register appeared 'not working'.
// Fix: trigger ONLY by explicit element IDs of real submit/reset buttons.
(function(){
  ...
  document.addEventListener('click', async (ev)=>{
    ...
    const id = (el.id||'');
    const isForgot = (id==='authForgotBtn' || id==='authResetPass');
    const isRegister = (id==='authRegisterBtn');
    const isLogin = (id==='authLoginBtn');
    ...
    try{
      ...
      if(isForgot && typeof doPasswordReset==='function'){
        await doPasswordReset();
        return;
      }
      if(isRegister && typeof handleRegister==='function'){
        await handleRegister();
        return;
      }
      if(isLogin && typeof handleLogin==='function'){
        await handleLogin();
        return;
      }
    }catch(e){
