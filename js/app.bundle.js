  ...
  try{
    ...
    function __mk_captureHandler(e){
      try{
        ...
        var btn = t.closest ? t.closest('#authLoginBtn,#authRegisterBtn,#authGoogleBtn,#authSwitchToRegister,#authSwitchToLogin') : null;
        if (!btn) return;
        ...
        // Switch buttons: let existing UI code handle view changes if present,
        // but we still prevent duplicates. We emulate a "click" via custom event.
        if (id === 'authSwitchToRegister' || id === 'authSwitchToLogin') {
          try{
            var ev = new CustomEvent('mk_auth_switch', { detail: { id: id } });
            document.dispatchEvent(ev);
          }catch(_){}
          // If the original UI switch logic lives elsewhere, it may listen to this custom event.
          return;
        }
      }catch(_e){}
    }
...
function _setAdminUI(is, user){
  ...
  try{
    document.querySelectorAll('.adm-only,.admin-only').forEach(el=>{
  ...
  for(const [msgId, msg] of Object.entries(msgs)){
    ...
    if(premiumMembers.length){
      for(const uid of premiumMembers){
        updates['archive/privateMessagesByUser/'+uid+'/'+roomId+'/'+msgId] = Object.assign({}, msg, { archivedAt:_now() });
      }
    }
...
async function runDailyCleanupIfAdmin(user){
  try{
    if(!user || !window.__isAdmin) return;
    const key = 'mk_last_cleanup_ts';
    const last = Number(localStorage.getItem(key)||0);
    const DAY = 24*60*60*1000;
    if(last && (_now()-last) < DAY) return;
...
document.getElementById('adminBroadcastClose')?.addEventListener('click', ()=>closeModal('adminBroadcastModal'));
  // Admin tiles (v17.65)
  document.getElementById('adminUsersBtn')?.addEventListener('click', ()=>{ try{ openModal('adminUsersModal'); }catch(e){} });
  document.getElementById('adminTicketsBtn')?.addEventListener('click', ()=>{ try{ supportOpenModal(); }catch(e){ try{ openModal('supportModal'); }catch{} } });
  document.getElementById('adminBroadcastBtn')?.addEventListener('click', ()=>{ try{ openModal('adminBroadcastModal'); }catch(e){} });
  document.getElementById('adminMapBtn')?.addEventListener('click', ()=>{ try{ openModal('adminMapPointsModal'); }catch(e){} });
  document.getElementById('adminBotsBtn')?.addEventListener('click', ()=>{ try{ openModal('modalBots'); }catch(e){} });
  document.getElementById('adminDesignBtn')?.addEventListener('click', ()=>{ try{ adminOpenDesignQuick(); }catch(e){} });
  document.getElementById('adminSoundsBtn')?.addEventListener('click', ()=>{ try{ adminOpenSoundsQuick(); }catch(e){} });
