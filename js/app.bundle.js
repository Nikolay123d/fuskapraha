...
(function(){
  try{
    ...
    async function __mk_login_google(){
      ...
      // Use redirect by default to avoid COOP/POPUP issues in modern browsers.
      // Do NOT await on some mobile browsers to preserve user-gesture context.
      var isMobile = /Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent || '');
      try{
        if (isMobile) { a.signInWithRedirect(provider); return; }
    ...
    // 3) CAPTURE shield: stop any other duplicate handlers and route auth actions once.
    function __mk_captureHandler(e){
      try{
        var t = e.target;
        if (!t) return;

        var btn = t.closest ? t.closest('#authLoginBtn,#authRegisterBtn,#authGoogleBtn,#authSwitchToRegister,#authSwitchToLogin,#authForgotBtn,#authResetPass') : null;
        if (!btn) return;

        // Stop event before it reaches other listeners (including those on the button itself).
        e.preventDefault();
        e.stopPropagation();
        ...
        if (id === 'authLoginBtn') return __mk_run(__mk_login_email);
        if (id === 'authRegisterBtn') return __mk_run(__mk_register_email);
        if (id === 'authGoogleBtn') return __mk_run(__mk_login_google);

        // Switch buttons: we control the UI here to avoid relying on other handlers.
        if (id === 'authSwitchToRegister') { __mk_setAuthMode('register'); return; }
        if (id === 'authSwitchToLogin') { __mk_setAuthMode('login'); return; }

        if (id === 'authForgotBtn' || id === 'authResetPass') return __mk_run(__mk_reset_password);

      }catch(_e){}
    }

    function __mk_install(){
      // capture = true is essential here
      document.addEventListener('click', __mk_captureHandler, true);
      ...
      row.innerHTML = `
        ...
        <div class="threadBody" style="min-width:0">
          <div class="threadTop" style="display:flex;justify-content:space-between;gap:10px;align-items:baseline">
            ...
            <div class="muted" style="font-size:12px;white-space:nowrap">${new Date(v[room].ts||0).toLocaleString()}</div>
          </div>
