...
function $(id){ return document.getElementById(id); }
  ...
  async function doGoogle(){
    ...
    // Preferred: Popup on desktop, Redirect on mobile.
    ...
    try{
      ...
      if(code.includes('unauthorized-domain')){
        ...
        say((e && e.message) || ('Chyba Google přihlášení: ' + code));
      }
      throw e;
    }
  }
...
function _setAdminUI(is, user, isMod){
  ...
  // Auto DB maintenance once per day when staff logs in
  try{
    if(window.__isStaff){
      setTimeout(()=>{ try{ runDailyCleanupIfAdmin(auth.currentUser, false); }catch(e){} }, 1200);
    }
  }catch(e){}
...
async function runDailyCleanupIfAdmin(user, force=false){
  try{
    ...
    let cities = (window.CITIES && Array.isArray(window.CITIES) && window.CITIES.length)
      ? window.CITIES
      : (Array.from(document.querySelectorAll('#citySelect option'))
           .map(o=>String(o.value||'').trim())
           .filter(Boolean));
    if(!cities || !cities.length){
      // fallback (match RTDB paths)
      cities = ['praha','brno','ostrava','plzen','liberec','olomouc','ceske_budejovice','hradec_kralove','pardubice','usti','zlin','karlovy_vary','jihlava','teplice'];
    }
