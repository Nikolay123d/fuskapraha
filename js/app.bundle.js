...
async function googleSignIn(){
  ...
  try{
    ...
    // Strategy:
    // - Desktop: Popup first (fast, no reload). If blocked, fallback to Redirect.
    // - Mobile / in-app browsers: Redirect (more reliable).
    if(!isMobile){
      try{
        ...
        window.__GOOGLE_AUTH_IN_FLIGHT__ = false;
        return;
      }catch(e){
        const code = String(e?.code||'');
        if(code.includes('unauthorized-domain')){
        ...
        // Popup blocked/closed/cancelled → fallback to redirect
        // Other errors: show and try redirect as last resort
        try{ toast(e?.message || 'Chyba Google přihlášení (popup). Zkouším redirect…'); }catch(_){}
      }
    }
    ...
    if(code.includes('unauthorized-domain')){
      ...
      toast('Google přihlášení: tento prohlížeč / režim blokuje přihlášení. Otevřete stránku v Chrome a zkuste znovu.');
    } else if(e?.message){
      toast(e.message);
    } else {
...
(()=>{const b=document.getElementById('buyVipWorker'); if(!b) return; if(b.dataset && b.dataset.mkBuyWired==='1') return; if(b.dataset) b.dataset.mkBuyWired='1'; b.addEventListener('click', (e)=>{ try{ e?.preventDefault?.(); }catch(_){} try{ closePrivModal?.(); }catch(_){} openPremiumBotDM('vip_worker'); });})();
(()=>{const b=document.getElementById('buyVip150'); if(!b) return; if(b.dataset && b.dataset.mkBuyWired==='1') return; if(b.dataset) b.dataset.mkBuyWired='1'; b.addEventListener('click', (e)=>{ try{ e?.preventDefault?.(); }catch(_){} try{ closePrivModal?.(); }catch(_){} openPremiumBotDM('vip150'); });})();
(()=>{const b=document.getElementById('buyPremium200'); if(!b) return; if(b.dataset && b.dataset.mkBuyWired==='1') return; if(b.dataset) b.dataset.mkBuyWired='1'; b.addEventListener('click', (e)=>{ try{ e?.preventDefault?.(); }catch(_){} try{ closePrivModal?.(); }catch(_){} openPremiumBotDM('premium200'); });})();
