...
// Hard admin UID (single super-admin)
window.ADMIN_UID = "urciknikolaj642@gmail.com";
// TEMP: Admin by email (Firebase Auth)
...
function isAdminUser(u){
  ...
  return !!(window.ADMIN_EMAIL && em === String(window.ADMIN_EMAIL).toLowerCase());
}
  ...
  const cb = async (snap)=>{
    ...
    for(const uid of uids){
      try{
        ...
        row.innerHTML = `<div class="ava"><img src="${esc(pu.avatar||window.DEFAULT_AVATAR)}"></div>`+
                        ...
                        (isAdminUser(auth.currentUser) ? `<button data-act="ban" data-uid="${esc(uid)}">Ban 24h</button><button data-act="mute" data-uid="${esc(uid)}">Mute 24h</button>` : ``)+
                        `</div>`;
