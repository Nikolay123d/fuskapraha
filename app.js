...
console.log('[MAKAME] app.js build: 2025-12-25_cityrooms');
...
async function ensureMyPublic(u){
  ...
  if(u.email){
    ...
    await db.ref('emails/'+key).set({uid:u.uid, email:u.email||null, ts: Date.now()});
  }
}
...
async function ensureUserPublic(u, extra={}){
  ...
  if(u.email){
    ...
    await db.ref('emails/'+key).update({uid: u.uid, email: u.email||null, ts: Date.now()});
  }
}
