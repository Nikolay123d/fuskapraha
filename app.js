...
function showView(id){
  ...
  if(id!=='view-dm'){ try{ document.body.classList.remove('dm-chat-open'); }catch(e){} }
  if(id==='view-admin'){ try{ loadAdminAllUsers(); }catch(e){} }
...
async function enforceVerifyWindow(u){
  ...
  window.__EMAIL_VERIFY_REQUIRED__ = false;
...
$('#sendBtn')?.addEventListener('click', async ()=>{
  try{
    ...
    if(u.emailVerified===false){
      const until=getVerifyDeadline(u);
      if(until && Date.now()>until){ window.__EMAIL_VERIFY_REQUIRED__ = false; toast('Potvrzení e-mailu vypršelo.'); openModalAuth('login'); return; }
    }
...
function openDMRoom(a,b){
  ...
  window.__dmRoom = room;
  // Mobile UX: hide thread list when chat opened
  try{ document.body.classList.add('dm-chat-open'); }catch(e){}
  renderDM(room);
}
...
$('#dmSend')?.addEventListener('click', async ()=>{
  try{
    ...
    // Pull public data (for thread titles)
    let myPub = null, toPub = null;
    try{
      [myPub, toPub] = await Promise.all([getUser(me.uid), getUser(to)]);
    }catch(e){
      myPub = myPub || {};
      toPub = toPub || {};
    }

    // Create message key first (so we can do an atomic multi-path update)
    const msgRef = db.ref('privateMessages/'+room).push();
    const msgId = msgRef.key;

    const updates = {};
    updates['privateMembers/'+room+'/'+me.uid] = true;
    updates['privateMembers/'+room+'/'+to] = true;

    updates['privateMessages/'+room+'/'+msgId] = {
      by: me.uid,
      ts: ts,
      text: text || null,
      img: img || null
    ...
    const lastText = (text && text.length) ? text.slice(0, 120) : (img ? '[photo]' : '');

    updates['inboxMeta/'+me.uid+'/'+room+'/ts'] = ts;
    updates['inboxMeta/'+me.uid+'/'+room+'/with'] = to;
    updates['inboxMeta/'+me.uid+'/'+room+'/title'] = (toPub && (toPub.nick || toPub.name)) ? (toPub.nick || toPub.name) : 'Uživatel';
    updates['inboxMeta/'+me.uid+'/'+room+'/lastTs'] = ts;
    updates['inboxMeta/'+me.uid+'/'+room+'/lastText'] = lastText;

    updates['inboxMeta/'+to+'/'+room+'/ts'] = ts;
    updates['inboxMeta/'+to+'/'+room+'/with'] = me.uid;
    updates['inboxMeta/'+to+'/'+room+'/title'] = (myPub && (myPub.nick || myPub.name)) ? (myPub.nick || myPub.name) : 'Uživatel';
    updates['inboxMeta/'+to+'/'+room+'/lastTs'] = ts;
    updates['inboxMeta/'+to+'/'+room+'/lastText'] = lastText;
...
// --- Admin: list all registered users (from usersPublic) ---
let __adminUsersCache = [];
async function loadAdminAllUsers(){
  const me = auth.currentUser;
  if(!me || !isAdmin(me)) return;
  const box = document.getElementById('adminAllUsers');
  if(!box) return;
  box.innerHTML = '<div class="muted">Načítání…</div>';
  try{
    const snap = await db.ref('usersPublic').limitToLast(5000).get();
    const arr=[];
    snap.forEach(ch=>{ const v=ch.val()||{}; arr.push({uid: ch.key, nick: v.nick||'Uživatel', email: v.email||'', city: v.city||'', role: v.role||'', plan: v.plan||''}); });
    // newest first if createdAt exists
    arr.sort((a,b)=> String(b.plan).localeCompare(String(a.plan)) || a.nick.localeCompare(b.nick));
    __adminUsersCache = arr;
    renderAdminUsers(arr);
  }catch(e){
    console.error(e);
...
function renderAdminUsers(arr){
  const box=document.getElementById('adminAllUsers'); if(!box) return;
  const q=(document.getElementById('adminUserSearch')?.value||'').trim().toLowerCase();
  const list = !q ? arr : arr.filter(x=> (x.uid||'').toLowerCase().includes(q) || (x.nick||'').toLowerCase().includes(q) || (x.email||'').toLowerCase().includes(q));
  box.innerHTML = '';
  list.slice(0, 500).forEach(u=>{
    const d=document.createElement('div'); d.className='msg';
    d.style.cursor='pointer';
    const badge = (String(u.plan).toLowerCase()!=='free' && String(u.plan).toLowerCase()!=='none' && u.plan) ? '<span class="badge premium">PREMIUM</span>' : '';
    d.innerHTML = `<div><b>${esc(u.nick||'Uživatel')}</b> ${badge}<div class="muted" style="margin-top:2px">${esc(u.email||'')} · ${esc(u.uid)}</div></div>`;
    d.addEventListener('click', ()=>{
      const inp=document.getElementById('roleUid'); if(inp) inp.value=u.uid;
      toast('UID vloženo');
    });
    box.appendChild(d);
  });
  if(list.length===0) box.innerHTML = '<div class="muted">Nic nenalezeno.</div>';
}
document.getElementById('adminUserSearch')?.addEventListener('input', ()=>renderAdminUsers(__adminUsersCache));
// === Auth state ===
auth.onAuthStateChanged(async (u)=>{
  ...
  try{ if(window.__isAdmin) loadAdminAllUsers(); }catch(e){}
...
(function(){
  ...
  function openModal(id){
    ...
    if(!el.__backdropBound){
      el.addEventListener('click', (e)=>{ if(e.target===el) closeModal(id, true); });
      el.__backdropBound=true;
    }
    el.hidden=false;
    document.body.classList.add('modal-open');
    // Mobile/Android back button support
    try{
      if(!history.state || history.state.modal!==id){
        history.pushState({modal:id}, '');
      }
      if(!window.__modalPopBound){
        window.addEventListener('popstate', ()=>{
          // close any open modal on back
          document.querySelectorAll('.modal:not([hidden])').forEach(m=>{ m.hidden=true; });
          document.body.classList.remove('modal-open');
        });
        window.__modalPopBound=true;
      }
    }catch(e){}
  }
  function closeModal(id, fromBackdrop=false){
    const el=document.getElementById(id);
    if(el) el.hidden=true;
    // remove class if no other modals open
    if(!document.querySelector('.modal:not([hidden])')) document.body.classList.remove('modal-open');
    // if user clicked X/backdrop, return from pushed state
    try{
      if(!fromBackdrop && history.state && history.state.modal===id) history.back();
      if(fromBackdrop && history.state && history.state.modal===id) history.back();
    }catch(e){}
  }
  ...
  document.getElementById('dmBackBtn')?.addEventListener('click', ()=>{ try{ document.body.classList.remove('dm-chat-open'); }catch(e){} });
...
async function startDM(toUid, opts = {}){
  ...
  // Pull public data for nicer titles in thread list
  let myPub = null, toPub = null;
  try{
    [myPub, toPub] = await Promise.all([getUser(me.uid), getUser(toUid)]);
  }catch(e){
    myPub = myPub || {};
    toPub = toPub || {};
  }

  // Atomic multi-path update to avoid "half-created" rooms
  const updates = {};
  updates['privateMembers/'+room+'/'+me.uid] = true;
  updates['privateMembers/'+room+'/'+toUid] = true;

  // IMPORTANT: loadDmThreads() sorts by child 'ts', keep consistent
  updates['inboxMeta/'+me.uid+'/'+room+'/ts'] = ts;
  updates['inboxMeta/'+me.uid+'/'+room+'/with'] = toUid;
  updates['inboxMeta/'+me.uid+'/'+room+'/title'] = (toPub && (toPub.nick || toPub.name)) ? (toPub.nick || toPub.name) : 'Uživatel';
  updates['inboxMeta/'+me.uid+'/'+room+'/lastTs'] = ts;

  updates['inboxMeta/'+toUid+'/'+room+'/ts'] = ts;
  updates['inboxMeta/'+toUid+'/'+room+'/with'] = me.uid;
  updates['inboxMeta/'+toUid+'/'+room+'/title'] = (myPub && (myPub.nick || myPub.name)) ? (myPub.nick || myPub.name) : 'Uživatel';
  updates['inboxMeta/'+toUid+'/'+room+'/lastTs'] = ts;

  try{
    await db.ref().update(updates);
  }catch(e){
    console.error('startDM update failed:', e);
    playSound?.('err');
    toast('Nelze otevřít DM (permission / rules).');
    return null;
  }
  ...
  try{
    const dmToEl = document.getElementById('dmTo');
    if(dmToEl) dmToEl.value = toUid;
  }catch(e){}

  if(opts && opts.closeModalId){
    try{ closeModal(opts.closeModalId); }catch(e){}
  }
  return room;
}
