...
// Start staged prefetch safely (never block initial UI)
function __startPrefetchStages(){
  try{
    // Delay a bit to let first paint happen
    setTimeout(()=>{ try{ __runPrefetchStages(); }catch(e){} }, 50);
  }catch(e){}
}
...
function schedulePromoOffer(){
  try{
    const lsKey='promo_seen_ts';
    const localSeen = +localStorage.getItem(lsKey)||0;

    const me = auth.currentUser;
    const userSeenRef = me ? db.ref('users/'+me.uid+'/promoSeenTs') : null;

    const tryPaths = ['announcements/global','settings/promo'];
    const showIfNew = async(p)=>{
      const ts = +p.ts||0;
      if(!ts) return;
      // Read per-user seen (cross-device). If unavailable, fall back to localStorage.
      let seen = localSeen;
      if(userSeenRef){
        try{
          const s = await userSeenRef.get();
          const v = +s.val()||0;
          if(v>seen) seen=v;
        }catch(e){}
      }
      ...
      // Mark seen immediately (best-effort)
      try{ localStorage.setItem(lsKey, String(ts)); }catch(e){}
      if(userSeenRef){
        try{ await userSeenRef.set(ts); }catch(e){}
      }

      // Render
      const overlay = document.getElementById('promoOverlay');
      if(overlay){
        document.getElementById('promoTitle').textContent = p.title || 'Oznámení';
        document.getElementById('promoText').textContent  = p.text  || '';
        const img = document.getElementById('promoImg');
        if(img){
          if(p.img){ img.src = p.img; img.hidden=false; } else { img.hidden=true; }
        }
        ...
        if(btn){
          if(p.btnText){
            ...
            btn.hidden = false;
            btn.onclick = ()=>{ try{ if(p.btnUrl) window.location.hash=p.btnUrl; }catch(e){} closeModal('promoOverlay'); };
    ...
    // Delay so it doesn't feel like "popup on load"
    setTimeout(()=>tryNext(0), 2500);
  }catch(e){}
}
...
async function ensureMyPublic(u){
  ...
  // Email index disabled (was causing permission_denied noise)
  // If you need "add friend by email" later, re-enable with dedicated secure rules.
...
async function resolveUidByEmail(email){
  // Disabled: email->uid index was removed to avoid permission_denied warnings.
  return null;
}
let DM_REF=null;
...
(function(){
  ...
  document.getElementById('friendAddBtn')?.addEventListener('click', async ()=>{
    ...
    if(me.emailVerified===false){
      ...
      const toS=await Promise.resolve({val:()=>null}); const uid=(toS.val()&&toS.val().uid);
  ...
  try{
    if(_adminUsersMode==='complaints'){
      ...
      for(const it of filtered){
        ...
        row.className='msg';
        row.innerHTML = `
          <div class="ava"><img src="${esc(it.avatar||window.DEFAULT_AVATAR)}"></div>
          <div class="bubble" style="width:100%;cursor:pointer">
            <div class="meta">
              <div class="name"><b>${esc(it.nick||'Uživatel')}</b> <span class="muted">${esc(it.role||'')}</span></div>
              <div class="time">${esc(it.plan||'free')}</div>
            </div>
            <div class="muted" style="font-size:12px">${esc(it.email||'')} · ${esc(it.uid)}</div>
          </div>`;
        row.addEventListener('click', (ev)=>{ try{ ev.stopPropagation(); }catch(e){} adminOpenUserCard(it.uid); });
        list.appendChild(row);
      }
    }
  }catch(e){
