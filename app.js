...
const OWNER_EMAIL = 'urciknikolaj642@gmail.com';
// in case UID was copied with O/0 or l/I confusion - both accepted
const OWNER_UIDS = new Set([
  'VrP5IzhgxmTOuKWc9UWXSCe6nM2',
...
let SOUND_URLS = {
  chat: './sounds/new_chat.wav',
  dm: './sounds/new_dm.wav',
  ok: './sounds/ok.wav',
  err: './sounds/err.wav',
  party: './sounds/celebration.wav'
};
function applySoundUrls(urls){
  if(!urls) return;
  Object.keys(SOUND_URLS).forEach(k=>{
    const v = urls[k];
    if(typeof v === 'string' && v.trim()){
      SOUND_URLS[k] = v.trim();
      try{ SND[k].src = SOUND_URLS[k]; }catch{}
    }
  });
}
...
function notify(title, body, kind="dm"){
  try{
    if(!('Notification' in window)) return;
...
// === Premium UI + payment requests (safe, does not affect chat/DM) ===
function renderPremiumPlans(){
  const host = document.getElementById('premiumPlans');
  if(!host) return;
  const plans = [
    {
      code:'premium',
      ...
      period:'měsíc',
      items:[
        'Prolistování lenty (bez limitu)',
        'Vlastní bot – interval 1 hod.',
        'Základní privilegia + badge PREMIUM'
      ]
    },
    {
      code:'premiumPlus',
      ...
      items:[
        'Vše z Premium',
        'Bot s avatarem + text+foto po příspěvku',
        'Zprávy botu chodí vám do DM s označením [REKLAMA]'
      ]
    }
  ];
  host.innerHTML = plans.map(p=>`
    <div class="plan" data-plan="${p.code}">
      <h3>${p.title}</h3>
...
async function requestNotifications(){
  try{
    if(!('Notification' in window)) return toast('Prohlížeč nepodporuje oznámení');
    const perm = await Notification.requestPermission();
    if(perm==='granted'){ localStorage.setItem('notifAllowed','1'); toast('Oznámení povolena'); playSound('ok'); }
    else { localStorage.setItem('notifAllowed','0'); toast('Oznámení zamítnuta'); playSound('err'); }
  }catch(e){ console.error(e); playSound('err'); }
}
document.getElementById('btnReqNotif')?.addEventListener('click', requestNotifications);

let _subRef=null;
window.__isPremium = false;
function setPremiumUi(active, plan){
  window.__isPremium = !!active;
  try{
    const b = document.getElementById('myPremium');
    if(b){
      b.style.display = active ? 'inline-flex' : 'none';
      b.textContent = active ? String(plan||'PREMIUM').toUpperCase() : 'PREMIUM';
    }
  }catch(_){}
}
...
async function submitPaymentRequest(){
  ...
  if(!f) return toast('Nahrajte screenshot platby');
  const st = document.getElementById('payStatus');
  if(st) st.textContent = 'Odesílám…';
  try{
    const proof = await fileToDataURL(f);
    const id = db.ref('paymentRequests').push().key;
    await db.ref('paymentRequests/'+id).set({
      uid: me.uid,
      email: me.email || null,
      plan,
      ts: Date.now(),
    ...
    if(st) st.textContent = 'Žádost odeslána. Admin ji potvrdí.';
    toast('Žádost odeslána'); playSound('ok');
    document.getElementById('payProof').value='';
  }catch(e){ console.error(e); if(st) st.textContent='Chyba při odesílání.'; playSound('err'); }
}
document.getElementById('paySubmit')?.addEventListener('click', submitPaymentRequest);

function watchPaymentsForAdmin(){
  const feed = document.getElementById('paymentsFeed');
  if(!feed) return;
  db.ref('paymentRequests').on('value', async (snap)=>{
    if(!window.__isAdmin){ feed.innerHTML=''; return; }
    const v=snap.val()||{};
    const rows = Object.entries(v)
      .filter(([_,r])=>r && r.status==='pending')
      .sort((a,b)=>(b[1].ts||0)-(a[1].ts||0));
    if(!rows.length){ feed.innerHTML='<div class="muted">Žádné žádosti.</div>'; return; }
    feed.innerHTML='';
    for(const [id,r] of rows){
      ...
      card.innerHTML = `
        <div class="row" style="justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap">
          <div>
            <b>${esc(r.plan||'premium')}</b> · <span class="muted">${when}</span><br>
            <span class="muted">${esc((r.email||'')+' · '+String(r.uid||'').slice(0,8))}</span>
          </div>
          <div class="row" style="gap:8px">
            <button class="btn" data-approve="${id}">Potvrdit</button>
            <button class="ghost" data-deny="${id}">Zamítnout</button>
          </div>
        </div>
        ${r.proof ? `<div style="margin-top:8px"><img src="${r.proof}" alt="proof" style="max-width:100%;border-radius:12px;border:1px solid rgba(255,255,255,.12)"></div>`:''}
      `;
      card.querySelector('[data-approve]')?.addEventListener('click', async ()=>{
        ...
        await db.ref('subscriptions/'+uid).set({plan:r.plan||'premium', since:now, until, by: (auth.currentUser?auth.currentUser.uid:null)});
        await db.ref('paymentRequests/'+id+'/status').set('approved');
        toast('Aktivováno'); playSound('ok');
      });
      card.querySelector('[data-deny]')?.addEventListener('click', async ()=>{
        await db.ref('paymentRequests/'+id+'/status').set('denied');
        toast('Zamítnuto'); playSound('ok');
      });
      feed.appendChild(card);
    }
  });
}
watchPaymentsForAdmin();
...
if(t && t.id==='chatWallCamera'){
  ...
  r.onload=()=>{
    const data=r.result;
    applyWallpaperVar('--wallChat', data);
    try{ localStorage.setItem('wall_chat', data); }catch(_){}
    db.ref('settings/wallpapers/chat').set({url:data, ts:Date.now(), by:(auth.currentUser?auth.currentUser.uid:null)});
    toast('Pozadí chatu bylo uloženo pro všechny uživatele.');
    t.value='';
  };
  r.readAsDataURL(f);
}
if(t && t.id==='dmWallCamera'){
  ...
  r.onload=()=>{
    const data=r.result;
    applyWallpaperVar('--wallDM', data);
    try{ localStorage.setItem('wall_dm', data); }catch(_){}
    db.ref('settings/wallpapers/dm').set({url:data, ts:Date.now(), by:(auth.currentUser?auth.currentUser.uid:null)});
    toast('Pozadí DM bylo uloženo pro všechny uživatele.');
    t.value='';
  };
  r.readAsDataURL(f);
}
if(t && t.id==='profileWallCamera'){
  if(!window.__isAdmin){ toast('Pouze administrátor může měnit pozadí.'); t.value=''; return; }
  const f=t.files && t.files[0]; if(!f) return;
  const r=new FileReader();
  r.onload=()=>{
    const data=r.result;
    applyWallpaperVar('--wallProfile', data);
    try{ localStorage.setItem('wall_profile', data); }catch(_){}
    db.ref('settings/wallpapers/profile').set({url:data, ts:Date.now(), by:(auth.currentUser?auth.currentUser.uid:null)});
    toast('Pozadí profilu bylo uloženo pro všechny uživatele.');
    t.value='';
  };
  r.readAsDataURL(f);
}
...
// --- settings watchers (sounds + wallpapers) ---
try{
  db.ref('settings/sounds').on('value', (snap)=>{
    const v = snap.val();
    if(v && typeof v === 'object') applySoundUrls(v);
  });
}catch(_){}

function applyWallpaperVar(cssVar, url){
  if(typeof url!=='string') return;
  document.documentElement.style.setProperty(cssVar, `url('${url}')`);
  document.documentElement.style.setProperty(cssVar+'Raw', url);
}
function watchWallpaper(key, cssVar, lsKey){
  try{
    const ref = db.ref('settings/wallpapers/'+key);
    ref.on('value', (snap)=>{
      const val=snap.val();
      const url = (typeof val==='string') ? val : (val && val.url);
      if(typeof url==='string' && url){
        try{ if(lsKey) localStorage.setItem(lsKey, url); }catch{}
        applyWallpaperVar(cssVar, url);
      }
    });
  }catch(_){}
}
// existing watchers already handle main/auth elsewhere; add extra scoped ones
watchWallpaper('chat','--wallChat','wall_chat');
watchWallpaper('dm','--wallDM','wall_dm');
watchWallpaper('profile','--wallProfile','wall_profile');
...
$('#dmSend')?.addEventListener('click', async ()=>{
  ...
  try{
    ...
    // 5) notify peer (for bell + dm badge)
    try{
      const preview = text ? text.slice(0,80) : (img ? 'Foto' : 'DM');
      await db.ref('notifications/'+currentDmPeerUid).push({type:'dm', from:me.uid, room, ts:Date.now(), text: preview});
      notify('Nová DM zpráva', preview, 'dm');
    }catch(_){}
...
async function refreshMe(){
  ...
  // badges/roles
  try{
    const a = document.getElementById('myAdmin');
    if(a) a.style.display = window.__isAdmin ? 'inline-flex' : 'none';
    const prem = document.getElementById('myPremium');
    if(prem) prem.style.display = window.__isPremium ? 'inline-flex' : 'none';
    const roleLine = document.getElementById('profileRoleLine');
    if(roleLine){
      const parts=[];
      if(window.__isAdmin) parts.push('ADMIN');
      if(window.__isPremium) parts.push((prem && prem.textContent) ? prem.textContent : 'PREMIUM');
      roleLine.textContent = parts.length ? ('Role: ' + parts.join(' · ')) : 'Role: user';
    }
  }catch(_){}
}
...
// --- Admin role watcher (RTDB) ---
// Admin is true if ANY of these passes:
// 1) /settings/admins/{uid} === true
// 2) /roles/{uid}/admin === true
// 3) uid/email is in OWNER allowlist (bootstrap)
let _roleRef = null;
let _adminRef = null;
function setAdminUi(on){
  ...
  try{
    ...
    // hard-hide by default, show only for admin
    ...
    const chatWrap = document.getElementById('chatWallCamWrap');
    if(chatWrap) chatWrap.style.display = window.__isAdmin ? '' : 'none';
    const dmWrap = document.getElementById('dmWallCamWrap');
    if(dmWrap) dmWrap.style.display = window.__isAdmin ? '' : 'none';
    const profWrap = document.getElementById('profileWallCamWrap');
    if(profWrap) profWrap.style.display = window.__isAdmin ? '' : 'none';
    const payCard = document.getElementById('adminPayments');
    if(payCard) payCard.style.display = window.__isAdmin ? '' : 'none';
  }catch{}
}
function watchAdminRole(user){
  setAdminUi(false);
  if(_roleRef){ try{ _roleRef.off(); }catch{} _roleRef=null; }
  if(_adminRef){ try{ _adminRef.off(); }catch{} _adminRef=null; }
  if(!user) return;

  // bootstrap by known owner
  const byOwner = (user.email && user.email.toLowerCase()===OWNER_EMAIL) || OWNER_UIDS.has(user.uid);
  if(byOwner) setAdminUi(true);

  // live checks from DB (either can enable admin)
  const recompute = ()=>{
    const on = byOwner || (_roleVal===true) || (_adminVal===true);
    setAdminUi(on);
  };
  let _roleVal=false, _adminVal=false;

  _roleRef = db.ref('roles/'+user.uid+'/admin');
  _roleRef.on('value', snap=>{ _roleVal=(snap.val()===true); recompute(); });

  _adminRef = db.ref('settings/admins/'+user.uid);
  _adminRef.on('value', snap=>{ _adminVal=(snap.val()===true); recompute(); });
}
...
auth.onAuthStateChanged(async (u)=>{
  ...
  // premium subscription watcher
  try{
    if(_subRef){ _subRef.off(); _subRef=null; }
    setPremiumUi(false);
    if(u){
      _subRef = db.ref('subscriptions/'+u.uid);
      _subRef.on('value', (snap)=>{
        const v=snap.val()||{};
        const until = v.until||0;
        const active = (typeof until==='number' && until> Date.now());
        setPremiumUi(active, v.plan||'premium');
      });
    }
  }catch(_){}
...
(function(){
  ...
  function listenNotifications(){
    ...
    NOTIF_REF.on('value', async snap=>{
      ...
      // dm badge (top icon) + friends top badge
      try{
        const dmCount = items.filter(x=>x && x.type==='dm').length;
        const dmB = document.getElementById('dmBadge');
        if(dmB){
          dmB.textContent = dmCount ? String(dmCount) : '';
          dmB.style.display = dmCount ? 'inline-flex' : 'none';
        }
        const frCount = items.filter(x=>x && x.type==='friend').length;
        const frB = document.getElementById('friendsTopBadge');
        if(frB){
          frB.textContent = frCount ? String(frCount) : '';
          frB.style.display = frCount ? 'inline-flex' : 'none';
        }
      }catch(_){}
