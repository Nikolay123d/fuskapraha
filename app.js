...
// Default avatar
window.DEFAULT_AVATAR = "default-avatar.svg";
...
async function loadChat(){
  ...
  const cb=async (snap)=>{
    ...
    d.innerHTML = `<div class="ava click-profile" data-uid="${esc(m.by)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}"></div>`+
...
async function renderDM(room){
  ...
  ref.on('child_added', async snap=>{
    ...
    el.innerHTML = `<div class="ava click-profile" data-uid="${esc(m.by)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}"></div>`+
...
async function friendItem(uid, st){
  ...
  wrap.innerHTML = `<div class="ava click-profile" data-uid="${esc(m.by)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}"></div>`+
                   `<div class="bubble"><div class="name" data-uid="${esc(m.by)}">${esc(u.nick||'Uživatel')}</div>`+
...
auth.onAuthStateChanged(async (u)=>{
  ...
  document.querySelectorAll(".adm-only").forEach(x=> x.style.display = (u && isAdmin(u)) ? "" : "none");
...
(function(){
  const db = firebase.database();
window.db = db; const auth = firebase.auth();
window.auth = auth;
  ...
  async function renderFriendCard(uid, st){
    ...
    const wrap=document.createElement('div'); wrap.className='msg';
    wrap.innerHTML = `
      <div class="ava click-profile" data-uid="${uid}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}" alt=""></div>
  ...
  async function loadDmThreads(){
    ...
    for(const room of rooms){
      ...
      row.className='msg';
      row.innerHTML = `
        <div class="ava click-profile" data-uid="${other}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}" alt=""></div>
...
// ===== UI: Drawer, Profile modal, Friends, Notifications =====
(function(){
  ...
  document.addEventListener("click",(e)=>{
    const it = e.target.closest(".drawer-item");
    if(!it) return;
    ...
    document.querySelector(map[go]||"")?.click();
  });
  ...
  document.addEventListener("click",(e)=>{
    if(e.target.closest('[data-close="profile"]')) hideProfile();
  });
  ...
  async function openProfile(uid){
    ...
    try{
      if(!uid) return;
      const me = auth.currentUser;
      if(!me){ toast('Přihlašte se'); return; }

      const u = await window.getUser(uid);
      if(!u){ toast('Profil nenalezen'); return; }

      // UI refs
      const modal = $('#profileModal');
      const avaEl = $('#pmAvatar');
      const nameEl = $('#pmName');
      const metaEl = $('#pmMeta');
      const infoEl = $('#pmInfo');
      const btnAdd = $('#pmAddFriend');
      const btnMsg = $('#pmMessage');
      const btnBlock = $('#pmBlock');
      const btnReport = $('#pmReport');

      const isSelf = me.uid === uid;

      // Fill basic data
      avaEl.src = u.avatar || 'assets/avatar-default.png';
      nameEl.textContent = u.nick || 'Uživatel';
      const city = u.city || (u.room ? (u.room.charAt(0).toUpperCase()+u.room.slice(1)) : '—');
      const role = u.role || '—';
      metaEl.textContent = `Město: ${city} • Role: ${role}`;

      const about = (u.about || '').trim();
      const lastSeen = u.lastSeen ? new Date(u.lastSeen).toLocaleString() : '';
      infoEl.innerHTML = `
        <div class="pmInfoGrid">
          <div><b>Nick:</b> ${esc(u.nick || '—')}</div>
          <div><b>Město:</b> ${esc(city)}</div>
          <div><b>Role:</b> ${esc(role)}</div>
          ${lastSeen ? `<div><b>Naposledy online:</b> ${esc(lastSeen)}</div>` : ``}
          ${about ? `<div class="pmInfoAbout"><b>O mně:</b><br>${esc(about)}</div>` : `<div class="pmInfoAbout muted">Bez popisu</div>`}
        </div>
      `;

      // hide self-actions
      btnAdd.style.display = isSelf ? 'none' : '';
      btnMsg.style.display = isSelf ? 'none' : '';
      btnBlock.style.display = isSelf ? 'none' : '';
      btnReport.style.display = isSelf ? 'none' : '';
      ...
      // --- Friend status / button state
      let friendStatus = null;
      try{
        const fs = await db.ref('friends/'+me.uid+'/'+uid).once('value');
        friendStatus = fs.val();
      }catch(e){}

      if(friendStatus === 'accepted'){
        btnAdd.textContent = 'Odebrat z přátel';
        btnAdd.classList.add('btn-ghost');
      }else if(friendStatus === 'pending'){
        btnAdd.textContent = 'Žádost odeslána';
        btnAdd.disabled = true;
      }else{
        btnAdd.textContent = 'Přidat do přátel';
        btnAdd.classList.remove('btn-ghost');
      }

      // --- Block state
      let blocked = false;
      try{
        const bs = await db.ref('blocks/'+me.uid+'/'+uid).once('value');
        blocked = !!bs.val();
      }catch(e){}
      btnBlock.textContent = blocked ? 'Odblokovat' : 'Zablokovat';

      // Handlers (use onclick to avoid duplicates)
      btnAdd.onclick = async ()=>{
        if(isSelf) return;
        const pathA = 'friends/'+me.uid+'/'+uid;
        const pathB = 'friends/'+uid+'/'+me.uid;

        if(friendStatus === 'accepted'){
          await db.ref(pathA).remove();
          await db.ref(pathB).remove();
          toast('Odebráno z přátel');
        ...
        const ts = Date.now();
        await db.ref(pathA).set('pending');
        await db.ref('friendRequests/'+uid+'/'+me.uid).set({from: me.uid, ts});
        toast('Žádost odeslána');
        btnAdd.textContent = 'Žádost odeslána';
        btnAdd.disabled = true;
      ...
      btnBlock.onclick = async ()=>{
        if(isSelf) return;
        const p = db.ref('blocks/'+me.uid+'/'+uid);
        if(blocked){
          await p.remove();
          blocked = false;
          btnBlock.textContent = 'Zablokovat';
          toast('Odblokováno');
        }else{
          await p.set(true);
          blocked = true;
          btnBlock.textContent = 'Odblokovat';
          toast('Zablokováno');
        }
      };
      ...
      btnReport.onclick = async ()=>{
        if(isSelf) return;
        const reason = (prompt('Co přesně je problém? (stručně)') || '').trim();
        if(!reason){ toast('Zrušeno'); return; }
        const item = { reporter: me.uid, target: uid, reason, ts: Date.now() };
        await db.ref('reports').push(item);
        toast('Nahlášení odesláno');
      };
...
// --- Hotfix: open user profile from chat by clicking avatar/name ---
(function(){
  const feed = document.getElementById('chatFeed');
  if(!feed) return;
  feed.addEventListener('click', (ev)=>{
    const ava = ev.target.closest('.ava,[data-uid].click-profile,.click-profile');
    if(!ava) return;
    const uid = ava.getAttribute('data-uid') || ava.dataset.uid;
  ...
  // Click any element with data-uid to open profile
  document.addEventListener("click",(e)=>{
  ...
  async function ensurePrivateMember(room, uid){
    const r = window.db.ref("privateMembers/"+room+"/"+uid);
    const s = await r.get();
    if(!s.exists()) await r.set(true);
  }

  pmAddFriend?.addEventListener("click", async ()=>{
    try{
      const me = window.__me;
      if(!me?.uid || !pmUid || me.uid===pmUid) return;
      await window.db.ref("friendRequests/"+pmUid+"/"+me.uid).set({
        from: me.uid,
        ts: Date.now(),
        nick: me.nick || "",
        avatar: me.avatar || ""
      });
      window.toast("Žádost odeslána.");
      pmAddFriend.textContent = "Odesláno";
      pmAddFriend.disabled = true;
      refreshBell();
    }catch(err){
      console.error(err);
      window.toast("Chyba odeslání žádosti.");
    }
  });

  pmMessage?.addEventListener("click", async ()=>{
    try{
      const me = window.__me;
      if(!me?.uid || !pmUid || me.uid===pmUid) return;
      const room = [me.uid, pmUid].sort().join("_");
      await ensurePrivateMember(room, me.uid);
      await ensurePrivateMember(room, pmUid);
      document.querySelector("#tab-dm")?.click();
      // If app has a DM opener, use it
      if(typeof window.openDMRoom === "function"){
        window.openDMRoom(room, pmUid);
      } else {
        // best-effort: store and let DM view load
        window.__activeDMRoom = room;
        window.__activeDMOther = pmUid;
        if(typeof window.loadDMRoom === "function") window.loadDMRoom(room);
      }
      hideProfile();
    }catch(err){
      console.error(err);
      window.toast("Chyba otevření DM.");
    }
  });
  ...
  async function refreshBell(){
    try{
      const me = window.__me;
      if(!me?.uid || !bellBadge) return;
      const snap = await window.db.ref("friendRequests/"+me.uid).get();
      const n = snap.exists() ? Object.keys(snap.val()||{}).length : 0;
      bellBadge.textContent = String(n);
      bellBadge.classList.toggle("hidden", n===0);
    }catch(e){}
  }
  window.refreshBell = refreshBell;

  btnBell?.addEventListener("click", ()=>document.querySelector("#tab-friends")?.click());
  btnMe?.addEventListener("click", ()=>document.querySelector("#tab-profile")?.click());

  // Patch avatar in header
  function syncMeAvatar(){
    const me = window.__me;
    if(meAvatar) meAvatar.src = me?.avatar || "default-avatar.svg";
  }

  // Observe auth / profile changes from app (it already sets state via setMe())
  const origSetMe = window.setMe;
  if(typeof origSetMe === "function"){
    window.setMe = function(me){
      window.__me = me;
      const r = origSetMe(me);
      syncMeAvatar();
      refreshBell();
      return r;
    }
    ...
    // Fallback poll
    setInterval(()=>{syncMeAvatar(); refreshBell();}, 8000);
  }
