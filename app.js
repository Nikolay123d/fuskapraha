...
const PREMIUM_PLANS = {
  ...
  premium: { code:'premium', title:'Premium', price:150, period:'měsíc', desc:'Premium: speciální design profilu, možnost vytvářet inzeráty, bot s omezenými možnostmi (bez avatara), interval 30 min.' },
...
async function ensureNotifications(){
  ...
  if(localStorage.getItem('cookiesAccepted')!=='1') return;
...
// Cookie banner (sounds + notifications only after consent)
function cookieBanner(){
  const b = $('#cookieBanner');
  if(!b) return;

  const ok = (localStorage.getItem('cookiesAccepted')==='1' || localStorage.getItem('cookiesAccepted')==='1');
  ...
  $('#cookieAcceptAll')?.addEventListener('click', async ()=>{
    ...
    localStorage.setItem('cookieAccepted','1'); // backward compat
    try{ document.cookie = 'cookiesAccepted=1; path=/; max-age=31536000'; }catch(_){}
    b.style.display='none';
    // After deliberate click: unlock sounds and ask notif permission
    try{ await unlockAudio(); }catch(_){}
    try{ await ensureNotifications(); }catch(_){}
  }, {once:true});
...
async function ensureMyPublic(u){
  ...
  // --- Room wallpaper (per city) ---
  if(t && t.id==='roomWallCamera'){
    if(!window.__isAdmin){ toast('Pouze administrátor může měnit pozadí místnosti.'); t.value=''; return; }
    const f=t.files && t.files[0]; if(!f) return;
    const city=getCity();
    const r=new FileReader();
    r.onload=()=>{
      const data=r.result;
      applyCityWallpaper(city, data);
      try{ localStorage.setItem('wall_room_'+city, data); }catch(_){}
      db.ref('settings/wallpapers/rooms/'+city).set({url:data, ts:Date.now(), by:(auth.currentUser?auth.currentUser.uid:null)});
      toast('Pozadí místnosti uloženo.');
    };
    r.readAsDataURL(f);
  }

  // --- DM wallpaper (per roomId) ---
  if(t && t.id==='dmWallCamera'){
    if(!window.__isAdmin){ toast('Pouze administrátor může měnit pozadí DM.'); t.value=''; return; }
    const f=t.files && t.files[0]; if(!f) return;
    if(!currentDmRoom){ toast('Nejdřív otevřete konverzaci.'); t.value=''; return; }
    const r=new FileReader();
    r.onload=()=>{
      const data=r.result;
      applyDmWallpaper(currentDmRoom, data);
      try{ localStorage.setItem('wall_dm_'+currentDmRoom, data); }catch(_){}
      db.ref('settings/wallpapers/dm/'+currentDmRoom).set({url:data, ts:Date.now(), by:(auth.currentUser?auth.currentUser.uid:null)});
      toast('Pozadí DM uloženo.');
    };
    r.readAsDataURL(f);
  }

  // --- User profile wallpaper (when viewing other profile) ---
  if(t && t.id==='userWallCamera'){
    if(!window.__isAdmin){ toast('Pouze administrátor může měnit pozadí profilu.'); t.value=''; return; }
    const f=t.files && t.files[0]; if(!f) return;
    if(!currentUserCardUid){ toast('Otevřete profil uživatele.'); t.value=''; return; }
    const r=new FileReader();
    r.onload=()=>{
      const data=r.result;
      applyUserWallpaper(currentUserCardUid, data);
      try{ localStorage.setItem('wall_user_'+currentUserCardUid, data); }catch(_){}
      db.ref('settings/wallpapers/users/'+currentUserCardUid).set({url:data, ts:Date.now(), by:(auth.currentUser?auth.currentUser.uid:null)});
      toast('Pozadí profilu uloženo.');
    };
    r.readAsDataURL(f);
  }
...
// --- Per-city (room) wallpaper sync/apply ---
function applyCityWallpaper(city, url){
  if(typeof url==='string' && url){
    document.documentElement.style.setProperty('--wall', `url('${url}')`);
  }else{
    // fallback main
    const main = localStorage.getItem('wall_main');
    if(main) document.documentElement.style.setProperty('--wall', `url('${main}')`);
  }
}
function startCityWallpaperSync(city){
  try{
    if(_roomWallUnsub){ _roomWallUnsub(); _roomWallUnsub=null; }
    const ref=db.ref('settings/wallpapers/rooms/'+city);
    const handler=(snap)=>{
      const val=snap.val();
      const url=(typeof val==='string')?val:(val && val.url);
      if(typeof url==='string' && url){
        try{ localStorage.setItem('wall_room_'+city, url); }catch{}
        applyCityWallpaper(city, url);
      }else{
        const cached=localStorage.getItem('wall_room_'+city);
        if(cached) applyCityWallpaper(city, cached);
      }
    };
    ref.on('value', handler);
    _roomWallUnsub=()=>ref.off('value', handler);
  }catch(e){}
}

// --- DM wallpaper sync/apply ---
function applyDmWallpaper(roomId, url){
  if(typeof url==='string' && url){
    document.documentElement.style.setProperty('--wall', `url('${url}')`);
  }else{
    const main = localStorage.getItem('wall_main');
...
function startDmWallpaperSync(roomId){
  try{
    if(_dmWallUnsub){ _dmWallUnsub(); _dmWallUnsub=null; }
    const ref=db.ref('settings/wallpapers/dm/'+roomId);
    const handler=(snap)=>{
      const val=snap.val();
      const url=(typeof val==='string')?val:(val && val.url);
      if(typeof url==='string' && url){
        try{ localStorage.setItem('wall_dm_'+roomId, url); }catch{}
        applyDmWallpaper(roomId, url);
      }else{
        const cached=localStorage.getItem('wall_dm_'+roomId);
        if(cached) applyDmWallpaper(roomId, cached);
      }
    };
    ref.on('value', handler);
...
// --- User card wallpaper (modal only) ---
function applyUserWallpaper(uid, url){
  try{
    const card=document.querySelector('#modalUserCard .usercard');
    if(!card) return;
    document.documentElement.style.setProperty('--userwall', url ? `url('${url}')` : 'none');
  }catch(e){}
}
function startUserWallpaperSync(uid){
  try{
    if(_userWallUnsub){ _userWallUnsub(); _userWallUnsub=null; }
    const ref=db.ref('settings/wallpapers/users/'+uid);
    const handler=(snap)=>{
      const val=snap.val();
      const url=(typeof val==='string')?val:(val && val.url);
      if(typeof url==='string' && url){
        try{ localStorage.setItem('wall_user_'+uid, url); }catch{}
        applyUserWallpaper(uid, url);
      }else{
        const cached=localStorage.getItem('wall_user_'+uid);
        if(cached) applyUserWallpaper(uid, cached);
        else applyUserWallpaper(uid, null);
      }
    };
    ref.on('value', handler);
    _userWallUnsub=()=>ref.off('value', handler);
  }catch(e){}
}
...
function loadPoi(){
  ...
  POI_REF.on('child_added', async snap=>{
    ...
    m.bindPopup(`<b>${esc(v.title||'Bod')}</b><br>${esc(v.type||'')}` + (v.desc?`<br>${esc(v.desc)}`:'') + (v.rating?`<br>⭐ ${esc(String(v.rating))}`:'') + (v.img?`<br><img src="${esc(v.img)}" style="max-width:240px;border-radius:12px">`:'') );
  });
  ...
  MAP.on('click', async (e)=>{
    ...
    const ratingStr=prompt('Rating 1-5 (číslo):','')||'';
    const rating=parseFloat(ratingStr)||0;
    const img=prompt('URL foto (nepovinné):','')||'';
    await db.ref('map/poi/'+city).push({lat:e.latlng.lat, lng:e.latlng.lng, title, type, desc, rating, img, by:u.uid, ts:Date.now()});
...
// --- DM threads / inbox ---
  ...
  window.openDMRoom = async function(a,b){
    ...
    try{ startDmWallpaperSync(currentDmRoom); const cached=localStorage.getItem('wall_dm_'+currentDmRoom); if(cached) applyDmWallpaper(currentDmRoom, cached); }catch(e){}
