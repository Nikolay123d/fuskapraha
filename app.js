...
async function loadChat(){
  ...
  const city=getCity();
  const ref=db.ref('chat/main').orderByChild('at').limitToLast(50);
  const cb=async (snap)=>{
    const m=snap.val()||{};
    const senderUid = m.from || m.by;
    if(m.deleted) return;
    const u=await getUser(senderUid);
    ...
    const name = `<span class="nick" data-uid="${esc(senderUid)}">${esc(u?.nick||u?.name||'Uživatel')}</span>`+
                 badges +
                 (u?.online?'<span class="online"></span>':'');
    d.innerHTML = `<div class="ava" data-uid="${esc(senderUid)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}"></div>`+
                  `<div class="bubble"><div class="name">${name}</div>`+
                  (m.text? `<div class="text">${esc(m.text)}</div>`:'' )+
                  (m.img||m.photo||m.image? `<div class="text"><img src="${esc(m.img||m.photo||m.image)}"></div>`:'')+
                  `</div>`;
    feed.appendChild(d);
    playSound('chat');
...
$('#sendBtn')?.addEventListener('click', async ()=>{
  try{
    ...
    const ref = db.ref('chat/main').push();
    const mePub = await getUser(u.uid);
    const nick = (mePub && (mePub.nick||mePub.name)) || u.displayName || 'Uživatel';
    const payload = { at: Date.now(), from: u.uid, id: ref.key, nick: nick, text: text||'', image: img||'' };
...
// --- DM threads / inbox ---
  ...
  async function botTick(){
    ...
    for(const [id,b] of Object.entries(v)){
      ...
      const ref = db.ref('chat/main').push();
      await ref.set({ at: now, from: botUid, id: ref.key, nick: (b.nick||'BOT'), text: b.text||'', image: b.img||'' });
