...
// Wallpaper helpers
function currentCity(){ return getCity(); }
function applyWallpaper(city, url){
  try{
    if(url){
      document.documentElement.style.setProperty('--wall', `url('${url}')`);
      localStorage.setItem('wall_'+city, url);
    }
  }catch{}
}
...
async function enforceVerifyWindow(u){ /* disabled hard-block */ }
...
auth.onAuthStateChanged(async (u)=>{
  ...
  $$(".adm-only").forEach(x=> x.style.display = (u && isAdmin(u)) ? "" : "nonee");
  window.__isAdmin = !!(u && isAdmin(u));
  const wallWrap = document.getElementById("wallCamWrap"); if(wallWrap) wallWrap.style.display = window.__isAdmin ? "" : "nonee";
  // profile display
  if(u){
