...
// === config injected ===
// ==== Firebase config
// === GLOBAL MODAL HELPERS (must exist even if other code throws) ===
(function(){
  function _byId(id){ return document.getElementById(id); }
  window.openModal = window.openModal || function(id){
    const el=_byId(id); if(!el) return;
    document.querySelectorAll('.modal').forEach(m=>{ if(m.id!==id) m.hidden=true; });
    el.hidden=false;
  ...
  window.closeModal = window.closeModal || function(id){
    const el=_byId(id); if(!el) return;
    el.hidden=true;
    // keep modal-open if some other modal is visible
...
(function(){
  ...
  $('#citySelect')?.addEventListener('change', ()=>{ setCity($('#citySelect').value); apply(); startWallpaperSync(); loadChat(); loadRent(); try{ loadFriends(); }catch(e){ console.error(e); } loadPoi(); });
...
document.getElementById('dmOpen')?.addEventListener('click', async ()=>{
  ...
  let to=(dmToEl?dmToEl.value:'').trim();
  if(!to) return;
  if(to.includes('@')){
...
async function openDMRoom(a,b){
  ...
  try{
    const me = auth.currentUser;
    if(me) currentDmPeerUid = (me.uid===a)? b : a;
  }catch(_){ /* noop */ }
...
$('#dmOpen')?.addEventListener('click', async ()=>{
  ...
  const dmToEl=document.getElementById('dmTo'); let to=(dmToEl?dmToEl.value:'').trim(); if(!to) return toast('Zadejte UID nebo e-mail (Nová DM)');
...
$('#dmSend')?.addEventListener('click', async ()=>{
  try{
    ...
    // Resolve current room/peer (dmTo exists only in "Nová DM" modal)
    let room = currentDmRoom;
    let to = currentDmPeerUid;

    // Fallback: if user is sending directly from modal, take value from dmTo
    const dmToEl = document.getElementById('dmTo');
    if((!to || !room) && dmToEl){
      let raw = (dmToEl.value || '').trim();
      if(!raw) return toast('Nejdřív otevřete konverzaci');
      if(raw.includes('@')){
        const uid = await resolveUidByEmail(raw);
        if(!uid) return alert('Email neznám');
        raw = uid;
        dmToEl.value = uid;
      }
      to = raw;
      room = dmKey(me.uid, to);
      currentDmRoom = room;
      currentDmPeerUid = to;
    }

    if(!room || !to){
      alert('Nejprve otevřete konverzaci');
      return;
    }
...
async function friendItem(uid, st){
  ...
  wrap.innerHTML =
    `<div class="ava" data-uid="${esc(uid)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}" alt=""></div>` +
    `<div class="bubble">` +
      `<div class="name" data-uid="${esc(uid)}">${esc(u.nick||u.name||'Uživatel')}</div>` +
      `<div class="muted">${esc(st||'')}</div>` +
      `<div class="actions">` +
        `<button data-act="chat">Napsat</button>` +
        (st==='pending' ? `<button data-act="accept">Přijmout</button>` : ``) +
        `<button data-act="remove">Odebrat</button>` +
      `</div>` +
    `</div>`;
  ...
  wrap.addEventListener('click', async (e)=>{
    ...
    if(a==='accept'){
      ...
      try{ await try{ loadFriends(); }catch(e){ console.error(e); } }catch(e){ console.error(e); }
      return;
    }
    if(a==='remove'){
...
// profile display
  if(u){
    ...
    await try{ loadFriends(); }catch(e){ console.error(e); }
