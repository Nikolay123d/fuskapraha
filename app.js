...
$('#sendBtn')?.addEventListener('click', async ()=>{
  try{
    ...
    console.error(e);
    toast('Chyba odeslání (rules/online).');
    playSound('err');
  }
    // cleanup
    $('#msgText').value='';
...
// === Cookies / permissions + legal modals ===
function cookiesAccepted(){ return localStorage.getItem('cookiesAccepted') === '1'; }
function showCookieBanner(){ const b=document.getElementById('cookieBanner'); if(b) b.classList.remove('hidden'); }
function hideCookieBanner(){ const b=document.getElementById('cookieBanner'); if(b) b.classList.add('hidden'); }

function unlockAudio(){
  try{
    // play a silent sound to unlock on mobile
    const a = window.__SND_UNLOCK || (window.__SND_UNLOCK = new Audio());
    a.muted = true;
    a.src = (document.querySelector('audio[data-unlock]')?.src) || '';
    a.play().catch(()=>{});
  }catch(e){}
}
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  document.getElementById('copyContactEmail')?.addEventListener('click', async ()=>{
    const t = document.getElementById('contactEmail')?.textContent || '';
    try{
