...
(function(){
  ...
  function subtabActivate(containerPrefix, key){
    document.querySelectorAll(`#${containerPrefix} .subtab`).forEach(x=>x.classList.toggle('active', x.dataset.sub===key));
    document.querySelectorAll(`#${containerPrefix} .subpage`).forEach(x=>x.classList.remove('visible'));
    const map = {login:"#authLogin", register:"#authRegister", reset:"#authReset"};
    (map[key] && document.querySelector(map[key]).classList.add('visible'));
  }
  $$('#authDlg .subtab').forEach(b=> b.onclick=()=> subtabActivate('authDlg', b.dataset.sub));
  $("#authClose").onclick=()=> authDlg.close();
  authDlg.addEventListener('cancel', (e)=>{ /* ESC close */ });

  $("#btnAuthOpen").onclick=()=>{ subtabActivate('authDlg','login'); authDlg.showModal(); };
  $("#btnJobsOpen").onclick=()=>{ $("#jobsCity").textContent=currentCity; $("#jobsDlg").showModal(); };

  // Login methods
  $("#btnGoogleLogin").onclick=async(e)=>{ e.preventDefault(); const p=new firebase.auth.GoogleAuthProvider(); try{await auth.signInWithPopup(p);}catch(err){await auth.signInWithRedirect(p);} finally{ authDlg.close(); } };
  $("#btnEmailLogin").onclick=async(e)=>{ e.preventDefault(); const email=$("#loginEmail").value.trim(), pass=$("#loginPass").value; if(!email||!pass) return toast('Заповніть email/пароль'); try{ await auth.signInWithEmailAndPassword(email,pass); authDlg.close(); }catch(err){ toast(err.message);} };
  $("#btnDoReset").onclick=async(e)=>{ e.preventDefault(); const email=$("#resetEmail").value.trim(); if(!email) return toast('Email?'); try{ await auth.sendPasswordResetEmail(email); toast('Посилання надіслано'); }catch(err){ toast(err.message);} };
  ...
  $("#btnDoRegister").onclick=async(e)=>{
    ...
    if(!role||!locale||!nick||!email||!pass){ toast('Заповніть всі поля'); return; }
    try{
  ...
  function updateRoleUI(){
    ...
    $("#btnAuthOpen").style.display = (auth.currentUser && !auth.currentUser.isAnonymous) ? 'none' : '';
  ...
  const gate=()=>{ const u=auth.currentUser; if(!u) return true; if(blockedByVerification(u)){ $("#verifyDlg").showModal(); return true; } return false; };
  ...
  function msgElement(v){
    ...
    const avatar = (v.avatarUrl || v.photoAvatar || v.photoURL);
    const el=document.createElement('div'); el.className='msg';
    const nick = v.nick || v.email || (uid?('user-'+uid.slice(0,6)):'Гість');
    const txt = (v.translations && my.locale && v.translations[my.locale]) ? v.translations[my.locale] : (v.text||'');
  ...
  $("#chatSend").onclick=async()=>{
    ...
    if(blockedByVerification(u)){ $("#verifyDlg").showModal(); return; }
    const url=await upFile($("#chatPhoto")); const input=$("#chatInput");
    if(!input.value.trim() && !url) return;
    await db.ref('messages/'+currentCity).push({uid:u.uid||null,email:u.email||null,nick: (u.displayName||'Користувач'), avatarUrl: (u.photoURL||null), text:input.value||null, photoUrl:url||null, ts:firebase.database.ServerValue.TIMESTAMP});
    input.value=''; $("#chatPhoto").value=''; $("#sndSend").play();
  };
  $("#rentSend").onclick=async()=>{
    const u=auth.currentUser; if(!u){ $("#authDlg").showModal(); return; }
    if(blockedByVerification(u)){ $("#verifyDlg").showModal(); return; }
    const url=await upFile($("#rentPhoto")); const input=$("#rentInput");
    if(!input.value.trim() && !url) return;
    await db.ref('rentMessages/'+currentCity).push({uid:u.uid||null,email:u.email||null,nick:(u.displayName||'Користувач'), avatarUrl:(u.photoURL||null), text:input.value||null, photoUrl:url||null, ts:firebase.database.ServerValue.TIMESTAMP});
    input.value=''; $("#rentPhoto").value=''; $("#sndSend").play();
  };
  ...
  $("#jobAddBtn").onclick=()=>{ const u=auth.currentUser; if(!u){ $("#authDlg").showModal(); return; } if(!(isAdmin || (u && u.uid && (my.role==='employer')))) return alert('Лише адмін або роботодавець'); $("#jobNewDlg").showModal(); };
  $("#jobAddBtn2").onclick=()=>{ const u=auth.currentUser; if(!u){ $("#authDlg").showModal(); return; } $("#jobNewDlg").showModal(); };
  ...
  function loadMembers(){
    ...
    const redraw = async (upsSnap)=>{
      list.innerHTML='';
      const plans = (await db.ref('plans').get()).val()||{};
      const status = (await db.ref('status').get()).val()||{};
      const arr=[]; upsSnap.forEach(ch=>arr.push({uid:ch.key, ...ch.val()}));
      arr.forEach(u=>{ u.plan=(plans[u.uid]?.name)||'Free'; u.status=(status[u.uid]?.state)||'offline'; });
    ...
    db.ref('usersPublic').off();
    db.ref('usersPublic').on('value', redraw);
  }
  ...
  function profSubtab(key){
    $$('#profileDlg .subtab').forEach(x=>x.classList.toggle('active', x.dataset.sub===key));
    $$('#profileDlg .subpage').forEach(x=>x.classList.remove('visible'));
  ...
  $$('#profileDlg .subtab').forEach(b=> b.onclick=()=> profSubtab(b.dataset.sub));
  ...
  async function openProfile(uid){
    ...
    if(me && me!==uid){
      ...
      $("#dmSend").onclick = async ()=>{
        ...
        const data={uid:me, text:$("#dmInput").value||null, photoUrl:f, ts:firebase.database.ServerValue.TIMESTAMP, avatarUrl: u.photoURL||null, nick: u.displayName||'Користувач'};
    ...
    $("#btnProfAdd").onclick = async()=>{
      const meUser=auth.currentUser; if(!meUser){ $("#authDlg").showModal(); return; }
      if(meUser.isAnonymous){ toast("Потрібно увійти, гість не може додавати друзів"); $("#authDlg").showModal(); return; }
      await db.ref("friendRequests/"+uid+"/"+meUser.uid).set({ts:firebase.database.ServerValue.TIMESTAMP});
  ...
  $("#actionPremiumSelected").onclick=async()=>{
    ...
    for(const el of list){
      const nameEl = el.querySelector('[data-act="profile"]'); if(!nameEl) continue;
      const name = nameEl.textContent;
      const uidSnap = await db.ref('usersPublic').orderByChild('nick').equalTo(name).limitToFirst(1).get();
      uidSnap.forEach(s=> ids.append if False)
