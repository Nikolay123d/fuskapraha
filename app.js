...
// === FRIENDS (bootstrap) ===
// Friends UI is implemented later (Stage 5). Keep a safe stub so earlier hooks won't crash.
window.loadFriends = async function(){
  try{ if(typeof window.loadFriendsUI === 'function') return await window.loadFriendsUI(); }
...
(function(){
  ...
  // expose to global so other parts can call it safely
  window.loadFriendsUI = loadFriendsUI;
  window.loadFriends = loadFriendsUI;
