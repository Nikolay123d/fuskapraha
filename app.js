...
function showView(id){
  ...
  try{
    ...
    if(id==='view-admin') setMiniLoad('adminUsersMiniLoad','Načítáme…', true);
  }catch(e){}
...
async function googleSignIn(){
  ...
  const isMobile = /Android|iPhone|iPad|iPod/i.test(ua) || (window.matchMedia && window.matchMedia('(max-width: 560px)').matches);
  const btn = document.getElementById('authGoogleBtn');
  try{ if(btn){ btn.disabled=true; btn.classList.add('loading'); } }catch(e){}
  try{
    if(isMobile){
    ...
    // If redirect is used, this page will reload; keeping disabled is fine.
    try{ if(btn && !isMobile){ btn.disabled=false; btn.classList.remove('loading'); } }catch(e){}
  }
}
...
async function doPasswordReset(){
  ...
  const email = (document.getElementById('authEmail') && document.getElementById('authEmail').value ? document.getElementById('authEmail').value.trim() : '') || (auth.currentUser && auth.currentUser.email) || (prompt('E-mail pro obnovu:')||'').trim();
  if(!email) return;
  try{
...
async function loadChat(){
  ...
  const __markChatReady = ()=>{
    ...
    // Do NOT block first paint; run only after user has seen the chat.
    if(!window.__prefetchScheduled){
      window.__prefetchScheduled = true;
      const run = ()=>{ try{ if(typeof __startPrefetchStages==='function') __startPrefetchStages(); }catch(e){} };
      if('requestIdleCallback' in window){
        try{ window.requestIdleCallback(()=>setTimeout(run, 1200)); }catch(e){ setTimeout(run, 2500); }
      }else{
        setTimeout(run, 2500);
      }
    }
  ...
  wrap.addEventListener('click', async (e)=>{
    ...
    try{
      if(a==='accept'){
        ...
        /* notif to receiver disabled (avoid permission_denied); receiver sees requests via friendRequests */
    }
  };
  ref.on('value', cb);
...
async function refreshMe(){
  ...
  // Lock nick/role after first set
  try{
    const nickEl=$('#setNick');
    const roleEl=$('#setRole');
    if(nickEl){ nickEl.placeholder = me.nick ? ('Nick: '+me.nick+' (změna přes žádost)') : 'Zadejte nick'; }
    if(roleEl){
      if(me.role){ roleEl.value = me.role; roleEl.disabled = true; }
      else{ roleEl.disabled = false; }
    }
    if(nickEl){ nickEl.disabled = !!me.nick; }
  }catch(e){}
...
$('#saveProfile')?.addEventListener('click', async ()=>{
  ...
  const cur = await fetchUserPublic(u.uid);
  ...
  // Nick/Role: first set is direct, later only via request
  const n=($('#setNick')?.value||'').trim();
  const r=$('#setRole')?.value||'';

  const wantsNick = n && n !== (cur.nick||'');
  const wantsRole = r && r !== (cur.role||'');

  const canSetNick = !cur.nick;
  const canSetRole = !cur.role;

  if(wantsNick && canSetNick) up.nick = n;
  if(wantsRole && canSetRole) up.role = r;

  if(Object.keys(up).length){
    await db.ref('usersPublic/'+u.uid).update(up);
  }

  // If user tries to change locked fields -> create request
  if((wantsNick && !canSetNick) || (wantsRole && !canSetRole)){
    const reqId = 'r_' + Date.now();
    await db.ref('profileChangeRequests/'+u.uid+'/'+reqId).set({
      ts: Date.now(),
      status: 'pending',
      wantNick: (wantsNick && !canSetNick) ? n : null,
      wantRole: (wantsRole && !canSetRole) ? r : null,
      curNick: cur.nick || null,
      curRole: cur.role || null
    });
...
/* =========================
   v17.27: Profile lock + Bell center + User VIP bot + Employer verify (MVP)
   ========================= */
...
// Ensure openPremium exists
if(typeof window.openPremium !== 'function'){
  window.openPremium = ()=>{ try{ openModal('modalPremium'); }catch(e){ console.warn(e); } };
}
if(typeof window.openPremiumBot !== 'function'){
  window.openPremiumBot = openPremiumBot;
}

function __adminEmailOk(){
  const me=auth.currentUser;
  const em=(me && me.email) || (me && me.providerData && me.providerData[0] && me.providerData[0].email) || '';
  return (em==='urciknikolaj642@gmail.com' || em==='urciknikolaj62@gmail.com');
}
function __isAdminNow(){
  const u=auth.currentUser;
  try{ if(u && typeof isAdmin==='function') return isAdmin(u); }catch{}
  return __adminEmailOk();
}
...
function initPremiumModal(){
  ...
  modal.querySelectorAll('.tab.mini').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      modal.querySelectorAll('.tab.mini').forEach(x=>x.classList.remove('active'));
      btn.classList.add('active');
      const key=btn.getAttribute('data-ptab');
      ['plans','bot','verify','admin'].forEach(k=>{
        const el=document.getElementById('pTab'+k.charAt(0).toUpperCase()+k.slice(1));
        if(!el) return;
        if(k===key){ el.hidden=false; el.classList.add('active'); }
        else{ el.hidden=true; el.classList.remove('active'); }
      });
      if(key==='admin'){ loadAdminPremiumPanels(); }
      if(key==='bot'){ loadUserBotPanel(); }
      if(key==='plans'){ loadMyPayments(); }
      if(key==='verify'){ loadMyVerifications(); }
    });
  });

  document.getElementById('openPremiumBot')?.addEventListener('click', ()=>{ try{ closeModal('modalPremium'); }catch{}; openPremiumBot(); });

  // quick buy buttons -> just open plan in selector
  document.getElementById('buyVipWorker')?.addEventListener('click', ()=>{
    document.getElementById('payPlan').value='vip_worker_100';
    toast('Vyberte screenshot platby a odešlete žádost.');
  });
  document.getElementById('buyPremium150')?.addEventListener('click', ()=>{
    document.getElementById('payPlan').value='premium_150';
    toast('Vyberte screenshot platby a odešlete žádost.');
  });
  document.getElementById('buyPremium200')?.addEventListener('click', ()=>{
    document.getElementById('payPlan').value='premium_plus_200';
    toast('Vyberte screenshot platby a odešlete žádost.');
  });

  let pendingPayShot = null;
  document.getElementById('payShot')?.addEventListener('change', async (e)=>{
    const f=e.target.files && e.target.files[0];
    if(!f){ pendingPayShot=null; return; }
    pendingPayShot = await fileToDataURL(f);
    toast('Screenshot připraven. Stiskněte Odeslat.');
  });

  document.getElementById('submitPayment')?.addEventListener('click', async ()=>{
    const me=auth.currentUser;
    if(!me){ openModalAuth('login'); return; }
    if(!pendingPayShot){ toast('Nahrajte screenshot'); return; }
    const plan=document.getElementById('payPlan')?.value || 'vip_worker_100';
    const reqId='p_'+Date.now();
    await db.ref('vipRequests/'+me.uid+'/'+reqId).set({
      ts: Date.now(),
      status: 'pending',
      plan,
      shot: pendingPayShot
    ...
    pendingPayShot=null;
    try{ document.getElementById('payShot').value=''; }catch{}
    toast('Žádost odeslána adminovi');
    loadMyPayments();
  });
  ...
  let pendingDocs = [];
  document.getElementById('verDocs')?.addEventListener('change', async (e)=>{
    const files=[...(e.target.files||[])];
    pendingDocs=[];
    for(const f of files.slice(0,5)){
      // for PDFs store as name marker (no OCR). images as dataURL
      if((f.type||'').includes('pdf')){
        const buf=await f.arrayBuffer();
        // store small PDFs only (MVP): cap 600KB
        if(buf.byteLength>600000){ toast('PDF je příliš velký (max 600KB)'); continue; }
        const b64=btoa(String.fromCharCode(...new Uint8Array(buf)));
        pendingDocs.push({type:'pdf', name:f.name, data:'data:application/pdf;base64,'+b64});
      }else{
        const data=await fileToDataURL(f);
        pendingDocs.push({type:'img', name:f.name, data});
      }
    }
    toast('Dokumenty připraveny ('+pendingDocs.length+')');
  });
  document.getElementById('submitVerify')?.addEventListener('click', async ()=>{
    const me=auth.currentUser;
    if(!me){ openModalAuth('login'); return; }
    if(!pendingDocs.length){ toast('Nahrajte dokumenty'); return; }
    const reqId='v_'+Date.now();
    await db.ref('employerVerifications/'+me.uid+'/'+reqId).set({
      ts: Date.now(),
      status: 'pending',
      docs: pendingDocs
    });
    pendingDocs=[];
    try{ document.getElementById('verDocs').value=''; }catch{}
    toast('Ověření odesláno adminovi');
    loadMyVerifications();
  });

  // user bot panel actions
  document.getElementById('uBotPhoto')?.addEventListener('change', async (e)=>{
    const f=e.target.files && e.target.files[0];
    if(!f) return;
    window.__pendingUserBotPhoto = await fileToDataURL(f);
    toast('Foto bota připraveno');
  });
  document.getElementById('uBotSave')?.addEventListener('click', async ()=>{
    const me=auth.currentUser;
    if(!me){ openModalAuth('login'); return; }
    const plan=(await db.ref('users/'+me.uid+'/plan').get()).val() || '';
    if(!(plan==='premium_150' || plan==='premium_plus_200')){ toast('Potřebujete Premium'); return; }

    const data={
      ts: Date.now(),
      nick: (document.getElementById('uBotNick').value||'').trim() || 'Bot',
      text: (document.getElementById('uBotText').value||'').trim() || '',
      templates: (document.getElementById('uBotTemplates').value||'').split('\n').map(s=>s.trim()).filter(Boolean).slice(0,30),
      city: document.getElementById('uBotCity').value || 'praha',
      intervalMin: Math.max(5, parseInt(document.getElementById('uBotInterval').value||'30',10)||30),
      photo: window.__pendingUserBotPhoto || null,
      active: false
    };
    await db.ref('userBots/'+me.uid).update(data);
    toast('Uloženo');
    loadUserBotPanel();
  });
  document.getElementById('uBotActivate')?.addEventListener('click', async ()=>{
    const me=auth.currentUser; if(!me){ openModalAuth('login'); return; }
    const plan=(await db.ref('users/'+me.uid+'/plan').get()).val() || '';
    if(!(plan==='premium_150' || plan==='premium_plus_200')){ toast('Potřebujete Premium'); return; }
    await db.ref('userBots/'+me.uid+'/active').set(true);
    toast('Aktivováno');
    loadUserBotPanel();
  });
  document.getElementById('uBotDeactivate')?.addEventListener('click', async ()=>{
    const me=auth.currentUser; if(!me){ openModalAuth('login'); return; }
    await db.ref('userBots/'+me.uid+'/active').set(false);
    toast('Deaktivováno');
    loadUserBotPanel();
  });
  document.getElementById('uBotDelete')?.addEventListener('click', async ()=>{
    const me=auth.currentUser; if(!me){ openModalAuth('login'); return; }
    if(!confirm('Smazat bota?')) return;
    await db.ref('userBots/'+me.uid).remove();
    toast('Smazáno');
    loadUserBotPanel();
  });
}

async function loadUserBotPanel(){
  const me=auth.currentUser; if(!me) return;
  // fill cities select from existing CITY_LIST or fallback
  const sel=document.getElementById('uBotCity');
  if(sel && !sel.dataset.filled){
    try{
      const cities=(window.CITY_LIST && Array.isArray(window.CITY_LIST)) ? window.CITY_LIST : ['praha','brno','ostrava','plzen','liberec','olomouc'];
      sel.innerHTML = cities.map(c=>`<option value="${c}">${c}</option>`).join('');
      sel.dataset.filled='1';
    }catch{}
  }

  const snap=await db.ref('userBots/'+me.uid).get();
  const b=snap.val()||{};
  document.getElementById('uBotNick').value = b.nick || '';
  document.getElementById('uBotText').value = b.text || '';
  document.getElementById('uBotTemplates').value = (b.templates||[]).join('\n');
  if(sel) sel.value = b.city || 'praha';
  document.getElementById('uBotInterval').value = b.intervalMin || 30;
  window.__pendingUserBotPhoto = null;

  const plan=(await db.ref('users/'+me.uid+'/plan').get()).val() || '';
  const allowed = (plan==='premium_150' || plan==='premium_plus_200');
  ['uBotNick','uBotText','uBotTemplates','uBotCity','uBotInterval','uBotPhoto','uBotSave','uBotActivate','uBotDeactivate','uBotDelete'].forEach(id=>{
    const el=document.getElementById(id);
    if(!el) return;
    el.disabled = !allowed;
  });
...
async function loadMyPayments(){
  const me=auth.currentUser; if(!me) return;
  const feed=document.getElementById('myPayments'); if(!feed) return;
  const s=await db.ref('vipRequests/'+me.uid).orderByChild('ts').limitToLast(20).get();
  const v=s.val()||{};
  const rows=Object.keys(v).sort((a,b)=>(v[b].ts||0)-(v[a].ts||0)).map(k=>{
    const r=v[k]||{};
    const st=r.status||'pending';
    return `<div class="item"><b>${escapeHtml(r.plan||'')}</b> · <span class="pill">${escapeHtml(st)}</span><div class="muted">${new Date(r.ts||0).toLocaleString()}</div></div>`;
  });
  feed.innerHTML = rows.join('') || '<div class="muted">Žádné žádosti</div>';
}

async function loadMyVerifications(){
  const me=auth.currentUser; if(!me) return;
  const feed=document.getElementById('myVerifications'); if(!feed) return;
  const s=await db.ref('employerVerifications/'+me.uid).orderByChild('ts').limitToLast(20).get();
  const v=s.val()||{};
  const rows=Object.keys(v).sort((a,b)=>(v[b].ts||0)-(v[a].ts||0)).map(k=>{
    const r=v[k]||{};
    return `<div class="item"><b>Ověření</b> · <span class="pill">${escapeHtml(r.status||'pending')}</span><div class="muted">${new Date(r.ts||0).toLocaleString()}</div></div>`;
  });
  feed.innerHTML = rows.join('') || '<div class="muted">Žádné žádosti</div>';
}

async function loadAdminPremiumPanels(){
  const me=auth.currentUser; if(!me || !__isAdminNow()) return;

  // Profile change requests
  const prFeed=document.getElementById('adminProfileReqs');
  if(prFeed){
    const s=await db.ref('profileChangeRequests').orderByChild('ts').limitToLast(50).get();
    const all=s.val()||{};
    const items=[];
    ...
    prFeed.innerHTML = items.map(r=>{
      const wantN=r.wantNick?`Nick → <b>${escapeHtml(r.wantNick)}</b>`:'';
      const wantR=r.wantRole?`Role → <b>${escapeHtml(r.wantRole)}</b>`:'';
      return `<div class="item">
        <div><b>${escapeHtml(r.uid)}</b> · <span class="pill">${escapeHtml(r.status||'pending')}</span></div>
        <div>${wantN} ${wantR}</div>
        <div class="row" style="justify-content:flex-end;margin-top:6px">
          <button class="ghost" type="button" data-apr="1" data-uid="${r.uid}" data-id="${r.id}">Schválit</button>
          <button class="ghost" type="button" data-rej="1" data-uid="${r.uid}" data-id="${r.id}">Zamítnout</button>
        </div>
      </div>`;
    }).join('') || '<div class="muted">Žádné žádosti</div>';

    prFeed.querySelectorAll('button[data-apr]').forEach(b=>b.addEventListener('click', async ()=>{
      const uid=b.getAttribute('data-uid'); const id=b.getAttribute('data-id');
      const snap=await db.ref('profileChangeRequests/'+uid+'/'+id).get();
      const r=snap.val(); if(!r) return;
      const up={};
      if(r.wantNick) up.nick=r.wantNick;
      if(r.wantRole) up.role=r.wantRole;
      if(Object.keys(up).length) await db.ref('usersPublic/'+uid).update(up);
      await db.ref('profileChangeRequests/'+uid+'/'+id).update({status:'approved', decidedTs:Date.now()});
      toast('Schváleno');
      loadAdminPremiumPanels();
    }));
    prFeed.querySelectorAll('button[data-rej]').forEach(b=>b.addEventListener('click', async ()=>{
      const uid=b.getAttribute('data-uid'); const id=b.getAttribute('data-id');
      await db.ref('profileChangeRequests/'+uid+'/'+id).update({status:'rejected', decidedTs:Date.now()});
      toast('Zamítnuto');
      loadAdminPremiumPanels();
    }));
  }
  ...
  // Payments
  const payFeed=document.getElementById('adminPayments');
  if(payFeed){
    const s=await db.ref('vipRequests').orderByChild('ts').limitToLast(50).get();
    const all=s.val()||{};
    const items=[];
    Object.keys(all).forEach(uid=>{
      const obj=all[uid]||{};
    ...
    items.sort((a,b)=>(b.ts||0)-(a.ts||0));
    payFeed.innerHTML = items.map(r=>{
      return `<div class="item">
        <div><b>${escapeHtml(r.uid)}</b> · <b>${escapeHtml(r.plan||'')}</b> · <span class="pill">${escapeHtml(r.status||'pending')}</span></div>
        ${r.shot?`<div style="margin-top:6px"><img src="${r.shot}" style="max-width:220px;border-radius:12px"/></div>`:''}
        <div class="row" style="justify-content:flex-end;margin-top:6px">
          <button class="ghost" type="button" data-payok="1" data-uid="${r.uid}" data-id="${r.id}" data-plan="${escapeHtml(r.plan||'')}">Potvrdit</button>
          <button class="ghost" type="button" data-payno="1" data-uid="${r.uid}" data-id="${r.id}">Zamítnout</button>
        </div>
      </div>`;
    }).join('') || '<div class="muted">Žádné platby</div>';

    payFeed.querySelectorAll('button[data-payok]').forEach(b=>b.addEventListener('click', async ()=>{
      const uid=b.getAttribute('data-uid'); const id=b.getAttribute('data-id'); const plan=b.getAttribute('data-plan');
      await db.ref('users/'+uid+'/plan').set(plan);
      await db.ref('vipRequests/'+uid+'/'+id).update({status:'approved', decidedTs:Date.now()});
      toast('Aktivováno');
      loadAdminPremiumPanels();
    }));
    payFeed.querySelectorAll('button[data-payno]').forEach(b=>b.addEventListener('click', async ()=>{
      const uid=b.getAttribute('data-uid'); const id=b.getAttribute('data-id');
      await db.ref('vipRequests/'+uid+'/'+id).update({status:'rejected', decidedTs:Date.now()});
      toast('Zamítnuto');
      loadAdminPremiumPanels();
    }));
  }
  ...
  const vFeed=document.getElementById('adminVerifs');
  if(vFeed){
    const s=await db.ref('employerVerifications').orderByChild('ts').limitToLast(50).get();
    const all=s.val()||{};
    const items=[];
    Object.keys(all).forEach(uid=>{
      const obj=all[uid]||{};
    ...
    items.sort((a,b)=>(b.ts||0)-(a.ts||0));
    vFeed.innerHTML = items.map(r=>{
      const docs=(r.docs||[]).slice(0,3).map(d=>d.type==='img'
        ? `<img src="${d.data}" style="max-width:160px;border-radius:12px;margin-right:8px"/>`
        : `<div class="pill">${escapeHtml(d.name||'PDF')}</div>`).join('');
      return `<div class="item">
        <div><b>${escapeHtml(r.uid)}</b> · <span class="pill">${escapeHtml(r.status||'pending')}</span></div>
        <div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:8px">${docs}</div>
        <div class="row" style="justify-content:flex-end;margin-top:6px">
          <button class="ghost" type="button" data-verok="1" data-uid="${r.uid}" data-id="${r.id}">Potvrdit</button>
          <button class="ghost" type="button" data-verno="1" data-uid="${r.uid}" data-id="${r.id}">Zamítnout</button>
        </div>
      </div>`;
    }).join('') || '<div class="muted">Žádná ověření</div>';

    vFeed.querySelectorAll('button[data-verok]').forEach(b=>b.addEventListener('click', async ()=>{
      const uid=b.getAttribute('data-uid'); const id=b.getAttribute('data-id');
      await db.ref('usersPublic/'+uid+'/verifiedEmployer').set(true);
      await db.ref('employerVerifications/'+uid+'/'+id).update({status:'approved', decidedTs:Date.now()});
      toast('Ověřeno');
      loadAdminPremiumPanels();
    }));
    vFeed.querySelectorAll('button[data-verno]').forEach(b=>b.addEventListener('click', async ()=>{
      const uid=b.getAttribute('data-uid'); const id=b.getAttribute('data-id');
      await db.ref('employerVerifications/'+uid+'/'+id).update({status:'rejected', decidedTs:Date.now()});
      toast('Zamítnuto');
      loadAdminPremiumPanels();
    }));
  }
...
// Bell center / notifications
let __notifUnsubs = [];
function stopNotifs(){
  __notifUnsubs.forEach(fn=>{ try{ fn(); }catch{} });
  __notifUnsubs=[];
}
  ...
  const upUI=()=>{
    ...
    if(feed){
      feed.innerHTML = top.map(ev=>{
        const pill = ev.kind ? `<span class="pill">${escapeHtml(ev.kind)}</span>` : '';
        const act = ev.action ? `<button class="ghost" type="button" data-act="${escapeHtml(ev.action)}" data-p="${escapeHtml(ev.payload||'')}">Otevřít</button>` : '';
        return `<div class="item"><div><b>${escapeHtml(ev.title||'')}</b> ${pill}</div><div class="muted">${escapeHtml(ev.body||'')}</div><div class="row" style="justify-content:flex-end">${act}</div></div>`;
      }).join('') || '<div class="muted">Žádné události</div>';

      feed.querySelectorAll('button[data-act]').forEach(b=>b.addEventListener('click', ()=>{
        const act=b.getAttribute('data-act'); const p=b.getAttribute('data-p');
        if(act==='dm'){ 
          try{ const parts=(p||'').split('|'); openDMRoom(me.uid, parts[0]); showView('view-dm'); }catch(e){ console.warn(e); }
        }
        if(act==='friends'){ try{ showView('view-friends'); }catch{} }
        if(act==='premium'){ try{ openModal('modalPremium'); }catch{} }
      }));
    }
    const cnt = events.filter(e=>e.unread).length;
    if(badge){ badge.textContent = cnt>0?String(cnt):''; badge.style.display = cnt>0?'inline-flex':'none'; }
  };

  // friend requests to me
  const frRef=db.ref('friendRequests/'+me.uid);
  const frCb=frRef.on('value', snap=>{
    // remove existing friend events
    for(let i=events.length-1;i>=0;i--) if(events[i].src==='fr') events.splice(i,1);
    const v=snap.val()||{};
    Object.keys(v).forEach(fromUid=>{
      const r=v[fromUid]||{};
      if((r.status||'pending')==='pending'){
        events.push({src:'fr', ts:r.ts||Date.now(), unread:true, kind:'Friend', title:'Nová žádost o přátelství', body:'Od: '+(r.fromNick||fromUid), action:'friends', payload:''});
      }
    });
    upUI();
  });
  __notifUnsubs.push(()=>frRef.off('value', frCb));

  // DM unread count from inboxMeta
  const imRef=db.ref('inboxMeta/'+me.uid);
  const imCb=imRef.on('value', snap=>{
    for(let i=events.length-1;i>=0;i--) if(events[i].src==='dm') events.splice(i,1);
    const v=snap.val()||{};
    Object.keys(v).forEach(room=>{
      const r=v[room]||{};
      const unread=r.unread||0;
      if(unread>0){
        const peer=r.peerUid || (r.peer||'');
        events.push({src:'dm', ts:r.lastTs||Date.now(), unread:true, kind:'DM', title:'Nová zpráva v L.S.', body:'Zprávy: '+unread, action:'dm', payload:(peer||'')+'|'+room});
      }
    });
    upUI();
  });
  __notifUnsubs.push(()=>imRef.off('value', imCb));

  // my vip requests status changes
  const vrRef=db.ref('vipRequests/'+me.uid);
  const vrCb=vrRef.on('value', snap=>{
    for(let i=events.length-1;i>=0;i--) if(events[i].src==='vip') events.splice(i,1);
    const v=snap.val()||{};
    Object.keys(v).forEach(id=>{
      const r=v[id]||{};
      if(r.status && r.status!=='pending'){
        events.push({src:'vip', ts:r.decidedTs||r.ts||Date.now(), unread:true, kind:'VIP', title:'Změna tarifu', body:`${r.plan} → ${r.status}`, action:'premium', payload:''});
      }
    });
    upUI();
  });
  __notifUnsubs.push(()=>vrRef.off('value', vrCb));

  // my profile change requests status changes
  const pcRef=db.ref('profileChangeRequests/'+me.uid);
  const pcCb=pcRef.on('value', snap=>{
    for(let i=events.length-1;i>=0;i--) if(events[i].src==='pc') events.splice(i,1);
    const v=snap.val()||{};
    Object.keys(v).forEach(id=>{
      const r=v[id]||{};
      if(r.status && r.status!=='pending'){
        events.push({src:'pc', ts:r.decidedTs||r.ts||Date.now(), unread:true, kind:'Profil', title:'Žádost o změnu profilu', body:`${r.status}`, action:'premium', payload:''});
      }
    });
    upUI();
  });
  __notifUnsubs.push(()=>pcRef.off('value', pcCb));

  // clear button
  document.getElementById('notifClear')?.addEventListener('click', ()=>{
    events.forEach(e=>e.unread=false);
    upUI();
  });
}

// VIP business logic: block worker -> employer DM without VIP worker
async function canWriteEmployerDM(meUid, peerUid){
  try{
    const meP=(await db.ref('usersPublic/'+meUid).get()).val()||{};
    const peerP=(await db.ref('usersPublic/'+peerUid).get()).val()||{};
    const meRole=(meP.role||'').toLowerCase();
    const peerRole=(peerP.role||'').toLowerCase();
    if(meRole==='worker' && peerRole==='employer'){
      const plan=(await db.ref('users/'+meUid+'/plan').get()).val()||'';
      if(plan!=='vip_worker_100' && plan!=='premium_150' && plan!=='premium_plus_200'){
        return false;
      }
    }
  }catch(e){}
...
// Wrap openDMRoom to enforce VIP rule (best-effort without breaking existing)
const __openDMRoom_orig = (typeof window.openDMRoom==='function') ? window.openDMRoom : null;
if(__openDMRoom_orig){
  window.openDMRoom = async function(meUid, peerUid){
    const ok = await canWriteEmployerDM(meUid, peerUid);
    if(!ok){
      toast('Pro psaní zaměstnavateli potřebujete VIP Pracovník (100 Kč).');
      try{ openModal('modalPremium'); }catch{}
      try{ openPremiumBot(); }catch{}
      return;
    }
    return __openDMRoom_orig.apply(this, arguments);
  }
}

// Patch drawer premium to open modalPremium (plus keep bot)
try{
  const premLink=document.getElementById('drawerPremium');
  premLink?.addEventListener('click', (e)=>{ e.preventDefault(); try{ openModal('modalPremium'); }catch{} });
}catch{}

// Start hooks
document.addEventListener('DOMContentLoaded', ()=>{
  try{ initPremiumModal(); }catch(e){ console.warn(e); }
});

// auth listener to start notifications
try{
  auth.onAuthStateChanged((u)=>{
    if(u){
      setTimeout(()=>{ try{ startNotifs(); }catch(e){ console.warn(e);} }, 350);
    }else{
      stopNotifs();
      const badge=document.getElementById('bellBadge'); if(badge){ badge.textContent=''; badge.style.display='none'; }
    }
  });
}catch(e){}

/* User bot runtime: posts into selected chat on interval when active (MVP) */
let __botTimer=null;
async function startUserBotRuntime(){
  if(__botTimer) clearInterval(__botTimer);
  const me=auth.currentUser; if(!me) return;
  __botTimer = setInterval(async ()=>{
    try{
      const bsnap=await db.ref('userBots/'+me.uid).get();
      const b=bsnap.val();
      if(!b || !b.active) return;
      const plan=(await db.ref('users/'+me.uid+'/plan').get()).val()||'';
      if(!(plan==='premium_150' || plan==='premium_plus_200')) return;
      const city=b.city||'praha';
      const intervalMin=Math.max(5, parseInt(b.intervalMin||30,10)||30);
      if(!b.__lastTs) b.__lastTs=0;
      const now=Date.now();
      if(now - b.__lastTs < intervalMin*60*1000) return;

      // choose template or base text
      const templates=Array.isArray(b.templates)?b.templates:[];
      const msgText = (templates.length ? templates[Math.floor(Math.random()*templates.length)] : (b.text||'')).trim();
      if(!msgText && !b.photo) return;

      const msg = { by: me.uid, ts: now, text: msgText, bot: true, botNick: b.nick||'Bot', botPhoto: b.photo||null };
      await db.ref('messages/'+city).push(msg);
      b.__lastTs = now;
      await db.ref('userBots/'+me.uid+'/__lastTs').set(now);
    }catch(e){}
  }, 5000); // check every 5s
}
try{
  auth.onAuthStateChanged(u=>{ if(u) startUserBotRuntime(); else if(__botTimer){ clearInterval(__botTimer); __botTimer=null; } });
}catch{}
