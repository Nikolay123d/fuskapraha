...
function cookieBanner(){
  ...
  $('#cookieDecline')?.addEventListener('click', ()=>{
    ...
    document.cookie = 'cookieAccepted=0; path=/; max-age=31536000';
    b.style.display='none';
    b.hidden = true;
  }, {once:true});
}
...
function setPresence(u){
  ...
  ref.on('value', snap=>{
    ...
    // Presence is best-effort. If rules are not published yet, do not break the app.
    try{
      up.onDisconnect().set({online:false,ts:Date.now()}).catch(()=>{});
      up.set({online:true,ts:Date.now()}).catch(()=>{});
    }catch(e){/* ignore */}
  });
}
...
$('#dmOpen')?.addEventListener('click', async ()=>{
  ...
  const dmToEl=$('#dmTo');
  if(!dmToEl){ toast('DM panel není připraven'); return; }
  let to=dmToEl.value.trim(); if(!to) return;
...
$('#dmSend')?.addEventListener('click', async ()=>{
  try{
    ...
    if(!dmToEl){ toast('DM panel není připraven'); return; }
    let to=dmToEl.value.trim(); if(!to) return;
    if(to.includes('@')){ const uid=await resolveUidByEmail(to); if(!uid) return alert('Email neznám'); to=uid; dmToEl.value=uid; }
...
(function(){
  ...
  document.getElementById('linkPrivacy')?.addEventListener('click', (e)=>{ e.preventDefault(); show('modalPrivacy'); });
  document.getElementById('linkTerms')?.addEventListener('click', (e)=>{ e.preventDefault(); show('modalTerms'); });
  document.getElementById('linkContact')?.addEventListener('click', (e)=>{ e.preventDefault(); show('modalContact'); });
  document.addEventListener('click', (e)=>{
    const c=e.target.closest('[data-close]');
    if(!c) return;
    const t=c.getAttribute('data-close');
    if(t==='privacy') hide('modalPrivacy');
    if(t==='terms') hide('modalTerms');
    if(t==='contact') hide('modalContact');
  });
  document.getElementById('copyContactEmail')?.addEventListener('click', async ()=>{
    try{
      const txt=document.getElementById('contactEmail')?.textContent||'';
      await navigator.clipboard.writeText(txt);
      toast('Zkopírováno');
      playSound('ok');
    }catch(e){ toast('Nelze kopírovat'); }
  });
})();
