...
async function ensureMyPublic(u){
  ...
  if(!s.exists()){
    const fallbackNick = (u.displayName || '...');
    await ref.set({ nick: fallbackNick, email: u.email||null, avatar: window.DEFAULT_AVATAR, role:'seeker', plan:'free', createdAt: Date.now() });
  }else{
    const v=s.val()||{};

    // strict nick: if legacy /users has nick and usersPublic.nick is missing, migrate once
    if(!v.nick){
      try{
        const legacy = (await db.ref('users/'+u.uid).get()).val()||{};
        const ln = (legacy.nick || legacy.name || '').toString().trim();
        if(ln){
          await ref.update({nick: ln});
        }
      }catch(e){}
    }
...
async function googleSignIn(){
  ...
  // Make sure the button never double-triggers (hard lock ~2.5s).
  const now = Date.now();
  if(window.__GOOGLE_AUTH_IN_FLIGHT__ || now < __GOOGLE_LOCK_UNTIL) return;
  window.__GOOGLE_AUTH_IN_FLIGHT__ = true;
  __GOOGLE_LOCK_UNTIL = now + 2500;
  // Auto-unlock as an additional failsafe (redirect will unload the page anyway).
  try{ setTimeout(()=>{ try{ window.__GOOGLE_AUTH_IN_FLIGHT__ = false; }catch(e){} }, 2600); }catch(e){}
...
// Expire premium plan on login (no server). Deterministic: only downgrades when planUntil < now.
let __planExpireLock = false;
async function enforcePlanExpiry(u){
  try{
    if(!u || __planExpireLock) return;
    __planExpireLock = true;
    const nowTs = Date.now();

    // Read from usersPublic first (UI source of truth), fall back to /users.
    const pubRef = db.ref('usersPublic/'+u.uid);
    const pub = (await pubRef.get()).val() || {};
    let planUntil = (pub.planUntil||0);
    let plan = pub.plan || '';
    if(!planUntil){
      try{
        const priv = (await db.ref('users/'+u.uid).get()).val() || {};
        planUntil = priv.planUntil || 0;
        if(!plan) plan = priv.plan || '';
      }catch(e){}
    }

    if(planUntil && Number(planUntil) < nowTs){
      const updates = {};
      // Public (UI)
      updates['usersPublic/'+u.uid+'/plan'] = '';
      updates['usersPublic/'+u.uid+'/planStatus'] = 'expired';
      updates['usersPublic/'+u.uid+'/planTs'] = nowTs;
      updates['usersPublic/'+u.uid+'/planUntil'] = null;

      // Private (if allowed by rules)
      updates['users/'+u.uid+'/plan'] = '';
      updates['users/'+u.uid+'/planStatus'] = 'expired';
      updates['users/'+u.uid+'/planTs'] = nowTs;
      updates['users/'+u.uid+'/planUntil'] = null;

      await db.ref().update(updates);
      console.log('[MAKAME] plan expired -> downgraded', u.uid, plan, planUntil);
    }
  }catch(e){
    console.warn('[MAKAME] enforcePlanExpiry failed', e);
  }finally{
    __planExpireLock = false;
  }
}
...
async function doPasswordReset(){
  ...
  const email = emailInput || auth.currentUser?.email || prompt('E-mail pro obnovu:');
  if(!email) return;
  ...
  try{
    ...
    const code = (e && e.code) ? String(e.code) : '';
    if(code.includes('auth/invalid-email')) return toast('Neplatný e-mail.');
    if(code.includes('auth/user-not-found')) return toast('Tento e-mail nemá účet.');
    if(code.includes('auth/too-many-requests')) return toast('Příliš mnoho pokusů. Zkuste to později.');
    if(code.includes('auth/network-request-failed')) return toast('Síťová chyba. Zkuste to znovu.');
    toast(e?.message || 'Chyba při odeslání resetu hesla.');
  }
}
...
const m=snap.val()||{};
  ...
  try{
    db.ref('messages/'+city).limitToLast(5).once('value').then((s5)=>{
      ...
      // Stage-2 should be perceptible: let the user see the first 5 messages first.
      setTimeout(()=>{ try{ __switchChatToFull(); }catch(e){} }, 900);
    }).catch(()=>{});
  }catch(e){}
  ...
  setTimeout(__switchChatToFull, 1800);
...
auth.onAuthStateChanged(async (u)=>{
  ...
  try{ if(u) await enforcePlanExpiry(u); }catch{}
  try{ if(u) await enforceVerifyWindow(u); }catch{}
  // admin-only visibility (role-based)
  watchAdminRole(u);
  try{ window.__authExtraAdminUi && window.__authExtraAdminUi(u); }catch(e){}
// profile display
  if(u){
...
// --- DM threads / inbox ---
  ...
  async function loadDmThreads(){
    ...
    const emptyStateHtml = '<div class="muted" style="padding:10px">U tebe zatím nejsou žádné soukromé zprávy.</div>';
    ...
    try{
      ...
      if(!rooms.length){
        ...
        if(label) label.textContent='Osobní zprávy';
        return;
      }
      ...
      if(!box.innerHTML) box.innerHTML = emptyStateHtml;
    }finally{
      setMiniLoad('dmMiniLoad','', false);
  ...
  // Payment result -> deterministic DM message from bot (no Cloud Functions).
  let __payResListenerOff = null;
  let __payResHandling = false;
  function __detachPayResListener(){
    try{ if(__payResListenerOff) __payResListenerOff(); }catch(e){}
    __payResListenerOff = null;
    __payResHandling = false;
  }
  function __attachPayResListener(room, otherUid){
    const me = auth.currentUser;
    if(!me) return;
    const botRoom = dmKey(me.uid, PREMIUM_BOT_UID);
    if(room !== botRoom || otherUid !== PREMIUM_BOT_UID){
      __detachPayResListener();
      return;
    }
    if(__payResListenerOff) return; // already attached

    const ref = db.ref('userState/'+me.uid+'/botInbox/paymentResult');
    const handler = async (snap)=>{
      try{
        // Only act when user is still inside the bot DM.
        if(currentDmRoom !== botRoom) return;
        const v = snap.val();
        if(!v || !v.status || __payResHandling) return;
        __payResHandling = true;

        const status = String(v.status);
        const plan = v.plan ? String(v.plan) : null;
        const ts = (typeof v.ts==='number') ? v.ts : Date.now();

        let text;
        if(status==='approved'){
          text = `✅ Platba byla potvrzena. Tarif aktivován: ${plan||'—'}.
Děkujeme!`;
        }else if(status==='declined'){
          text = `❌ Platba byla zamítnuta.
Pokud jste už zaplatil(a), pošlete prosím znovu screenshot nebo kontaktujte podporu.`;
        }else{
          __payResHandling = false;
          return;
        }

        // Write message as the current user, but render as bot.
        await db.ref('privateMessages/'+botRoom).push({
          by: me.uid,
          ts,
          text,
          bot: true,
        ...
        // Clear flag (deterministic, prevents double-send).
        await ref.set(null);
      }catch(e){
        console.warn('paymentResult bot post failed', e?.code||e);
      }finally{
        __payResHandling = false;
      }
    };
    ref.on('value', handler);
    __payResListenerOff = ()=>ref.off('value', handler);
  }
  ...
  window.openDMRoom = async function(a,b){
    ...
    // Attach deterministic payment-result listener only inside Premium bot DM.
    try{ __attachPayResListener(currentDmRoom, other); }catch(e){}
...
document.getElementById('botAdd')?.addEventListener('click', async ()=>{
  ...
  window.__authExtraAdminUi = async (u)=>{
    try{
...
async function ensureUserPublicBasics(){
  ...
  if(!nick){
    ...
    patch.nick = (displayName || fromEmail || 'Nováček').slice(0,28);
  }
  if(!role){
    patch.role = 'seeker';
  }
...
async function __adminSendPaymentResultDm(uid, ok, plan){
  try{
    const now = Date.now();
    const room = dmKey(uid, PREMIUM_BOT_UID);

    // Ensure DM room membership exists (user + bot)
    const members = {};
    members[uid] = true;
    members[PREMIUM_BOT_UID] = true;
    try{ await db.ref('privateMembers/'+room).transaction(cur=>cur||members); }catch(e){}

    // Ensure inbox meta for the user
    try{
      await db.ref('inboxMeta/'+uid+'/'+room).update({
        with: PREMIUM_BOT_UID,
        ts: now,
        lastTs: now,
        title: 'Bot — Privilege'
      });
    ...
    const txt = ok
      ? `✅ Platba potvrzena. Aktivováno: ${plan || ''}.`
      : `❌ Platba zamítnuta. Pokud je to omyl, pošli prosím znovu screenshot.`;
    await db.ref('privateMessages/'+room).push({by: PREMIUM_BOT_UID, botUid: PREMIUM_BOT_UID, bot:true, text: txt, ts: now});
  }catch(e){
    console.warn('[admin] DM notify failed', e);
  }
}

async function __adminAudit(action, data){
  try{
    const key = db.ref('payments/audit').push().key;
    await db.ref('payments/audit/'+key).set({ts: Date.now(), action, ...data});
  }catch(e){}
}
...
async function adminApprovePayment(uid, reqId, plan){
  ...
  // Plan lives in usersPublic (client UI) + users (server-side/private)
  updates[`usersPublic/${uid}/plan`] = plan;
  updates[`usersPublic/${uid}/planTs`] = now;
  updates[`usersPublic/${uid}/planUntil`] = until;
  updates[`usersPublic/${uid}/planStatus`] = 'active';
  ...
  // Deterministic bot-inbox flag: client bot will post DM message when user opens bot DM.
  updates[`userState/${uid}/botInbox/paymentResult`] = { status:'approved', plan, ts: now };

  await db.ref().update(updates);

  // Audit log
  await __adminAudit('approve', {uid, reqId, plan, by: me.uid});
}
...
async function adminDeclinePayment(uid, reqId){
  ...
  const nid = db.ref('notifications/'+uid).push().key;
  updates[`notifications/${uid}/${nid}`] = { ts: now, type:'payment_declined', read:false, reqId };

  updates[`userState/${uid}/botInbox/paymentResult`] = { status:'declined', plan: null, ts: now };

  await db.ref().update(updates);

  await __adminAudit('decline', {uid, reqId, by: me.uid});
}
