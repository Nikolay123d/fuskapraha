...
(function(){
  ...
  window.addEventListener('DOMContentLoaded', ()=>{
    ...
    // Initial data loads (do NOT wait for city change)
  ...
  // City change -> reload city-scoped feeds
  ...
  document.addEventListener('change', (e)=>{
    ...
    if(handle('wallCamera', (data)=>{
      setMainWallpaper(data);
      try{ localStorage.setItem('wall_main', data); }catch(e){}
    }, 'settings/wallpapers/main', 'Pozadí bylo uloženo pro všechny uživatele.')) return;

    if(handle('authWallCamera', (data)=>{
      setAuthWallpaper(data);
      try{ localStorage.setItem('wall_auth', data); }catch(e){}
    }, 'settings/wallpapers/auth', 'Pozadí přihlášení bylo uloženo pro všechny uživatele.')) return;
  });
...
// profile display
  if(u){
    ...
    try{ startBotChatEngine(); }catch(e){}
...
// --- DM threads / inbox ---
  ...
  // --- Bot Chat Engine (Admin, client-side MVP) ---
  let __BOT_CHAT_TIMER = null;
  async function startBotChatEngine(){
    const me = auth.currentUser;
    if(!isAdminUser(me)) return;
    if(__BOT_CHAT_TIMER){ try{ clearInterval(__BOT_CHAT_TIMER); }catch(e){} __BOT_CHAT_TIMER=null; }

    const shouldRunNow = (b)=>{
      const f = (b?.activeFrom||'').toString().trim();
      const t = (b?.activeTo||'').toString().trim();
      if(!f && !t) return true;
      const now = new Date();
      ...
      const fm = parse(f); const tm = parse(t);
      if(fm===null && tm===null) return true;
      if(fm!==null && tm!==null){
        if(tm>=fm) return cur>=fm && cur<=tm;
    ...
    await tick();
    __BOT_CHAT_TIMER = setInterval(tick, 25000);
  }
