...
function renderMessage(container, m){
  ...
  text.innerHTML = escapeHtml(m.text||'') + (m.photo?`<div><img src="${m.photo}" alt=""></div>`:'');
  ...
  document.addEventListener('click', (e)=>{
    if(e.target && e.target.id==='chatSend'){
      const action = async ()=>{
        ...
        await db.ref('messages/'+city).push({text:txt||null,photo:photo||null,by:auth.currentUser.uid,ts:Date.now()});
  ...
  document.addEventListener('click', (e)=>{
    if(e.target && e.target.id==='rentSend'){
      const action = async ()=>{
        ...
        const city=window.CURRENT_CITY; if(photo) await decQuota(auth.currentUser.uid,'photo'); else await decQuota(auth.currentUser.uid,'text');
        await db.ref('rentMessages/'+city).push({text:txt||null,photo:photo||null,by:auth.currentUser.uid,ts:Date.now()});
...
// ===== Friends
function sendFriendRequest(toUid){ const me=auth.currentUser.uid; const req={from:me,ts:Date.now(),status:'pending'}; db.ref('friendRequests/'+toUid+'/'+me).set(req); playSound('notif'); }
  ...
  for(const uid in all){
    ...
    for(const key in items){
      ...
      row.querySelector('[data-approve]').onclick = async()=>{
        ...
        const lim = plan==='premium_plus'? {text:5000, photo:2500} : {text:1000, photo:500};
