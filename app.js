...
function initDmMobileModal(){
  ...
  function open(){
    ...
    // Use the shared modal-manager so other overlays (drawer, admin, etc.) cannot stack on top.
    ...
    try{
      ...
      if(typeof window.openModal === 'function'){
        window.openModal('dmMobileOverlay');
      }else{
  ...
  function close(){
    try{
      ...
      if(typeof window.closeModal === 'function') window.closeModal('dmMobileOverlay');
...
document.getElementById('authGoogleBtn')?.addEventListener('click', async ()=>{
  ...
  // --- Public helpers (used by other modules/buttons) ---
  // Some modules (Premium/Bots) call `openAuth()` directly.
  // Keep a stable global alias to the auth modal.
  window.openAuth = function openAuth(mode='login'){
    try{ openModalAuth(mode); }catch(e){ console.warn('openAuth failed', e); }
  };
