...
(function(){
  ...
  if(t && t.id==='wallCamera'){
    ...
    r.onload=()=>{
      ...
      db.ref('settings/wallpapers/'+c).set(String(data));
...
function startWallpaperSync(){
  try{
    ...
    const handler=(snap)=>{
      const raw = snap.val();
      // Backward compatible: accept either a raw dataURL string or {url: dataURL}
      const val = (typeof raw === 'string') ? raw : (raw && typeof raw.url === 'string' ? raw.url : null);
...
$('#sendBtn')?.addEventListener('click', async ()=>{
  try{
    ...
    const mt=$('#msgText'); const text=(mt && mt.value ? mt.value : '').trim();
...
function openDMRoom(a,b){
  ...
  try{ await db.ref('privateMembers/'+room+'/'+a).set(true); }catch(e){ console.warn(e); }
  try{ await db.ref('privateMembers/'+room+'/'+b).set(true); }catch(e){ console.warn(e); }
  // render
  renderDM(room);
}
...
// === Global helpers ===
window.closeModal = function(){ try{ document.querySelectorAll('.modal,.popup,.overlay').forEach(el=>el.classList.remove('open','active','show')); }catch(e){} };

if(!window.__AUDIO_UNLOCK_BOUND__){ window.__AUDIO_UNLOCK_BOUND__=true; window.addEventListener('pointerdown', ()=>{ unlockAudio(); }, {once:true}); }

async function selfTestRules(){
  const u = auth.currentUser;
  if(!u) return;
  // Presence write test
  try{
    await db.ref('presence/'+u.uid).set({ts: Date.now()});
  }catch(e){
    console.warn('Rules test failed (presence)', e);
    toast('Firebase Rules не применены: permission_denied (presence). Открой RTDB → Rules → вставь database.rules.json и Publish.');
    return;
  }
  // Emails write test (keyed)
  if(u.email){
    const key=u.email.toLowerCase().replace(/\./g,',');
    try{
      await db.ref('emails/'+key).set({uid:u.uid, ts: Date.now()});
    }catch(e){
      console.warn('Rules test failed (emails)', e);
      toast('Firebase Rules не применены: permission_denied (emails). Проверь, что редактируешь rules именно для базы praga-4baee-default-rtdb.');
      return;
    }
  }
}
auth.onAuthStateChanged(()=>{ setTimeout(selfTestRules, 500); });
