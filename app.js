    ...
    ref.on('value', async (snap)=>{
      try{
        ...
        await Promise.all(uids.slice(0,150).map(async (uid)=>{
          ...
          else if(r==='worker' || r==='hledam' || r==='hledam_praci' || r==='hledampraci' || r==='hledám práci') workers++;
        }));
...
async function ensureMyPublic(u){
  ...
  if(!s.exists()){
    ...
    await ref.set({ nick: fallbackNick, email: u.email||null, avatar: window.DEFAULT_AVATAR, role:'seeker', plan:'free', createdAt: Date.now() });
  }else{
    const v=s.val()||{};
...
async function requireProfileComplete(u){
  ...
  const needRole = !pub || !pub.role || (pub.role!=='worker' && pub.role!=='employer');
  ...
  if(selRole) selRole.value = (needRole ? 'worker' : pub.role);
  ...
  if(!__setupProfileBound && btnSave){
    ...
    btnSave.addEventListener('click', async ()=>{
      ...
      if(role!=='worker' && role!=='employer') return toast('Vyberte roli');
