...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  try{ startTypingSim(); }catch(e){}
  try{ __startHomeTicker(); }catch(e){}
  try{ startOnlineCounts(); }catch(e){}
...
async function ensureMyPublic(u){
  ...
  if(!s.exists()){
    ...
    // migrate legacy nick/role from private /users (older builds) into usersPublic (and lock it)
    if((!v.nick || String(v.nick).trim()==="") || (!v.role || String(v.role).trim()==="")) {
      try{
        const legacySnap = await db.ref("users/"+u.uid).get();
        const legacy = legacySnap.val()||{};
        const patch = {};
        if((!v.nick || String(v.nick).trim()==="") && legacy.nick) patch.nick = String(legacy.nick);
        if((!v.role || String(v.role).trim()==="") && legacy.role) patch.role = String(legacy.role);
        if(Object.keys(patch).length){
          patch.nickLocked = true;
...
async function googleSignIn(){
  ...
  try{
    ...
    // If we redirected, the page unloads; if not, clear the flag below in getRedirectResult.finally()
    // (keep it true here so rapid double taps are ignored).
  }
}
  ...
  try{
    refFast.once('value').then((s)=>{
      ...
      if(__chatBootTarget === 0 && !__chatBootDone){
        __chatBootDone = true;
          setMiniLoad('chatMiniLoad','', false);
        setTimeout(()=>{ try{ __switchChatToFull(); }catch(e){} }, 1500);
        if(stopSeq) stopSeq();
      }
    }).catch(()=>{
...
async function loadRent(){
  ...
  const v=s.val()||{};
    ...
    // migrate legacy nick/role from private /users (older builds) into usersPublic
    if((!v.nick || String(v.nick).trim()==="") || (!v.role || String(v.role).trim()==="")){
      try{
        const legacySnap = await db.ref("users/"+u.uid).get();
        const legacy = legacySnap.val()||{};
...
(function initMobileNav(){
  ...
  try{ if(typeof window.closeDrawer==='function') window.closeDrawer(); }catch(e){}
  try{ setMiniLoad('dmMiniLoad'); }catch(e){}
  showView('view-dm');
});
...
async function ensureUserPublicBasics(){
  ...
  if(!nick){
    ...
    patch.nick = (displayName || fromEmail || 'Uživatel').slice(0,28);
  }
  if(!role){
    patch.role = 'seeker';
  }
...
(function(){
  ...
  // --- Overlay closer (drawer, dm overlays, system modals) ---
  window.__closeAllOverlays = function(){
    try{ closeDrawer(); }catch(e){}
    try{ closeDmMobile(); }catch(e){}
    try{ closeDmPopup(); }catch(e){}
    try{ closeModal('sysModal'); }catch(e){}
    try{ closeModal('privModal'); }catch(e){}
    try{ closeModal('payModal'); }catch(e){}
    // remove stray blockers that can freeze UI
    try{ document.body.classList.remove('modal-open'); }catch(e){}
    try{ const b=document.getElementById('modalBackdrop'); if(b) b.style.display='none'; }catch(e){}
  };
