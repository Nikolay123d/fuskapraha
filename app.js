...
function initCookieBanner(){
  ...
  a.onclick=()=>{
    ...
    b.hidden=true;
    toast('Děkujeme. Nastavení uloženo.');
  };
  d.onclick=()=>{
    localStorage.setItem('cookieConsent','0');
    b.hidden=true;
    toast('Bez cookies budou některá nastavení omezená.');
  };
}
...
window.addEventListener('DOMContentLoaded', ()=>{
  showView(localStorage.getItem('lastView')||'view-chat');
  initCookieBanner();
  document.addEventListener('click', ()=>{ if(localStorage.getItem('soundAllowed')!=='1') unlockAudio(); }, {once:true});
});
// City state
function getCity(){
...
function premiumBotOnUserMessage(room, text){
  try{
    const t=(text||'').toLowerCase().trim();
    if(!t) return;
    // allow typed choices: vip / premium / premium+ / premium plus
    let plan=null;
    if(t.includes('vip')) plan='vip';
    else if(t.includes('premium+') || t.includes('premium plus') || t.includes('premium +')) plan='premium_plus';
    ...
    localStorage.setItem(stateKey, JSON.stringify({step:'pay', plan, proof:null}));
    botSay(room, `✅ Vybráno: ${PREMIUM_PLANS[plan].label}. Teď prosím zaplaťte podle QR.`).catch(()=>{});
    ensureBotQR(room).catch(()=>{});
    showPremiumBotPanel(room);
  }catch(e){}
}
...
$('#dmSend')?.addEventListener('click', async ()=>{
  try{
    ...
    if(isPremiumBotRoom(room)) { premiumBotOnUserMessage(room, text||''); }
...
(function(){
  ...
  const btnTopDm = $("#btnTopDm");
  const btnTopFriends = $("#btnTopFriends");
  ...
  async function openProfile(uid){
    try{
      ...
      pmAvatar.src = u.avatar || "./img/default-avatar.svg";
      pmNick.textContent = u.nick || u.name || "User";
      pmCity.textContent = u.city ? ("Město: "+u.city) : "";
      pmInfo.textContent = u.bio || u.about || "";

      const me = window.__me;
      if(!me?.uid){
        pmAddFriend.disabled = true;
        pmMessage.disabled = true;
        pmAddFriend.textContent = "Přidat";
      } else {
        pmAddFriend.disabled = (me.uid===uid);
        pmMessage.disabled = (me.uid===uid);

        const frSnap = await window.db.ref("friendRequests/"+uid+"/"+me.uid).get();
        const fSnap = await window.db.ref("friends/"+me.uid+"/"+uid).get();

        if(fSnap.exists()){
          pmAddFriend.textContent = "Přátelé";
          pmAddFriend.disabled = true;
        } else if(frSnap.exists()){
          pmAddFriend.textContent = "Odesláno";
          pmAddFriend.disabled = true;
        } else {
          pmAddFriend.textContent = "Přidat";
          pmAddFriend.disabled = false;
        }
      }
...
(function(){
  ...
  btnBell?.addEventListener("click", async ()=>{
    ...
    document.querySelector("#tab-friends")?.click();
  });
  btnMe?.addEventListener("click", ()=>document.querySelector("#tab-profile")?.click());
  btnTopDm?.addEventListener("click", ()=>document.querySelector("#tab-dm")?.click());
  btnTopFriends?.addEventListener("click", ()=>document.querySelector("#tab-friends")?.click());

  // Patch avatar in header
  function syncMeAvatar(){
    const me = window.__me;
    if(meAvatar) meAvatar.src = me?.avatar || "./img/default-avatar.svg";
  }
