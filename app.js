...
function setPresence(u){
  // NOTE: do NOT read .info/connected (often causes permission_denied with strict rules).
  // We mark online immediately and rely on onDisconnect().
  try{
    ...
    console.warn('setPresence skipped:', e && e.message ? e.message : e);
  }
}
...
async function loadChat(){
  ...
  // reset feed + detach previous listeners
  if(!window.__chatState) window.__chatState = { off:null, city:null, oldestTs:null, newestTs:null, loading:false };
  const st = window.__chatState;

  const city = getCity() || 'praha';
  st.city = city;
  feed.innerHTML = '';
  st.oldestTs = null;
  st.newestTs = null;

  if(st.off){ try{ st.off(); }catch{} st.off=null; }

  const baseRef = db.ref('messages/'+city).orderByChild('ts');
  const PAGE = 15;

  const renderOne = async (snap, opts={})=>{
    const m = snap.val() || {};
    if(m.deleted) return;
    const by = m.by || m.from || m.uid || 'unknown';
    const ts = (typeof m.ts === 'number') ? m.ts : (typeof m.at === 'number' ? m.at : Date.now());
    const text = m.text || '';
    const img = m.img || m.photo || m.image || '';

    const u = await getUser(by);
    const d = document.createElement('div');
    d.className = 'msg';
    const name = (u?.nick||u?.name||'Uživatel') + (u?.online?'<span class="online"></span>':'');
    d.innerHTML =
      `<div class="ava" data-uid="${esc(by)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}"></div>`+
      `<div class="bubble">`+
        `<div class="name">${name}</div>`+
        (text ? `<div class="text">${esc(text)}</div>` : '')+
        (img ? `<div class="img"><img loading="lazy" src="${esc(img)}"></div>` : '')+
    ...
    if(st.oldestTs === null || ts < st.oldestTs) st.oldestTs = ts;
    if(st.newestTs === null || ts > st.newestTs) st.newestTs = ts;

    // sounds/notifications only for live incoming (not initial/history) and not own messages
    if(!opts.silent){
      const me = auth?.currentUser?.uid;
      if(me && by !== me){
        playSound('chat');
        notify('Nová zpráva', (u?.nick||'Uživatel') + ': ' + (text || ''));
      }
    }
  };
  ...
  const loadInitial = async ()=>{
    try{
      const snap = await baseRef.limitToLast(PAGE).get();
      const items = [];
      snap.forEach(ch => items.push(ch));
      // render oldest -> newest
      for(const ch of items){ await renderOne(ch, {silent:true}); }
      // scroll to bottom
      feed.scrollTop = feed.scrollHeight;
    }catch(e){
      console.error('loadInitial chat failed', e);
      showToast('Chyba načtení chatu', 'error');
    }
  };

  const loadOlder = async ()=>{
    if(st.loading) return;
    if(st.oldestTs === null) return;
    st.loading = true;
    try{
      const prevScrollHeight = feed.scrollHeight;
      const prevScrollTop = feed.scrollTop;

      const snap = await baseRef.endAt(st.oldestTs - 1).limitToLast(PAGE).get();
      const items = [];
      snap.forEach(ch => items.push(ch));
      // prepend newest->oldest so order at top stays correct after multiple prepends
      for(let i=items.length-1;i>=0;i--){
        await renderOne(items[i], {prepend:true, silent:true});
      }
      // maintain scroll position
      const newScrollHeight = feed.scrollHeight;
      feed.scrollTop = (newScrollHeight - prevScrollHeight) + prevScrollTop;
    }catch(e){
      console.error('loadOlder chat failed', e);
    }finally{
      st.loading = false;
    }
  };
  ...
  // live listener from newestTs
  const liveCb = async (snap)=>{
    const m = snap.val() || {};
    const ts = (typeof m.ts === 'number') ? m.ts : (typeof m.at === 'number' ? m.at : 0);
    // ignore anything we already have from initial load
    if(st.newestTs !== null && ts <= st.newestTs) return;
    await renderOne(snap, {silent:false});
    // auto-stick to bottom if user is near bottom
  ...
  const attachLive = ()=>{
    const startTs = (st.newestTs || 0) + 1;
    const q = baseRef.startAt(startTs);
    q.on('child_added', liveCb);
    st.off = ()=> q.off('child_added', liveCb);
  };

  loadInitial().then(attachLive);
}

function initChatOnce(){
  // default to Praha on first visit
  const saved = localStorage.getItem('city');
  if(!saved){
    localStorage.setItem('city','praha');
    const sel = $('#citySelect'); if(sel) sel.value = 'praha';
  }
  // always open chat tab on start if such function exists
  try{ if(typeof openTab === 'function') openTab('chat'); }catch{}
  loadChat();
  $('#citySelect')?.addEventListener('change', loadChat);
}

window.addEventListener('DOMContentLoaded', initChatOnce);
...
async function resolveUidByEmail(email){
  try{
    ...
    // If rules do not allow /emails, do not break UI.
    return null;
  }
}
let DM_REF=null;
async function openDMRoom(a,b){
  ...
  try{
    // set membership: first self, then peer
    await db.ref('privateMembers/'+room+'/'+a).set(true);
    await db.ref('privateMembers/'+room+'/'+b).set(true);
  }catch(e){
    console.error('openDMRoom membership failed', e);
  }
  renderDM(room);
}
...
(function(){
  ...
  async function showUserCard(uid){
    ...
    const isPrem = !!(u.plan||u.tier||u.premium);
    if(plan) plan.style.display = isPrem ? '' : 'none';
