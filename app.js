...
function showView(id){
  ...
  try{
    if(id==='view-chat') setMiniLoad('chatMiniLoad','Načítáme chat…', true);
  ...
  try{
    if(id==='view-chat'){
      if(!window.__chatLoaded){ window.__chatLoaded=true; try{ loadChat(); }catch{} }
        ...
        // Chat already loaded: keep UX consistent by briefly showing loader.
        setTimeout(()=>{ try{ setMiniLoad('chatMiniLoad','', false); }catch(e){} }, 250);
      }
      return;
    }
    if(id==='view-rent'){
  ...
  try{
    ...
    ref.on('value', async (snap)=>{
      try{
        ...
        // update UI
        const t1=document.getElementById('homeOnlineTotal'); if(t1) t1.textContent=String(total);
        const t2=document.getElementById('homeOnlineEmployers'); if(t2) t2.textContent=String(employers);
        const t3=document.getElementById('homeOnlineWorkers'); if(t3) t3.textContent=String(workers);
      }catch(e){}
    });
  }catch(e){}
}
