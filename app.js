...
function notify(title, body, kind){
  try{
    // In browsers, OS notifications only work while the site is open (no Push here).
    if(!('Notification' in window)) return;

    // Respect user's choice
    if(localStorage.getItem('notifAllowed')!=='1') return;

    // Only show OS notification when tab is hidden (avoids spam)
    ...
    // Play sound only if user has unlocked audio
    if(kind==='chat') playSound('chat');
    else if(kind==='dm') playSound('dm');
    else if(kind==='friend') playSound('ok');
  }catch(e){}
}
...
async function loadChat(){
  ...
  // Detach previous listener safely
  if(offChat){ try{ offChat(); }catch(e){} offChat=null; }

  // Keep set of rendered message keys to avoid duplicates (optimistic UI / reconnect)
  window.__chatSeenKeys = window.__chatSeenKeys || new Set();
  const seen = window.__chatSeenKeys;
  // Keep per-city last timestamp so we can subscribe only to NEW after initial load
  window.__chatLastTs = window.__chatLastTs || {};
  const lastTsMap = window.__chatLastTs;

  // Render helper (no sound for initial history)
  const renderOne = async (m, key, isLive)=>{
    if(!m || m.deleted) return;
    if(key && seen.has(key)) return;
    if(key) seen.add(key);

    const u=await getUser(m.by);
    const d=document.createElement('div'); d.className='msg '+((m.by===auth.currentUser?.uid)?'me':'other');
    ...
    d.innerHTML =
      ...
      `<div class="bubble">`+
        `<div class="name" data-uid="${esc(m.by)}">${esc(u?.nick||u?.name||'Uživatel')} ${badges}</div>`+
        (m.text?`<div class="text">${esc(m.text)}</div>`:'')+
        (m.img||m.photo? `<div class="text"><img src="${esc(m.img||m.photo)}"></div>`:'')+
        `<div class="muted">${new Date(m.ts||Date.now()).toLocaleString()}</div>`+
      `</div>`;
    feed.appendChild(d);
    // autoscroll bottom only if user near bottom
    try{
    ...
    if(isLive){
      notify('Nová zpráva', (u?.nick||'Uživatel')+': '+(m.text||''), 'chat');
    }
  };

  (async ()=>{
    // Initial history: deterministic load
    feed.innerHTML='';
    seen.clear();

    let lastTs = 0;
    try{
      const snap = await db.ref('messages/'+city).orderByChild('ts').limitToLast(50).get();
      snap.forEach(s=>{
        const v=s.val()||{};
        lastTs = Math.max(lastTs, (typeof v.ts==='number'?v.ts:0));
      });
      // second pass to render in correct order
      const items=[];
      snap.forEach(s=> items.push({key:s.key, val:s.val()||{}}));
      items.sort((a,b)=>((a.val.ts||0)-(b.val.ts||0)));
      for(const it of items){
        await renderOne(it.val, it.key, false);
      }
    }catch(e){
      console.error('chat history load failed', e);
    }
    lastTsMap[city] = lastTs;

    // Live subscription: only new messages after lastTs (prevents re-playing sounds on reload)
    const ref = db.ref('messages/'+city).orderByChild('ts').startAt((lastTs||0)+1);
    const cb = async (snap)=>{
      const m=snap.val()||{};
      const ts = (typeof m.ts==='number'?m.ts:0);
      if(ts> (lastTsMap[city]||0)) lastTsMap[city]=ts;
      await renderOne(m, snap.key, true);
    };
...
async function renderDM(room){
  ...
  if(DM_REF){ try{ DM_REF.off(); }catch(e){} DM_REF=null; }

  box.innerHTML='';
  showPremiumBotPanel(room);

  window.__dmSeenKeys = window.__dmSeenKeys || {};
  const seen = window.__dmSeenKeys[room] = window.__dmSeenKeys[room] || new Set();
  ...
  const renderOne = async (m, key, isLive)=>{
    ...
    const u=await getUser(m.by);
    const el=document.createElement('div'); el.className='msg '+((m.by===auth.currentUser?.uid)?'me':'other');
    el.innerHTML =
      `<div class="ava" data-uid="${esc(m.by)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}"></div>`+
    ...
    if(isLive){
      notify('Nová DM', (u.nick||'Uživatel')+': '+(m.text||''), 'dm');
    }
  };

  // Initial history (no sounds)
  let lastTs=0;
  try{
    ...
    console.error('dm history load failed', e);
  }

  // Live subscription only for new
  const ref = db.ref('privateMessages/'+room).orderByChild('ts').startAt((lastTs||0)+1);
  const cb = async (snap)=>{
    const m=snap.val()||{};
    const ts=(typeof m.ts==='number'?m.ts:0);
...
// Realtime subscriptions for friends (requests + accepted list)
let FRIEND_REQ_REF=null;
let FRIEND_LIST_REF=null;
function subscribeFriendsRealtime(meUid){
  if(!meUid) return;
  try{ FRIEND_REQ_REF?.off(); }catch(e){}
  try{ FRIEND_LIST_REF?.off(); }catch(e){}
  FRIEND_REQ_REF = db.ref('friendRequests/'+meUid);
  FRIEND_LIST_REF = db.ref('friends/'+meUid);

  const reload = ()=>{ try{ loadFriendsUI(); }catch(e){} };

  // value listener is simplest & reliable for MVP
  FRIEND_REQ_REF.on('value', reload);
  FRIEND_LIST_REF.on('value', reload);

  // also show badge in topbar (if present)
  FRIEND_REQ_REF.on('value', (s)=>{
    try{
      const v=s.val()||{};
...
auth.onAuthStateChanged(async (u)=>{
  ...
  if(u){
    ...
    setPresence(u);
    await refreshMe();
    try{ subscribeFriendsRealtime(u.uid); }catch(e){}
