...
// Basic device hint (used to pick Redirect vs Popup for Google sign-in)
const IS_MOBILE = /Android|iPhone|iPad|iPod|Opera Mini|IEMobile/i.test(navigator.userAgent);
...
(function(){
  ...
  if(t && t.id==='wallCamera'){
    ...
    r.onload=()=>{
      ...
      // Keep a local fallback, but the source of truth is Firebase (same for all devices)
...
// Auth (login) background sync (separate from site wallpaper)
function applyAuthBg(url){
  const left = document.querySelector('#modalAuth .auth-left');
  if(left) left.style.backgroundImage = url ? `url(${url})` : '';
}
db.ref('settings/authBg').on('value', (s)=>{
  const url = s.val();
  if(url){
    try{ localStorage.setItem('authBg', url); }catch(_){}
  }
  applyAuthBg(url || (localStorage.getItem('authBg')||''));
});
...
// Handle Google redirect return (mobile). Some browsers need explicit handling
// to close the auth modal and to create the public profile record.
try{
  auth.getRedirectResult().then(async (res)=>{
    if(res && res.user){
      await ensureUserPublic(res.user, {
        nick: res.user.displayName || '',
        avatar: res.user.photoURL || window.DEFAULT_AVATAR
      });
      // Ask to complete required fields (nick/role) if missing
      const pub = (await db.ref('usersPublic/'+res.user.uid).get()).val() || {};
      if(!pub.nick || !pub.role){
        openModalAuth('complete');
      }else{
        closeModalAuth();
      }
    }
...
auth.onAuthStateChanged(async (u)=>{
  ...
  document.querySelectorAll(".adm-only, .admin-only").forEach(x=> x.style.display = (u && isAdmin(u)) ? "" : "none");
...
// Extra wiring: admin wallpapers + auth background (kept separate)
window.addEventListener('DOMContentLoaded', ()=>{
  const wallBtn = document.getElementById('wallCam');
  const wallFile = document.getElementById('wallFile');
  if(wallBtn && wallFile){
    wallBtn.addEventListener('click', ()=>wallFile.click());
    wallFile.addEventListener('change', async ()=>{
      try{
        const u = auth.currentUser;
        if(!u || !isAdmin(u)) return toast('Pouze admin');
        const f = wallFile.files && wallFile.files[0];
        if(!f) return;
        const city = document.getElementById('citySel')?.value || 'Praha';
        toast('Nahrávám pozadí…');
        const ref = stg.ref(`wallpapers/${city.toLowerCase()}_${Date.now()}.jpg`);
        await ref.put(f);
        const url = await ref.getDownloadURL();
        await db.ref(`settings/wallpaper/${city}`).set(url);
        try{ localStorage.setItem('wall_'+city, url); }catch(_){}
        toast('Pozadí bylo změněno');
      }catch(e){
        console.error(e); toast('Chyba při nahrávání');
      }finally{
        wallFile.value='';
      }
    });
  ...
  const authBgBtn = document.getElementById('authBgCamBtn');
  const authBgFile = document.getElementById('authBgFile');
  if(authBgBtn && authBgFile){
    authBgBtn.addEventListener('click', ()=>authBgFile.click());
    authBgFile.addEventListener('change', async ()=>{
      try{
        const u = auth.currentUser;
        if(!u || !isAdmin(u)) return toast('Pouze admin');
        const f = authBgFile.files && authBgFile.files[0];
        if(!f) return;
        toast('Nahrávám pozadí přihlášení…');
        const ref = stg.ref(`authbg/auth_${Date.now()}.jpg`);
        await ref.put(f);
        const url = await ref.getDownloadURL();
        await db.ref('settings/authBg').set(url);
        try{ localStorage.setItem('authBg', url); }catch(_){}
        toast('Pozadí přihlášení bylo změněno');
      }catch(e){
        console.error(e); toast('Chyba při nahrávání');
      }finally{
        authBgFile.value='';
  ...
  // Chat input: extra defense against browser autofill
