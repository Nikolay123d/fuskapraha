...
async function initPushForUser(u){
  try{
    ...
    // Guard: Firebase expects a Base64URL-encoded VAPID public key.
    // If the key is malformed, getToken() may throw (atob invalid base64).
    if(!/^[A-Za-z0-9_-]{40,}$/.test(vapidKey)){
      console.warn('[FCM] Invalid WEB_PUSH_VAPID_KEY format (must be Base64URL).');
      return;
    }
    // register SW once
    if(!__swReg){
...
async function loadChat(){
  ...
  // Initial chat preloader (center) + rotating tips below it.
  setMiniLoad('chatMiniLoad','Načítáme chat…', true);
  try{ startMiniTips('chatMiniTip'); }catch(e){}
...
// --- DM threads / inbox ---
  ...
  async function loadDmThreads(){
    ...
    try{
      ...
      // Inbox meta (room list) — requires rules for /inboxMeta/{uid}
      ...
      console.warn('[DM] loadDmThreads failed', e);
    } finally {
      try{ if(stopSeq) stopSeq(); }catch(e){}
...
async function _tryAcquireBotHost(){
  ...
  try{
    ...
    finally{ try{ if(stopSeq) stopSeq(); }catch(e){} try{ if(ml) ml.style.display='none'; }catch(e){} }
  ...
  document.addEventListener('click', (e)=>{
    ...
    if(t){
      setTimeout(()=>{
        try{
          if(!isAdminUser(auth.currentUser)) return;
          Promise.resolve(loadAdminRequests()).catch(console.warn);
          Promise.resolve(loadBotsUI()).catch(console.warn);
          Promise.resolve(loadBotInboxUI()).catch(console.warn);
          Promise.resolve(loadBotProfiles()).catch(console.warn);
