...
// Helpers: admin list + bot UID detection
function getAdminUids(){
  const a = Array.isArray(window.ADMIN_UIDS) && window.ADMIN_UIDS.length ? window.ADMIN_UIDS.slice() : [];
  if(window.ADMIN_UID && !a.includes(window.ADMIN_UID)) a.unshift(window.ADMIN_UID);
  return a.filter(Boolean);
}
function isBotUid(uid){
  uid = String(uid||'');
  return uid === 'bot_premium' || uid.startsWith('bot_');
}
...
async function ensurePremiumBotPublic(){
  try{
    await db.ref('usersPublic/'+PREMIUM_BOT_UID).update({
      nick: 'Bot — Privilegia',
      avatar: './img/rose.svg',
      role: 'bot',
      plan: 'bot',
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  // Google redirect-only login: finalize sign-in after returning from Google
  try{ handleGoogleRedirectResultOnce(); }catch(e){}
...
// Ensure premium bot public profile exists (for DM + admin inbox UI)
try{ ensurePremiumBotPublic().catch(()=>{}); }catch(e){}
...
function openModalAuth(mode='login'){
  ...
  // Close any other open modals to avoid invisible backdrops blocking clicks
  try{ document.querySelectorAll('.modal').forEach(x=>{ if(x && !x.hidden && x.id!=='modalAuth') x.hidden = true; }); }catch(e){}

  document.body.classList.add('auth-open');
  // also lock background via the generic modal-open rule set
  ...
  // pushState so Android back button closes auth first
  try{ history.pushState({modal:'modalAuth'}, '', location.href); }catch(e){}
}
...
// Policy: we use signInWithRedirect() on all platforms for maximum stability.
// The redirect result MUST be handled on page load, otherwise it looks like
// "Google opened and returned, but login didn't happen".
let __GOOGLE_AUTH_IN_FLIGHT__ = false;
async function googleSignIn(){
  if(__GOOGLE_AUTH_IN_FLIGHT__) return;
  __GOOGLE_AUTH_IN_FLIGHT__ = true;

  // Respect "remember me" for redirect login as well
  try{
  ...
  // Optional: request profile/email explicitly
  try{ provider.addScope('email'); provider.addScope('profile'); }catch{}
  await auth.signInWithRedirect(provider);
}

async function handleGoogleRedirectResultOnce(){
  // Prevent duplicate execution (some browsers re-run scripts on BFCache restore)
  try{
    if(sessionStorage.getItem('mk_google_redirect_handled') === '1') return;
    sessionStorage.setItem('mk_google_redirect_handled', '1');
  }catch{}

  try{
    const res = await auth.getRedirectResult();
    if(res && res.user){
      // Ensure public profile exists (nick fallback to displayName)
      const u = res.user;
      await ensureUserPublic(u, { nick: u.displayName || '' });
      try{ closeModalAuth(); }catch{}
      toast('Přihlášení přes Google: OK');
    }
  }catch(e){
    // Make the common production failure obvious
    const code = String(e && e.code || '');
    if(code === 'auth/unauthorized-domain'){
      toast('Firebase: nepovolená doména (Authorized domains)');
    }else if(code){
      toast(code);
    }else{
    ...
    __GOOGLE_AUTH_IN_FLIGHT__ = false;
  }
}
...
async function ensureUserPublic(u, extra={}){
  ...
  // Robust nick fallback (Google users may not have displayName)
  const emailNick = (u.email||'').split('@')[0] || '';
  const nickFallback = (extra.nick || u.displayName || cur.nick || emailNick || 'Uživatel').trim();

  const merged = {
    email: u.email || cur.email || '',
    nick: cur.nick || nickFallback,
    role: cur.role || extra.role || 'job',
...
// Convenience wrapper used by FAB / topbar
function openAuth(){
  openModalAuth('login');
}
...
async function openDMRoom(a,b){
  ...
  // Open existing DM room by id without rewriting members.
  // Useful for admin viewing bot/user rooms from inbox.
  function openDMByRoomId(room, peerHint){
    currentDmRoom = room;
    const meUid = auth.currentUser ? auth.currentUser.uid : null;
    currentDmPeerUid = peerHint || null;
    if(currentDmPeerUid){
      getUser(currentDmPeerUid).then(u=>{ window._lastDmPeerPublic = u || null; try{ const t=document.getElementById('dmMobileTitle'); if(t) t.textContent = u && u.nick ? ('DM: '+u.nick) : 'Konverzace'; }catch(e){} }).catch(()=>{});
    }
    renderDM(room);
  }
  window.openDMByRoomId = openDMByRoomId;
...
$('#dmSend')?.addEventListener('click', async ()=>{
  ...
  try{
    ...
    // If user writes to a bot (including premium bot), forward a copy to admin inbox.
    // This makes sure admin sees bot messages even when the admin site is closed.
    if(isBotUid(currentDmPeerUid)){
      const botUid = currentDmPeerUid;
      const admins = getAdminUids();
      // Try to add admins as members so they can open the DM if needed.
      for(const a of admins){
        if(!a || a===me.uid) continue;
        try{ await db.ref('privateMembers/'+room+'/'+a).set(true); }catch(e){}
      }
      // bot profile snapshot for nicer admin UI
      let botNick='Bot', botAvatar=window.DEFAULT_AVATAR;
      try{
        if(botUid===PREMIUM_BOT_UID){ botNick='Bot — Privilegia'; botAvatar='./img/rose.svg'; }
        else{
          const b = await _getBotConfigByUid(botUid);
          if(b && b.nick) botNick = b.nick;
          if(b && b.avatar) botAvatar = b.avatar;
        }
      }catch(e){}
      const payload = { ts, botUid, botNick, botAvatar, from: me.uid, room, text: text||'', img: img||'' };
      for(const a of admins){
        if(!a) continue;
        try{ await db.ref('botsInbox/'+a+'/'+botUid).push(payload); }catch(e){}
      }
    }
...
(function(){
  ...
  window.addEventListener('popstate', ()=>{
    // Close regular modal stack first
    if(__openModals.size){
      const last = Array.from(__openModals).slice(-1)[0];
      closeModal(last);
      return;
    }
    // Also close auth modal (it uses its own open/close helpers)
    try{ if(document.body.classList.contains('auth-open')) closeModalAuth(); }catch(e){}
  });
  ...
  async function loadFriendsUI(){
    ...
    // badges (mobile top + tabs + inline)
    const badgeTop = document.getElementById('friendsBadgeTop');
    const badgeTab = document.getElementById('friendsBadgeTab');
    const badge2 = document.getElementById('friendsBadgeInline');
    const n = rqUids.length;
    if(badgeTop){ badgeTop.style.display = n ? '' : 'none'; badgeTop.textContent = n ? String(n) : ''; }
    if(badgeTab){ badgeTab.textContent = n ? `(${n})` : ''; }
  ...
  try{
    ...
    try{
      if(isBotUid(toUid)){
        ...
        if(toUid===PREMIUM_BOT_UID){ try{ await ensurePremiumBotPublic(); }catch(e){} }
...
// --- DM threads / inbox ---
  ...
  async function loadAdminRequests(){
    ...
    // (Optional) payment requests list lives in adminPremiumRequests in this build.
  ...
  async function startBotDmEngine(){
    ...
    db.ref('botRooms').orderByChild('ts').limitToLast(200).on('child_added', async (snap)=>{
      ...
      ref.on('child_added', async (ms)=>{
        ...
        try{
          let botNick = (botUid===PREMIUM_BOT_UID) ? 'Bot — Privilegia' : 'Bot';
          let botAvatar = (botUid===PREMIUM_BOT_UID) ? './img/rose.svg' : window.DEFAULT_AVATAR;
          try{
            if(botUid!==PREMIUM_BOT_UID){
              const b = await _getBotConfigByUid(botUid);
              if(b && b.nick) botNick = b.nick;
              if(b && b.avatar) botAvatar = b.avatar;
            }
...
const renderFor = async (botUid)=>{
  ...
  // Shared state for inbox UI
  window.__botInboxState = window.__botInboxState || { botUid:null, selectedUserUid:null, data:null };
  const state = window.__botInboxState;
  state.botUid = botUid;
  state.selectedUserUid = null;
  state.data = null;

  // Wire once: event delegation for buttons
  if(!box.__botInboxWired){
    box.__botInboxWired = true;
    box.addEventListener('click', async (e)=>{
      ...
      const bot = (window.__botInboxState && window.__botInboxState.botUid) ? window.__botInboxState.botUid : '';

      if(act==='back'){
        window.__botInboxState.selectedUserUid = null;
        try{ await window.__botInboxRedraw(); }catch(err){}
        return;
      }
      if(act==='thread'){
        window.__botInboxState.selectedUserUid = uid || null;
        try{ await window.__botInboxRedraw(); }catch(err){}
        return;
      }
      if(act==='open'){
        if(!room || !bot) return;
        try{ closeModal('modalBotInbox'); }catch(err){}
        try{ openDMByRoomId(room, bot); }catch(err){ console.warn(err); }
        try{ showView('view-dm'); }catch(err){}
        return;
      }
      if(act==='del'){
        if(!mid || !bot) return;
        try{ await db.ref('botsInbox/'+adminUid+'/'+bot+'/'+mid).remove(); }catch(err){}
        return;
      }
    });
  }
  ...
  const redraw = async ()=>{
    const vv = state.data || {};
    const entries = Object.entries(vv).map(([k,it])=>({id:k, ...(it||{})}));
    ...
    // Bot header
    let botU=null;
    try{ botU = await getUser(botUid); }catch(e){}
    const head=document.createElement('div');
    head.className='msg';
    head.innerHTML = `<div class="ava"><img src="${esc(botU?.avatar||window.DEFAULT_AVATAR)}"></div>
      <div class="bubble" style="width:100%">
        <div class="name"><b>${esc(botU?.nick||botUid)}</b> <span class="muted">(${esc(botUid)})</span></div>
        <div class="muted">Inbox: dialogy uživatelů v rámci tohoto bota</div>
      </div>`;
    ...
    if(entries.length===0){
      ...
      em.className='muted';
      em.textContent='Zatím žádné zprávy.';
      box.appendChild(em);
      return;
    }

    // Build threads by user
    const threads = new Map();
    for(const m of entries){
      const uid = m.from || '';
      if(!uid) continue;
      if(!threads.has(uid)) threads.set(uid, {uid, lastTs:0, lastText:'', lastImg:'', room:''});
      const th = threads.get(uid);
      const ts = +m.ts||0;
      if(ts > th.lastTs){
        th.lastTs = ts;
        th.lastText = m.text || '';
        th.lastImg = m.img || '';
        th.room = m.room || dmKey(uid, botUid);
      }
    }
    const threadArr = Array.from(threads.values()).sort((a,b)=>b.lastTs-a.lastTs);

    const selected = state.selectedUserUid;
    if(!selected){
      // Thread list view
      for(const th of threadArr){
        const fromU = await getUser(th.uid).catch(()=>null);
        const row=document.createElement('div');
        row.className='msg';
        const preview = th.lastText ? th.lastText : (th.lastImg ? '[foto]' : '');
        row.innerHTML = `
          <div class="ava" data-uid="${esc(th.uid)}"><img src="${esc(fromU?.avatar||window.DEFAULT_AVATAR)}"></div>
          <div class="bubble" style="width:100%">
            <div class="name"><b>${esc(fromU?.nick||'Uživatel')}</b> <span class="muted">${new Date(th.lastTs||0).toLocaleString()}</span></div>
            <div class="text">${esc(preview).slice(0,160)}</div>
            <div class="actions">
              <button data-act="thread" data-uid="${esc(th.uid)}" type="button">Dialog</button>
              <button data-act="open" data-room="${esc(th.room)}" type="button">Otevřít DM</button>
            </div>
          </div>`;
        box.appendChild(row);
      }
    ...
    const uPub = await getUser(selected).catch(()=>null);
    const room = dmKey(selected, botUid);

    const top=document.createElement('div');
    top.className='row';
    top.style.cssText='justify-content:space-between;align-items:center;margin:10px 0;gap:10px;flex-wrap:wrap';
    top.innerHTML = `
      <div><b>Dialog:</b> ${esc(uPub?.nick||'Uživatel')} <span class="muted">(${esc(selected)})</span></div>
      <div class="row" style="gap:8px">
        <button data-act="back" type="button" class="ghost">← Zpět</button>
        <button data-act="open" data-room="${esc(room)}" type="button">Otevřít DM</button>
      </div>`;
    box.appendChild(top);

    const filtered = entries.filter(m=>m.from===selected).sort((a,b)=>(+a.ts||0)-(+b.ts||0));
    for(const m of filtered){
      const el=document.createElement('div');
      el.className='msg';
      el.innerHTML = `
        <div class="ava" data-uid="${esc(m.from||'')}"><img src="${esc(uPub?.avatar||window.DEFAULT_AVATAR)}"></div>
        <div class="bubble" style="width:100%">
          <div class="name"><b>${esc(uPub?.nick||'Uživatel')}</b> <span class="muted">→ ${esc(botU?.nick||'Bot')}</span> <span class="muted">${new Date(m.ts||0).toLocaleString()}</span></div>
          ${m.text?`<div class="text">${esc(m.text)}</div>`:''}
          ${m.img?`<div class="text"><img src="${esc(m.img)}"></div>`:''}
          <div class="actions">
            <button data-act="del" data-mid="${esc(m.id)}" type="button" class="danger">Smazat</button>
          </div>
        </div>`;
      box.appendChild(el);
    }
  ...
  // expose redraw for button actions
  window.__botInboxRedraw = redraw;

  // subscribe
  __BOT_INBOX_REF = db.ref('botsInbox/'+adminUid+'/'+botUid).orderByChild('ts').limitToLast(200);
  __BOT_INBOX_REF.on('value', (snap)=>{
    state.data = snap.val() || {};
    redraw().catch(console.warn);
  });
};
...
async function botTick(){
  ...
  async function loadBotInboxUI(){
    const me=auth.currentUser; if(!me) return;
    const box=document.getElementById('botInboxFeedModal') || document.getElementById('botInboxFeed');
    if(!box) return;
    box.innerHTML = '<div class="muted">Načítám…</div>';
...
function initFabMenu(){
  ...
  // Safety fallback: if openAuth ever fails, at least show the real modal.
  loginBtn?.addEventListener('click', ()=>{ close(); try{ openAuth(); }catch(e){ const m=document.getElementById('modalAuth'); if(m) m.hidden=false; } });
  ...
  // Premium / privileges modal
  premBtn?.addEventListener('click', ()=>{ close(); try{ openPremiumBot(); }catch(e){ console.warn(e); toast('Privilegia nejsou připravena'); } });
