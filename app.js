...
// --- Mini loader helpers ---
// Mini-loaders (overlay spinners for each screen)
function showMiniLoad(id, label){
  const el = document.getElementById(id);
  if(!el) return;
  try{
    const t = el.querySelector('.mini-text');
    if(t && label) t.textContent = label;
    el.classList.add('show');
  }catch(e){}
}
function hideMiniLoad(id){
  const el = document.getElementById(id);
  if(!el) return;
  try{ el.classList.remove('show'); }catch(e){}
}

// ensure cookie banner is always clickable even when other modals/overlays disable pointer events
let __cookiePrevModalOpen = null;
function __cookieForceClickable(on){
  try{
    if(on){
      if(__cookiePrevModalOpen===null) __cookiePrevModalOpen = document.body.classList.contains('modal-open');
      // Some UI patterns disable pointer-events on #views when modal-open is present.
      // Cookie consent must be clickable, so temporarily disable modal-open.
      document.body.classList.remove('modal-open');
      // Also remove inert/aria-hidden that may block interactions.
      ...
      if(__cookiePrevModalOpen===true) document.body.classList.add('modal-open');
      __cookiePrevModalOpen = null;
    }
  }catch(e){}
}
...
// Wrap common loaders to show/hide mini spinners without changing core logic
(function(){
  const wrapAsync = (fnName, showId, label)=>{
    const orig = window[fnName];
    if(typeof orig !== 'function') return;
    window[fnName] = async function(...args){
      try{ showMiniLoad(showId, label); }catch(e){}
      try{ return await orig.apply(this, args); }
      finally{ try{ hideMiniLoad(showId); }catch(e){} }
    };
  };
  wrapAsync('loadFriends', 'miniFriends', 'Načítáme přátele…');
  wrapAsync('loadDmThreads', 'miniDM', 'Načítáme soukromé zprávy…');
  wrapAsync('loadMembers', 'miniMembers', 'Načítáme účastníky…');
  wrapAsync('loadChat', 'miniChat', 'Načítáme chat…');
})();
