...
(function(){
  ...
  if(t && t.id==='wallCamera'){
    ...
    r.onload=()=>{
      ...
      db.ref('siteHelperConfig/wallpapers/'+c).set({url:data, ts:Date.now(), by: (auth.currentUser?auth.currentUser.uid:null)});
...
function startWallpaperSync(){
  try{
    ...
    const city=getCity();
    const ref=db.ref('siteHelperConfig/wallpapers');
    const apply=(data)=>{
      const mainCity = data?.main?.[city];
      const mainDef  = data?.main?._default;
      const authUrl  = data?.auth;
      const wallUrl = (typeof mainCity==='string' && mainCity) ? mainCity : (typeof mainDef==='string' ? mainDef : null);
      if(wallUrl){
        try{ localStorage.setItem('wall_'+city, wallUrl); }catch{}
        document.documentElement.style.setProperty('--wall', `url('${wallUrl}')`);
      }
      if(typeof authUrl==='string' && authUrl){
        try{ localStorage.setItem('authwall', authUrl); }catch{}
        document.documentElement.style.setProperty('--authwall', `url('${authUrl}')`);
      }
    };
    const handler=(snap)=> apply(snap.val()||{});
...
async function loadChat(){
  ...
  const city=getCity();
  const ref=db.ref('chat/'+city).limitToLast(50);
  const cb=async (snap)=>{
    const m=snap.val()||{};
    if(m.deleted) return;
...
$('#sendBtn')?.addEventListener('click', async ()=>{
  try{
    ...
    await db.ref('chat/'+getCity()).push(m);
    // cleanup
    $('#msgText').value='';
    $('#msgPhoto').value='';
    pendingChatImg=null;
    ...
    console.error(e);
    toast('Chyba odeslání. Zkontrolujte připojení a pravidla DB.');
    playSound('err');
  }
});
...
// === USER PROFILE MODAL (click on avatar/name) ===
async function openUserProfile(uid){
  try{
    ...
    if(!m) return;
    m.querySelector('[data-up="nick"]').textContent = u.nick || u.name || 'Uživatel';
    m.querySelector('[data-up="uid"]').textContent = uid;
    const ava = m.querySelector('[data-up="avatar"]');
    ava.src = u.avatar || window.DEFAULT_AVATAR;
    m.querySelector('[data-up="city"]').textContent = u.city || '';
    m.querySelector('[data-up="about"]').textContent = u.about || '';
    // buttons
    const btnDm = m.querySelector('#userDmBtn');
    btnDm.onclick = ()=>{
      if(!me){ openModalAuth('login'); return; }
      document.getElementById('tabDm')?.click();
      // try open room directly
      openDMRoom(me.uid, uid);
      m.hidden = true;
    };
    const btnFriend = m.querySelector('#userFriendBtn');
    btnFriend.onclick = ()=>{
      if(!me){ openModalAuth('login'); return; }
...
auth.onAuthStateChanged(async (u)=>{
  ...
  1900(".adm-only").forEach(x=> x.style.display = (u && isAdmin(u)) ? "" : "none");
...
(function(){
  ...
  async function botTick(){
    ...
    for(const [id,b] of Object.entries(v)){
      ...
      await db.ref('chat/'+city).push({by:botUid, ts:now, text:b.text||'', img:b.img||''});
...
// === Admin: change AUTH background from login modal (camera button) ===
(function initAuthBgAdmin(){
  const ADMIN_UID = 'VrP5IzhgxmT0uKWc9UWlXSCe6nM2';
  document.addEventListener('click', async (e)=>{
    if(e.target.closest('#authCamBtn')){
      const u=auth.currentUser;
      if(!u || u.uid!==ADMIN_UID){ toast('Jen admin může měnit pozadí.'); return; }
      document.getElementById('authBgFile')?.click();
    }
  });
  document.getElementById('authBgFile')?.addEventListener('change', async (e)=>{
    try{
      const u=auth.currentUser;
      if(!u || u.uid!==ADMIN_UID) return;
      const f=e.target.files && e.target.files[0];
      if(!f) return;
      toast('Nahrávám pozadí…');
      const ref=stg.ref('backgrounds/auth.jpg');
      await ref.put(f);
      const url=await ref.getDownloadURL();
      await db.ref('siteHelperConfig/wallpapers/auth').set(url);
      await db.ref('siteHelperConfig/wallpapers/_v').set(Date.now());
      document.documentElement.style.setProperty('--authwall', `url('${url}')`);
      toast('Hotovo');
      playSound('ok');
    }catch(err){
      console.error(err);
      toast('Chyba nahrávání pozadí.');
      playSound('err');
    }finally{
