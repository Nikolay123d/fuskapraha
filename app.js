...
window.FCM_PUBLIC_VAPID_KEY = window.FCM_PUBLIC_VAPID_KEY || 'PASTE_PUBLIC_VAPID_KEY_HERE';
...
(function(){
  ...
  window.addEventListener('DOMContentLoaded', ()=>{ apply(); startWallpaperSync(); startAuthWallpaperSync(); startChatWallpaperSync(); startDmWallpaperSync(); startProfileWallpaperSync(); startSoundsSync(); });
...
async function initPushForUser(u){
  try{
    ...
    const vapidKey = String(window.WEB_PUSH_VAPID_KEY||'').trim();
...
async function loadMembers(){
  ...
  // Listen to last active users (presence)
...
function initFabMenu(){
  ...
  botsBtn?.addEventListener('click', ()=>{ close(); try{ openAdmin(); setTimeout(()=>{ document.getElementById('botList')?.scrollIntoView({behavior:'smooth', block:'start'}); }, 200); }catch(e){ console.warn(e); } });
  botInboxBtn?.addEventListener('click', ()=>{ close(); try{ openAdmin(); setTimeout(()=>{ document.getElementById('botList')?.scrollIntoView({behavior:'smooth', block:'start'}); }, 200); toast('Otevři sekci „Boti“ v Admin panelu.'); }catch(e){ console.warn(e); } });
  ...
  refreshAdminBtn();
  window.addEventListener('admin-changed', refreshAdminBtn);
}
