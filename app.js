...
function startMiniTipsElement(el){
  if(!el) return;
  const TIPS = window.__TIPS__ || [
    "Tip: Klikni na nick nebo avatar a otevřeš profil.",
    "Věděl jsi, že Premium může mít vlastní boty na inzerci?",
    "Tip: Obálka = soukromé zprávy. Kolokol = upozornění.",
    "Tip: Mapu otevři až když ji potřebuješ — start bude rychlejší."
  ];
  let i = 0;
  el.textContent = TIPS[0];
  if(el.__tipTimer) clearInterval(el.__tipTimer);
  el.__tipTimer = setInterval(()=>{ i=(i+1)%TIPS.length; el.textContent=TIPS[i]; }, 3000);
}

// Never let a single permission_denied / network hiccup break the whole UI.
// We keep errors visible in console, but prevent startup freezes caused by uncaught promise rejections.
window.addEventListener('unhandledrejection', (e)=>{
  try{ console.warn('[MAKAME] unhandledrejection', e?.reason||e); }catch(_){ }
  try{ e.preventDefault && e.preventDefault(); }catch(_){ }
});
window.addEventListener('error', (e)=>{
  try{ console.warn('[MAKAME] error', e?.message||e); }catch(_){ }
});
...
function showView(id){
  ...
  try{
    ...
    if(id==='view-dm'){
      ...
      if(!window.__dmThreadsLoaded){
        window.__dmThreadsLoaded=true;
        try{
          if(typeof window.loadDmThreads==='function') window.loadDmThreads();
          else if(typeof window.loadDMThreads==='function') window.loadDMThreads();
        }catch(e){}
      }
      return;
    }
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  // Idle prefetch: warm-up other tabs after chat is loaded
  setTimeout(()=>{
    try{ if(auth.currentUser && !window.__dmThreadsLoaded) loadDmThreads(); }catch(e){}
    try{ if(auth.currentUser && !window.__friendsLoaded) loadFriends(); }catch(e){}
    try{ if(!window.__membersLoaded) loadMembers(); }catch(e){}
    try{ listenOnlineCounts(); }catch(e){}
  }, 1500);
});
...
async function loadChat(){
  ...
  const tipEl = ml ? ml.querySelector('.mini-tip') : null;
  try{ if(ml){ ml.style.display='block'; const st=ml.querySelector('.mini-status'); if(st) st.textContent='Načítáme chat…'; } }catch(e){}
  try{ if(tipEl) startMiniTipsElement(tipEl); }catch(e){}
...
function openPremium(){ return openPremiumBot(); }
...
function loadPoi(){
  ...
  POI_REF.on('child_added', async snap=>{
    ...
    try{
      ...
      m.bindPopup(`<b>${esc(v.title||'Bod')}</b><br>${esc(v.type||'')}${v.desc?'<br>'+esc(v.desc):''}${v.photoUrl?'<br><img src="'+v.photoUrl+'" style="max-width:220px;border-radius:10px;margin-top:6px">':''}`);
    }catch{}
  });
  POI_REF.on('value', snap=>{
    const list=[];
    snap.forEach(ch=>{ const v=ch.val()||{}; v.id=ch.key; list.push(v); });
    renderPoiList(city, list);
  }, err=>{ console.warn('poi list failed', err); });
  ...
  MAP.on('click', async (e)=>{
    ...
    if(!isAdmin() && !mod) return;
    try{ window.__onMapClickFill && window.__onMapClickFill(e.latlng.lat, e.latlng.lng); }catch(e){}
    toast('Souřadnice vloženy do formuláře');
  });
}
...
let NOTIF_REF=null;
  function listenNotifications(){
    ...
    NOTIF_REF.on('value', async snap=>{
      ...
      const bellCount = ids.length;
      if(bellB){ bellB.textContent = bellCount?String(bellCount):''; bellB.style.display = bellCount?'inline-flex':'none'; }
  ...
  const stopSeq = startMiniSequence('notifMiniLoad', [
    ...
    'Synchronizace…'
      ...
      try{ stopSeq && stopSeq(); }catch(e){}
      try{ setMiniLoad('notifMiniLoad','', false); }catch(e){}
...
async function startDM(toUid, opts={}){
  ...
  showView('view-dm');
  // Mobile: open conversation overlay immediately
  try{ if(typeof window.openDmMobile==='function') window.openDmMobile(); }catch(e){}
...
// --- DM threads / inbox ---
  ...
  async function loadDmThreads(){
    const ml = document.getElementById('dmMiniLoad');
    const tipEl = document.getElementById('dmMiniTip');
    let stopSeq = null;
    try{ if(ml){ ml.style.display='block'; const st=ml.querySelector('.mini-status'); if(st) st.textContent='Načítáme konverzace…'; } }catch(e){}
    try{ if(tipEl) startMiniTipsElement(tipEl); }catch(e){}
    try{ stopSeq = startMiniSequence('dmMiniLoad', ['Načítáme konverzace…','Synchronizace…','Hotovo…'], 650); }catch(e){}
    try{
    ...
    if(label && !currentDmRoom) label.textContent='Osobní zprávy';
      ...
      try{ stopSeq && stopSeq(); }catch(e){}
      try{ if(ml) ml.style.display='none'; }catch(e){}
    }
  }

  // expose both spellings (historical)
...
// === Online counters (total / employers / workers) ===
let _onlineStatsRef=null;
function listenOnlineCounts(){
  try{
    if(_onlineStatsRef){ _onlineStatsRef.off(); }
  }catch(e){}
  _onlineStatsRef = db.ref('onlineStats/public');
  const el = document.getElementById('onlineCounts');
  const apply = (v)=>{
    if(!el) return;
    const t = v?.total||0;
    const e = v?.employers||0;
    const w = v?.workers||0;
    el.textContent = `${t} • ${e} • ${w}`;
  };
  _onlineStatsRef.on('value', (snap)=>{
    const v = snap.val();
    if(v && typeof v==='object'){ apply(v); }
    else { // fallback: show only total online from presence if stats not deployed
      try{ listenOnlineCountsClientFallback(); }catch(e){}
    }
  }, (err)=>{
    console.warn('onlineStats read failed', err);
    try{ listenOnlineCountsClientFallback(); }catch(e){}
  });
}

// Fallback: count from presence + usersPublic.role (client-side, heavier; used only if functions not deployed)
let _fallbackPresenceRef=null;
let _fallbackUsersRef=null;
function listenOnlineCountsClientFallback(){
  if(window.__onlineFallbackStarted) return;
  window.__onlineFallbackStarted = true;
  const el = document.getElementById('onlineCounts');
  if(!el) return;
  const online = new Map(); // uid -> true
  const roles  = new Map(); // uid -> role
  const recompute = ()=>{
    let total = 0, employers=0, workers=0;
    online.forEach((on, uid)=>{
      if(!on) return;
      total++;
      const r = roles.get(uid);
      if(r==='employer') employers++;
      else if(r==='worker') workers++;
    });
    el.textContent = `${total} • ${employers} • ${workers}`;
  };
  _fallbackPresenceRef = db.ref('presence');
  _fallbackPresenceRef.on('child_added', s=>{ online.set(s.key, true); recompute(); });
  _fallbackPresenceRef.on('child_removed', s=>{ online.delete(s.key); recompute(); });
  _fallbackUsersRef = db.ref('usersPublic');
  _fallbackUsersRef.on('child_added', s=>{ const v=s.val()||{}; if(v.role) roles.set(s.key, v.role); recompute(); });
  _fallbackUsersRef.on('child_changed', s=>{ const v=s.val()||{}; if(v.role) roles.set(s.key, v.role); recompute(); });
}


// === Map POI editor (admin/mod) ===
async function isModerator(){
  const u = auth.currentUser;
  if(!u) return false;
  try{ const v = (await db.ref('roles/'+u.uid+'/moderator').get()).val(); return v===true; }catch(e){ return false; }
}

function initMapAdminUI(){
  const box = document.getElementById('mapAdminBox');
  const btn = document.getElementById('poiSave');
  const latI = document.getElementById('poiLat');
  const lngI = document.getElementById('poiLng');
  if(!box || !btn) return;

  // Show/hide based on admin/mod
  const show = async ()=>{
    const u = auth.currentUser;
    if(!u){ box.style.display='none'; return; }
    const mod = await isModerator();
    box.style.display = (isAdmin() || mod) ? '' : 'none';
  };
  try{ auth.onAuthStateChanged(()=>{ show(); }); }catch(e){}

  btn.addEventListener('click', async ()=>{
    const u = auth.currentUser; if(!u){ openModalAuth('login'); return; }
    const mod = await isModerator();
    if(!isAdmin() && !mod){ toast('Nemáte práva.'); return; }
    ...
    const lat   = parseFloat((latI?.value||'').trim());
    const lng   = parseFloat((lngI?.value||'').trim());
    if(!title || !Number.isFinite(lat) || !Number.isFinite(lng)){
      toast('Vyplňte název + souřadnice.');
      return;
    }
    ...
    let photoUrl = '';
    try{
      const f = document.getElementById('poiPhoto')?.files?.[0];
      if(f){
        const id = db.ref().push().key;
        const ref = stg.ref('public/poi/'+city+'/'+id+'_'+Date.now()+'.jpg');
        await ref.put(f);
        photoUrl = await ref.getDownloadURL();
        await db.ref('map/poi/'+city+'/'+id).set({ id, title, type, desc, lat, lng, photoUrl, by:u.uid, ts: Date.now() });
      }else{
        const ref = db.ref('map/poi/'+city).push();
        await ref.set({ id: ref.key, title, type, desc, lat, lng, photoUrl:'', by:u.uid, ts: Date.now() });
      }
      toast('Bod uložen'); playSound('ok');
      // clear fields (keep type)
      try{ document.getElementById('poiTitle').value=''; }catch(e){}
      try{ document.getElementById('poiDesc').value=''; }catch(e){}
      try{ document.getElementById('poiPhoto').value=''; }catch(e){}
    }catch(e){
      console.warn(e);
  ...
  // click on map to fill coords
  try{
    window.__onMapClickFill = (lat, lng)=>{
      try{ if(latI) latI.value = String(lat); if(lngI) lngI.value = String(lng); }catch(e){}
    };
  }catch(e){}
}

function renderPoiList(city, list){
  const box = document.getElementById('poiList');
  const cnt = document.getElementById('mapCounts');
  if(cnt) cnt.textContent = 'Body: ' + list.length;
  if(!box) return;
  box.innerHTML = '';
  ...
  list.forEach(p=>{
    const div = document.createElement('div');
    div.className = 'item';
    div.innerHTML = `<b>${esc(p.title||'Bod')}</b><div class="muted">${esc(p.type||'')}</div>${p.desc?`<div>${esc(p.desc)}</div>`:''}${p.photoUrl?`<div style="margin-top:6px"><img src="${p.photoUrl}" style="max-width:100%;border-radius:12px"></div>`:''}`;
    div.onclick = ()=>{ try{ if(window.__MAP) window.__MAP.setView([p.lat||50.08,p.lng||14.43], 15); }catch(e){} };
    box.appendChild(div);
  });
}