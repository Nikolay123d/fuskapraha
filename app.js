...
async function loadChat(){
  ...
  const cb=async (snap)=>{
    ...
    const name = `<span class="nick" data-uid="${esc(m.from)}">${esc(u?.nick||u?.name||'Uživatel')}</span>`+
                 badges +
                 (u?.online?'<span class="online"></span>':'');
    d.innerHTML = `<div class="ava" data-uid="${esc(m.from)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}"></div>`+
                  `<div class="bubble"><div class="name">${name}</div>`+
                  (m.text? `<div class="text">${esc(m.text)}</div>`:'' )+
                  (m.image||m.photo? `<div class="text"><img src="${esc(m.image||m.photo)}"></div>`:'')+
                  `</div>`;
    feed.appendChild(d);
    playSound('chat');
...
$('#sendBtn')?.addEventListener('click', async ()=>{
  try{
    ...
    if(!text && !img) return;
    const m={from:u.uid, at:Date.now(), nick:(u.displayName||u.email||'User'), text: (text||null), image: (img||null)};
...
function openDMRoom(a,b){
  const u = auth.currentUser;
  if(!u){ toast('Musíte být přihlášen'); openAuth(); return; }
  const room = dmKey(a,b);
  // Membership: each user can only add THEMSELVES (secure). Peer joins when they open the room.
  db.ref('privateMembers/'+room+'/'+u.uid).set(true).catch(e=>{
    console.error('openDMRoom membership failed', e);
    toast('Nelze otevřít DM (permissions)');
  });
  renderDM(room);
}
...
Po výběru ti pošlu QR kód. Zaplať a pošli sem fotku/printscreen platby, pak stiskni "Podat žádost".`;
  ...
  function addBtn(text, fn, cls){
    ...
    b.type='button';
    b.textContent=text;
    b.className = (cls==='ghost')?'ghost':((cls==='primary')?'primary':'');
    b.onclick=fn;
