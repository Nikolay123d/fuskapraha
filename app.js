...
function bubble({id,name,avatar,text,photo,city}){
  ...
  const n=document.createElement('div'); n.className='name'; n.textContent=name||'Uživatel';
  ...
  actions.innerHTML=`
    ...
    <button data-dm>✉️ Napsat</button>
    <button data-del class="adm-only" style="display:none">🗑 Smazat</button>`;
  ...
  actions.addEventListener('click', async (e)=>{
    ...
    if(e.target.hasAttribute('data-report')){
      ...
      showToast('#globalToast','Stížnost odeslána. Děkujeme!');
    }
...
async function ensureMyPublic(u){
  ...
  if(!s.exists()){
    await db.ref('usersPublic/'+u.uid).set({ name: u.displayName || 'Uživatel', avatar: window.DEFAULT_AVATAR, role: 'seeker', plan: 'free' });
  }
}
...
$('#participantsBtn').addEventListener('click', async()=>{
  ...
  for(const [uid,info] of Object.entries(all)){
    ...
    row.innerHTML=`<div class="ava"><img src="${info.avatar||window.DEFAULT_AVATAR}"></div>
      <div class="bubble"><div class="name">${esc(info.name||'Uživatel')}</div>
  ...
  list.onclick=async e=>{
    ...
    if(e.target.dataset.add){ if(!me) return alert('Přihlaste se'); await db.ref('friendRequests/'+uid+'/'+me.uid).set({from:me.uid,ts:Date.now(),status:'pending'}); }
  };
  box.hidden=false;
});
...
async function subChat(){
  ...
  CHAT_REF.on('child_added', async s=>{
    ...
    m._name=up.name||'Uživatel'; m._avatar=up.avatar||window.DEFAULT_AVATAR;
...
$('#chatSend').onclick = async()=>{
  ...
  let photo=null; const f=$('#chatFile').files[0]; if(f) { try{ photo=await fileToUrl(f); }catch(e){ showToast('#globalToast','⚠️ Nepodařilo se nahrát fotku'); } }
  ...
  $('#chatInput').value=''; $('#chatFile').value=''; showToast('#globalToast','✔️ Zpráva odeslána');
};

$('#clearChat').onclick = async()=>{
  const me=auth.currentUser; if(!me) return;
  const isMod=(await db.ref('roles/'+me.uid+'/moderator').get()).val()===true;
  if(!isAdmin(me) && !isMod) return alert('Pouze admin/moderátor');
  if(!confirm('Vyčistit celý chat?')) return;
...
$('#rentSend').onclick = async()=>{
  if(!auth.currentUser) return ensureAuth();
  let photo=null; const f=$('#rentFile').files[0]; if(f) { try{ photo=await fileToUrl(f); }catch(e){ showToast('#globalToast','⚠️ Nepodařilo se nahrát fotku'); } }
  ...
  $('#rentInput').value=''; $('#rentFile').value=''; showToast('#globalToast','✔️ Inzerát odeslán');
};
...
function openDmWith(uid){
  ...
  DM_REF.on('child_added', async s=>{
    const m=s.val(); const up=await userPublic(m.by)||{};
    const el=bubble({id:s.key,name:up.name||'Uživatel',avatar:up.avatar||window.DEFAULT_AVATAR,text:m.text,photo:m.photo,city:'dm'}); $('#dmMessages').appendChild(el);
    $('#dmMessages').scrollTop = $('#dmMessages').scrollHeight;
  });
  $('#dmSend').onclick = async ()=>{
    const txt=$('#dmInput').value.trim(); let photo=null; const f=$('#dmFile').files[0];
    if(f){ try{ photo=await fileToUrl(f); }catch(e){ showToast('#globalToast','⚠️ Fotka se nenahrála'); } }
    if(!txt && !photo) return;
    await db.ref('privateMessages/'+tid).push({by:me,text:txt||null,photo:photo||null,ts:Date.now()});
    $('#dmInput').value=''; $('#dmFile').value=''; showToast('#globalToast','✔️ Zpráva odeslána');
  };
}
...
function subHelp(){
  ...
  db.ref('help/'+CURRENT_CITY+'/cards').on('child_added', s=>{
    ...
    c.innerHTML=`${v.image?`<img src="${v.image}" style="width:100%;border-radius:10px">`:''}<b>${esc(v.title||'')}</b>${v.link?` · <a href="${v.link}" target="_blank">Otevřít</a>`:''}`;
    $('#helpGrid').prepend(c);
  });
}
subHelp();
...
function loadPoi(){
  ...
  db.ref('map/poi/'+CURRENT_CITY).on('child_added', s=>{
    ...
    m.addTo(map);
    m.bindPopup(`<div><b>${esc(v.title||'Bod')}</b></div><div>Тип: ${esc(v.type||'')}</div>${v.photo?`<div><img src="${v.photo}" style="max-width:180px;border-radius:8px;margin-top:6px"></div>`:''}<div class="muted">lat: ${v.lat} · lng: ${v.lng}</div>`);
    markers.push(m);
  });
}
loadPoi();
$('#poiAdd').onclick = async()=>{
  ...
  if(!isAdmin(u) && !isMod) return alert('Pouze admin/moderátor');
...
async function refreshMe(){
  ...
  $('#myName').textContent=me.name||'Uživatel';
...
async function loadReports(){ const box=$('#reportsBox'); if(!box) return;
  ...
  Object.entries(all).forEach(([msgId,items])=>{ Object.values(items||{}).forEach(rep=>{
    ...
    <div class="actions"><button data-godel data-id="${msgId}">Smazat повідомлення</button></div></div>`;
    box.appendChild(div); }); });
  box.onclick=async e=>{
    if(e.target.dataset.godel){
      ...
      showToast('#globalToast','✔️ Smazáno');
    }
  };
}
...
$('#ban30')?.addEventListener('click', async()=>{
  ...
  await db.ref('bans/'+uid).set({until: Date.now()+30*60*1000, reason}); showToast('#globalToast','⛔ Ban 30 min');
});
...
async function ensureAuth(){
  ...
  try{
    ...
    try { cred = await auth.signInWithEmailAndPassword(email,pass); }
           ...
           try{ await cred.user.sendEmailVerification(); showToast('#globalToast','Potvrzovací e-mail odeslán. ⚠️ Zkontrolujte SPAM!'); }catch{} }
    await ensureMyPublic(cred.user); showToast('#globalToast','Vítejte! Jste přihlášen.');
    return cred.user;
...
$('#resetPass')?.addEventListener('click', async()=>{
  ...
  if(!email) return;
  try{ await auth.sendPasswordResetEmail(email); showToast('#globalToast','E-mail pro obnovu odeslán. ⚠️ Zkontrolujte SPAM!'); }
