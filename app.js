...
function openPurchaseBot(reason){
  // Opens Privilegia modal and shows purchase assistant
  openTab('privilegia');
  const modal = document.getElementById('privilegiaModal');
  if (modal) modal.classList.add('open');
  const tab = document.querySelector('[data-priv-tab="koupit"]');
  if (tab) tab.click();
  const box = document.getElementById('privBotChat');
  if (box){
    box.innerHTML = `
      <div class="botMsg"><b>MAKÁME bot</b><div>Pro psaní zaměstnavatelům v L.S. potřebuješ <b>VIP Pracovník</b> (100 Kč navždy).</div>
      <div class="muted">Vyber možnost:</div>
      <div class="botBtns">
        <button class="btn primary" id="buyVipWorkerBtn">Koupit VIP Pracovník (100 Kč)</button>
        <button class="btn" id="closePrivBtn">Zavřít</button>
      </div></div>`;
    setTimeout(()=>{
      const b=document.getElementById('buyVipWorkerBtn');
      if (b) b.onclick=()=>{ document.getElementById('privBuySection')?.scrollIntoView({behavior:'smooth'}); };
      const c=document.getElementById('closePrivBtn');
      if (c) c.onclick=()=>{ modal?.classList.remove('open'); };
    },0);
  }
}
...
async function loadChat(){
  ...
  const city=getCity();
  const ref=db.ref('messages/'+city).orderByChild('ts').limitToLast(20);
