    ...
    btn.addEventListener('click', async () => {
      try {
        ...
        if (err && err.code && err.code.indexOf('operation-not-supported') >= 0) {
          ...
          if (box) box.textContent = err && err.message ? err.message : 'Помилка входу Google';
        }
      }
    });
  }
...
firebase.auth().onAuthStateChanged(async (user) => {
  ...
  try {
    ...
    if (uid) {
      const displayName = user.displayName || (window.localStorage.getItem('nick') || 'Користувач');
...
(function(){
  ...
  function openDm(uid, nick) {
    currentPeer = uid;
    dmTitle.textContent = nick || 'Особисті';
    dmThread.innerHTML = '';
    // subscribe
  ...
  async function sendDmMessage(text, file) {
    ...
    updates['inboxMeta/'+my+'/'+peer]  = { last: msg.text||'📷 Фото', ts: msg.ts };
    updates['inboxMeta/'+peer+'/'+my]  = { last: msg.text||'📷 Фото', ts: msg.ts };
    await firebase.database().ref().update(updates);
  }
