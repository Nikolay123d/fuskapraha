...
// Build stamp (helps confirm that GitHub Pages deployed the new version)
try{
  const build = document.body?.dataset?.build || 'unknown';
  console.log('[MAKAME] build:', build);
}catch(e){}
...
loadChat(); loadRent(); loadFriendsUI().catch(console.error); loadPoi(); });
...
async function loadChat(){
  ...
  const cb=async (snap)=>{
    ...
    const isAdm = !!(auth.currentUser && uid===auth.currentUser.uid && window.__isAdmin);
    const badges = (isAdm?'<span class="badge admin">ADMIN</span>':'') + (isPrem?'<span class="badge premium">PREMIUM</span>':'');
    const name = `<span class="nick" data-uid="${esc(uid)}">${esc(u?.nick||u?.name||'Uživatel')}</span>`+
                 badges +
...
async function renderDM(room){
  ...
  ref.on('child_added', async snap=>{
    ...
    el.innerHTML = `<div class="ava" data-uid="${esc(uid)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}"></div>`+
...
// Friends UI tends to flicker if we re-render too often.
// We keep a single set of listeners per UID and debounce UI refresh.
let FR_REQ_REF=null;
let FR_LIST_REF=null;
let __friendsListenUid=null;
let __friendsTimer=null;

function scheduleLoadFriendsUI(){
  clearTimeout(__friendsTimer);
  __friendsTimer=setTimeout(()=>{ loadFriendsUI().catch(console.error); }, 120);
}

function ensureFriendsListeners(uid){
  if(!uid) return;
  if(__friendsListenUid===uid) return;
  __friendsListenUid=uid;
  try{ FR_REQ_REF?.off?.(); }catch(e){}
  try{ FR_LIST_REF?.off?.(); }catch(e){}
  FR_REQ_REF=db.ref('friendRequests/'+uid);
  FR_LIST_REF=db.ref('friends/'+uid);
  FR_REQ_REF.on('value', scheduleLoadFriendsUI);
  FR_LIST_REF.on('value', scheduleLoadFriendsUI);
}

async function loadFriendsUI(){
  const me=auth.currentUser; if(!me) return;
  ensureFriendsListeners(me.uid);

  const wrap=$('#friendsRequestsWrap');
  const hint=$('#friendsNewHint');
  const reqBox=$('#friendsRequests');
  const listBox=$('#friendsList');
  const listEmpty=$('#friendsListEmpty');
  const mini=$('#friendsMiniLoad');
...
(function(){
  ...
  function isAdminUser(u){ return !!(u && window.__isAdmin===true); }
  ...
  async function renderFriendCard(uid, st){
    ...
    wrap.innerHTML = `
      ...
      <div class="bubble" style="width:100%">
        <div class="name" data-uid="${esc(r.uid)}"><b>${esc(u.nick||'Uživatel')}</b> <span class="muted">${u.online?'online':'offline'}</span></div>
        <div class="actions">
...
// --- DM threads / inbox ---
  ...
  async function loadDmThreads(){
    ...
    for(const room of rooms){
      ...
      row.className='msg';
      row.innerHTML = `
        <div class="ava" data-uid="${esc(r.uid)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}" alt=""></div>
...
// Admins management (roles/{uid}/admin)
const adminUidEl = document.getElementById('adminUid');
const adminListEl = document.getElementById('adminList');
const addBtn = document.getElementById('adminAddBtn');
const delBtn = document.getElementById('adminDelBtn');

async function refreshAdminList(){
  if(!adminListEl) return;
  adminListEl.innerHTML = '<div class="muted">Načítám…</div>';
  try{
    const snap = await db.ref('roles').get();
    const v = snap.val() || {};
    const admins = Object.keys(v).filter(uid=>v[uid] && v[uid].admin===true);
    admins.sort();
    if(admins.length===0){
      adminListEl.innerHTML = '<div class="muted">Zatím žádní admini.</div>';
      return;
    }
    ...
    for(const uid of admins){
      ...
      const row = document.createElement('div');
      row.className = 'msg';
      row.innerHTML = `<div class="ava"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}"></div>
        <div class="bubble" style="width:100%">
          <div class="name"><b>${esc(u.nick||'Uživatel')}</b> <span class="muted">${esc(uid)}</span></div>
          <div class="actions">
            <button data-uid="${esc(uid)}" data-act="rm">Odebrat admin</button>
          </div>
        </div>`;
      row.addEventListener('click', async (e)=>{
        ...
        if(act==='rm' && targetUid){
          if(!confirm('Odebrat admin práva: '+targetUid+' ?')) return;
          await adminSet('roles/'+targetUid+'/admin', false);
          toast('Admin odebrán');
          refreshAdminList();
        }
      });
    ...
    console.warn(e);
    adminListEl.innerHTML = '<div class="muted">Chyba načtení.</div>';
  }
}
...
addBtn?.addEventListener('click', async ()=>{
  try{
    ...
    const uid = (adminUidEl?.value||'').trim();
    if(!uid) return toast('Zadejte UID');
    await adminSet('roles/'+uid+'/admin', true);
    toast('Admin přidán');
    if(adminUidEl) adminUidEl.value='';
    refreshAdminList();
...
delBtn?.addEventListener('click', async ()=>{
  try{
    ...
    const uid = (adminUidEl?.value||'').trim();
    if(!uid) return toast('Zadejte UID');
    await adminSet('roles/'+uid+'/admin', false);
    toast('Admin odebrán');
    if(adminUidEl) adminUidEl.value='';
    refreshAdminList();
  }catch(e){ console.warn(e); toast('Chyba'); }
...
// auto refresh list when admin view opened
document.addEventListener('click', (e)=>{
  const t=e.target.closest('[data-view]');
  if(t && t.dataset.view==='view-admin'){
    refreshAdminList();
  }
});
  // Wallpapers uploads
