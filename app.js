...
function openNotifsPanel(){
  ...
  const items = (window.__NOTIFS_CACHE__||[]).slice().sort((a,b)=> (b.ts||0)-(a.ts||0));
  if(!items.length){
    list.innerHTML = '<div class="muted" style="padding:10px">Zatím žádná oznámení.</div>';
    return;
  }
  for(const n of items){
    const row = document.createElement('div');
    row.className = 'notifRow'+(n.read? ' read':'');
    const when = n.ts ? new Date(n.ts).toLocaleString() : '';
    const title = (n.type==='dm') ? 'Nová zpráva (L.S.)' :
                  (n.type==='friend') ? 'Nový přítel' :
                  (n.type==='broadcast') ? 'Systém' : (n.type||'Oznámení');
    const txt = esc(n.text || n.preview || '');
    row.innerHTML = `
      <div class="notifMain">
        <div class="notifTitle">${esc(title)}</div>
        <div class="notifText">${txt}</div>
        <div class="notifMeta">${esc(when)}</div>
      </div>
      <div class="notifActions">
        <button class="btn tiny" data-act="open">Otevřít</button>
        <button class="btn tiny" data-act="read">Přečteno</button>
      </div>`;
    row.querySelector('[data-act="read"]').onclick = async ()=>{
      try{
        if(n._key){
          await db.ref('notifications/'+currentUser.uid+'/'+n._key+'/read').set(true);
        }
      }catch(e){}
    };
    row.querySelector('[data-act="open"]').onclick = async ()=>{
      try{
        if(n.type==='dm' && n.chatId && n.peerUid){
          box.classList.add('hidden');
          await openDMRoom(n.peerUid, n.chatId);
