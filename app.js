...
function schedulePromoOffer(){
  try{
    ...
    // Admin can publish promo in settings/promo: {ts, title, text, img, sound}
    db.ref('settings/promo').get().then(s=>{
      const p=s.val();
      if(!p || !p.ts) return;
      const ts=+p.ts||0;
      if(ts<=seen) return;
      ...
      // show simple modal if exists else toast
      try{
        const box=document.getElementById('promoModalBox');
        if(box){
          box.querySelector('.promo-title')?.textContent = p.title||'Oznámení';
          box.querySelector('.promo-text')?.textContent = p.text||'';
          const img=box.querySelector('.promo-img');
