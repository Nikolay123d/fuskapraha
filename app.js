...
async function loadChatWallpaper(city){
  if(!city) city = getCity();
  window.__currentCity = city;
  try{
    const s = await db.ref('settings/wallpapers/chat/'+city).get();
    const v=s.val();
    if(v && v.url){ applyChatWallpaper(city, v.url); return; }
  }catch(e){}
  const ls = localStorage.getItem('wall_chat_'+city);
  if(ls) applyChatWallpaper(city, ls); else applyChatWallpaper(city, '');
}

function applyChatWallpaper(city, dataUrl){
  const feed = document.getElementById('chatFeed');
  if(!feed) return;
  feed.style.backgroundImage = dataUrl ? `url('${dataUrl}')` : '';
}
function applyDmWallpaper(room, dataUrl){
  const feed = document.getElementById('dmFeed');
  if(!feed) return;
  feed.style.backgroundImage = dataUrl ? `url('${dataUrl}')` : '';
}
function applyUserCardWallpaper(uid, dataUrl){
  const card = document.querySelector('#modalUserCard .modal-card');
  if(!card) return;
  card.style.backgroundImage = dataUrl ? `url('${dataUrl}')` : '';
}
}
...
let premiumQrUrl = PREMIUM_QR_IMG;
async function loadPremiumQr(){
  try{
    const s = await db.ref('settings/payment/qrUrl').get();
    const v = s.val();
    if(v && typeof v === 'string') premiumQrUrl = v;
    try{ const im=document.getElementById('adminQrPreview'); if(im) im.src=premiumQrUrl; }catch(e){}
  }catch(e){}
}
function getPremiumQr(){ return premiumQrUrl || PREMIUM_QR_IMG; }

const PREMIUM_PLANS = {
  premium: { code:'premium', title:'Premium', price:150, period:'měsíc', desc:'Bot pro 1 místnost. Interval 30 min. Zprávy lidem (reklama) přijdou vám do DM.' },
  premiumPlus: { code:'premiumPlus', title:'Premium+', price:200, period:'měsíc', desc:'Bot pro všechny místnosti. Interval 20 min. Zprávy lidem (reklama) přijdou vám do DM.' }
};
...
const SND = {
  ...
  dm: new Audio('./sounds/new_message-di-ding.mp3'),
  ok: new Audio('./sounds/ok.wav'),
  err: new Audio('./sounds/err.wav'),
  party: new Audio('./sounds/celebration.wav')
};
// normalize volumes
try{ Object.values(SND).forEach(a=>{a.volume=0.6;}); }catch(e){}
function playSound(k){
  try{
    if(localStorage.getItem('consentSounds')!=='1') return;
...
async function unlockAudio(){
  try{
    localStorage.setItem('consentSounds','1');
...
function initConsentUI(){
  const saved = localStorage.getItem('consentSaved')==='1';
  const modal = document.getElementById('modalConsent');
  if(!saved && modal){
    // default: sounds on, notifications off
    document.getElementById('consentSounds').checked = (localStorage.getItem('consentSounds')!=='0');
    document.getElementById('consentNotif').checked = (localStorage.getItem('consentNotif')==='1');
    openModal('modalConsent');
  }
  document.getElementById('consentSave')?.addEventListener('click', async ()=>{
    const sOk = document.getElementById('consentSounds')?.checked;
    const nOk = document.getElementById('consentNotif')?.checked;
    localStorage.setItem('consentSaved','1');
    localStorage.setItem('consentSounds', sOk?'1':'0');
    localStorage.setItem('consentNotif', nOk?'1':'0');
    closeModal('modalConsent');
    if(sOk) await unlockAudio();
    if(nOk){
...
function setCity(c){ localStorage.setItem('city', c); window.__currentCity = c; try{ loadChatWallpaper(c); }catch(e){} }
...
(function(){
  ...
  window.addEventListener('DOMContentLoaded', ()=>{ apply(); startWallpaperSync(); startAuthWallpaperSync(); });
...
if(t && t.id==='chatWallCamera'){
  if(!window.__isAdmin){ toast('Pouze administrátor může měnit pozadí.'); t.value=''; return; }
  const f=t.files && t.files[0]; if(!f) return;
  const city = (window.__currentCity || 'praha');
  const r=new FileReader();
  r.onload=()=>{
    const data=r.result;
    try{ localStorage.setItem('wall_chat_'+city, data); }catch(_){}
    db.ref('settings/wallpapers/chat/'+city).set({url:data, ts:Date.now(), by:(auth.currentUser?auth.currentUser.uid:null)});
    applyChatWallpaper(city, data);
    toast('Pozadí místnosti bylo uloženo.');
    t.value='';
  };
  r.readAsDataURL(f);
}
if(t && t.id==='dmWallCamera'){
  if(!window.__isAdmin){ toast('Pouze administrátor může měnit pozadí.'); t.value=''; return; }
  const f=t.files && t.files[0]; if(!f) return;
  const room = window.__currentDmRoom;
  if(!room){ toast('Otevřete nejprve DM konverzaci.'); t.value=''; return; }
  const r=new FileReader();
  r.onload=()=>{
    const data=r.result;
    try{ localStorage.setItem('wall_dm_'+room, data); }catch(_){}
    db.ref('settings/wallpapers/dm/'+room).set({url:data, ts:Date.now(), by:(auth.currentUser?auth.currentUser.uid:null)});
    applyDmWallpaper(room, data);
    toast('Pozadí DM bylo uloženo.');
    t.value='';
  };
  r.readAsDataURL(f);
}
if(t && t.id==='userWallCamera'){
  if(!window.__isAdmin){ toast('Pouze administrátor může měnit pozadí.'); t.value=''; return; }
  const f=t.files && t.files[0]; if(!f) return;
  const uid = window.__currentUserCardUid;
  if(!uid){ toast('Otevřete nejprve profil uživatele.'); t.value=''; return; }
  const r=new FileReader();
  r.onload=()=>{
    const data=r.result;
    try{ localStorage.setItem('wall_profile_'+uid, data); }catch(_){}
    db.ref('settings/wallpapers/profile/'+uid).set({url:data, ts:Date.now(), by:(auth.currentUser?auth.currentUser.uid:null)});
    applyUserCardWallpaper(uid, data);
    toast('Pozadí profilu bylo uloženo.');
    t.value='';
  };
  r.readAsDataURL(f);
}
...
async function renderDM(room){
  window.__currentDmRoom = room;
  try{ const s=await db.ref('settings/wallpapers/dm/'+room).get(); const v=s.val(); if(v&&v.url) applyDmWallpaper(room, v.url); else { const ls=localStorage.getItem('wall_dm_'+room); if(ls) applyDmWallpaper(room, ls); else applyDmWallpaper(room, ''); } }catch(e){ const ls=localStorage.getItem('wall_dm_'+room); if(ls) applyDmWallpaper(room, ls); }
...
async function botSendQR(room){
  ...
  await db.ref('privateMessages/'+room).push({by: auth.currentUser.uid, botUid: PREMIUM_BOT_UID, img: getPremiumQr(), text:'QR platba', ts: Date.now(), bot:true});
}
...
async function ensureBotQR(room){
  ...
  const v=snap.val()||{};
  const hasQr = Object.values(v).some(m=>m && m.bot===true && m.botUid===PREMIUM_BOT_UID && m.img===getPremiumQr());
  if(!hasQr){
...
async function openPremiumBot(){
  try{ await loadPremiumQr(); }catch(e){}
  const u=auth.currentUser;
...
$('#dmOpen')?.addEventListener('click', async ()=>{
  ...
  let to=$('#dmTo').value.trim();
    // If user writes to pseudo-bot (bot_<id>), forward message to owner (Premium) with tag
    if(to.startsWith('bot_')){
      const botId = to.slice(4);
      try{
        const owner = (await db.ref('botsPublic/'+botId+'/ownerUid').get()).val();
        const botNick = (await db.ref('botsPublic/'+botId+'/nick').get()).val() || 'Bot';
        if(owner){
          to = owner;
          $('#dmTo').value = owner;
          // keep original recipient shown in header by using dmWithName; message will be tagged
          const origText = $('#dmText').value.trim();
          if(origText){
            $('#dmText').value = `[REKLAMA → ${botNick}] ${origText}`;
          }
        }else{
          toast('Bot není aktivní');
          return;
        }
      }catch(e){}
    }
...
$('#dmSend')?.addEventListener('click', async ()=>{
  try{
    ...
    await db.ref('inboxMeta/'+to+'/'+room).set({with: me.uid, ts: Date.now(), lastTs: Date.now()});
    await db.ref('inboxMeta/'+me.uid+'/'+room).set({with: to, ts: Date.now(), lastTs: Date.now()});
...
function watchAdminRole(user){
  window.__isAdmin = false;
  try{
    document.querySelectorAll('.admin-only,.adm-only').forEach(el=>{ el.style.display='none'; });
  }catch{}
  if(_roleRef){ try{ _roleRef.off(); }catch{} _roleRef=null; }
  if(!user) return;
  _roleRef = db.ref('roles/'+user.uid+'/admin');
  const handler = (snap)=>{
    window.__isAdmin = (snap.val() === true);
    try{
      document.querySelectorAll('.admin-only,.adm-only').forEach(el=>{
        el.style.display = window.__isAdmin ? '' : 'none';
      });
      const wallWrap = document.getElementById('wallCamWrap');
      if(wallWrap) wallWrap.style.display = window.__isAdmin ? '' : 'none';
      const authWrap = document.getElementById('authWallCamWrap');
      if(authWrap) authWrap.style.display = window.__isAdmin ? '' : 'none';
      document.body.classList.toggle('is-admin', window.__isAdmin);
    }catch{}
  };
  _roleRef.on('value', handler);
}
...
// profile display
  if(u){
      document.documentElement.classList.toggle('is-admin', isAdminUser(u));
      window.__isAdmin = isAdminUser(u);
      try{ await loadPremiumQr(); }catch(e){}
...
(function(){
  ...
  async function showUserCard(uid){
    window.__currentUserCardUid = uid;
    try{ const s=await db.ref('settings/wallpapers/profile/'+uid).get(); const v=s.val(); const data=(v&&v.url)||localStorage.getItem('wall_profile_'+uid)||''; applyUserCardWallpaper(uid, data); }catch(e){ const ls=localStorage.getItem('wall_profile_'+uid); if(ls) applyUserCardWallpaper(uid, ls); }
...
async function loadMyPremiumState(u){
  if(!u) return null;
  try{
    const s = await db.ref('usersPublic/'+u.uid+'/plan').get();
    return s.val() || 'none';
  }catch(e){ return 'none'; }
}
...
async function loadMyBotsUI(){
  const u=auth.currentUser;
  const card=document.getElementById('myBotsCard');
  const list=document.getElementById('myBotsList');
  if(!u || !card || !list) return;
  const plan = await loadMyPremiumState(u);
  if(plan!=='premium' && plan!=='premiumPlus'){ card.style.display='none'; return; }
  card.style.display='block';
  const snap = await db.ref('userBots/'+u.uid).get();
  const v = snap.val()||{};
  myBotsCache = v || {};
  list.innerHTML='';
  for(const [id,b] of Object.entries(v)){
    if(!b) continue;
    const el=document.createElement('div'); el.className='msg';
    el.innerHTML = `<div class="meta">${esc(b.scope||'one')} • ${esc(String(b.intervalMin||''))} min</div>
                    <div><b>${esc(b.nick||('Bot '+id))}</b> <span class="muted">${esc(b.city||'')}</span></div>
                    <div class="muted">${esc(b.text||'')}</div>
                    <div class="row" style="justify-content:flex-end;gap:8px">
                      <button class="ghost" data-act="toggle" data-id="${id}">${b.enabled===false?'Zapnout':'Vypnout'}</button>
                      <button class="ghost" data-act="del" data-id="${id}">Smazat</button>
                    </div>`;
    el.querySelectorAll('button').forEach(btn=>btn.addEventListener('click', async ()=>{
      const act=btn.dataset.act, bid=btn.dataset.id;
      if(act==='toggle'){
        const cur = (await db.ref('userBots/'+u.uid+'/'+bid+'/enabled').get()).val();
        await db.ref('userBots/'+u.uid+'/'+bid+'/enabled').set(cur===false?true:false);
        loadMyBotsUI();
      }
      if(act==='del'){
        await db.ref('userBots/'+u.uid+'/'+bid).remove();
        loadMyBotsUI();
      }
    }));
    list.appendChild(el);
  ...
  // scheduler
  startMyBotsScheduler(plan);
}

function startMyBotsScheduler(plan){
  const u=auth.currentUser; if(!u) return;
  if(myBotTimer){ clearInterval(myBotTimer); myBotTimer=null; }
  const tickEveryMs = 20000;
  myBotTimer=setInterval(async ()=>{
    try{
      const snap = await db.ref('userBots/'+u.uid).get();
      const bots = snap.val()||{};
      const now=Date.now();
      for(const [id,b] of Object.entries(bots)){
        ...
        const intervalMin = (plan==='premiumPlus') ? 20 : 30;
        const iv = Math.max(intervalMin, parseInt(b.intervalMin||intervalMin,10)||intervalMin);
        const last = b.lastSent||0;
        if(now - last < iv*60*1000) continue;
        // scope
        const cities = (plan==='premiumPlus' && b.scope==='all') ? ['praha','brno','olomouc','ostrava'] : [b.city||'praha'];
        for(const city of cities){
          const ref = db.ref('messages/'+city).push();
          await ref.set({by:u.uid, ts: now, text: b.text||'', bot:true, botId:id, botNick:b.nick||''});
        }
        await db.ref('userBots/'+u.uid+'/'+id+'/lastSent').set(now);
      }
    }catch(e){}
  }, tickEveryMs);
}
...
async function createMyBot(){
  const u=auth.currentUser; if(!u) return toast('Nejprve se přihlaste');
  const plan = await loadMyPremiumState(u);
  if(plan!=='premium' && plan!=='premiumPlus') return toast('Vyžaduje Premium');
  const nick = (document.getElementById('myBotNick')?.value||'').trim() || 'Bot';
  const scope = document.getElementById('myBotScope')?.value || 'one';
  const city = document.getElementById('myBotCity')?.value || 'praha';
  const text = (document.getElementById('myBotText')?.value||'').trim();
  if(!text) return toast('Zadejte text bota');
  if(plan==='premium' && scope==='all') return toast('Premium umožňuje pouze 1 místnost');
  const intervalMin = (plan==='premiumPlus') ? 20 : 30;
  const id = db.ref('userBots/'+u.uid).push().key;
  await db.ref('userBots/'+u.uid+'/'+id).set({nick, scope, city, text, intervalMin, enabled:true, createdAt:Date.now(), lastSent:0});
  // public index for forwarding
  await db.ref('botsPublic/'+id).set({ownerUid:u.uid, nick, scope, city, intervalMin, enabled:true, ts:Date.now()});
  // create public pseudo-user for DM discovery
  await db.ref('usersPublic/bot_'+id).set({nick, avatar: window.DEFAULT_AVATAR, plan:'bot', createdAt:Date.now()});
  toast('Bot vytvořen');
  loadMyBotsUI();
}
...
async function loadMyProfileUI(u){
  ...
  auth.onAuthStateChanged(async (u)=>{
    if(u){
      ...
      try{ await loadMyProfileUI(u);
      try{ await loadMyBotsUI(); }catch(e){}
 }catch(e){}
      // admin panels
...
(function initMobileNav(){
  ...
  document.getElementById('adminQrFile')?.addEventListener('change', (e)=>{
    const f=e.target.files && e.target.files[0]; if(!f) return;
    const r=new FileReader();
    r.onload=()=>{ document.getElementById('adminQrPreview').src = r.result; document.getElementById('adminQrPreview').dataset.qr = r.result; };
    r.readAsDataURL(f);
  });
  document.getElementById('adminQrSave')?.addEventListener('click', async ()=>{
    const me=auth.currentUser; if(!isAdminUser(me)) return toast('Jen admin');
    const data = document.getElementById('adminQrPreview')?.dataset.qr;
    if(!data) return toast('Nahrajte QR');
    await db.ref('settings/payment/qrUrl').set(data);
    premiumQrUrl = data;
    toast('QR uloženo');
  });
