...
async function ensureMyPublic(u){
  ...
  if(u.email){
    ...
    try{ await db.ref('emails/'+key).set({uid:u.uid, ts: Date.now()}); }catch(e){ /* non-critical */ }
  }
}
...
async function googleSignIn(){
  ...
  // Mobile browsers often block popups; redirect is the most reliable.
  const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent||'') || (window.innerWidth && window.innerWidth<760);

  // Make sure the button never double-triggers.
  ...
  try{
    ...
    // Desktop: try popup first, but if blocked, fall back to redirect.
    try{
      await auth.signInWithPopup(provider);
      // Popup path must release the lock (otherwise later auth actions look "broken").
      ...
      try{
        ...
        if(code.includes('unauthorized-domain')){
          toast('Chyba: nepovolený doménový původ. Přidejte doménu webu do Firebase Auth → Settings → Authorized domains.');
        }
      }catch(_e){}
      try{ sessionStorage.setItem('authRedirecting','1'); }catch(_e){}
      didRedirect = true;
      await auth.signInWithRedirect(provider);
      return;
    }
  } finally {
    // If we did not redirect (popup failed/canceled), release lock.
    if(!didRedirect){
...
async function ensureUserPublic(u, extra={}){
  ...
  if(u.email){
    const key = emailKey(u.email);
    try{ await db.ref('emails/'+key).update({uid: u.uid}); }catch(e){ /* non-critical */ }
  }
}
    ...
    try{
      ...
      row.innerHTML = `
        ...
        <div class="bubble" style="width:100%;cursor:pointer">
          <div class="name"><b>${esc(u.nick||'...')}</b> <span class="muted">${u.online?'online':'offline'}</span></div>
    ...
    if(_adminUsersMode==='complaints'){
      ...
      for(const it of items){
        ...
        row.innerHTML = `
          ...
          <div class="bubble" style="width:100%">
            <div class="meta"><div class="name"><b>${esc(u.nick||'...')}</b> <span class="muted">${new Date(it.ts||0).toLocaleString()}</span></div></div>
