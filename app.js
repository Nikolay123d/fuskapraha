...
document.getElementById("openSupportFromProfile")?.addEventListener("click", () => { __closeAllOverlays?.(); supportOpenModal(); });
document.getElementById("openBotsFromProfile")?.addEventListener("click", () => {
  try { __closeAllOverlays?.(); } catch (e) {}
  if (typeof openPrivModal === "function") return openPrivModal("bots");
  // fallback: open DM with bot if exists
...
// profile display
  if(u){
    ...
    // Premium-bot hosting runs ONLY when admin/mod is online (client-side limitation).
    try{
      const br = (localStorage.getItem("mk_bot_runner_enabled") !== "0");
      if ((IS_ADMIN || IS_MOD) && br) startBotHostEngine();
    }catch(e){}
...
document.getElementById('adminBroadcastClose')?.addEventListener('click', ()=>closeModal('adminBroadcastModal'));
  ...
  // Bot Runner toggle (admin/mod only). Premium bots can run only when someone trusted is online.
  (function(){
    const btn = document.getElementById("adminBotRunnerBtn");
    const stateEl = document.getElementById("botRunnerState");
    if (!btn) return;
    function isAllowed(){ return (typeof IS_ADMIN !== "undefined" && IS_ADMIN) || (typeof IS_MOD !== "undefined" && IS_MOD); }
    function getEnabled(){ return localStorage.getItem("mk_bot_runner_enabled") !== "0"; }
    function setEnabled(v){ localStorage.setItem("mk_bot_runner_enabled", v ? "1" : "0"); }
    function render(){
      const allowed = isAllowed();
      const enabled = allowed && getEnabled();
      btn.style.display = allowed ? "" : "none";
      if (stateEl) stateEl.textContent = enabled ? "Zapnuto" : "Vypnuto";
    }
    btn.addEventListener("click", () => {
      if (!isAllowed()){ toast?.("Pouze admin/moderátor."); return; }
      const next = !getEnabled();
      setEnabled(next);
      render();
      try { if (next) startBotHostEngine(); else stopBotHostEngine(); } catch(e){}
    });
