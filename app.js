...
async function chatLoadInitial(city){
  ...
  const snap=await db.ref('messages/'+city).orderByChild('ts').limitToLast(15).get();
  const items=[];
...
async function chatLoadOlder(){
  ...
  const city=getCity();
  const snap=await db.ref('messages/'+city).orderByChild('ts').endAt(endTs).limitToLast(15).get();
  const items=[];
  snap.forEach(s=>items.push({key:s.key, val:s.val()||{}}));
  items.sort((a,b)=>(a.val.ts||0)-(b.val.ts||0));
...
window.addEventListener('DOMContentLoaded', ()=>{
  try{ localStorage.setItem('city','praha'); const sel=document.getElementById('citySelect'); if(sel) sel.value='praha'; }catch{}
...
(function(){
  ...
  async function loadVacanciesInto(container, uid){
    ...
    const snap = await db.ref('vacancies/'+uid).orderByChild('ts').limitToLast(15).get();
    const v = snap.val()||{};
  ...
  async function loadMyVacancies(){
    ...
    feed.innerHTML='';
    const snap = await db.ref('vacancies/'+me.uid).orderByChild('ts').limitToLast(15).get();
    const v=snap.val()||{};
    const ids=Object.keys(v).sort((a,b)=>(v[b].ts||0)-(v[a].ts||0));
    for(const id of ids){
      const it=v[id]||{};
