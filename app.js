...
// Admin bootstrap (email fallback). You can override in index.html via window.ADMIN_EMAIL
window.ADMIN_EMAIL = window.ADMIN_EMAIL || 'urciknikolaj642@gmail.com';
...
function openDmWithCreator(){
  ...
  db.ref('emails/'+key).get().then(s=>{
    ...
    showView('view-dm');
    // set input
    (function(){const _e=$('#dmTo'); if(_e) _e.value = creatorUid;})();
  });
}
...
async function loadChat(){
  ...
  feed.innerHTML='';
  feed.innerHTML = '<div class=\"mini-loading\">Načítáme chat…</div>';
  if(offChat){ offChat(); offChat=null; }
  const city=getCity();
  const ref=db.ref('messages/'+city).limitToLast(50);
  const cb=async (snap)=>{
    // first data -> remove mini loader
    try{ const ml = feed.querySelector('.mini-loading'); if(ml) feed.innerHTML=''; }catch{}
    const m=snap.val()||{};
    if(m.deleted) return;
...
async function renderDM(room){
  ...
  box.innerHTML='';
  box.innerHTML = '<div class=\"mini-loading\">Načítáme zprávy…</div>';
  showPremiumBotPanel(room);
  if(DM_REF){ try{ DM_REF.off(); }catch{} }
  const ref=db.ref('privateMessages/'+room).limitToLast(50);
  DM_REF = ref;
  ref.on('child_added', async snap=>{
    try{ const ml = box.querySelector('.mini-loading'); if(ml) box.innerHTML=''; }catch{}
    const m=snap.val()||{};
    const u = (m.bot && m.botUid===PREMIUM_BOT_UID) ? {nick:'Bot — Privilegia', avatar:'./img/rose.svg'} : await getUser(m.by);
...
async function friendItem(uid, st){
  ...
  wrap.className='msg friend-item';
  const u=await getUser(uid);

  const name = esc(u.nick||'Uživatel');
  const avatar = esc(u.avatar||window.DEFAULT_AVATAR);

  const actions = [];
  actions.push(`<button data-act="chat">Napsat</button>`);
  if(st==='pending'){
    actions.push(`<button data-act="accept" class="primary">Přijmout</button>`);
    actions.push(`<button data-act="decline" class="ghost">Odmítnout</button>`);
  }else{
    actions.push(`<button data-act="remove" class="ghost">Odebrat</button>`);
  }

  wrap.innerHTML =
    `<div class="ava" data-uid="${esc(uid)}"><img src="${avatar}" alt=""></div>`+
    `<div class="bubble">`+
      `<div class="name" data-uid="${esc(uid)}">${name}</div>`+
  ...
  wrap.addEventListener('click', async (e)=>{
    ...
    try{
      ...
      if(a==='accept'){
        ...
        // notify
        try{ await db.ref('notifications/'+uid).push({ts:Date.now(), type:'friend', by: me.uid}); }catch(_){}
...
function watchAdminRole(user){
  // Hide admin UI by default (hard CSS gate is in style.css via body.is-admin too)
  window.__isAdmin = false;
  try{ document.body.classList.remove('is-admin'); }catch{}
  try{ document.querySelectorAll('.admin-only,.adm-only').forEach(el=>{ el.style.display='none'; }); }catch{}

  // remove previous listeners
  try{ if(window.__adminRefs){ window.__adminRefs.forEach(r=>{ try{ r.ref.off('value', r.cb); }catch{} }); } }catch{}
  window.__adminRefs = [];

  if(!user) return;

  const state = { role:false, uid:false, email:false };

  const apply = ()=>{
    const isAdm = !!(state.role || state.uid || state.email);
    window.__isAdmin = isAdm;
    try{
      document.body.classList.toggle('is-admin', isAdm);
      document.querySelectorAll('.admin-only,.adm-only').forEach(el=>{ el.style.display = isAdm ? '' : 'none'; });
      // legacy camera wrappers
      const wallWrap = document.getElementById('wallCamWrap'); if(wallWrap) wallWrap.style.display = isAdm ? '' : 'none';
      const authWrap = document.getElementById('authWallCamWrap'); if(authWrap) authWrap.style.display = isAdm ? '' : 'none';
    }catch{}
    try{ console.log('ADMIN =', isAdm, {uid:user.uid, email:user.email||null, ...state}); }catch{}
  };

  // 1) roles/{uid}/admin === true
  const roleRef = db.ref('roles/'+user.uid+'/admin');
  const roleCb = (snap)=>{ state.role = (snap.val()===true); apply(); };
  roleRef.on('value', roleCb);
  window.__adminRefs.push({ref:roleRef, cb:roleCb});

  // 2) settings/admins/{uid} === true
  const uidRef = db.ref('settings/admins/'+user.uid);
  const uidCb = (snap)=>{ state.uid = (snap.val()===true); apply(); };
  uidRef.on('value', uidCb);
  window.__adminRefs.push({ref:uidRef, cb:uidCb});

  // 3) email fallback:
  // - if settings/adminEmail exists and equals user.email
  // - OR window.ADMIN_EMAIL equals user.email
  const emailRef = db.ref('settings/adminEmail');
  const emailCb = (snap)=>{
    ...
    state.email = !!(
      uEmail &&
      (
        (dbEmail && String(dbEmail).toLowerCase()===String(uEmail).toLowerCase()) ||
        (window.ADMIN_EMAIL && String(window.ADMIN_EMAIL).toLowerCase()===String(uEmail).toLowerCase())
      )
    );
    apply();
  };
  emailRef.on('value', emailCb);
  window.__adminRefs.push({ref:emailRef, cb:emailCb});

  // initial apply (in case refs are slow)
  apply();
}
