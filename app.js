...
async function googleSignIn(){
  ...
  const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent||'') || (window.innerWidth && window.innerWidth<560);
  if(isMobile){
...
try{
  auth.getRedirectResult().then((r)=>{ if(r && r.user){ try{ closeModalAuth(); }catch(e){} try{ toast('Přihlášeno přes Google'); }catch(e){} } }).finally(()=>{ try{ sessionStorage.removeItem('authRedirecting'); }catch(e){} }).catch(()=>{});
}catch{}
