...
// === DB paths (chat compatibility) ===
const CHAT_NODE = 'chat';
const CHAT_LEGACY_NODE = 'messages';
function chatRef(city){ return db.ref(CHAT_NODE + '/' + city); }
function legacyChatRef(city){ return db.ref(CHAT_LEGACY_NODE + '/' + city); }
function msgUid(m){ return (m && (m.by || m.uid || m.from)) || null; }
function msgTs(m){ return (m && (m.ts || m.at)) || 0; }
function msgImg(m){ return (m && (m.img || m.image)) || null; }
...
async function loadChat(){
  ...
  const city=getCity();
  const ref=chatRef(city).limitToLast(50);
  // fallback: if chat node empty, use legacy messages
  const legacyRef=legacyChatRef(city).limitToLast(50);
  const hasChat = (await chatRef(city).limitToFirst(1).get()).exists();
  const activeRef = hasChat ? ref : legacyRef;
  const cb=async (snap)=>{
    const m=snap.val()||{};
    if(m.deleted) return;
    const u=await getUser(msgUid(m));
  ...
  activeRef.on('child_added', cb);
  offChat=()=> activeRef.off('child_added', cb);
}
...
$('#sendBtn')?.addEventListener('click', async ()=>{
  try{
    ...
    const pub=await getUser(u.uid);
    const nick = pub?.nick || null;
    const payload={by:u.uid, from:u.uid, ts:m.ts, at:m.ts, nick:nick, text: m.text||null, img: m.img||null, image: m.img||null};
    await chatRef(getCity()).push(payload);
    // also mirror to legacy node for backward compatibility
    try{ await legacyChatRef(getCity()).push(payload); }catch(_e){}
    // cleanup
    $('#msgText').value='';
    $('#msgPhoto').value='';
    pendingChatImg=null;
...
async function startDM(toUid, opts={}){
  ...
  const room = await openDMRoom(me.uid, toUid);

  // inbox meta for fast list (best-effort; room already exists)
  try{
    const ts = Date.now();
    const mePub = await getUser(me.uid);
    const toPub = await getUser(toUid);
    await Promise.all([
      db.ref('inboxMeta/'+me.uid+'/'+room).update({ with: toUid, ts, lastTs: ts, title: (toPub?.nick||'Uživatel') }),
      db.ref('inboxMeta/'+toUid+'/'+room).update({ with: me.uid, ts, lastTs: ts, title: (mePub?.nick||'Uživatel') })
    ]);
  }catch(e){}
...
async function openDMRoom(a,b){
  ...
  // membership must be sequential (rules depend on it)
...
(function(){
  ...
  async function renderFriendCard(uid, st){
    ...
    wrap.addEventListener('click', async (e)=>{
      ...
      if(act==='dm'){ startDM(uid); showView('view-dm'); }
      if(act==='accept'){
        await db.ref('friends/'+me.uid+'/'+uid).set('accepted');
        await db.ref('friends/'+uid+'/'+me.uid).set('accepted');
        await db.ref('friendRequests/'+me.uid+'/'+uid).remove();
...
// --- DM threads / inbox ---
  ...
  async function botTick(){
    ...
    for(const [id,b] of Object.entries(v)){
      ...
      await chatRef(city).push({by:botUid, ts:now, text:b.text||'', img:b.img||''});
