...
function watchAdminRole(user){
  ...
  if(!user) return;
  // Prefer settings/admins/{uid} as admin source (by UID), fallback to roles/{uid}/admin if exists
const adminRef1 = db.ref('settings/admins/'+user.uid);
const adminRef2 = db.ref('roles/'+user.uid+'/admin');
_roleRef = adminRef1;
  const handler = (snap)=>{
  ...
  // watch settings/admins first
let _admin1 = false;
let _admin2 = false;
adminRef1.on('value', (s)=>{ _admin1 = (s.val()===true); handler({ val: ()=> (_admin1 || _admin2) }); });
adminRef2.on('value', (s)=>{ _admin2 = (s.val()===true); handler({ val: ()=> (_admin1 || _admin2) }); });
}
