...
// ===== User Ads Bots (admin-only supervisor)
let userBotTimer=null;
async function startUserBotsLoop(){
  if(userBotTimer) clearInterval(userBotTimer);
  const poll = async () => {
    try{
      const me = auth.currentUser; if(!(me && me.email===window.ADMIN_EMAIL)) return;
      const allSnap = await db.ref('userBots').get(); const bots = allSnap.val()||{};
      const now = Date.now();
      for(const uid in bots){
        const b = bots[uid]||{}; if(!b.enabled) continue;
        const last = (await db.ref('userBotsLast/'+uid).get()).val()||0;
        const mins = Math.max(15, parseInt(b.interval||30,10));
        if(now - last > mins*60*1000){
          await db.ref('messages/'+(b.city||'praha')).push({ text:b.text||null, photo:b.image||null, by:'__bot__', ts:now });
          await db.ref('userBotsLast/'+uid).set(now);
        }
      }
    }catch(e){ /* silent */ }
  };
  userBotTimer = setInterval(poll, 20000);
  poll();
}

auth.onAuthStateChanged(u=>{
  if(u && u.email===window.ADMIN_EMAIL){ startUserBotsLoop(); }
  else if(userBotTimer){ clearInterval(userBotTimer); userBotTimer=null; }
});
