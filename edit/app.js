...
// __V17_33_PATCH__
// Robust mobile Google sign-in (redirect on mobile, popup on desktop) + redirect result handling.
function isMobileUA(){
  return /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent || '');
}

async function handleGoogleRedirectOnce(){
  if (window.__googleRedirectHandled) return;
  window.__googleRedirectHandled = true;
  try{
    const { getRedirectResult } = firebaseAuth;
    if (!getRedirectResult) return;
    const res = await getRedirectResult(auth);
    if (res && res.user){
      toast('✅ Přihlášeno přes Google');
    }
    try{ sessionStorage.removeItem('google_redirect_pending'); }catch(e){}
    try{ sessionStorage.removeItem('authRedirecting'); }catch(e){}
  }catch(e){
    // ignore when there is no redirect pending
    const code = e && (e.code || e.message || '');
    if (!String(code).includes('auth/no-auth-event') && !String(code).includes('auth/redirect-cancelled-by-user')){
...
async function signInGoogleSmart(){
  try{
    const { GoogleAuthProvider, signInWithRedirect, signInWithPopup } = firebaseAuth;
    const provider = new GoogleAuthProvider();
    provider.setCustomParameters({ prompt: 'select_account' });
    // avoid double taps
    if (window.__googleSigning) return;
    window.__googleSigning = true;
    miniPreloader('auth', true, 'Přihlašujeme přes Google…');
    if (isMobileUA()){
    ...
    toast('❌ Google přihlášení selhalo. Zkus to znovu.');
  }finally{
    window.__googleSigning = false;
    miniPreloader('auth', false);
  }
}

// Notifications (adult bell): unified feed with read/unread + basic actions
function notifRef(uid){ return firebaseDB.ref(db, 'notifications/' + uid); }

async function pushNotif(toUid, payload){
  try{
    const ref = notifRef(toUid).push();
    await ref.set(Object.assign({ ts: Date.now(), read: false }, payload));
  }catch(e){
...
function renderBellList(){
  ...
  notifRef(me.uid).orderByChild('ts').limitToLast(60).once('value', snap=>{
    const items=[];
    snap.forEach(ch=> items.push({ id: ch.key, ...ch.val() }));
    items.sort((a,b)=> (b.ts||0)-(a.ts||0));
    if (!items.length){ box.innerHTML='<div class="muted">Žádná oznámení.</div>'; return; }
    box.innerHTML = items.map(it=>{
      const unread = it.read ? '' : ' unread';
      const title = it.title || (it.type==='dm' ? 'Nová zpráva' : it.type==='friendRequest' ? 'Žádost o přátelství' : it.type || 'Událost');
      const sub = it.text || it.preview || '';
      const time = new Date(it.ts||Date.now()).toLocaleString();
      return `<div class="bellItem${unread}" data-id="${it.id}">
        <div class="bellHead"><b>${esc(title)}</b> <span class="muted">${esc(time)}</span></div>
        ${sub ? `<div class="bellBody">${esc(sub)}</div>`:''}
        <div class="bellActions">
          <button class="btn sm" data-act="open">Otevřít</button>
          <button class="btn sm" data-act="read">Přečteno</button>
        </div>
      </div>`;
    }).join('');
    box.querySelectorAll('.bellItem').forEach(el=>{
      el.addEventListener('click', async (ev)=>{
        const id = el.getAttribute('data-id');
        const actBtn = ev.target.closest('button');
        const act = actBtn ? actBtn.getAttribute('data-act') : null;
        const it = items.find(x=>x.id===id);
        if (!it) return;
        if (act==='read'){
          await notifRef(me.uid).child(id).update({ read:true });
        ...
        if (act==='open' || !act){
          await notifRef(me.uid).child(id).update({ read:true });
          // basic routes
          if (it.type==='dm' && it.chatId){
            closeBellModal();
            openDMById(it.chatId, it.peerUid || null);
          } else if (it.type==='friendRequest'){
            closeBellModal();
            openTab('friends');
          } else if (it.type==='premium'){
            closeBellModal();
            openTab('privilegia');
          }
        }
...
function listenBellBadge(){
  if (!me || !me.uid) return;
  const badge = document.getElementById('bellBadge');
  if (!badge) return;
  notifRef(me.uid).orderByChild('read').equalTo(false).on('value', snap=>{
...
// DM gating: free worker cannot DM employer -> open purchase bot modal
async function canDM(toUid){
  if (!me || !me.uid || !toUid) return { ok:false, reason:'auth' };
  const myPub = await firebaseDB.ref(db,'usersPublic/'+me.uid).once('value').then(s=>s.val()||{});
  const toPub = await firebaseDB.ref(db,'usersPublic/'+toUid).once('value').then(s=>s.val()||{});
  const myPlan = await firebaseDB.ref(db,'users/'+me.uid+'/plan').once('value').then(s=>s.val()||'free');
  if ((myPub.role==='worker' || myPub.role==='hleda') && (toPub.role==='employer' || toPub.role==='zam')){
    if (String(myPlan).toLowerCase()!=='vip_worker_life' && String(myPlan).toLowerCase()!=='vip'){
      return { ok:false, reason:'vip_worker_needed', toPub };
    }
  }
  return { ok:true };
}

function openPurchaseBot(reason){
  // Unified purchase assistant inside modalPriv (Koupit tab)
  try{
    openPrivModal('buy');
  }catch(e){}
  const box = document.getElementById('privBotChat');
  if(!box) return;

  const title = 'MAKÁME bot';
  let msg = 'Vyber možnost a postupuj podle instrukcí.';
  if(reason==='vip_worker_needed'){
    msg = 'Pro psaní zaměstnavatelům v L.S. potřebuješ <b>VIP Pracovník</b> (100 Kč navždy).';
  }
  box.innerHTML = `
    <div class="botMsg">
      <b>${title}</b>
      <div>${msg}</div>
      <div class="muted" style="margin-top:6px">1) Klikni na balíček • 2) Zaplať • 3) Nahraj/pošli screenshot (žádost se odešle adminovi).</div>
      <div class="botBtns" style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
        <button class="btn primary" id="botGoVipWorker" type="button">VIP Pracovník 100 Kč</button>
        <button class="btn" id="botGoVip150" type="button">VIP 150 Kč</button>
        <button class="btn" id="botGoPremium200" type="button">Premium 200 Kč</button>
        <button class="btn" id="botClosePriv" type="button">Zavřít</button>
      </div>
    </div>`;
  setTimeout(()=>{
    document.getElementById('botGoVipWorker')?.addEventListener('click', ()=>{ document.getElementById('buyVipWorker')?.click(); });
    document.getElementById('botGoVip150')?.addEventListener('click', ()=>{ document.getElementById('buyVip150')?.click(); });
    document.getElementById('botGoPremium200')?.addEventListener('click', ()=>{ document.getElementById('buyPremium200')?.click(); });
    document.getElementById('botClosePriv')?.addEventListener('click', ()=>{ try{ closeModal('modalPriv'); }catch(e){} try{ document.getElementById('modalPriv').hidden=true; document.body.classList.remove('modal-open'); }catch(e){} });
  },0);
}


// Nick/role hard scheme: if already set -> create request instead of overwriting
async function requestProfileChange(newNick, newRole){
  if (!me || !me.uid) return;
  const pubRef = firebaseDB.ref(db,'usersPublic/'+me.uid);
  const cur = await pubRef.once('value').then(s=>s.val()||{});
  const payload = {
    ts: Date.now(),
    status: 'pending',
    fromNick: cur.nick || '',
  ...
  await firebaseDB.ref(db,'profileChangeRequests/'+me.uid).set(payload);
        ...
        if(overlay){
          ...
          if(buyBtn){
            ...
            buyBtn.onclick = ()=>{
              ...
              // default: open Privilegia -> DM -> bot dialog
              try{ openPremiumBot(); }catch(e){ toast(title); }
            };
          }
...
async function doPasswordReset(){
  try{
    ...
    if(!email){
      toast('Zadej e‑mail.','warn');
      return;
    }
    // compat API
    await auth.sendPasswordResetEmail(email);
    toast('Odesláno. Zkontroluj e‑mail (i spam).','ok');
  }catch(e){
    console.error(e);
    toast('Nelze odeslat reset. Zkontroluj e‑mail.','warn');
  }
}
...
// Chat boot staging: paint 5 immediately, then background-fill to 20.
...
async function loadChat(){
  ...
  // Reset staged boot counters (each time the user opens chat).
  try{ __chatBootKeys.clear(); }catch(e){}
  __chatBootDone = false;
  __chatBootTarget = 5;
  ...
  const city=getCity();
  const refFast=db.ref('messages/'+city).orderByChild('ts').limitToLast(5);
  const refFull=db.ref('messages/'+city).orderByChild('ts').limitToLast(20);
...
if(moreBtn){
  ...
  try{
    // Stage 0: check if chat exists (do not block UI on fetching all 20)
    ref.once('value').then((s)=>{
      const total = (s && s.exists && s.exists()) ? (s.numChildren ? s.numChildren() : 0) : 0;
      if(total===0){
        feed.innerHTML='';
        __chatBootTarget = 0;
        if(!__chatBootDone){
          __chatBootDone = true;
          // Background fill to 20 (once): fetch last 20 and render missing messages without blocking UI.
          try{
            if(!window.__chatFilled20){
              window.__chatFilled20 = true;
              setTimeout(async ()=>{
                try{
                  const snap20 = await refFull.get();
                  const list = [];
                  snap20.forEach(s=>{ list.push({key:s.key, val:(s.val()||{})}); });
                  list.sort((a,b)=>(Number(a.val.ts||0)-Number(b.val.ts||0)));
                  for(const it of list){
                    if(__chatBootKeys.has(it.key)) continue;
                    // Reuse existing upsert path by simulating a child_changed insert
                    try{
                      const m = it.val||{};
                      if(m.deleted) continue;
                      __chatBootKeys.add(it.key);
                      let d = feed.querySelector(`[data-mid="${it.key}"]`);
                      const isNew = !d;
                      if(!d){ d=document.createElement('div'); d.className='msg'; d.dataset.mid = it.key; }
                      const u = await resolveMsgUser(m);
                      const planVal = String(u.plan||'').toLowerCase();
                      const isPrem = !!planVal && !['free','none','0','basic'].includes(planVal);
                      const isAdm = (isAdminUser(auth.currentUser) && auth.currentUser && m.by === auth.currentUser.uid);
                      ...
                      const name = `<span class="nick" data-uid="${esc(m.by)}">${esc(u?.nick||u?.name||'Uživatel')}</span>`+badges+(u?.online?'<span class="online"></span>':'');
                      d.innerHTML =
                        `<div class="ava" data-uid="${esc(m.by)}"><img src="${esc(u.avatar||window.DEFAULT_AVATAR)}"></div>`+
                        `<div class="bubble"><div class="meta"><div class="name">${name}</div><div class="time">${fmtTime(m.ts||m.createdAt||Date.now())}</div></div>`+
                        (m.text? `<div class="text">${esc(m.text)}</div>`:'' )+
                        (m.img||m.photo? `<div class="text"><img src="${esc(m.img||m.photo)}"></div>`:'')+
                        `</div>`;
      ...
      // Stage 2 (background): after the first paint of 5, ensure the rest of the last 20 is rendered.
      // This keeps the "feel" fast while still bringing the full recent context.
      setTimeout(async ()=>{
        try{
          const s20 = await db.ref('messages/'+city).orderByChild('ts').limitToLast(20).get();
          const v20 = s20.val()||{};
          const items = Object.entries(v20).map(([k,v])=>({k,v})).sort((a,b)=>((a.v&&a.v.ts)||0)-((b.v&&b.v.ts)||0));
          for(const it of items){
            if(feed.querySelector(`[data-mid="${it.k}"]`)) continue;
            await upsert({ key: it.k, val: ()=>it.v });
          }
          // Stop mini-loader after 5 (stage1) to keep UX snappy.
          // Remaining items continue to arrive via child_added.
          if(!__chatBootDone){
            __chatBootDone = true;
            setMiniLoad('chatMiniLoad','', false);
            try{ stopSeq && stopSeq(); }catch(e){}
      ...
      // Fail-safe: never keep the chat mini-loader forever.
      __markChatReady();
      __chatCheckHasMore();
      setTimeout(()=>{ 
        if(!__chatBootDone){ 
  ...
  async function upsert(snap){
    // Boot gating: stop mini-loader after the first stage (5).
    try{
...
const m=snap.val()||{};
  ...
  try{
    db.ref('messages/'+city).orderByChild('ts').limitToLast(5).once('value').then((s5)=>{
      const arr=[];
      if(s5 && s5.forEach){
...
async function openPremiumBot(){
  // Scenario: Privilegia -> open DM view and immediately show purchase bot dialog.
  if(!auth.currentUser){
    try{ openModalAuth('login'); }catch(e){ try{ openAuth(); }catch(_){ } }
    return;
  }
  ...
  try{
    ...
    document.querySelector('.tab[data-view="view-dm"]')?.classList.add('active');
  }catch(e){}
  // open privilegia modal with bot assistant
  setTimeout(()=>{
    try{ openPurchaseBot('promo'); }catch(e){ try{ openPrivModal('buy'); }catch(_){ } }
  }, 60);
}
...
document.getElementById('authGoogleBtntn')?.addEventListener('click', async ()=>{
...
async function startDM(toUid, opts={}){
  const gate = await canDM(arguments[0]);
  if(!gate.ok){ if(gate.reason==='vip_worker_needed'){ openPurchaseBot(gate.reason); } return; }
  ...
  // (fix) removed stray chat-boot vars that accidentally landed inside DM flow
...
// Init v17
window.addEventListener('DOMContentLoaded', ()=>{
  try{ handleGoogleRedirectOnce(); }catch(e){}
